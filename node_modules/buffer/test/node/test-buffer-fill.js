...
// Buffer
function deepStrictEqualValues(buf, arr) ***REMOVED***
  for (var [index, value] of buf.entries()) ***REMOVED***
...
function writeToFill(string, offset, end, encoding) ***REMOVED***
  ...
  ***REMOVED*** else if (end === undefined) ***REMOVED***
    end = buf2.length;
  ***REMOVED***
  ...
  do ***REMOVED***
    ...
    if (written === 0) ***REMOVED***
      if (wasZero)
        ...
        wasZero = true;
    ***REMOVED***
  ***REMOVED*** while (offset < buf2.length);

  // Correction for UCS2 operations.
  if (os.endianness() === 'BE' && encoding === 'ucs2') ***REMOVED***
    for (var i = 0; i < buf2.length; i += 2) ***REMOVED***
      var tmp = buf2[i];
      buf2[i] = buf2[i + 1];
      buf2[i + 1] = tmp;
    ***REMOVED***
...
function testBufs(string, offset, length, encoding) ***REMOVED***
  bufReset();
