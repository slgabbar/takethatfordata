...
module.exports =
  ...
  'subnetMatch does not fail on empty range': (test) ->
    ipaddr.subnetMatch(new ipaddr.IPv4([1,2,3,4]), ***REMOVED******REMOVED***, false)
    ipaddr.subnetMatch(new ipaddr.IPv4([1,2,3,4]), ***REMOVED***subnet: []***REMOVED***, false)
    test.done()

  'subnetMatch returns default subnet on empty range': (test) ->
    test.equal(ipaddr.subnetMatch(new ipaddr.IPv4([1,2,3,4]), ***REMOVED******REMOVED***, false), false)
    test.equal(ipaddr.subnetMatch(new ipaddr.IPv4([1,2,3,4]), ***REMOVED***subnet: []***REMOVED***, false), false)
    test.done()

  'subnetMatch does not fail on IPv4 when looking for IPv6': (test) ->
    rangelist = ***REMOVED***subnet6: ipaddr.parseCIDR('fe80::/64')***REMOVED***
  ...
  'subnetMatch does not fail on IPv6 when looking for IPv4': (test) ->
    rangelist = ***REMOVED***subnet4: ipaddr.parseCIDR('1.2.3.0/24')***REMOVED***
  ...
  'subnetMatch can use a hybrid IPv4/IPv6 range list': (test) ->
    rangelist = ***REMOVED***dual64: [ipaddr.parseCIDR('1.2.4.0/24'), ipaddr.parseCIDR('2001:1:2:3::/64')]***REMOVED***
