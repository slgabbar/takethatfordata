...
function testIt(priv, run) ***REMOVED***
  test('r is coprime with n ' + (run + 1), function (t) ***REMOVED***
    var len = 30;
    t.plan(len);
    var i = 0;
    while(i++ < len) ***REMOVED***
...
function testMessage(key, run) ***REMOVED***
  var len = 40;
  var i = 0;
  while (len--) ***REMOVED***
    test('round trip key ' + (run + 1) + ' run ' + (++i), function (t) ***REMOVED***
      t.plan(1);
      var priv = parseKey(key);
      var len = priv.modulus.byteLength();
      var r = new bn(crypto.randomBytes(len));
      while (r.cmp(priv.modulus) >=  0) ***REMOVED***
        r = new bn(crypto.randomBytes(len));
      ***REMOVED***
      var buf = new Buffer(r.toArray());
      if (buf.byteLength < priv.modulus.byteLength()) ***REMOVED***
        var tmp = new Buffer(priv.modulus.byteLength() - buf.byteLength);
        tmp.fill(0);
        buf = Buffer.concat([tmp, buf]);
      ***REMOVED***
      var nodeEncrypt = crypto.privateDecrypt(***REMOVED***
        padding: constants.RSA_NO_PADDING,
        key: key
      ***REMOVED***, buf).toString('hex');
