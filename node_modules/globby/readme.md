...
```js
(async () => ***REMOVED***
	const paths = await globby('images', ***REMOVED***
		expandDirectories: ***REMOVED***
...
Returns an `Array<Object>` in the format `***REMOVED***pattern: string, opts: Object***REMOVED***`, which can be passed as arguments to [`node-glob`](https://github.com/isaacs/node-glob). This is useful for other globbing-related packages.
...
Note that the options affect the results. If `noext: true` is set, then `+(a|b)` will not be considered a magic pattern. If the pattern has a brace expansion, like `a/***REMOVED***b/c,x/y***REMOVED***`, then that is considered magical, unless `nobrace: true` is set.
...
```js
const ***REMOVED***gitignore***REMOVED*** = require('globby');
...
- `***REMOVED******REMOVED***` allows for a comma-separated list of "or" expressions
