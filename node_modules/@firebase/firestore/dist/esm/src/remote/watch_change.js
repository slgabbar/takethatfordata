/**
 * Copyright 2017 Google Inc.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *   http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
import ***REMOVED*** maybeDocumentMap ***REMOVED*** from '../model/collections';
import ***REMOVED*** emptyByteString ***REMOVED*** from '../platform/platform';
import ***REMOVED*** assert, fail ***REMOVED*** from '../util/assert';
import * as objUtils from '../util/obj';
import ***REMOVED*** CurrentStatusUpdate, RemoteEvent, ResetMapping, UpdateMapping ***REMOVED*** from './remote_event';
/**
 * Represents a changed document and a list of target ids to which this change
 * applies.
 *
 * If document has been deleted NoDocument will be provided.
 */
var DocumentWatchChange = /** @class */ (function () ***REMOVED***
    function DocumentWatchChange(
        /** The new document applies to all of these targets. */
        updatedTargetIds, 
        /** The new document is removed from all of these targets. */
        removedTargetIds, 
        /** The key of the document for this change. */
        key, 
        /**
         * The new document or NoDocument if it was deleted. Is null if the
         * document went out of view without the server sending a new document.
         */
        newDoc) ***REMOVED***
        this.updatedTargetIds = updatedTargetIds;
        this.removedTargetIds = removedTargetIds;
        this.key = key;
        this.newDoc = newDoc;
    ***REMOVED***
    return DocumentWatchChange;
***REMOVED***());
export ***REMOVED*** DocumentWatchChange ***REMOVED***;
var ExistenceFilterChange = /** @class */ (function () ***REMOVED***
    function ExistenceFilterChange(targetId, existenceFilter) ***REMOVED***
        this.targetId = targetId;
        this.existenceFilter = existenceFilter;
    ***REMOVED***
    return ExistenceFilterChange;
***REMOVED***());
export ***REMOVED*** ExistenceFilterChange ***REMOVED***;
export var WatchTargetChangeState;
(function (WatchTargetChangeState) ***REMOVED***
    WatchTargetChangeState[WatchTargetChangeState["NoChange"] = 0] = "NoChange";
    WatchTargetChangeState[WatchTargetChangeState["Added"] = 1] = "Added";
    WatchTargetChangeState[WatchTargetChangeState["Removed"] = 2] = "Removed";
    WatchTargetChangeState[WatchTargetChangeState["Current"] = 3] = "Current";
    WatchTargetChangeState[WatchTargetChangeState["Reset"] = 4] = "Reset";
***REMOVED***)(WatchTargetChangeState || (WatchTargetChangeState = ***REMOVED******REMOVED***));
var WatchTargetChange = /** @class */ (function () ***REMOVED***
    function WatchTargetChange(
        /** What kind of change occurred to the watch target. */
        state, 
        /** The target IDs that were added/removed/set. */
        targetIds, 
        /**
         * An opaque, server-assigned token that allows watching a query to be
         * resumed after disconnecting without retransmitting all the data that
         * matches the query. The resume token essentially identifies a point in
         * time from which the server should resume sending results.
         */
        resumeToken, 
        /** An RPC error indicating why the watch failed. */
        cause) ***REMOVED***
        if (resumeToken === void 0) ***REMOVED*** resumeToken = emptyByteString(); ***REMOVED***
        if (cause === void 0) ***REMOVED*** cause = null; ***REMOVED***
        this.state = state;
        this.targetIds = targetIds;
        this.resumeToken = resumeToken;
        this.cause = cause;
    ***REMOVED***
    return WatchTargetChange;
***REMOVED***());
export ***REMOVED*** WatchTargetChange ***REMOVED***;
/**
 * A helper class to accumulate watch changes into a RemoteEvent and other
 * target information.
 */
var WatchChangeAggregator = /** @class */ (function () ***REMOVED***
    function WatchChangeAggregator(snapshotVersion, listenTargets, pendingTargetResponses) ***REMOVED***
        this.snapshotVersion = snapshotVersion;
        this.listenTargets = listenTargets;
        /** The existence filter - if any - for the given target IDs. */
        this.existenceFilters = ***REMOVED******REMOVED***;
        /** Keeps track of the current target mappings */
        this.targetChanges = ***REMOVED******REMOVED***;
        /** Keeps track of document to update */
        this.documentUpdates = maybeDocumentMap();
        /** Whether this aggregator was frozen and can no longer be modified */
        this.frozen = false;
        this.pendingTargetResponses = objUtils.shallowCopy(pendingTargetResponses);
    ***REMOVED***
    /** Aggregates a watch change into the current state */
    WatchChangeAggregator.prototype.add = function (watchChange) ***REMOVED***
        assert(!this.frozen, 'Trying to modify frozen WatchChangeAggregator.');
        if (watchChange instanceof DocumentWatchChange) ***REMOVED***
            this.addDocumentChange(watchChange);
        ***REMOVED***
        else if (watchChange instanceof WatchTargetChange) ***REMOVED***
            this.addTargetChange(watchChange);
        ***REMOVED***
        else if (watchChange instanceof ExistenceFilterChange) ***REMOVED***
            this.addExistenceFilterChange(watchChange);
        ***REMOVED***
        else ***REMOVED***
            fail('Unknown watch change: ' + watchChange);
        ***REMOVED***
    ***REMOVED***;
    /** Aggregates all provided watch changes to the current state in order */
    WatchChangeAggregator.prototype.addChanges = function (watchChanges) ***REMOVED***
        var _this = this;
        assert(!this.frozen, 'Trying to modify frozen WatchChangeAggregator.');
        watchChanges.forEach(function (change) ***REMOVED*** return _this.add(change); ***REMOVED***);
    ***REMOVED***;
    /**
     * Converts the current state into a remote event with the snapshot version
     * provided via the constructor.
     */
    WatchChangeAggregator.prototype.createRemoteEvent = function () ***REMOVED***
        var _this = this;
        var targetChanges = this.targetChanges;
        // Remove all the non-active targets from the remote event.
        objUtils.forEachNumber(this.targetChanges, function (targetId) ***REMOVED***
            if (!_this.isActiveTarget(targetId)) ***REMOVED***
                delete targetChanges[targetId];
            ***REMOVED***
        ***REMOVED***);
        // Mark this aggregator as frozen so no further modifications are made
        this.frozen = true;
        return new RemoteEvent(this.snapshotVersion, targetChanges, this.documentUpdates);
    ***REMOVED***;
    WatchChangeAggregator.prototype.ensureTargetChange = function (targetId) ***REMOVED***
        var change = this.targetChanges[targetId];
        if (!change) ***REMOVED***
            // Create an UpdateMapping by default, since resets are always explicit.
            change = ***REMOVED***
                currentStatusUpdate: CurrentStatusUpdate.None,
                snapshotVersion: this.snapshotVersion,
                mapping: new UpdateMapping(),
                resumeToken: emptyByteString()
            ***REMOVED***;
            this.targetChanges[targetId] = change;
        ***REMOVED***
        return change;
    ***REMOVED***;
    /**
     * We need to wait for watch to ack targets before we process those events,
     * so to know if a target is active, there must be no pending acks we're
     * waiting for and it must be in the current list of targets that the client
     * cares about.
     *
     * This method is visible for testing.
     */
    WatchChangeAggregator.prototype.isActiveTarget = function (targetId) ***REMOVED***
        return (!objUtils.contains(this.pendingTargetResponses, targetId) &&
            objUtils.contains(this.listenTargets, targetId));
    ***REMOVED***;
    WatchChangeAggregator.prototype.addDocumentChange = function (docChange) ***REMOVED***
        var relevant = false;
        for (var _i = 0, _a = docChange.updatedTargetIds; _i < _a.length; _i++) ***REMOVED***
            var targetId = _a[_i];
            if (this.isActiveTarget(targetId)) ***REMOVED***
                var change = this.ensureTargetChange(targetId);
                change.mapping.add(docChange.key);
                relevant = true;
            ***REMOVED***
        ***REMOVED***
        for (var _b = 0, _c = docChange.removedTargetIds; _b < _c.length; _b++) ***REMOVED***
            var targetId = _c[_b];
            if (this.isActiveTarget(targetId)) ***REMOVED***
                var change = this.ensureTargetChange(targetId);
                change.mapping.delete(docChange.key);
                relevant = true;
            ***REMOVED***
        ***REMOVED***
        // Only update the document if there is a new document to replace to an
        // active target that is being listened to, this might be just a target
        // update instead.
        if (docChange.newDoc && relevant) ***REMOVED***
            this.documentUpdates = this.documentUpdates.insert(docChange.key, docChange.newDoc);
        ***REMOVED***
    ***REMOVED***;
    WatchChangeAggregator.prototype.addTargetChange = function (targetChange) ***REMOVED***
        var _this = this;
        targetChange.targetIds.forEach(function (targetId) ***REMOVED***
            var change = _this.ensureTargetChange(targetId);
            switch (targetChange.state) ***REMOVED***
                case WatchTargetChangeState.NoChange:
                    if (_this.isActiveTarget(targetId)) ***REMOVED***
                        // Creating the change above satisfies the semantics of no-change.
                        applyResumeToken(change, targetChange.resumeToken);
                    ***REMOVED***
                    break;
                case WatchTargetChangeState.Added:
                    // We need to decrement the number of pending acks needed from watch
                    // for this targetId.
                    _this.recordTargetResponse(targetId);
                    if (!objUtils.contains(_this.pendingTargetResponses, targetId)) ***REMOVED***
                        // We have a freshly added target, so we need to reset any state
                        // that we had previously This can happen e.g. when remove and add
                        // back a target for existence filter mismatches.
                        change.mapping = new UpdateMapping();
                        change.currentStatusUpdate = CurrentStatusUpdate.None;
                        delete _this.existenceFilters[targetId];
                    ***REMOVED***
                    applyResumeToken(change, targetChange.resumeToken);
                    break;
                case WatchTargetChangeState.Removed:
                    // We need to keep track of removed targets to we can
                    // post-filter and remove any target changes.
                    // We need to decrement the number of pending acks needed from watch
                    // for this targetId.
                    _this.recordTargetResponse(targetId);
                    assert(!targetChange.cause, 'WatchChangeAggregator does not handle errored targets');
                    break;
                case WatchTargetChangeState.Current:
                    if (_this.isActiveTarget(targetId)) ***REMOVED***
                        change.currentStatusUpdate = CurrentStatusUpdate.MarkCurrent;
                        applyResumeToken(change, targetChange.resumeToken);
                    ***REMOVED***
                    break;
                case WatchTargetChangeState.Reset:
                    if (_this.isActiveTarget(targetId)) ***REMOVED***
                        // Overwrite any existing target mapping with a reset
                        // mapping. Every subsequent update will modify the reset
                        // mapping, not an update mapping.
                        change.mapping = new ResetMapping();
                        applyResumeToken(change, targetChange.resumeToken);
                    ***REMOVED***
                    break;
                default:
                    fail('Unknown target watch change state: ' + targetChange.state);
            ***REMOVED***
        ***REMOVED***);
    ***REMOVED***;
    /**
     * Record that we get a watch target add/remove by decrementing the number of
     * pending target responses that we have.
     */
    WatchChangeAggregator.prototype.recordTargetResponse = function (targetId) ***REMOVED***
        var newCount = (this.pendingTargetResponses[targetId] || 0) - 1;
        if (newCount === 0) ***REMOVED***
            delete this.pendingTargetResponses[targetId];
        ***REMOVED***
        else ***REMOVED***
            this.pendingTargetResponses[targetId] = newCount;
        ***REMOVED***
    ***REMOVED***;
    WatchChangeAggregator.prototype.addExistenceFilterChange = function (change) ***REMOVED***
        if (this.isActiveTarget(change.targetId)) ***REMOVED***
            this.existenceFilters[change.targetId] = change.existenceFilter;
        ***REMOVED***
    ***REMOVED***;
    return WatchChangeAggregator;
***REMOVED***());
export ***REMOVED*** WatchChangeAggregator ***REMOVED***;
/**
 * Applies the resume token to the TargetChange, but only when it has a new
 * value. null and empty resumeTokens are discarded.
 */
function applyResumeToken(change, resumeToken) ***REMOVED***
    if (resumeToken.length > 0) ***REMOVED***
        change.resumeToken = resumeToken;
    ***REMOVED***
***REMOVED***

//# sourceMappingURL=watch_change.js.map
