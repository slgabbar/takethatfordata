...
var indexeddb_mutation_queue_1 = require("./indexeddb_mutation_queue");
var indexeddb_query_cache_1 = require("./indexeddb_query_cache");
var indexeddb_remote_document_cache_1 = require("./indexeddb_remote_document_cache");
var indexeddb_schema_1 = require("./indexeddb_schema");
var indexeddb_schema_2 = require("./indexeddb_schema");
var local_serializer_1 = require("./local_serializer");
 ...
 */
var IndexedDbPersistence = /** @class */ (function () ***REMOVED***
    function IndexedDbPersistence(prefix, serializer) ***REMOVED***
        this.ownerId = this.generateOwnerId();
        this.dbName = prefix + IndexedDbPersistence.MAIN_DATABASE;
        this.serializer = new local_serializer_1.LocalSerializer(serializer);
        this.localStoragePrefix = prefix;
    ***REMOVED***
    IndexedDbPersistence.prototype.start = function () ***REMOVED***
        var _this = this;
        if (!IndexedDbPersistence.isAvailable()) ***REMOVED***
            this.persistenceError = new error_1.FirestoreError(error_1.Code.UNIMPLEMENTED, UNSUPPORTED_PLATFORM_ERROR_MSG);
            return Promise.reject(this.persistenceError);
        ***REMOVED***
        assert_1.assert(!this.started, 'IndexedDbPersistence double-started!');
        this.started = true;
        return simple_db_1.SimpleDb.openOrCreate(this.dbName, indexeddb_schema_2.SCHEMA_VERSION, indexeddb_schema_2.createOrUpgradeDb)
            .then(function (db) ***REMOVED***
            _this.simpleDb = db;
        ***REMOVED***)
            .then(function () ***REMOVED*** return _this.tryAcquireOwnerLease(); ***REMOVED***)
            .then(function () ***REMOVED***
            _this.scheduleOwnerLeaseRefreshes();
            _this.attachWindowUnloadHook();
        ***REMOVED***);
    ***REMOVED***;
    IndexedDbPersistence.prototype.shutdown = function () ***REMOVED***
        var _this = this;
        assert_1.assert(this.started, 'IndexedDbPersistence shutdown without start!');
        this.started = false;
        this.detachWindowUnloadHook();
        this.stopOwnerLeaseRefreshes();
        return this.releaseOwnerLease().then(function () ***REMOVED***
            _this.simpleDb.close();
        ***REMOVED***);
    ***REMOVED***;
    IndexedDbPersistence.prototype.getMutationQueue = function (user) ***REMOVED***
        return indexeddb_mutation_queue_1.IndexedDbMutationQueue.forUser(user, this.serializer);
    ***REMOVED***;
    IndexedDbPersistence.prototype.getQueryCache = function () ***REMOVED***
        return new indexeddb_query_cache_1.IndexedDbQueryCache(this.serializer);
    ***REMOVED***;
    IndexedDbPersistence.prototype.getRemoteDocumentCache = function () ***REMOVED***
        return new indexeddb_remote_document_cache_1.IndexedDbRemoteDocumentCache(this.serializer);
    ***REMOVED***;
    IndexedDbPersistence.prototype.runTransaction = function (action, operation) ***REMOVED***
        var _this = this;
        if (this.persistenceError) ***REMOVED***
            return Promise.reject(this.persistenceError);
        ***REMOVED***
        log.debug(LOG_TAG, 'Starting transaction:', action);
        // Do all transactions as readwrite against all object stores, since we
        // are the only reader/writer.
        return this.simpleDb.runTransaction('readwrite', indexeddb_schema_1.ALL_STORES, function (txn) ***REMOVED***
            // Verify that we still have the owner lease as part of every transaction.
            return _this.ensureOwnerLease(txn).next(function () ***REMOVED*** return operation(txn); ***REMOVED***);
        ***REMOVED***);
    ***REMOVED***;
    IndexedDbPersistence.isAvailable = function () ***REMOVED***
        return simple_db_1.SimpleDb.isAvailable();
    ***REMOVED***;
    /**
     * Generates a string used as a prefix when storing data in IndexedDB and
     * LocalStorage.
     */
    IndexedDbPersistence.buildStoragePrefix = function (databaseInfo) ***REMOVED***
        ...
        if (!databaseInfo.databaseId.isDefaultDatabase) ***REMOVED***
    ...
    ***REMOVED***;
     ...
     * promise.
     */
    IndexedDbPersistence.prototype.tryAcquireOwnerLease = function () ***REMOVED***
        var _this = this;
        // NOTE: Don't use this.runTransaction, since it requires us to already
        // have the lease.
        return this.simpleDb.runTransaction('readwrite', [indexeddb_schema_1.DbOwner.store], function (txn) ***REMOVED***
            var store = txn.store(indexeddb_schema_1.DbOwner.store);
            return store.get('owner').next(function (dbOwner) ***REMOVED***
                if (!_this.validOwner(dbOwner)) ***REMOVED***
                    var newDbOwner = new indexeddb_schema_1.DbOwner(_this.ownerId, Date.now());
                ...
                ***REMOVED***
                    ...
                    _this.persistenceError = new error_1.FirestoreError(error_1.Code.FAILED_PRECONDITION, EXISTING_OWNER_ERROR_MSG);
                    return persistence_promise_1.PersistencePromise.reject(_this.persistenceError);
                ***REMOVED***
            ***REMOVED***);
        ***REMOVED***);
    ***REMOVED***;
    /** Checks the owner lease and deletes it if we are the current owner. */
    IndexedDbPersistence.prototype.releaseOwnerLease = function () ***REMOVED***
        var _this = this;
        // NOTE: Don't use this.runTransaction, since it requires us to already
        // have the lease.
        return this.simpleDb.runTransaction('readwrite', [indexeddb_schema_1.DbOwner.store], function (txn) ***REMOVED***
            var store = txn.store(indexeddb_schema_1.DbOwner.store);
            return store.get('owner').next(function (dbOwner) ***REMOVED***
                if (dbOwner !== null && dbOwner.ownerId === _this.ownerId) ***REMOVED***
    ...
    ***REMOVED***;
     ...
     */
    IndexedDbPersistence.prototype.ensureOwnerLease = function (txn) ***REMOVED***
        var _this = this;
        var store = txn.store(indexeddb_schema_1.DbOwner.store);
        return store.get('owner').next(function (dbOwner) ***REMOVED***
            if (dbOwner === null || dbOwner.ownerId !== _this.ownerId) ***REMOVED***
    ...
    ***REMOVED***;
     ...
     */
    IndexedDbPersistence.prototype.validOwner = function (dbOwner) ***REMOVED***
        var now = Date.now();
        var minAcceptable = now - OWNER_LEASE_MAX_AGE_MS;
        var maxAcceptable = now;
        if (dbOwner === null) ***REMOVED***
            return false; // no owner.
        ***REMOVED***
        else if (dbOwner.leaseTimestampMs < minAcceptable) ***REMOVED***
            return false; // owner lease has expired.
        ***REMOVED***
        else if (dbOwner.leaseTimestampMs > maxAcceptable) ***REMOVED***
            log.error('Persistence owner-lease is in the future. Discarding.', dbOwner);
            return false;
        ***REMOVED***
        else if (dbOwner.ownerId === this.getZombiedOwnerId()) ***REMOVED***
    ...
    ***REMOVED***;
     ...
     */
    IndexedDbPersistence.prototype.scheduleOwnerLeaseRefreshes = function () ***REMOVED***
        var _this = this;
        // NOTE: This doesn't need to be scheduled on the async queue and doing so
        // would increase the chances of us not refreshing on time if the queue is
        // backed up for some reason.
        this.ownerLeaseRefreshHandle = setInterval(function () ***REMOVED***
            var txResult = _this.runTransaction('Refresh owner timestamp', function (txn) ***REMOVED***
                // NOTE: We don't need to validate the current owner contents, since
                // runTransaction does that automatically.
                var store = txn.store(indexeddb_schema_1.DbOwner.store);
                return store.put('owner', new indexeddb_schema_1.DbOwner(_this.ownerId, Date.now()));
            ***REMOVED***);
            txResult.catch(function (reason) ***REMOVED***
                // Probably means we lost the lease. Report the error and stop trying to
                // refresh the lease.
                log.error(reason);
                _this.stopOwnerLeaseRefreshes();
            ***REMOVED***);
        ***REMOVED***, OWNER_LEASE_REFRESH_INTERVAL_MS);
    ***REMOVED***;
    IndexedDbPersistence.prototype.stopOwnerLeaseRefreshes = function () ***REMOVED***
        if (this.ownerLeaseRefreshHandle) ***REMOVED***
    ...
    ***REMOVED***;
     ...
     */
    IndexedDbPersistence.prototype.attachWindowUnloadHook = function () ***REMOVED***
        var _this = this;
        this.windowUnloadHandler = function () ***REMOVED***
    ...
    ***REMOVED***;
    IndexedDbPersistence.prototype.detachWindowUnloadHook = function () ***REMOVED***
        if (this.windowUnloadHandler) ***REMOVED***
    ...
    ***REMOVED***;
     ...
     * record exists.
     */
    IndexedDbPersistence.prototype.getZombiedOwnerId = function () ***REMOVED***
        try ***REMOVED***
    ...
    ***REMOVED***;
     ...
     */
    IndexedDbPersistence.prototype.setZombiedOwnerId = function (zombieOwnerId) ***REMOVED***
        try ***REMOVED***
            if (zombieOwnerId === null) ***REMOVED***
        ...
        ***REMOVED***
    ***REMOVED***;
    IndexedDbPersistence.prototype.zombiedOwnerLocalStorageKey = function () ***REMOVED***
        return this.localStoragePrefix + ZOMBIE_OWNER_LOCALSTORAGE_SUFFIX;
    ***REMOVED***;
    IndexedDbPersistence.prototype.generateOwnerId = function () ***REMOVED***
        // For convenience, just use an AutoId.
        return misc_1.AutoId.newId();
    ***REMOVED***;
    /**
