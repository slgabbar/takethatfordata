 ...
 *     print "static const uint8_t %s[15 * 2 * 32] = ***REMOVED***" % name,
 ...
 */
static const uint8_t kSpakeNSmallPrecomp[15 * 2 * 32] = ***REMOVED***
...
static const uint8_t kSpakeMSmallPrecomp[15 * 2 * 32] = ***REMOVED***
...
enum spake2_state_t ***REMOVED***
  spake2_state_init = 0,
  spake2_state_msg_generated,
  spake2_state_key_generated,
***REMOVED***;

struct spake2_ctx_st ***REMOVED***
...
SPAKE2_CTX *SPAKE2_CTX_new(enum spake2_role_t my_role,
                           ...
                           const uint8_t *their_name, size_t their_name_len) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (!CBS_stow(&my_name_cbs, &ctx->my_name, &ctx->my_name_len) ||
      !CBS_stow(&their_name_cbs, &ctx->their_name, &ctx->their_name_len)) ***REMOVED***
    SPAKE2_CTX_free(ctx);
    return NULL;
  ***REMOVED***
...
void SPAKE2_CTX_free(SPAKE2_CTX *ctx) ***REMOVED***
  if (ctx == NULL) ***REMOVED***
    return;
  ***REMOVED***
 ...
 * order. */
static void left_shift_3(uint8_t n[32]) ***REMOVED***
  uint8_t carry = 0;
  unsigned i;
...
int SPAKE2_generate_msg(SPAKE2_CTX *ctx, uint8_t *out, size_t *out_len,
                         ...
                         size_t password_len) ***REMOVED***
  if (ctx->state != spake2_state_init) ***REMOVED***
    return 0;
  ***REMOVED***

  if (max_out_len < sizeof(ctx->my_msg)) ***REMOVED***
    return 0;
  ***REMOVED***
...
int SPAKE2_process_msg(SPAKE2_CTX *ctx, uint8_t *out_key, size_t *out_key_len,
                       ...
                       size_t their_msg_len) ***REMOVED***
  if (ctx->state != spake2_state_msg_generated ||
      their_msg_len != 32) ***REMOVED***
    return 0;
  ***REMOVED***

  ge_p3 Qstar;
  if (0 != x25519_ge_frombytes_vartime(&Qstar, their_msg)) ***REMOVED***
  ...
  ***REMOVED***
  ...
  SHA512_CTX sha;
  SHA512_Init(&sha);
  if (ctx->my_role == spake2_role_alice) ***REMOVED***
