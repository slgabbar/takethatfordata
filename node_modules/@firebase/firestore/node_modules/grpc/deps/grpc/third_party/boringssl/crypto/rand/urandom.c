 ...
 * once. */
static void init_once(void) ***REMOVED***
  ...
  if (getrandom_ret == 1) ***REMOVED***
    urandom_fd = kHaveGetrandom;
    return;
  ***REMOVED*** else if (getrandom_ret == -1 && errno == EAGAIN) ***REMOVED***
    fprintf(stderr,
    ...
    ***REMOVED*** while (getrandom_ret == -1 && errno == EINTR);
  ...
  ***REMOVED***
  ...
  if (fd == kUnset) ***REMOVED***
    do ***REMOVED***
      fd = open("/dev/urandom", O_RDONLY);
    ***REMOVED*** while (fd == -1 && errno == EINTR);
  ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (flags == -1) ***REMOVED***
    ...
    if (errno != ENOSYS) ***REMOVED***
      abort();
    ***REMOVED***
  ***REMOVED*** else ***REMOVED***
    flags |= FD_CLOEXEC;
    if (fcntl(fd, F_SETFD, flags) == -1) ***REMOVED***
      abort();
    ***REMOVED***
  ***REMOVED***
  urandom_fd = fd;
...
void RAND_set_urandom_fd(int fd) ***REMOVED***
  fd = dup(fd);
  if (fd < 0) ***REMOVED***
    abort();
  ***REMOVED***
  ...
  if (urandom_fd == kHaveGetrandom) ***REMOVED***
    close(fd);
  ***REMOVED*** else if (urandom_fd != fd) ***REMOVED***
    abort();  // Already initialized.
  ***REMOVED***
***REMOVED***

void RAND_enable_fork_unsafe_buffering(int fd) ***REMOVED***
  if (fd >= 0) ***REMOVED***
    fd = dup(fd);
    if (fd < 0) ***REMOVED***
      abort();
  ...
  ***REMOVED***
  ...
  CRYPTO_once(&once, init_once);
  if (urandom_buffering != 1) ***REMOVED***
    abort();  // Already initialized
  ***REMOVED***
...
static struct rand_buffer *get_thread_local_buffer(void) ***REMOVED***
...
static char fill_with_entropy(uint8_t *out, size_t len) ***REMOVED***
  while (len > 0) ***REMOVED***
    ssize_t r;
...
static void read_from_buffer(struct rand_buffer *buf,
                             uint8_t *out, size_t requested) ***REMOVED***
  size_t remaining = BUF_SIZE - buf->used;

  while (requested > remaining) ***REMOVED***
    OPENSSL_memcpy(out, &buf->rand[buf->used], remaining);
    buf->used += remaining;
    out += remaining;
    requested -= remaining;

    if (!fill_with_entropy(buf->rand, BUF_SIZE)) ***REMOVED***
      abort();
      return;
    ***REMOVED***
    buf->used = 0;
...
void CRYPTO_sysrand(uint8_t *out, size_t requested) ***REMOVED***
  if (requested == 0) ***REMOVED***
    return;
  ***REMOVED***

  CRYPTO_once(&once, init_once);
  if (urandom_buffering && requested < BUF_SIZE) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (!fill_with_entropy(out, requested)) ***REMOVED***
    abort();
  ***REMOVED***
***REMOVED***
