...
struct Bytes ***REMOVED***
  Bytes(const uint8_t *data_arg, size_t len_arg)
      : data(data_arg), len(len_arg) ***REMOVED******REMOVED***

  Bytes(const char *str)
      : data(reinterpret_cast<const uint8_t *>(str)), len(strlen(str)) ***REMOVED******REMOVED***

  template <size_t N>
  Bytes(const uint8_t (&array)[N]) : data(array), len(N) ***REMOVED******REMOVED***

  const uint8_t *data;
  size_t len;
***REMOVED***;

inline bool operator==(const Bytes &a, const Bytes &b) ***REMOVED***
  return a.len == b.len && OPENSSL_memcmp(a.data, b.data, a.len) == 0;
***REMOVED***

inline bool operator!=(const Bytes &a, const Bytes &b) ***REMOVED*** return !(a == b); ***REMOVED***
