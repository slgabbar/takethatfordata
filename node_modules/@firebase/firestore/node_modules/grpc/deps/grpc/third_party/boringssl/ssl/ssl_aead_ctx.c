...
SSL_AEAD_CTX *SSL_AEAD_CTX_new(enum evp_aead_direction_t direction,
                               ...
                               const uint8_t *fixed_iv, size_t fixed_iv_len) ***REMOVED***
  const EVP_AEAD *aead;
  ...
  if (!ssl_cipher_get_evp_aead(&aead, &expected_mac_key_len,
      ...
      expected_mac_key_len != mac_key_len) ***REMOVED***
    OPENSSL_PUT_ERROR(SSL, ERR_R_INTERNAL_ERROR);
    return 0;
  ***REMOVED***

  uint8_t merged_key[EVP_AEAD_MAX_KEY_LENGTH];
  if (mac_key_len > 0) ***REMOVED***
    /* This is a "stateful" AEAD (for compatibility with pre-AEAD cipher
     * suites). */
    if (mac_key_len + enc_key_len + fixed_iv_len > sizeof(merged_key)) ***REMOVED***
      OPENSSL_PUT_ERROR(SSL, ERR_R_INTERNAL_ERROR);
      return 0;
    ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (!EVP_AEAD_CTX_init_with_direction(
          ...
          EVP_AEAD_DEFAULT_TAG_LENGTH, direction)) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (mac_key_len == 0) ***REMOVED***
    ...
    if (cipher->algorithm_enc & SSL_CHACHA20POLY1305) ***REMOVED***
    ...
    ***REMOVED***
    ...
    if (cipher->algorithm_enc & (SSL_AES128GCM | SSL_AES256GCM)) ***REMOVED***
...
void SSL_AEAD_CTX_free(SSL_AEAD_CTX *aead) ***REMOVED***
...
size_t SSL_AEAD_CTX_explicit_nonce_len(const SSL_AEAD_CTX *aead) ***REMOVED***
#if defined(BORINGSSL_UNSAFE_FUZZER_MODE)
  aead = NULL;
#endif

  if (aead != NULL && aead->variable_nonce_included_in_record) ***REMOVED***
    return aead->variable_nonce_len;
  ***REMOVED***
  return 0;
***REMOVED***

size_t SSL_AEAD_CTX_max_overhead(const SSL_AEAD_CTX *aead) ***REMOVED***
#if defined(BORINGSSL_UNSAFE_FUZZER_MODE)
  aead = NULL;
#endif
...
static size_t ssl_aead_ctx_get_ad(SSL_AEAD_CTX *aead, uint8_t out[13],
                                  ...
                                  size_t plaintext_len) ***REMOVED***
  if (aead->omit_ad) ***REMOVED***
    return 0;
  ***REMOVED***
  ...
  size_t len = 8;
  out[len++] = type;
  if (!aead->omit_version_in_ad) ***REMOVED***
    out[len++] = (uint8_t)(wire_version >> 8);
    out[len++] = (uint8_t)wire_version;
  ***REMOVED***
  if (!aead->omit_length_in_ad) ***REMOVED***
...
int SSL_AEAD_CTX_open(SSL_AEAD_CTX *aead, CBS *out, uint8_t type,
  ...
  ***REMOVED***
  ...
  if (aead->xor_fixed_nonce) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (aead->variable_nonce_included_in_record) ***REMOVED***
    if (in_len < aead->variable_nonce_len) ***REMOVED***
      /* Publicly invalid. */
      OPENSSL_PUT_ERROR(SSL, SSL_R_BAD_PACKET_LENGTH);
      return 0;
    ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (aead->xor_fixed_nonce) ***REMOVED***
    ...
    for (size_t i = 0; i < aead->fixed_nonce_len; i++) ***REMOVED***
...
int SSL_AEAD_CTX_seal(SSL_AEAD_CTX *aead, uint8_t *out, size_t *out_len,
  ...
  ***REMOVED***
  ...
  if (aead->random_variable_nonce) ***REMOVED***
    assert(aead->variable_nonce_included_in_record);
    if (!RAND_bytes(nonce + nonce_len, aead->variable_nonce_len)) ***REMOVED***
      return 0;
    ***REMOVED***
  ***REMOVED*** else ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (aead->variable_nonce_included_in_record) ***REMOVED***
    ...
    if (max_out < aead->variable_nonce_len) ***REMOVED***
      OPENSSL_PUT_ERROR(SSL, SSL_R_BUFFER_TOO_SMALL);
      return 0;
    ***REMOVED***
    if (out < in + in_len && in < out + aead->variable_nonce_len) ***REMOVED***
