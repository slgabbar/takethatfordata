...
namespace absl ***REMOVED***
namespace numbers_internal ***REMOVED***
...
template <typename int_type>
char* FastIntToBuffer(int_type i, char* buffer) ***REMOVED***
  static_assert(sizeof(i) <= 64 / 8,
                "FastIntToBuffer works only with 64-bit-or-less integers.");
  // TODO(jorg): This signed-ness check is used because it works correctly
  // with enums, and it also serves to check that int_type is not a pointer.
  // If one day something like std::is_signed<enum E> works, switch to it.
  if (static_cast<int_type>(1) - 2 < 0) ***REMOVED***  // Signed
    if (sizeof(i) > 32 / 8) ***REMOVED***           // 33-bit to 64-bit
      return numbers_internal::FastInt64ToBuffer(i, buffer);
    ***REMOVED*** else ***REMOVED***  // 32-bit or less
      return numbers_internal::FastInt32ToBuffer(i, buffer);
    ***REMOVED***
  ***REMOVED*** else ***REMOVED***                     // Unsigned
    if (sizeof(i) > 32 / 8) ***REMOVED***  // 33-bit to 64-bit
...
***REMOVED***  // namespace numbers_internal
...
// integer type.
template <typename int_type>
ABSL_MUST_USE_RESULT bool SimpleAtoi(absl::string_view s, int_type* out) ***REMOVED***
  ...
  // If one day something like std::is_signed<enum E> works, switch to it.
  if (static_cast<int_type>(1) - 2 < 0) ***REMOVED***  // Signed
    if (sizeof(*out) == 64 / 8) ***REMOVED***       // 64-bit
      int64_t val;
      parsed = numbers_internal::safe_strto64_base(s, &val, 10);
      *out = static_cast<int_type>(val);
    ***REMOVED*** else ***REMOVED***  // 32-bit
      int32_t val;
