...
static bool TestSIMD(FileTest *t, unsigned excess,
                     ...
                     const std::vector<uint8_t> &mac) ***REMOVED***
  ...
  ***REMOVED***
  ...
  for (;;) ***REMOVED***
    ...
    if (done + 128 + excess > in.size()) ***REMOVED***
      break;
    ***REMOVED***
    ...
    if (done + excess > in.size()) ***REMOVED***
      break;
    ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (!t->ExpectBytesEqual(mac.data(), mac.size(), out, 16)) ***REMOVED***
...
static bool TestPoly1305(FileTest *t, void *arg) ***REMOVED***
  std::vector<uint8_t> key, in, mac;
  if (!t->GetBytes(&key, "Key") ||
      !t->GetBytes(&in, "Input") ||
      !t->GetBytes(&mac, "MAC")) ***REMOVED***
    return false;
  ***REMOVED***
  if (key.size() != 32 || mac.size() != 16) ***REMOVED***
  ...
  ***REMOVED***
  ...
  alignas(16) uint8_t out[16];
  CRYPTO_poly1305_finish(&state, out);
  if (!t->ExpectBytesEqual(out, 16, mac.data(), mac.size())) ***REMOVED***
  ...
  ***REMOVED***
  ...
  CRYPTO_poly1305_init(&state, key.data());
  for (size_t i = 0; i < in.size(); i++) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (!TestSIMD(t, 0, key, in, mac) ||
      ...
      !TestSIMD(t, 48, key, in, mac)) ***REMOVED***
    return false;
  ***REMOVED***
