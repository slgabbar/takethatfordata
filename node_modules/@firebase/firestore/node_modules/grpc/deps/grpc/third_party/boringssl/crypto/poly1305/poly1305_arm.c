...
static void freeze(fe1305x2 *r) ***REMOVED***
  int i;
...
static void fe1305x2_tobytearray(uint8_t *r, fe1305x2 *x) ***REMOVED***
  uint32_t x0 = x->v[0];
  uint32_t x1 = x->v[2];
  uint32_t x2 = x->v[4];
  uint32_t x3 = x->v[6];
...
static uint32_t load32(uint8_t *t) ***REMOVED***
  uint32_t tmp;
...
static void fe1305x2_frombytearray(fe1305x2 *r, const uint8_t *x,
                                   unsigned long long xlen) ***REMOVED***
  unsigned i;
  uint8_t t[17];

  for (i = 0; (i < 16) && (i < xlen); i++) ***REMOVED***
    t[i] = x[i];
  ***REMOVED***
  xlen -= i;
  x += i;
...
struct poly1305_state_st ***REMOVED***
...
void CRYPTO_poly1305_init_neon(poly1305_state *state, const uint8_t key[32]) ***REMOVED***
...
void CRYPTO_poly1305_update_neon(poly1305_state *state, const uint8_t *in,
  ...
  if (st->buf_used) ***REMOVED***
    unsigned int todo = 32 - st->buf_used;
    if (todo > in_len) ***REMOVED***
      todo = in_len;
    ***REMOVED***
    for (i = 0; i < todo; i++) ***REMOVED***
    ...
    ***REMOVED***
    ...
    if (st->buf_used == sizeof(st->buf) && in_len) ***REMOVED***
  ...
  ***REMOVED***
  ...
  while (in_len > 32) ***REMOVED***
    unsigned int tlen = 1048576;
    if (in_len < tlen) ***REMOVED***
      tlen = in_len;
    ***REMOVED***
...
void CRYPTO_poly1305_finish_neon(poly1305_state *state, uint8_t mac[16]) ***REMOVED***
  struct poly1305_state_st *st = (struct poly1305_state_st *)(state);
  fe1305x2 *const r = (fe1305x2 *)(st->data + (15 & (-(int)st->data)));
  fe1305x2 *const h = r + 1;
  fe1305x2 *const c = h + 1;
  ...
  ***REMOVED*** else if (st->buf_used > 0) ***REMOVED***
    fe1305x2_frombytearray(c, st->buf, st->buf_used);
    r->v[1] = 1;
    r->v[3] = 0;
    r->v[5] = 0;
