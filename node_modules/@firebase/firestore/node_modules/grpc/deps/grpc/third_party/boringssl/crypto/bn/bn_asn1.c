...
int BN_parse_asn1_unsigned(CBS *cbs, BIGNUM *ret) ***REMOVED***
  CBS child;
  ...
  ***REMOVED***
  ...
  if (CBS_data(&child)[0] & 0x80) ***REMOVED***
...
int BN_parse_asn1_unsigned_buggy(CBS *cbs, BIGNUM *ret) ***REMOVED***
  CBS child;
  if (!CBS_get_asn1(cbs, &child, CBS_ASN1_INTEGER) ||
      CBS_len(&child) == 0) ***REMOVED***
    OPENSSL_PUT_ERROR(BN, BN_R_BAD_ENCODING);
...
int BN_marshal_asn1(CBB *cbb, const BIGNUM *bn) ***REMOVED***
