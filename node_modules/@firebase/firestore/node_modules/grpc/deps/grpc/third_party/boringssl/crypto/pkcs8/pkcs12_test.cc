...
static const uint8_t kOpenSSL[] = ***REMOVED***
...
static const uint8_t kNSS[] = ***REMOVED***
...
static const uint8_t kWindows[] = ***REMOVED***
...
static bool Test(const char *name, const uint8_t *der, size_t der_len) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (!PKCS12_get_key_and_certs(&key, certs.get(), &pkcs12, kPassword)) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (sk_X509_num(certs.get()) != 1 || key == nullptr) ***REMOVED***
...
static bool TestCompat(const uint8_t *der, size_t der_len) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (PKCS12_verify_mac(p12.get(), "badpass", 7)) ***REMOVED***
    fprintf(stderr, "PKCS12_verify_mac accepted bad password.\n");
    return false;
  ***REMOVED***

  if (!PKCS12_verify_mac(p12.get(), kPassword, sizeof(kPassword) - 1)) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (!PKCS12_parse(p12.get(), kPassword, &key, &cert, &ca_certs)) ***REMOVED***
    fprintf(stderr, "PKCS12_parse failed.\n");
    ERR_print_errors_fp(stderr);
    return false;
  ***REMOVED***
  ...
  if (key == nullptr || cert == nullptr) ***REMOVED***
    fprintf(stderr, "Bad result from PKCS12_parse.\n");
    return false;
  ***REMOVED***

  if (sk_X509_num(ca_certs) != 0) ***REMOVED***
    fprintf(stderr, "Bad result from PKCS12_parse.\n");
    return false;
  ***REMOVED***
...
int main(int argc, char **argv) ***REMOVED***
  ...
  if (!Test("OpenSSL", kOpenSSL, sizeof(kOpenSSL)) ||
      ...
      !TestCompat(kWindows, sizeof(kWindows))) ***REMOVED***
    return 1;
  ***REMOVED***
