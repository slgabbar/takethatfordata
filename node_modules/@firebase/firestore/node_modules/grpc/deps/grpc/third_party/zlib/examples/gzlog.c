...
#   define BAIL(n) do ***REMOVED*** if (n == gzlog_bail && gzlog_count-- == 0) \
                            longjmp(gzlog_jump, gzlog_bail); ***REMOVED*** while (0)
#else
#   define BAIL(n)
#endif
...
#define PUT2(p,a) do ***REMOVED***(p)[0]=a;(p)[1]=(a)>>8;***REMOVED*** while(0)
#define PUT4(p,a) do ***REMOVED***PUT2(p,a);PUT2(p+2,a>>16);***REMOVED*** while(0)
#define PUT8(p,a) do ***REMOVED***PUT4(p,a);PUT4(p+4,a>>32);***REMOVED*** while(0)
...
local unsigned char log_gzhead[] = ***REMOVED***
...
local unsigned char log_gzext[] = ***REMOVED***
...
local unsigned char log_gzbody[] = ***REMOVED***
...
***REMOVED***
    ...
    strcpy(log->end, ".lock");
    while ((fd = open(log->path, O_CREAT | O_EXCL, 0644)) < 0) ***REMOVED***
        if (errno != EEXIST)
            return -1;
        if (stat(log->path, &st) == 0 && time(NULL) - st.st_mtime > PATIENCE) ***REMOVED***
            unlink(log->path);
            continue;
        ***REMOVED***
...
***REMOVED***
    ...
    if (lseek(log->fd, 0, SEEK_SET) < 0 ||
        ...
        memcmp(buf, log_gzhead, HEAD)) ***REMOVED***
        return -1;
    ***REMOVED***
    ...
    if (len) ***REMOVED***
        ...
        ***REMOVED***
        ...
        if (lseek(log->fd, log->first - (log->back > 8 ? 2 : 1),
                SEEK_SET) < 0 ||
            read(log->fd, buf, 1) != 1 || lseek(log->fd, -1, SEEK_CUR) < 0) ***REMOVED***
            deflateEnd(&strm);
            return -1;
        ***REMOVED***
        ...
        do ***REMOVED***
            ...
            do ***REMOVED***
                ...
                if (got && write(log->fd, buf, got) != got) ***REMOVED***
                    deflateEnd(&strm);
                    return -1;
                ***REMOVED***
                log_touch(log);
            ***REMOVED*** while (strm.avail_out == 0);
        ***REMOVED*** while (len);
        deflateEnd(&strm);
        BAIL(5);
...
***REMOVED***
    ...
    if (op == APPEND_OP || op == COMPRESS_OP) ***REMOVED***
        strcpy(log->end, ".add");
        if (stat(log->path, &st) == 0 && st.st_size) ***REMOVED***
            len = (size_t)(st.st_size);
            if ((off_t)len != st.st_size ||
                    (data = malloc(st.st_size)) == NULL) ***REMOVED***
                log_log(log, op, "allocation failure");
                return -2;
            ***REMOVED***
            if ((fd = open(log->path, O_RDONLY, 0)) < 0) ***REMOVED***
...
***REMOVED***
    ...
    ***REMOVED***
    ...
    if (lseek(log->fd, 0, SEEK_END) == 0) ***REMOVED***
        if (write(log->fd, log_gzhead, HEAD) != HEAD ||
            write(log->fd, log_gzext, EXTRA) != EXTRA ||
            write(log->fd, log_gzbody, BODY) != BODY) ***REMOVED***
            log_close(log);
            return -1;
        ***REMOVED***
        strcpy(log->end, ".dict");
    ...
    ***REMOVED***
    ...
    if ((op = log_head(log)) < 0) ***REMOVED***
        log_close(log);
        return -1;
    ***REMOVED***

    /* check for interrupted process and if so, recover */
    if (op != NO_OP && log_recover(log, op)) ***REMOVED***
        log_close(log);
        return -1;
    ***REMOVED***
