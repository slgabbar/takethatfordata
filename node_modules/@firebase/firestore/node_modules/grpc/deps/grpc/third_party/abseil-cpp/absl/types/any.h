...
namespace any_internal ***REMOVED***
...
inline size_t FastTypeId() ***REMOVED***
...
***REMOVED***  // namespace any_internal
...
//
//   void my_function() ***REMOVED******REMOVED***
...
class any ***REMOVED***
 ...
 public:
  ...
  any(const any& other)
      : obj_(other.has_value() ? other.obj_->Clone()
                               : std::unique_ptr<ObjInterface>()) ***REMOVED******REMOVED***
  ...
  any(T&& value) : obj_(new Obj<VT>(in_place, std::forward<T>(value))) ***REMOVED******REMOVED***
  ...
  explicit any(in_place_type_t<T> /*tag*/, Args&&... args)
      : obj_(new Obj<VT>(in_place, std::forward<Args>(args)...)) ***REMOVED******REMOVED***
  ...
  explicit any(in_place_type_t<T> /*tag*/, std::initializer_list<U> ilist,
               Args&&... args)
      : obj_(new Obj<VT>(in_place, ilist, std::forward<Args>(args)...)) ***REMOVED******REMOVED***

  // Assignment operators

  // Copy assigns an `absl::any` object with a "contained object" of the
  // passed type.
  any& operator=(const any& rhs) ***REMOVED***
    any(rhs).swap(*this);
    return *this;
  ***REMOVED***
  ...
  any& operator=(any&& rhs) noexcept ***REMOVED***
  ...
  ***REMOVED***
  ...
  template <
      ...
      absl::enable_if_t<std::is_copy_constructible<VT>::value &&
                        std::is_constructible<VT, std::initializer_list<U>&,
                                              Args...>::value>* = nullptr>
  VT& emplace(std::initializer_list<U> ilist, Args&&... args) ***REMOVED***
    reset();  // NOTE: reset() is required here even in the world of exceptions.
    Obj<VT>* const object_ptr =
  ...
  ***REMOVED***
  ...
  // if present.
  void reset() noexcept ***REMOVED*** obj_ = nullptr; ***REMOVED***

  // any::swap()
  //
  // Swaps the passed value and the value of this `absl::any` object.
  void swap(any& other) noexcept ***REMOVED*** obj_.swap(other.obj_); ***REMOVED***
  ...
  // returns `false`.
  bool has_value() const noexcept ***REMOVED*** return obj_ != nullptr; ***REMOVED***

#if ABSL_ANY_DETAIL_HAS_RTTI
  // Returns: typeid(T) if *this has a contained object of type T, otherwise
  // typeid(void).
  const std::type_info& type() const noexcept ***REMOVED***
    if (has_value()) ***REMOVED***
      return obj_->Type();
    ***REMOVED***
  ...
  ***REMOVED***;
  ...
  template <typename T>
  class Obj : public ObjInterface ***REMOVED***
   public:
    template <typename... Args>
    explicit Obj(in_place_t /*tag*/, Args&&... args)
        : value(std::forward<Args>(args)...) ***REMOVED******REMOVED***

    std::unique_ptr<ObjInterface> Clone() const final ***REMOVED***
      return std::unique_ptr<ObjInterface>(new Obj(in_place, value));
    ***REMOVED***

    size_t type_id() const noexcept final ***REMOVED*** return IdForType<T>(); ***REMOVED***

#if ABSL_ANY_DETAIL_HAS_RTTI
    const std::type_info& Type() const noexcept final ***REMOVED*** return typeid(T); ***REMOVED***
#endif  // ABSL_ANY_DETAIL_HAS_RTTI

    T value;
  ***REMOVED***;

  std::unique_ptr<ObjInterface> CloneObj() const ***REMOVED***
    if (!obj_) return nullptr;
    return obj_->Clone();
  ***REMOVED***

  template <typename T>
  static size_t IdForType() ***REMOVED***
  ...
  ***REMOVED***
  ...
  size_t GetObjTypeId() const ***REMOVED***
...
template <typename T>
struct any::IsInPlaceType : std::false_type ***REMOVED******REMOVED***;

template <typename T>
struct any::IsInPlaceType<in_place_type_t<T>> : std::true_type ***REMOVED******REMOVED***;

inline void swap(any& x, any& y) noexcept ***REMOVED*** x.swap(y); ***REMOVED***

// Description at the declaration site (top of file).
template <typename T, typename... Args>
any make_any(Args&&... args) ***REMOVED***
  return any(in_place_type_t<T>(), std::forward<Args>(args)...);
***REMOVED***

// Description at the declaration site (top of file).
template <typename T, typename U, typename... Args>
any make_any(std::initializer_list<U> il, Args&&... args) ***REMOVED***
  return any(in_place_type_t<T>(), il, std::forward<Args>(args)...);
***REMOVED***

// Description at the declaration site (top of file).
template <typename ValueType>
ValueType any_cast(const any& operand) ***REMOVED***
...
// Description at the declaration site (top of file).
template <typename ValueType>
ValueType any_cast(any& operand) ***REMOVED***  // NOLINT(runtime/references)
  using U = typename std::remove_cv<
      typename std::remove_reference<ValueType>::type>::type;
...
// Description at the declaration site (top of file).
template <typename ValueType>
ValueType any_cast(any&& operand) ***REMOVED***
  using U = typename std::remove_cv<
      typename std::remove_reference<ValueType>::type>::type;
  static_assert(std::is_constructible<ValueType, U>::value,
                "Invalid ValueType");
...
// Description at the declaration site (top of file).
template <typename T>
const T* any_cast(const any* operand) noexcept ***REMOVED***
...
// Description at the declaration site (top of file).
template <typename T>
T* any_cast(any* operand) noexcept ***REMOVED***
  return operand && operand->GetObjTypeId() == any::IdForType<T>()
             ? std::addressof(
