...
#define N 4
   ...
   ***REMOVED*** queue;
   ...
   /* Initialize *q. */
   void queue_init(queue *q) ***REMOVED***
     gpr_mu_init(&q->mu);
   ...
   ***REMOVED***
   ...
   void queue_destroy(queue *q) ***REMOVED***
     gpr_mu_destroy(&q->mu);
   ...
   ***REMOVED***
   ...
   void queue_append(queue *q, int x) ***REMOVED***
     gpr_mu_lock(&q->mu);
     ...
     while (q->length == N) ***REMOVED***
       gpr_cv_wait(&q->non_full, &q->mu, gpr_inf_future);
     ***REMOVED***
     if (q->length == 0) ***REMOVED***  /* Wake threads blocked in queue_remove(). */
   ...
   ***REMOVED***
   ...
   /* If it can be done without blocking, append x to *q and return non-zero.
      Otherwise return 0. */
   int queue_try_append(queue *q, int x) ***REMOVED***
     int result = 0;
     if (gpr_mu_trylock(&q->mu)) ***REMOVED***
       if (q->length != N) ***REMOVED***
         if (q->length == 0) ***REMOVED***  /* Wake threads blocked in queue_remove(). */
           gpr_cv_broadcast(&q->non_empty);
         ***REMOVED***
         q->elem[(q->head + q->length) % N] = x;
   ...
   ***REMOVED***
   ...
   int queue_remove(queue *q, int *head, gpr_timespec abs_deadline) ***REMOVED***
     int result = 0;
     gpr_mu_lock(&q->mu);
     /* To wait for a predicate with a deadline, loop on the negation of the
        ...
        lock. */
     while (q->length == 0 &&
            !gpr_cv_wait(&q->non_empty, &q->mu, abs_deadline)) ***REMOVED***
     ***REMOVED***
     if (q->length != 0) ***REMOVED***    /* Queue is non-empty. */
       result = 1;
       if (q->length == N) ***REMOVED***  /* Wake threads blocked in queue_append(). */
         gpr_cv_broadcast(&q->non_full);
       ***REMOVED***
       *head = q->elem[q->head];
       q->head = (q->head + 1) % N;
       q->length--;
     ***REMOVED*** /* else deadline exceeded */
     gpr_mu_unlock(&q->mu);
     return result;
   ***REMOVED***
#endif /* 0 */
...
class mu_guard ***REMOVED***
 public:
  mu_guard(gpr_mu* mu) : mu_(mu) ***REMOVED*** gpr_mu_lock(mu); ***REMOVED***
  ~mu_guard() ***REMOVED*** gpr_mu_unlock(mu_); ***REMOVED***
