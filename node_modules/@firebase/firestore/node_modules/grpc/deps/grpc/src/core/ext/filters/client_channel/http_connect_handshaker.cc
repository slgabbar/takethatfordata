...
typedef struct http_connect_handshaker ***REMOVED***
...
***REMOVED*** http_connect_handshaker;

// Unref and clean up handshaker.
static void http_connect_handshaker_unref(grpc_exec_ctx* exec_ctx,
                                          http_connect_handshaker* handshaker) ***REMOVED***
  if (gpr_unref(&handshaker->refcount)) ***REMOVED***
    gpr_mu_destroy(&handshaker->mu);
    if (handshaker->endpoint_to_destroy != nullptr) ***REMOVED***
      grpc_endpoint_destroy(exec_ctx, handshaker->endpoint_to_destroy);
    ***REMOVED***
    if (handshaker->read_buffer_to_destroy != nullptr) ***REMOVED***
...
static void cleanup_args_for_failure_locked(
    grpc_exec_ctx* exec_ctx, http_connect_handshaker* handshaker) ***REMOVED***
...
static void handshake_failed_locked(grpc_exec_ctx* exec_ctx,
  ...
  ***REMOVED***
  if (!handshaker->shutdown) ***REMOVED***
...
static void on_write_done(grpc_exec_ctx* exec_ctx, void* arg,
  ...
  if (error != GRPC_ERROR_NONE || handshaker->shutdown) ***REMOVED***
...
static void on_read_done(grpc_exec_ctx* exec_ctx, void* arg,
  ...
  ***REMOVED***
  ...
  for (size_t i = 0; i < handshaker->args->read_buffer->count; ++i) ***REMOVED***
    if (GRPC_SLICE_LENGTH(handshaker->args->read_buffer->slices[i]) > 0) ***REMOVED***
      ...
      if (error != GRPC_ERROR_NONE) ***REMOVED***
        ...
        goto done;
      ***REMOVED***
      if (handshaker->http_parser.state == GRPC_HTTP_BODY) ***REMOVED***
        // Remove the data we've already read from the read buffer,
        // leaving only the leftover bytes (if any).
        grpc_slice_buffer tmp_buffer;
        grpc_slice_buffer_init(&tmp_buffer);
        if (body_start_offset <
            GRPC_SLICE_LENGTH(handshaker->args->read_buffer->slices[i])) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (handshaker->http_response.status < 200 ||
      handshaker->http_response.status >= 300) ***REMOVED***
    char* msg;
...
static void http_connect_handshaker_destroy(grpc_exec_ctx* exec_ctx,
                                            grpc_handshaker* handshaker_in) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (arg != nullptr) ***REMOVED***
    ...
    headers = (grpc_http_header*)gpr_malloc(sizeof(grpc_http_header) *
                                            num_header_strings);
    for (size_t i = 0; i < num_header_strings; ++i) ***REMOVED***
      char* sep = strchr(header_strings[i], ':');
      if (sep == nullptr) ***REMOVED***
...
static const grpc_handshaker_vtable http_connect_handshaker_vtable = ***REMOVED***
    http_connect_handshaker_destroy, http_connect_handshaker_shutdown,
    http_connect_handshaker_do_handshake***REMOVED***;

static grpc_handshaker* grpc_http_connect_handshaker_create() ***REMOVED***
  http_connect_handshaker* handshaker =
...
static void handshaker_factory_destroy(grpc_exec_ctx* exec_ctx,
                                       grpc_handshaker_factory* factory) ***REMOVED******REMOVED***

static const grpc_handshaker_factory_vtable handshaker_factory_vtable = ***REMOVED***
    handshaker_factory_add_handshakers, handshaker_factory_destroy***REMOVED***;

static grpc_handshaker_factory handshaker_factory = ***REMOVED***
    &handshaker_factory_vtable***REMOVED***;

void grpc_http_connect_register_handshaker_factory() ***REMOVED***
