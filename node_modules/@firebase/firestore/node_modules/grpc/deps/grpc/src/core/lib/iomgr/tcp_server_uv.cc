...
struct grpc_tcp_listener ***REMOVED***
  uv_tcp_t* handle;
...
struct grpc_tcp_server ***REMOVED***
  gpr_refcount refs;
...
grpc_error* grpc_tcp_server_create(grpc_exec_ctx* exec_ctx,
  ...
  for (size_t i = 0; i < (args == NULL ? 0 : args->num_args); i++) ***REMOVED***
    if (0 == strcmp(GRPC_ARG_RESOURCE_QUOTA, args->args[i].key)) ***REMOVED***
      if (args->args[i].type == GRPC_ARG_POINTER) ***REMOVED***
...
static void handle_close_callback(uv_handle_t* handle) ***REMOVED***
  grpc_tcp_listener* sp = (grpc_tcp_listener*)handle->data;
  grpc_exec_ctx exec_ctx = GRPC_EXEC_CTX_INIT;
  sp->server->open_ports--;
  if (sp->server->open_ports == 0 && sp->server->shutdown) ***REMOVED***
    finish_shutdown(&exec_ctx, sp->server);
  ***REMOVED***
  grpc_exec_ctx_finish(&exec_ctx);
***REMOVED***

static void close_listener(grpc_tcp_listener* sp) ***REMOVED***
  if (!sp->closed) ***REMOVED***
    sp->closed = true;
...
static void finish_accept(grpc_exec_ctx* exec_ctx, grpc_tcp_listener* sp) ***REMOVED***
...
static void on_connect(uv_stream_t* server, int status) ***REMOVED***
  ...
  ***REMOVED***
  ...
  // Create acceptor.
  if (sp->server->on_accept_cb) ***REMOVED***
...
static grpc_error* add_addr_to_server(grpc_tcp_server* s,
                                      ...
                                      unsigned port_index,
                                      grpc_tcp_listener** listener) ***REMOVED***
  ...
  if (family == AF_INET || family == AF_INET6) ***REMOVED***
    int fd;
...
void grpc_tcp_server_start(grpc_exec_ctx* exec_ctx, grpc_tcp_server* server,
                           ...
                           grpc_tcp_server_cb on_accept_cb, void* cb_arg) ***REMOVED***
  grpc_tcp_listener* sp;
  (void)pollsets;
  (void)pollset_count;
  GRPC_UV_ASSERT_SAME_THREAD();
  ...
  ***REMOVED***
  ...
  for (sp = server->head; sp; sp = sp->next) ***REMOVED***
    if (sp->has_pending_connection) ***REMOVED***
      finish_accept(exec_ctx, sp);
...
void grpc_tcp_server_shutdown_listeners(grpc_exec_ctx* exec_ctx,
                                        grpc_tcp_server* s) ***REMOVED******REMOVED***
