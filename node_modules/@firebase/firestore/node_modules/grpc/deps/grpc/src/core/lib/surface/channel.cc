...
typedef struct registered_call ***REMOVED***
  grpc_mdelem path;
  grpc_mdelem authority;
  struct registered_call* next;
***REMOVED*** registered_call;

struct grpc_channel ***REMOVED***
  int is_client;
...
grpc_channel* grpc_channel_create_with_builder(
    ...
    grpc_channel_stack_type channel_stack_type) ***REMOVED***
  char* target = gpr_strdup(grpc_channel_stack_builder_get_target(builder));
  grpc_channel_args* args = grpc_channel_args_copy(
      grpc_channel_stack_builder_get_channel_arguments(builder));
  grpc_channel* channel;
  if (channel_stack_type == GRPC_SERVER_CHANNEL) ***REMOVED***
  ...
  ***REMOVED***
  ...
  for (size_t i = 0; i < args->num_args; i++) ***REMOVED***
    if (0 == strcmp(args->args[i].key, GRPC_ARG_DEFAULT_AUTHORITY)) ***REMOVED***
      if (args->args[i].type != GRPC_ARG_STRING) ***REMOVED***
        gpr_log(GPR_ERROR, "%s ignored: it must be a string",
                GRPC_ARG_DEFAULT_AUTHORITY);
      ***REMOVED*** else ***REMOVED***
        if (!GRPC_MDISNULL(channel->default_authority)) ***REMOVED***
      ...
      ***REMOVED***
    ***REMOVED*** else if (0 ==
               strcmp(args->args[i].key, GRPC_SSL_TARGET_NAME_OVERRIDE_ARG)) ***REMOVED***
      if (args->args[i].type != GRPC_ARG_STRING) ***REMOVED***
        gpr_log(GPR_ERROR, "%s ignored: it must be a string",
    ...
    ***REMOVED*** else if (0 == strcmp(args->args[i].key,
                           GRPC_COMPRESSION_CHANNEL_DEFAULT_LEVEL)) ***REMOVED***
      channel->compression_options.default_level.is_set = true;
      channel->compression_options.default_level.level =
          (grpc_compression_level)grpc_channel_arg_get_integer(
              &args->args[i],
              ***REMOVED***GRPC_COMPRESS_LEVEL_NONE, GRPC_COMPRESS_LEVEL_NONE,
               GRPC_COMPRESS_LEVEL_COUNT - 1***REMOVED***);
    ***REMOVED*** else if (0 == strcmp(args->args[i].key,
                           GRPC_STREAM_COMPRESSION_CHANNEL_DEFAULT_LEVEL)) ***REMOVED***
      ...
      channel->compression_options.default_stream_compression_level.level =
          (grpc_stream_compression_level)grpc_channel_arg_get_integer(
              &args->args[i],
              ***REMOVED***GRPC_STREAM_COMPRESS_LEVEL_NONE, GRPC_STREAM_COMPRESS_LEVEL_NONE,
               GRPC_STREAM_COMPRESS_LEVEL_COUNT - 1***REMOVED***);
    ***REMOVED*** else if (0 == strcmp(args->args[i].key,
                           GRPC_COMPRESSION_CHANNEL_DEFAULT_ALGORITHM)) ***REMOVED***
      channel->compression_options.default_algorithm.is_set = true;
      channel->compression_options.default_algorithm.algorithm =
          (grpc_compression_algorithm)grpc_channel_arg_get_integer(
              &args->args[i], ***REMOVED***GRPC_COMPRESS_NONE, GRPC_COMPRESS_NONE,
                               GRPC_COMPRESS_ALGORITHMS_COUNT - 1***REMOVED***);
    ***REMOVED*** else if (0 == strcmp(args->args[i].key,
                           GRPC_STREAM_COMPRESSION_CHANNEL_DEFAULT_ALGORITHM)) ***REMOVED***
      ...
      channel->compression_options.default_stream_compression_algorithm
          ...
          (grpc_stream_compression_algorithm)grpc_channel_arg_get_integer(
              &args->args[i],
              ***REMOVED***GRPC_STREAM_COMPRESS_NONE, GRPC_STREAM_COMPRESS_NONE,
               GRPC_STREAM_COMPRESS_ALGORITHMS_COUNT - 1***REMOVED***);
    ***REMOVED*** else if (0 ==
               strcmp(args->args[i].key,
                      GRPC_COMPRESSION_CHANNEL_ENABLED_ALGORITHMS_BITSET)) ***REMOVED***
    ...
    ***REMOVED*** else if (0 ==
               strcmp(
                   args->args[i].key,
                   GRPC_STREAM_COMPRESSION_CHANNEL_ENABLED_ALGORITHMS_BITSET)) ***REMOVED***
...
grpc_channel* grpc_channel_create(grpc_exec_ctx* exec_ctx, const char* target,
                                  ...
                                  grpc_transport* optional_transport) ***REMOVED***
  grpc_channel_stack_builder* builder = grpc_channel_stack_builder_create();
  grpc_channel_stack_builder_set_channel_arguments(exec_ctx, builder,
                                                   input_args);
  grpc_channel_stack_builder_set_target(builder, target);
  grpc_channel_stack_builder_set_transport(builder, optional_transport);
  if (!grpc_channel_init_create_stack(exec_ctx, builder, channel_stack_type)) ***REMOVED***
...
size_t grpc_channel_get_call_size_estimate(grpc_channel* channel) ***REMOVED***
...
void grpc_channel_update_call_size_estimate(grpc_channel* channel,
  ...
  ***REMOVED*** else if (cur == size) ***REMOVED***
    /* no change: holding pattern */
  ***REMOVED*** else if (cur > 0) ***REMOVED***
...
char* grpc_channel_get_target(grpc_channel* channel) ***REMOVED***
  GRPC_API_TRACE("grpc_channel_get_target(channel=%p)", 1, (channel));
  return gpr_strdup(channel->target);
***REMOVED***

void grpc_channel_get_info(grpc_channel* channel,
                           const grpc_channel_info* channel_info) ***REMOVED***
...
static grpc_call* grpc_channel_create_call_internal(
    ...
    grpc_mdelem authority_mdelem, grpc_millis deadline) ***REMOVED***
  ...
  if (!GRPC_MDISNULL(authority_mdelem)) ***REMOVED***
    send_metadata[num_metadata++] = authority_mdelem;
  ***REMOVED*** else if (!GRPC_MDISNULL(channel->default_authority)) ***REMOVED***
...
grpc_call* grpc_channel_create_pollset_set_call(
    ...
    const grpc_slice* host, grpc_millis deadline, void* reserved) ***REMOVED***
  GPR_ASSERT(!reserved);
...
void* grpc_channel_register_call(grpc_channel* channel, const char* method,
                                 const char* host, void* reserved) ***REMOVED***
...
static void destroy_channel(grpc_exec_ctx* exec_ctx, void* arg,
  ...
  while (channel->registered_calls) ***REMOVED***
...
void grpc_channel_destroy(grpc_channel* channel) ***REMOVED***
...
grpc_channel_stack* grpc_channel_get_channel_stack(grpc_channel* channel) ***REMOVED***
  return CHANNEL_STACK_FROM_CHANNEL(channel);
***REMOVED***

grpc_compression_options grpc_channel_compression_options(
    const grpc_channel* channel) ***REMOVED***
  return channel->compression_options;
***REMOVED***

grpc_mdelem grpc_channel_get_reffed_status_elem(grpc_exec_ctx* exec_ctx,
                                                grpc_channel* channel, int i) ***REMOVED***
