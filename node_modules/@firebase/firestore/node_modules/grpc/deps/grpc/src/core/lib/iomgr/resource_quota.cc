...
***REMOVED*** grpc_resource_user_link;
...
struct grpc_resource_user ***REMOVED***
...
struct grpc_resource_quota ***REMOVED***
  /* refcount */
  gpr_refcount refs;
...
static grpc_resource_user* rulist_pop_head(grpc_resource_quota* resource_quota,
  ...
  if (resource_user == nullptr) ***REMOVED***
    return nullptr;
  ***REMOVED***
  if (resource_user->links[list].next == resource_user) ***REMOVED***
    *root = nullptr;
  ***REMOVED*** else ***REMOVED***
    resource_user->links[list].next->links[list].prev =
...
static void rulist_remove(grpc_resource_user* resource_user, grpc_rulist list) ***REMOVED***
  if (resource_user->links[list].next == nullptr) return;
  grpc_resource_quota* resource_quota = resource_user->resource_quota;
  if (resource_quota->roots[list] == resource_user) ***REMOVED***
...
static void rq_step(grpc_exec_ctx* exec_ctx, void* rq, grpc_error* error) ***REMOVED***
  grpc_resource_quota* resource_quota = (grpc_resource_quota*)rq;
  resource_quota->step_scheduled = false;
  do ***REMOVED***
    if (rq_alloc(exec_ctx, resource_quota)) goto done;
  ***REMOVED*** while (rq_reclaim_from_per_user_free_pool(exec_ctx, resource_quota));

  if (!rq_reclaim(exec_ctx, resource_quota, false)) ***REMOVED***
...
static void rq_step_sched(grpc_exec_ctx* exec_ctx,
                          grpc_resource_quota* resource_quota) ***REMOVED***
...
static void rq_update_estimate(grpc_resource_quota* resource_quota) ***REMOVED***
  gpr_atm memory_usage_estimation = MEMORY_USAGE_ESTIMATION_MAX;
  if (resource_quota->size != 0) ***REMOVED***
...
static bool rq_alloc(grpc_exec_ctx* exec_ctx,
  ...
  while ((resource_user = rulist_pop_head(resource_quota,
                                          GRPC_RULIST_AWAITING_ALLOCATION))) ***REMOVED***
    gpr_mu_lock(&resource_user->mu);
    if (grpc_resource_quota_trace.enabled()) ***REMOVED***
      gpr_log(GPR_DEBUG,
    ...
    ***REMOVED***
    if (gpr_atm_no_barrier_load(&resource_user->shutdown)) ***REMOVED***
    ...
    ***REMOVED***
    if (resource_user->free_pool < 0 &&
        -resource_user->free_pool <= resource_quota->free_pool) ***REMOVED***
      ...
      ***REMOVED***
    ***REMOVED*** else if (grpc_resource_quota_trace.enabled() &&
               resource_user->free_pool >= 0) ***REMOVED***
      gpr_log(GPR_DEBUG, "RQ %s %s: discard already satisfied alloc request",
              resource_quota->name, resource_user->name);
    ***REMOVED***
    if (resource_user->free_pool >= 0) ***REMOVED***
      resource_user->allocating = false;
      resource_user->outstanding_allocations = 0;
      GRPC_CLOSURE_LIST_SCHED(exec_ctx, &resource_user->on_allocated);
      gpr_mu_unlock(&resource_user->mu);
...
static bool rq_reclaim_from_per_user_free_pool(
    grpc_exec_ctx* exec_ctx, grpc_resource_quota* resource_quota) ***REMOVED***
  grpc_resource_user* resource_user;
  while ((resource_user = rulist_pop_head(resource_quota,
                                          GRPC_RULIST_NON_EMPTY_FREE_POOL))) ***REMOVED***
    gpr_mu_lock(&resource_user->mu);
    if (resource_user->free_pool > 0) ***REMOVED***
      int64_t amt = resource_user->free_pool;
      resource_user->free_pool = 0;
...
static bool rq_reclaim(grpc_exec_ctx* exec_ctx,
                       grpc_resource_quota* resource_quota, bool destructive) ***REMOVED***
...
typedef struct ***REMOVED***
  ...
  grpc_resource_user* resource_user;
  size_t size;
***REMOVED*** ru_slice_refcount;

static void ru_slice_ref(void* p) ***REMOVED***
  ru_slice_refcount* rc = (ru_slice_refcount*)p;
  gpr_ref(&rc->refs);
***REMOVED***

static void ru_slice_unref(grpc_exec_ctx* exec_ctx, void* p) ***REMOVED***
  ru_slice_refcount* rc = (ru_slice_refcount*)p;
  if (gpr_unref(&rc->refs)) ***REMOVED***
    grpc_resource_user_free(exec_ctx, rc->resource_user, rc->size);
    gpr_free(rc);
  ***REMOVED***
***REMOVED***

static const grpc_slice_refcount_vtable ru_slice_vtable = ***REMOVED***
...
static void ru_allocate(grpc_exec_ctx* exec_ctx, void* ru, grpc_error* error) ***REMOVED***
...
static bool ru_post_reclaimer(grpc_exec_ctx* exec_ctx,
  ...
  if (gpr_atm_acq_load(&resource_user->shutdown) > 0) ***REMOVED***
...
static void ru_post_benign_reclaimer(grpc_exec_ctx* exec_ctx, void* ru,
  ...
  if (!rulist_empty(resource_user->resource_quota,
      ...
      rulist_empty(resource_user->resource_quota,
                   GRPC_RULIST_NON_EMPTY_FREE_POOL) &&
      rulist_empty(resource_user->resource_quota,
                   GRPC_RULIST_RECLAIMER_BENIGN)) ***REMOVED***
    rq_step_sched(exec_ctx, resource_user->resource_quota);
  ***REMOVED***
...
static void ru_post_destructive_reclaimer(grpc_exec_ctx* exec_ctx, void* ru,
  ...
  if (!rulist_empty(resource_user->resource_quota,
      ...
      rulist_empty(resource_user->resource_quota,
                   GRPC_RULIST_RECLAIMER_BENIGN) &&
      rulist_empty(resource_user->resource_quota,
                   GRPC_RULIST_RECLAIMER_DESTRUCTIVE)) ***REMOVED***
    rq_step_sched(exec_ctx, resource_user->resource_quota);
  ***REMOVED***
  rulist_add_tail(resource_user, GRPC_RULIST_RECLAIMER_DESTRUCTIVE);
***REMOVED***

static void ru_shutdown(grpc_exec_ctx* exec_ctx, void* ru, grpc_error* error) ***REMOVED***
  if (grpc_resource_quota_trace.enabled()) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (resource_user->allocating) ***REMOVED***
    rq_step_sched(exec_ctx, resource_user->resource_quota);
  ***REMOVED***
***REMOVED***

static void ru_destroy(grpc_exec_ctx* exec_ctx, void* ru, grpc_error* error) ***REMOVED***
  grpc_resource_user* resource_user = (grpc_resource_user*)ru;
  GPR_ASSERT(gpr_atm_no_barrier_load(&resource_user->refs) == 0);
  for (int i = 0; i < GRPC_RULIST_COUNT; i++) ***REMOVED***
...
static void ru_allocated_slices(grpc_exec_ctx* exec_ctx, void* arg,
  ...
  if (error == GRPC_ERROR_NONE) ***REMOVED***
    for (size_t i = 0; i < slice_allocator->count; i++) ***REMOVED***
...
***REMOVED*** rq_resize_args;

static void rq_resize(grpc_exec_ctx* exec_ctx, void* args, grpc_error* error) ***REMOVED***
...
/* Public API */
grpc_resource_quota* grpc_resource_quota_create(const char* name) ***REMOVED***
  grpc_resource_quota* resource_quota =
...
void grpc_resource_quota_unref_internal(grpc_exec_ctx* exec_ctx,
                                        grpc_resource_quota* resource_quota) ***REMOVED***
  if (gpr_unref(&resource_quota->refs)) ***REMOVED***
...
/* Public API */
void grpc_resource_quota_unref(grpc_resource_quota* resource_quota) ***REMOVED***
...
/* Public API */
void grpc_resource_quota_ref(grpc_resource_quota* resource_quota) ***REMOVED***
  grpc_resource_quota_ref_internal(resource_quota);
***REMOVED***
...
size_t grpc_resource_quota_peek_size(grpc_resource_quota* resource_quota) ***REMOVED***
...
grpc_resource_quota* grpc_resource_quota_from_channel_args(
    const grpc_channel_args* channel_args) ***REMOVED***
  for (size_t i = 0; i < channel_args->num_args; i++) ***REMOVED***
    if (0 == strcmp(channel_args->args[i].key, GRPC_ARG_RESOURCE_QUOTA)) ***REMOVED***
      if (channel_args->args[i].type == GRPC_ARG_POINTER) ***REMOVED***
...
static void* rq_copy(void* rq) ***REMOVED***
  grpc_resource_quota_ref((grpc_resource_quota*)rq);
  return rq;
***REMOVED***

static void rq_destroy(grpc_exec_ctx* exec_ctx, void* rq) ***REMOVED***
  grpc_resource_quota_unref_internal(exec_ctx, (grpc_resource_quota*)rq);
***REMOVED***

static int rq_cmp(void* a, void* b) ***REMOVED*** return GPR_ICMP(a, b); ***REMOVED***

const grpc_arg_pointer_vtable* grpc_resource_quota_arg_vtable(void) ***REMOVED***
  static const grpc_arg_pointer_vtable vtable = ***REMOVED***rq_copy, rq_destroy, rq_cmp***REMOVED***;
  return &vtable;
***REMOVED***
...
grpc_resource_user* grpc_resource_user_create(
    grpc_resource_quota* resource_quota, const char* name) ***REMOVED***
  grpc_resource_user* resource_user =
...
grpc_resource_quota* grpc_resource_user_quota(
    grpc_resource_user* resource_user) ***REMOVED***
  return resource_user->resource_quota;
***REMOVED***

static void ru_ref_by(grpc_resource_user* resource_user, gpr_atm amount) ***REMOVED***
  GPR_ASSERT(amount > 0);
  GPR_ASSERT(gpr_atm_no_barrier_fetch_add(&resource_user->refs, amount) != 0);
***REMOVED***

static void ru_unref_by(grpc_exec_ctx* exec_ctx,
                        grpc_resource_user* resource_user, gpr_atm amount) ***REMOVED***
  GPR_ASSERT(amount > 0);
  gpr_atm old = gpr_atm_full_fetch_add(&resource_user->refs, -amount);
  GPR_ASSERT(old >= amount);
  if (old == amount) ***REMOVED***
    GRPC_CLOSURE_SCHED(exec_ctx, &resource_user->destroy_closure,
                       GRPC_ERROR_NONE);
  ***REMOVED***
***REMOVED***

void grpc_resource_user_ref(grpc_resource_user* resource_user) ***REMOVED***
...
void grpc_resource_user_shutdown(grpc_exec_ctx* exec_ctx,
                                 grpc_resource_user* resource_user) ***REMOVED***
  if (gpr_atm_full_fetch_add(&resource_user->shutdown, 1) == 0) ***REMOVED***
    GRPC_CLOSURE_SCHED(
        exec_ctx,
        GRPC_CLOSURE_CREATE(
...
void grpc_resource_user_alloc(grpc_exec_ctx* exec_ctx,
                              ...
                              grpc_closure* optional_on_done) ***REMOVED***
  gpr_mu_lock(&resource_user->mu);
...
void grpc_resource_user_free(grpc_exec_ctx* exec_ctx,
                             grpc_resource_user* resource_user, size_t size) ***REMOVED***
  gpr_mu_lock(&resource_user->mu);
  ...
  ***REMOVED***
  ...
  if (is_bigger_than_zero && was_zero_or_negative &&
      !resource_user->added_to_free_pool) ***REMOVED***
...
void grpc_resource_user_slice_allocator_init(
    grpc_resource_user_slice_allocator* slice_allocator,
    grpc_resource_user* resource_user, grpc_iomgr_cb_func cb, void* p) ***REMOVED***
...
void grpc_resource_user_alloc_slices(
    ...
    size_t count, grpc_slice_buffer* dest) ***REMOVED***
