...
typedef struct fd_node ***REMOVED***
...
struct grpc_ares_ev_driver ***REMOVED***
...
static grpc_ares_ev_driver* grpc_ares_ev_driver_ref(
    grpc_ares_ev_driver* ev_driver) ***REMOVED***
  gpr_log(GPR_DEBUG, "Ref ev_driver %" PRIuPTR, (uintptr_t)ev_driver);
  gpr_ref(&ev_driver->refs);
  return ev_driver;
***REMOVED***

static void grpc_ares_ev_driver_unref(grpc_ares_ev_driver* ev_driver) ***REMOVED***
  gpr_log(GPR_DEBUG, "Unref ev_driver %" PRIuPTR, (uintptr_t)ev_driver);
  if (gpr_unref(&ev_driver->refs)) ***REMOVED***
...
static void fd_node_destroy(grpc_exec_ctx* exec_ctx, fd_node* fdn) ***REMOVED***
...
static void fd_node_shutdown(grpc_exec_ctx* exec_ctx, fd_node* fdn) ***REMOVED***
  gpr_mu_lock(&fdn->mu);
  fdn->shutting_down = true;
  if (!fdn->readable_registered && !fdn->writable_registered) ***REMOVED***
...
void grpc_ares_ev_driver_destroy(grpc_ares_ev_driver* ev_driver) ***REMOVED***
...
static fd_node* pop_fd_node(fd_node** head, int fd) ***REMOVED***
  fd_node dummy_head;
  dummy_head.next = *head;
  fd_node* node = &dummy_head;
  while (node->next != nullptr) ***REMOVED***
    if (grpc_fd_wrapped_fd(node->next->fd) == fd) ***REMOVED***
...
static void on_readable_cb(grpc_exec_ctx* exec_ctx, void* arg,
  ...
  if (fdn->shutting_down && !fdn->writable_registered) ***REMOVED***
    gpr_mu_unlock(&fdn->mu);
    fd_node_destroy(exec_ctx, fdn);
    grpc_ares_ev_driver_unref(ev_driver);
    return;
  ***REMOVED***
  ...
  if (error == GRPC_ERROR_NONE) ***REMOVED***
    ...
    ***REMOVED*** while (grpc_ares_is_fd_still_readable(ev_driver, fd));
  ***REMOVED*** else ***REMOVED***
...
static void on_writable_cb(grpc_exec_ctx* exec_ctx, void* arg,
  ...
  if (fdn->shutting_down && !fdn->readable_registered) ***REMOVED***
    gpr_mu_unlock(&fdn->mu);
    fd_node_destroy(exec_ctx, fdn);
    grpc_ares_ev_driver_unref(ev_driver);
    return;
...
ares_channel* grpc_ares_ev_driver_get_channel(grpc_ares_ev_driver* ev_driver) ***REMOVED***
...
static void grpc_ares_notify_on_event_locked(grpc_exec_ctx* exec_ctx,
  ...
  if (!ev_driver->shutting_down) ***REMOVED***
    ares_socket_t socks[ARES_GETSOCK_MAXNUM];
    int socks_bitmask =
        ares_getsock(ev_driver->channel, socks, ARES_GETSOCK_MAXNUM);
    for (size_t i = 0; i < ARES_GETSOCK_MAXNUM; i++) ***REMOVED***
      if (ARES_GETSOCK_READABLE(socks_bitmask, i) ||
          ARES_GETSOCK_WRITABLE(socks_bitmask, i)) ***REMOVED***
        fd_node* fdn = pop_fd_node(&ev_driver->fds, socks[i]);
        // Create a new fd_node if sock[i] is not in the fd_node list.
        if (fdn == nullptr) ***REMOVED***
          char* fd_name;
        ...
        ***REMOVED***
        ...
        if (ARES_GETSOCK_READABLE(socks_bitmask, i) &&
            !fdn->readable_registered) ***REMOVED***
        ...
        ***REMOVED***
        ...
        if (ARES_GETSOCK_WRITABLE(socks_bitmask, i) &&
            !fdn->writable_registered) ***REMOVED***
  ...
  ***REMOVED***
  ...
  // the list.
  while (ev_driver->fds != nullptr) ***REMOVED***
