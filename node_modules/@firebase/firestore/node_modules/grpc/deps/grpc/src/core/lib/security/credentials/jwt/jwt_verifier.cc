...
const char* grpc_jwt_verifier_status_to_string(
    grpc_jwt_verifier_status status) ***REMOVED***
...
static const EVP_MD* evp_md_from_alg(const char* alg) ***REMOVED***
  if (strcmp(alg, "RS256") == 0) ***REMOVED***
    return EVP_sha256();
  ***REMOVED*** else if (strcmp(alg, "RS384") == 0) ***REMOVED***
    return EVP_sha384();
  ***REMOVED*** else if (strcmp(alg, "RS512") == 0) ***REMOVED***
    return EVP_sha512();
...
static const char* validate_string_field(const grpc_json* json,
  ...
  if (json->type != GRPC_JSON_STRING) ***REMOVED***
...
static gpr_timespec validate_time_field(const grpc_json* json,
  ...
  if (json->type != GRPC_JSON_NUMBER) ***REMOVED***
    gpr_log(GPR_ERROR, "Invalid %s field [%s]", key, json->value);
    return result;
  ***REMOVED***
...
static void jose_header_destroy(grpc_exec_ctx* exec_ctx, jose_header* h) ***REMOVED***
...
static jose_header* jose_header_from_json(grpc_exec_ctx* exec_ctx,
                                          grpc_json* json, grpc_slice buffer) ***REMOVED***
  grpc_json* cur;
  jose_header* h = (jose_header*)gpr_zalloc(sizeof(jose_header));
  h->buffer = buffer;
  for (cur = json->child; cur != nullptr; cur = cur->next) ***REMOVED***
    if (strcmp(cur->key, "alg") == 0) ***REMOVED***
      /* We only support RSA-1.5 signatures for now.
         Beware of this if we add HMAC support:
         https://auth0.com/blog/2015/03/31/critical-vulnerabilities-in-json-web-token-libraries/
       */
      if (cur->type != GRPC_JSON_STRING || strncmp(cur->value, "RS", 2) ||
          evp_md_from_alg(cur->value) == nullptr) ***REMOVED***
        gpr_log(GPR_ERROR, "Invalid alg field [%s]", cur->value);
        goto error;
      ***REMOVED***
      h->alg = cur->value;
    ***REMOVED*** else if (strcmp(cur->key, "typ") == 0) ***REMOVED***
      h->typ = validate_string_field(cur, "typ");
      if (h->typ == nullptr) goto error;
    ***REMOVED*** else if (strcmp(cur->key, "kid") == 0) ***REMOVED***
      h->kid = validate_string_field(cur, "kid");
      if (h->kid == nullptr) goto error;
    ***REMOVED***
  ***REMOVED***
  if (h->alg == nullptr) ***REMOVED***
...
struct grpc_jwt_claims ***REMOVED***
...
void grpc_jwt_claims_destroy(grpc_exec_ctx* exec_ctx, grpc_jwt_claims* claims) ***REMOVED***
  grpc_json_destroy(claims->json);
  grpc_slice_unref_internal(exec_ctx, claims->buffer);
  gpr_free(claims);
***REMOVED***

const grpc_json* grpc_jwt_claims_json(const grpc_jwt_claims* claims) ***REMOVED***
  if (claims == nullptr) return nullptr;
  return claims->json;
***REMOVED***

const char* grpc_jwt_claims_subject(const grpc_jwt_claims* claims) ***REMOVED***
  if (claims == nullptr) return nullptr;
  return claims->sub;
***REMOVED***

const char* grpc_jwt_claims_issuer(const grpc_jwt_claims* claims) ***REMOVED***
  if (claims == nullptr) return nullptr;
  return claims->iss;
***REMOVED***

const char* grpc_jwt_claims_id(const grpc_jwt_claims* claims) ***REMOVED***
  if (claims == nullptr) return nullptr;
  return claims->jti;
***REMOVED***

const char* grpc_jwt_claims_audience(const grpc_jwt_claims* claims) ***REMOVED***
  if (claims == nullptr) return nullptr;
  return claims->aud;
***REMOVED***

gpr_timespec grpc_jwt_claims_issued_at(const grpc_jwt_claims* claims) ***REMOVED***
  if (claims == nullptr) return gpr_inf_past(GPR_CLOCK_REALTIME);
  return claims->iat;
***REMOVED***

gpr_timespec grpc_jwt_claims_expires_at(const grpc_jwt_claims* claims) ***REMOVED***
  if (claims == nullptr) return gpr_inf_future(GPR_CLOCK_REALTIME);
  return claims->exp;
***REMOVED***

gpr_timespec grpc_jwt_claims_not_before(const grpc_jwt_claims* claims) ***REMOVED***
  if (claims == nullptr) return gpr_inf_past(GPR_CLOCK_REALTIME);
  return claims->nbf;
***REMOVED***
...
grpc_jwt_verifier_status grpc_jwt_claims_check(const grpc_jwt_claims* claims,
  ...
  if (gpr_time_cmp(skewed_now, claims->nbf) < 0) ***REMOVED***
    gpr_log(GPR_ERROR, "JWT is not valid yet.");
    return GRPC_JWT_VERIFIER_TIME_CONSTRAINT_FAILURE;
  ***REMOVED***
  skewed_now =
      gpr_time_sub(gpr_now(GPR_CLOCK_REALTIME), grpc_jwt_verifier_clock_skew);
  if (gpr_time_cmp(skewed_now, claims->exp) > 0) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (grpc_jwt_issuer_email_domain(claims->iss) != nullptr &&
      claims->sub != nullptr && strcmp(claims->iss, claims->sub) != 0) ***REMOVED***
    gpr_log(GPR_ERROR,
...
static verifier_cb_ctx* verifier_cb_ctx_create(
    ...
    grpc_jwt_verification_done_cb cb) ***REMOVED***
...
void verifier_cb_ctx_destroy(grpc_exec_ctx* exec_ctx, verifier_cb_ctx* ctx) ***REMOVED***
  ...
  for (size_t i = 0; i < HTTP_RESPONSE_COUNT; i++) ***REMOVED***
...
gpr_timespec grpc_jwt_verifier_clock_skew = ***REMOVED***60, 0, GPR_TIMESPAN***REMOVED***;
...
typedef struct ***REMOVED***
  char* email_domain;
  char* key_url_prefix;
***REMOVED*** email_key_mapping;

struct grpc_jwt_verifier ***REMOVED***
...
static grpc_json* json_from_http(const grpc_httpcli_response* response) ***REMOVED***
  grpc_json* json = nullptr;

  if (response == nullptr) ***REMOVED***
...
static EVP_PKEY* extract_pkey_from_x509(const char* x509_str) ***REMOVED***
  X509* x509 = nullptr;
...
static BIGNUM* bignum_from_base64(grpc_exec_ctx* exec_ctx, const char* b64) ***REMOVED***
  BIGNUM* result = nullptr;
  grpc_slice bin;

  if (b64 == nullptr) return nullptr;
  bin = grpc_base64_decode(exec_ctx, b64, 1);
  if (GRPC_SLICE_IS_EMPTY(bin)) ***REMOVED***
...
static int RSA_set0_key(RSA* r, BIGNUM* n, BIGNUM* e, BIGNUM* d) ***REMOVED***
  /* If the fields n and e in r are NULL, the corresponding input
   * parameters MUST be non-NULL for n and e.  d may be
   * left NULL (in case only the public key is used).
   */
  if ((r->n == nullptr && n == nullptr) || (r->e == nullptr && e == nullptr)) ***REMOVED***
    return 0;
  ***REMOVED***

  if (n != nullptr) ***REMOVED***
    BN_free(r->n);
    r->n = n;
  ***REMOVED***
  if (e != nullptr) ***REMOVED***
    BN_free(r->e);
    r->e = e;
  ***REMOVED***
  if (d != nullptr) ***REMOVED***
    BN_free(r->d);
    r->d = d;
  ***REMOVED***
...
static EVP_PKEY* pkey_from_jwk(grpc_exec_ctx* exec_ctx, const grpc_json* json,
  ...
  if (strcmp(kty, "RSA") != 0) ***REMOVED***
    gpr_log(GPR_ERROR, "Unsupported key type %s.", kty);
    goto end;
  ***REMOVED***
  rsa = RSA_new();
  if (rsa == nullptr) ***REMOVED***
    gpr_log(GPR_ERROR, "Could not create rsa key.");
    goto end;
  ***REMOVED***
  for (key_prop = json->child; key_prop != nullptr; key_prop = key_prop->next) ***REMOVED***
    if (strcmp(key_prop->key, "n") == 0) ***REMOVED***
      tmp_n =
          bignum_from_base64(exec_ctx, validate_string_field(key_prop, "n"));
      if (tmp_n == nullptr) goto end;
    ***REMOVED*** else if (strcmp(key_prop->key, "e") == 0) ***REMOVED***
      tmp_e =
          bignum_from_base64(exec_ctx, validate_string_field(key_prop, "e"));
      if (tmp_e == nullptr) goto end;
    ***REMOVED***
  ***REMOVED***
  if (tmp_e == nullptr || tmp_n == nullptr) ***REMOVED***
    gpr_log(GPR_ERROR, "Missing RSA public key field.");
    goto end;
  ***REMOVED***
  if (!RSA_set0_key(rsa, tmp_n, tmp_e, nullptr)) ***REMOVED***
...
static EVP_PKEY* find_verification_key(grpc_exec_ctx* exec_ctx,
                                       ...
                                       const char* header_kid) ***REMOVED***
  const grpc_json* jkey;
  const grpc_json* jwk_keys;
  /* Try to parse the json as a JWK set:
     https://tools.ietf.org/html/rfc7517#section-5. */
  jwk_keys = find_property_by_name(json, "keys");
  if (jwk_keys == nullptr) ***REMOVED***
    /* Use the google proprietary format which is:
       ***REMOVED*** <kid1>: <x5091>, <kid2>: <x5092>, ... ***REMOVED*** */
    const grpc_json* cur = find_property_by_name(json, header_kid);
    if (cur == nullptr) return nullptr;
    return extract_pkey_from_x509(cur->value);
  ***REMOVED***

  if (jwk_keys->type != GRPC_JSON_ARRAY) ***REMOVED***
    gpr_log(GPR_ERROR,
  ...
  ***REMOVED***
  ...
  for (jkey = jwk_keys->child; jkey != nullptr; jkey = jkey->next) ***REMOVED***
    grpc_json* key_prop;
    ...
    for (key_prop = jkey->child; key_prop != nullptr;
         key_prop = key_prop->next) ***REMOVED***
      if (strcmp(key_prop->key, "alg") == 0 &&
          key_prop->type == GRPC_JSON_STRING) ***REMOVED***
        alg = key_prop->value;
      ***REMOVED*** else if (strcmp(key_prop->key, "kid") == 0 &&
                 key_prop->type == GRPC_JSON_STRING) ***REMOVED***
        kid = key_prop->value;
      ***REMOVED*** else if (strcmp(key_prop->key, "kty") == 0 &&
                 key_prop->type == GRPC_JSON_STRING) ***REMOVED***
        kty = key_prop->value;
      ***REMOVED***
    ***REMOVED***
    if (alg != nullptr && kid != nullptr && kty != nullptr &&
        strcmp(kid, header_kid) == 0 && strcmp(alg, header_alg) == 0) ***REMOVED***
...
static int verify_jwt_signature(EVP_PKEY* key, const char* alg,
                                grpc_slice signature, grpc_slice signed_data) ***REMOVED***
  EVP_MD_CTX* md_ctx = EVP_MD_CTX_create();
  const EVP_MD* md = evp_md_from_alg(alg);
  int result = 0;

  GPR_ASSERT(md != nullptr); /* Checked before. */
  if (md_ctx == nullptr) ***REMOVED***
    gpr_log(GPR_ERROR, "Could not create EVP_MD_CTX.");
    goto end;
  ***REMOVED***
  if (EVP_DigestVerifyInit(md_ctx, nullptr, md, nullptr, key) != 1) ***REMOVED***
    gpr_log(GPR_ERROR, "EVP_DigestVerifyInit failed.");
    goto end;
  ***REMOVED***
  if (EVP_DigestVerifyUpdate(md_ctx, GRPC_SLICE_START_PTR(signed_data),
                             GRPC_SLICE_LENGTH(signed_data)) != 1) ***REMOVED***
    gpr_log(GPR_ERROR, "EVP_DigestVerifyUpdate failed.");
    goto end;
  ***REMOVED***
  if (EVP_DigestVerifyFinal(md_ctx, GRPC_SLICE_START_PTR(signature),
                            GRPC_SLICE_LENGTH(signature)) != 1) ***REMOVED***
...
static void on_keys_retrieved(grpc_exec_ctx* exec_ctx, void* user_data,
  ...
  ***REMOVED***
  ...
  if (verification_key == nullptr) ***REMOVED***
...
static void on_openid_config_retrieved(grpc_exec_ctx* exec_ctx, void* user_data,
  ...
  if (cur == nullptr) ***REMOVED***
    gpr_log(GPR_ERROR, "Could not find jwks_uri in openid config.");
    goto error;
  ***REMOVED***
  jwks_uri = validate_string_field(cur, "jwks_uri");
  if (jwks_uri == nullptr) goto error;
  if (strstr(jwks_uri, "https://") != jwks_uri) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (req.http.path == nullptr) ***REMOVED***
    req.http.path = (char*)"";
  ***REMOVED*** else ***REMOVED***
...
static email_key_mapping* verifier_get_mapping(grpc_jwt_verifier* v,
                                               const char* email_domain) ***REMOVED***
  size_t i;
  if (v->mappings == nullptr) return nullptr;
  for (i = 0; i < v->num_mappings; i++) ***REMOVED***
    if (strcmp(email_domain, v->mappings[i].email_domain) == 0) ***REMOVED***
      return &v->mappings[i];
    ***REMOVED***
  ***REMOVED***
  return nullptr;
...
static void verifier_put_mapping(grpc_jwt_verifier* v, const char* email_domain,
                                 const char* key_url_prefix) ***REMOVED***
  email_key_mapping* mapping = verifier_get_mapping(v, email_domain);
  GPR_ASSERT(v->num_mappings < v->allocated_mappings);
  if (mapping != nullptr) ***REMOVED***
...
/* Very non-sophisticated way to detect an email address. Should be good
   enough for now... */
const char* grpc_jwt_issuer_email_domain(const char* issuer) ***REMOVED***
...
static void retrieve_key_and_verify(grpc_exec_ctx* exec_ctx,
  ...
  GPR_ASSERT(ctx != nullptr && ctx->header != nullptr &&
             ctx->claims != nullptr);
  iss = ctx->claims->iss;
  if (ctx->header->kid == nullptr) ***REMOVED***
    gpr_log(GPR_ERROR, "Missing kid in jose header.");
    goto error;
  ***REMOVED***
  if (iss == nullptr) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (email_domain != nullptr) ***REMOVED***
    ...
    ***REMOVED***
    ...
    if (path_prefix == nullptr) ***REMOVED***
...
grpc_jwt_verifier* grpc_jwt_verifier_create(
  ...
  if (mappings != nullptr) ***REMOVED***
    size_t i;
    for (i = 0; i < num_mappings; i++) ***REMOVED***
...
void grpc_jwt_verifier_destroy(grpc_exec_ctx* exec_ctx, grpc_jwt_verifier* v) ***REMOVED***
  size_t i;
  if (v == nullptr) return;
  grpc_httpcli_context_destroy(exec_ctx, &v->http_ctx);
  if (v->mappings != nullptr) ***REMOVED***
    for (i = 0; i < v->num_mappings; i++) ***REMOVED***
