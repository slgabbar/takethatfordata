...
static void assert_valid_list(grpc_mdelem_list* list) ***REMOVED***
#ifndef NDEBUG
  ...
  for (l = list->head; l; l = l->next) ***REMOVED***
...
static void assert_valid_callouts(grpc_exec_ctx* exec_ctx,
                                  grpc_metadata_batch* batch) ***REMOVED***
#ifndef NDEBUG
  for (grpc_linked_mdelem* l = batch->list.head; l != nullptr; l = l->next) ***REMOVED***
    grpc_slice key_interned = grpc_slice_intern(GRPC_MDKEY(l->md));
    grpc_metadata_batch_callouts_index callout_idx =
        GRPC_BATCH_INDEX_OF(key_interned);
    if (callout_idx != GRPC_BATCH_CALLOUTS_COUNT) ***REMOVED***
...
#ifndef NDEBUG
void grpc_metadata_batch_assert_ok(grpc_metadata_batch* batch) ***REMOVED***
  assert_valid_list(&batch->list);
***REMOVED***
#endif /* NDEBUG */

void grpc_metadata_batch_init(grpc_metadata_batch* batch) ***REMOVED***
...
void grpc_metadata_batch_destroy(grpc_exec_ctx* exec_ctx,
                                 grpc_metadata_batch* batch) ***REMOVED***
  grpc_linked_mdelem* l;
  for (l = batch->list.head; l; l = l->next) ***REMOVED***
    GRPC_MDELEM_UNREF(exec_ctx, l->md);
  ***REMOVED***
***REMOVED***

grpc_error* grpc_attach_md_to_error(grpc_error* src, grpc_mdelem md) ***REMOVED***
...
static grpc_error* maybe_link_callout(grpc_metadata_batch* batch,
                                      grpc_linked_mdelem* storage) ***REMOVED***
  grpc_metadata_batch_callouts_index idx =
      GRPC_BATCH_INDEX_OF(GRPC_MDKEY(storage->md));
  if (idx == GRPC_BATCH_CALLOUTS_COUNT) ***REMOVED***
...
static void link_tail(grpc_mdelem_list* list, grpc_linked_mdelem* storage) ***REMOVED***
  assert_valid_list(list);
  GPR_ASSERT(!GRPC_MDISNULL(storage->md));
  storage->prev = list->tail;
  storage->next = nullptr;
  storage->reserved = nullptr;
  if (list->tail != nullptr) ***REMOVED***
...
static void unlink_storage(grpc_mdelem_list* list,
                           grpc_linked_mdelem* storage) ***REMOVED***
  assert_valid_list(list);
  if (storage->prev != nullptr) ***REMOVED***
    storage->prev->next = storage->next;
  ***REMOVED*** else ***REMOVED***
    list->head = storage->next;
  ***REMOVED***
  if (storage->next != nullptr) ***REMOVED***
...
grpc_error* grpc_metadata_batch_substitute(grpc_exec_ctx* exec_ctx,
                                           grpc_metadata_batch* batch,
                                           grpc_linked_mdelem* storage,
                                           grpc_mdelem new_mdelem) ***REMOVED***
  assert_valid_callouts(exec_ctx, batch);
  grpc_error* error = GRPC_ERROR_NONE;
  grpc_mdelem old_mdelem = storage->md;
  if (!grpc_slice_eq(GRPC_MDKEY(new_mdelem), GRPC_MDKEY(old_mdelem))) ***REMOVED***
    maybe_unlink_callout(batch, storage);
    storage->md = new_mdelem;
...
bool grpc_metadata_batch_is_empty(grpc_metadata_batch* batch) ***REMOVED***
  return batch->list.head == nullptr &&
         batch->deadline == GRPC_MILLIS_INF_FUTURE;
***REMOVED***

size_t grpc_metadata_batch_size(grpc_metadata_batch* batch) ***REMOVED***
  size_t size = 0;
  for (grpc_linked_mdelem* elem = batch->list.head; elem != nullptr;
       elem = elem->next) ***REMOVED***
...
static void add_error(grpc_error** composite, grpc_error* error,
                      const char* composite_error_string) ***REMOVED***
...
grpc_error* grpc_metadata_batch_filter(grpc_exec_ctx* exec_ctx,
  ...
  while (l) ***REMOVED***
    ...
    if (GRPC_MDISNULL(new_mdelem.md)) ***REMOVED***
      grpc_metadata_batch_remove(exec_ctx, batch, l);
    ***REMOVED*** else if (new_mdelem.md.payload != l->md.payload) ***REMOVED***
