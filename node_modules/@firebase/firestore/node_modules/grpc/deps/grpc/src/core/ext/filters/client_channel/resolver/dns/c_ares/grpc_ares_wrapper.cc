...
struct grpc_ares_request ***REMOVED***
...
typedef struct grpc_ares_hostbyname_request ***REMOVED***
  ...
  bool is_balancer;
***REMOVED*** grpc_ares_hostbyname_request;

static void do_basic_init(void) ***REMOVED*** gpr_mu_init(&g_init_mu); ***REMOVED***

static uint16_t strhtons(const char* port) ***REMOVED***
  if (strcmp(port, "http") == 0) ***REMOVED***
    return htons(80);
  ***REMOVED*** else if (strcmp(port, "https") == 0) ***REMOVED***
    return htons(443);
  ***REMOVED***
  return htons((unsigned short)atoi(port));
***REMOVED***

static void grpc_ares_request_ref(grpc_ares_request* r) ***REMOVED***
  gpr_ref(&r->pending_queries);
***REMOVED***

static void grpc_ares_request_unref(grpc_exec_ctx* exec_ctx,
                                    grpc_ares_request* r) ***REMOVED***
  /* If there are no pending queries, invoke on_done callback and destroy the
     request */
  if (gpr_unref(&r->pending_queries)) ***REMOVED***
    /* TODO(zyc): Sort results with RFC6724 before invoking on_done. */
    if (exec_ctx == nullptr) ***REMOVED***
...
static void destroy_hostbyname_request(grpc_exec_ctx* exec_ctx,
                                       grpc_ares_hostbyname_request* hr) ***REMOVED***
...
static void on_hostbyname_done_cb(void* arg, int status, int timeouts,
  ...
  if (status == ARES_SUCCESS) ***REMOVED***
    ...
    ***REMOVED***
    ...
    for (i = prev_naddr; i < (*lb_addresses)->num_addresses; i++) ***REMOVED***
      switch (hostent->h_addrtype) ***REMOVED***
        case AF_INET6: ***REMOVED***
      ...
      ***REMOVED***
    ***REMOVED***
  ***REMOVED*** else if (!r->success) ***REMOVED***
    char* error_msg;
    gpr_asprintf(&error_msg, "C-ares status is not ARES_SUCCESS: %s",
                 ares_strerror(status));
    grpc_error* error = GRPC_ERROR_CREATE_FROM_COPIED_STRING(error_msg);
    gpr_free(error_msg);
    if (r->error == GRPC_ERROR_NONE) ***REMOVED***
      r->error = error;
    ***REMOVED*** else ***REMOVED***
...
static void on_srv_query_done_cb(void* arg, int status, int timeouts,
                                 unsigned char* abuf, int alen) ***REMOVED***
  ...
  if (status == ARES_SUCCESS) ***REMOVED***
    ...
    if (parse_status == ARES_SUCCESS) ***REMOVED***
      ares_channel* channel = grpc_ares_ev_driver_get_channel(r->ev_driver);
      for (struct ares_srv_reply* srv_it = reply; srv_it != nullptr;
           srv_it = srv_it->next) ***REMOVED***
...
static void on_txt_done_cb(void* arg, int status, int timeouts,
                           unsigned char* buf, int len) ***REMOVED***
  ...
  for (result = reply; result != nullptr; result = result->next) ***REMOVED***
...
static grpc_ares_request* grpc_dns_lookup_ares_impl(
    ...
    char** service_config_json) ***REMOVED***
  grpc_error* error = GRPC_ERROR_NONE;
  grpc_ares_hostbyname_request* hr = nullptr;
  grpc_ares_request* r = nullptr;
  ares_channel* channel = nullptr;
  /* TODO(zyc): Enable tracing after #9603 is checked in */
  /* if (grpc_dns_trace) ***REMOVED***
  ...
  if (host == nullptr) ***REMOVED***
    error = grpc_error_set_str(
        GRPC_ERROR_CREATE_FROM_STATIC_STRING("unparseable host:port"),
        GRPC_ERROR_STR_TARGET_ADDRESS, grpc_slice_from_copied_string(name));
    goto error_cleanup;
  ***REMOVED*** else if (port == nullptr) ***REMOVED***
    if (default_port == nullptr) ***REMOVED***
      error = grpc_error_set_str(
  ...
  ***REMOVED***
  ...
  if (dns_server != nullptr) ***REMOVED***
    gpr_log(GPR_INFO, "Using DNS server %s", dns_server);
    grpc_resolved_address addr;
    if (grpc_parse_ipv4_hostport(dns_server, &addr, false /* log_errors */)) ***REMOVED***
    ...
    ***REMOVED*** else if (grpc_parse_ipv6_hostport(dns_server, &addr,
                                        false /* log_errors */)) ***REMOVED***
...
void grpc_cancel_ares_request(grpc_exec_ctx* exec_ctx, grpc_ares_request* r) ***REMOVED***
  if (grpc_dns_lookup_ares == grpc_dns_lookup_ares_impl) ***REMOVED***
    grpc_ares_ev_driver_shutdown(exec_ctx, r->ev_driver);
  ***REMOVED***
***REMOVED***

grpc_error* grpc_ares_init(void) ***REMOVED***
...
void grpc_ares_cleanup(void) ***REMOVED***
  gpr_mu_lock(&g_init_mu);
...
typedef struct grpc_resolve_address_ares_request ***REMOVED***
  /** the pointer to receive the resolved addresses */
...
***REMOVED*** grpc_resolve_address_ares_request;
...
static void on_dns_lookup_done_cb(grpc_exec_ctx* exec_ctx, void* arg,
  ...
  if (r->lb_addrs == nullptr || r->lb_addrs->num_addresses == 0) ***REMOVED***
  ...
  ***REMOVED*** else ***REMOVED***
    *resolved_addresses =
    ...
    for (size_t i = 0; i < (*resolved_addresses)->naddrs; i++) ***REMOVED***
...
static void grpc_resolve_address_ares_impl(grpc_exec_ctx* exec_ctx,
                                           ...
                                           grpc_closure* on_done,
                                           grpc_resolved_addresses** addrs) ***REMOVED***
  grpc_resolve_address_ares_request* r =
