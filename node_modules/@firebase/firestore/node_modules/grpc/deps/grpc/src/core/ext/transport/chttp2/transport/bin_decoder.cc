...
static uint8_t decode_table[] = ***REMOVED***
    ...
    0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40,
    0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40,
    0x40, 0x40, 0x40, 0x40***REMOVED***;

static const uint8_t tail_xtra[4] = ***REMOVED***0, 0, 1, 2***REMOVED***;

static bool input_is_valid(uint8_t* input_ptr, size_t length) ***REMOVED***
  size_t i;

  for (i = 0; i < length; ++i) ***REMOVED***
    if ((decode_table[input_ptr[i]] & 0xC0) != 0) ***REMOVED***
      gpr_log(GPR_ERROR,
...
bool grpc_base64_decode_partial(struct grpc_base64_decode_context* ctx) ***REMOVED***
  size_t input_tail;

  if (ctx->input_cur > ctx->input_end || ctx->output_cur > ctx->output_end) ***REMOVED***
    return false;
  ***REMOVED***

  // Process a block of 4 input characters and 3 output bytes
  while (ctx->input_end >= ctx->input_cur + 4 &&
         ctx->output_end >= ctx->output_cur + 3) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (input_tail == 4) ***REMOVED***
    // Process the input data with pad chars
    if (ctx->input_cur[3] == '=') ***REMOVED***
      if (ctx->input_cur[2] == '=' && ctx->output_end >= ctx->output_cur + 1) ***REMOVED***
        if (!input_is_valid(ctx->input_cur, 2)) return false;
        *(ctx->output_cur++) = COMPOSE_OUTPUT_BYTE_0(ctx->input_cur);
        ctx->input_cur += 4;
      ***REMOVED*** else if (ctx->output_end >= ctx->output_cur + 2) ***REMOVED***
  ...
  ***REMOVED*** else if (ctx->contains_tail && input_tail > 1) ***REMOVED***
    // Process the input data without pad chars, but constains_tail is set
    if (ctx->output_end >= ctx->output_cur + tail_xtra[input_tail]) ***REMOVED***
      if (!input_is_valid(ctx->input_cur, input_tail)) return false;
      switch (input_tail) ***REMOVED***
        case 3:
          ctx->output_cur[1] = COMPOSE_OUTPUT_BYTE_1(ctx->input_cur);
        /* fallthrough */
        case 2:
...
grpc_slice grpc_chttp2_base64_decode(grpc_exec_ctx* exec_ctx,
                                     grpc_slice input) ***REMOVED***
  size_t input_length = GRPC_SLICE_LENGTH(input);
  size_t output_length = input_length / 4 * 3;
  struct grpc_base64_decode_context ctx;
  grpc_slice output;

  if (input_length % 4 != 0) ***REMOVED***
    gpr_log(GPR_ERROR,
  ...
  ***REMOVED***
  ...
  if (input_length > 0) ***REMOVED***
    uint8_t* input_end = GRPC_SLICE_END_PTR(input);
    if (*(--input_end) == '=') ***REMOVED***
      output_length--;
      if (*(--input_end) == '=') ***REMOVED***
        output_length--;
      ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (!grpc_base64_decode_partial(&ctx)) ***REMOVED***
...
grpc_slice grpc_chttp2_base64_decode_with_length(grpc_exec_ctx* exec_ctx,
                                                 grpc_slice input,
                                                 size_t output_length) ***REMOVED***
  size_t input_length = GRPC_SLICE_LENGTH(input);
  grpc_slice output = GRPC_SLICE_MALLOC(output_length);
  struct grpc_base64_decode_context ctx;

  // The length of a base64 string cannot be 4 * n + 1
  if (input_length % 4 == 1) ***REMOVED***
    gpr_log(GPR_ERROR,
            "Base64 decoding failed, input of "
  ...
  ***REMOVED***
  ...
  if (output_length > input_length / 4 * 3 + tail_xtra[input_length % 4]) ***REMOVED***
    gpr_log(GPR_ERROR,
