...
static const int8_t base64_bytes[] = ***REMOVED***
    ...
    0x31, 0x32, 0x33, -1,   -1,   -1,   -1,   -1***REMOVED***;
...
void grpc_base64_encode_core(char* result, const void* vdata, size_t data_size,
                             int url_safe, int multiline) ***REMOVED***
  ...
  while (data_size >= 3) ***REMOVED***
    ...
    *current++ = base64_chars[data[i + 2] & 0x3F];
    ...
    data_size -= 3;
    i += 3;
    if (multiline && (++num_blocks == GRPC_BASE64_MULTILINE_NUM_BLOCKS)) ***REMOVED***
      *current++ = '\r';
      *current++ = '\n';
      num_blocks = 0;
    ***REMOVED***
...
static int decode_group(const unsigned char* codes, size_t num_codes,
                        unsigned char* result, size_t* result_offset) ***REMOVED***
  ...
  if (num_codes == 1) ***REMOVED***
    ...
    return 0;
  ***REMOVED***
  if (num_codes == 2) ***REMOVED***
    decode_one_char(codes, result, result_offset);
    return 1;
  ***REMOVED***
  if (num_codes == 3) ***REMOVED***
  ...
  ***REMOVED***
  ...
  GPR_ASSERT(num_codes == 4);
  if (codes[0] == GRPC_BASE64_PAD_BYTE || codes[1] == GRPC_BASE64_PAD_BYTE) ***REMOVED***
    gpr_log(GPR_ERROR, "Invalid padding detected.");
    return 0;
  ***REMOVED***
  if (codes[2] == GRPC_BASE64_PAD_BYTE) ***REMOVED***
    if (codes[3] == GRPC_BASE64_PAD_BYTE) ***REMOVED***
      decode_one_char(codes, result, result_offset);
    ***REMOVED*** else ***REMOVED***
      gpr_log(GPR_ERROR, "Invalid padding detected.");
      return 0;
    ***REMOVED***
  ***REMOVED*** else if (codes[3] == GRPC_BASE64_PAD_BYTE) ***REMOVED***
    decode_two_chars(codes, result, result_offset);
  ***REMOVED*** else ***REMOVED***
    /* No padding. */
...
grpc_slice grpc_base64_decode_with_len(grpc_exec_ctx* exec_ctx, const char* b64,
                                       size_t b64_len, int url_safe) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (num_codes != 0 &&
      !decode_group(codes, num_codes, current, &result_size)) ***REMOVED***
    goto fail;
  ***REMOVED***
