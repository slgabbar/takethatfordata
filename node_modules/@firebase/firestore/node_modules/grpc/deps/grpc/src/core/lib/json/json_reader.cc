...
int grpc_json_reader_is_complete(grpc_json_reader* reader) ***REMOVED***
  return ((reader->depth == 0) &&
          ((reader->state == GRPC_JSON_STATE_END) ||
           (reader->state == GRPC_JSON_STATE_VALUE_END)));
***REMOVED***

grpc_json_reader_status grpc_json_reader_run(grpc_json_reader* reader) ***REMOVED***
  uint32_t c, success;
  ...
  for (;;) ***REMOVED***
    ...
    switch (c) ***REMOVED***
      ...
      case GRPC_JSON_READ_CHAR_EOF:
        if (grpc_json_reader_is_complete(reader)) ***REMOVED***
      ...
      case ']':
        switch (reader->state) ***REMOVED***
          case GRPC_JSON_STATE_OBJECT_KEY_STRING:
          case GRPC_JSON_STATE_VALUE_STRING:
            if (reader->unicode_high_surrogate != 0) ***REMOVED***
              return GRPC_JSON_PARSE_ERROR;
            ***REMOVED***
            json_reader_string_add_char(reader, c);
            break;
          ...
          case GRPC_JSON_STATE_VALUE_NUMBER_ZERO:
          case GRPC_JSON_STATE_VALUE_NUMBER_EPM:
            if (reader->depth == 0) ***REMOVED***
              return GRPC_JSON_PARSE_ERROR;
            ***REMOVED*** else if ((c == '***REMOVED***') && !reader->in_object) ***REMOVED***
              return GRPC_JSON_PARSE_ERROR;
            ***REMOVED*** else if ((c == ']') && !reader->in_array) ***REMOVED***
              return GRPC_JSON_PARSE_ERROR;
            ***REMOVED***
            success = (uint32_t)json_reader_set_number(reader);
            if (!success) return GRPC_JSON_PARSE_ERROR;
          ...
          case GRPC_JSON_STATE_VALUE_BEGIN:
            if (c == ',') ***REMOVED***
              if (reader->state != GRPC_JSON_STATE_VALUE_END) ***REMOVED***
                return GRPC_JSON_PARSE_ERROR;
              ***REMOVED***
              if (reader->in_object) ***REMOVED***
                reader->state = GRPC_JSON_STATE_OBJECT_KEY_BEGIN;
              ***REMOVED*** else if (reader->in_array) ***REMOVED***
                reader->state = GRPC_JSON_STATE_VALUE_BEGIN;
              ***REMOVED*** else ***REMOVED***
                return GRPC_JSON_PARSE_ERROR;
              ***REMOVED***
            ***REMOVED*** else ***REMOVED***
              ...
              if ((c == '***REMOVED***') && !reader->in_object) ***REMOVED***
                return GRPC_JSON_PARSE_ERROR;
              ***REMOVED***
              if ((c == '***REMOVED***') &&
                  (reader->state == GRPC_JSON_STATE_OBJECT_KEY_BEGIN) &&
                  !reader->container_just_begun) ***REMOVED***
                return GRPC_JSON_PARSE_ERROR;
              ***REMOVED***
              ...
              ***REMOVED***
              reader->state = GRPC_JSON_STATE_VALUE_END;
              switch (grpc_json_reader_container_ends(reader)) ***REMOVED***
      ...
      case '\\':
        switch (reader->state) ***REMOVED***
          ...
          case GRPC_JSON_STATE_STRING_ESCAPE:
            ...
            if (reader->escaped_string_was_key) ***REMOVED***
        ...
        switch (reader->state) ***REMOVED***
          ...
          case GRPC_JSON_STATE_STRING_ESCAPE:
            ...
            ***REMOVED*** else ***REMOVED***
              reader->state = GRPC_JSON_STATE_VALUE_STRING;
            ***REMOVED***
            if (reader->unicode_high_surrogate && c != 'u') ***REMOVED***
              return GRPC_JSON_PARSE_ERROR;
            ***REMOVED***
            switch (c) ***REMOVED***
              case '"':
            ...
            ***REMOVED***
            ...
            switch (reader->state) ***REMOVED***
              ...
              case GRPC_JSON_STATE_STRING_ESCAPE_U4:
                 ...
                 */
                if ((reader->unicode_char & 0xfc00) == 0xd800) ***REMOVED***
                  /* high surrogate utf-16 */
                  if (reader->unicode_high_surrogate != 0)
                    return GRPC_JSON_PARSE_ERROR;
                  reader->unicode_high_surrogate = reader->unicode_char;
                ***REMOVED*** else if ((reader->unicode_char & 0xfc00) == 0xdc00) ***REMOVED***
