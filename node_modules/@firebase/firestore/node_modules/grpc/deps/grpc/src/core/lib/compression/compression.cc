...
int grpc_compression_algorithm_parse(grpc_slice name,
                                     grpc_compression_algorithm* algorithm) ***REMOVED***
  /* we use strncmp not only because it's safer (even though in this case it
   * doesn't matter, given that we are comparing against string literals, but
   * because this way we needn't have "name" nil-terminated (useful for slice
   * data, for example) */
  if (grpc_slice_eq(name, GRPC_MDSTR_IDENTITY)) ***REMOVED***
    *algorithm = GRPC_COMPRESS_NONE;
    return 1;
  ***REMOVED*** else if (grpc_slice_eq(name, GRPC_MDSTR_GZIP)) ***REMOVED***
    *algorithm = GRPC_COMPRESS_GZIP;
    return 1;
  ***REMOVED*** else if (grpc_slice_eq(name, GRPC_MDSTR_DEFLATE)) ***REMOVED***
...
int grpc_stream_compression_algorithm_parse(
    grpc_slice name, grpc_stream_compression_algorithm* algorithm) ***REMOVED***
  if (grpc_slice_eq(name, GRPC_MDSTR_IDENTITY)) ***REMOVED***
...
int grpc_stream_compression_algorithm_name(
    grpc_stream_compression_algorithm algorithm, const char** name) ***REMOVED***
  GRPC_API_TRACE(
...
grpc_slice grpc_stream_compression_algorithm_slice(
    grpc_stream_compression_algorithm algorithm) ***REMOVED***
  switch (algorithm) ***REMOVED***
    case GRPC_STREAM_COMPRESS_NONE:
      return GRPC_MDSTR_IDENTITY;
    case GRPC_STREAM_COMPRESS_GZIP:
...
void grpc_compression_options_init(grpc_compression_options* opts) ***REMOVED***
...
void grpc_compression_options_enable_algorithm(
    grpc_compression_options* opts, grpc_compression_algorithm algorithm) ***REMOVED***
...
grpc_compression_algorithm grpc_compression_algorithm_for_level(
    grpc_compression_level level, uint32_t accepted_encodings) ***REMOVED***
  GRPC_API_TRACE("grpc_compression_algorithm_for_level(level=%d)", 1,
                 ((int)level));
  if (level > GRPC_COMPRESS_LEVEL_HIGH) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (level == GRPC_COMPRESS_LEVEL_NONE || num_supported == 0) ***REMOVED***
  ...
  ***REMOVED***
  ...
  const grpc_compression_algorithm algos_ranking[] = ***REMOVED***GRPC_COMPRESS_GZIP,
                                                      GRPC_COMPRESS_DEFLATE***REMOVED***;
  ...
  for (size_t i = 0; i < GPR_ARRAY_SIZE(algos_ranking); i++) ***REMOVED***
    const grpc_compression_algorithm alg = algos_ranking[i];
    for (size_t j = 0; j < num_supported; j++) ***REMOVED***
      if (GPR_BITGET(accepted_encodings, alg) == 1) ***REMOVED***
...
grpc_stream_compression_algorithm_for_level(
    grpc_stream_compression_level level, uint32_t accepted_stream_encodings) ***REMOVED***
  GRPC_API_TRACE("grpc_stream_compression_algorithm_for_level(level=%d)", 1,
                 ((int)level));
  if (level > GRPC_STREAM_COMPRESS_LEVEL_HIGH) ***REMOVED***
    gpr_log(GPR_ERROR, "Unknown compression level %d.", (int)level);
    abort();
  ***REMOVED***
