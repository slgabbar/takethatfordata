...
static bool get_user_agent_mdelem(const grpc_metadata_batch* batch,
  ...
  if (batch->idx.named.user_agent != nullptr) ***REMOVED***
...
static void recv_initial_metadata_ready(grpc_exec_ctx* exec_ctx,
  ...
  if (GRPC_ERROR_NONE == error) ***REMOVED***
    grpc_mdelem md;
    if (get_user_agent_mdelem(calld->recv_initial_metadata, &md)) ***REMOVED***
      grpc_workaround_user_agent_md* user_agent_md = grpc_parse_user_agent(md);
      if (user_agent_md
              ->workaround_active[GRPC_WORKAROUND_ID_CRONET_COMPRESSION]) ***REMOVED***
...
static void start_transport_stream_op_batch(
  ...
  ***REMOVED***
  ...
  if (op->send_message) ***REMOVED***
    ...
    if (calld->workaround_active) ***REMOVED***
...
static bool parse_user_agent(grpc_mdelem md) ***REMOVED***
  ...
  while (head != nullptr) ***REMOVED***
    if (!grpc_objc_specifier_seen &&
        0 == strncmp(head, grpc_objc_specifier, grpc_objc_specifier_len)) ***REMOVED***
      major_version_str = head + grpc_objc_specifier_len;
      grpc_objc_specifier_seen = true;
    ***REMOVED*** else if (grpc_objc_specifier_seen &&
               0 == strncmp(head, cronet_specifier, cronet_specifier_len)) ***REMOVED***
    ...
    ***REMOVED***
    ...
    head = strtok(nullptr, " ");
  ***REMOVED***
  if (grpc_objc_specifier_seen) ***REMOVED***
...
const grpc_channel_filter grpc_workaround_cronet_compression_filter = ***REMOVED***
    ...
    "workaround_cronet_compression"***REMOVED***;
...
static bool register_workaround_cronet_compression(
  ...
  if (a == nullptr) ***REMOVED***
    return true;
  ***REMOVED***
  if (grpc_channel_arg_get_bool(a, false) == false) ***REMOVED***
    return true;
  ***REMOVED***
...
extern "C" void grpc_workaround_cronet_compression_filter_init(void) ***REMOVED***
...
extern "C" void grpc_workaround_cronet_compression_filter_shutdown(void) ***REMOVED******REMOVED***
