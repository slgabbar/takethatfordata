...
bool grpc_parse_unix(const grpc_uri* uri,
                     grpc_resolved_address* resolved_addr) ***REMOVED***
  if (strcmp("unix", uri->scheme) != 0) ***REMOVED***
...
bool grpc_parse_ipv4_hostport(const char* hostport, grpc_resolved_address* addr,
  ...
  if (inet_pton(AF_INET, host, &in->sin_addr) == 0) ***REMOVED***
    if (log_errors) gpr_log(GPR_ERROR, "invalid ipv4 address: '%s'", host);
    goto done;
  ***REMOVED***
  // Parse port.
  if (port == nullptr) ***REMOVED***
    if (log_errors) gpr_log(GPR_ERROR, "no port given for ipv4 scheme");
    goto done;
  ***REMOVED***
  int port_num;
  if (sscanf(port, "%d", &port_num) != 1 || port_num < 0 || port_num > 65535) ***REMOVED***
...
bool grpc_parse_ipv4(const grpc_uri* uri,
                     grpc_resolved_address* resolved_addr) ***REMOVED***
  if (strcmp("ipv4", uri->scheme) != 0) ***REMOVED***
...
bool grpc_parse_ipv6_hostport(const char* hostport, grpc_resolved_address* addr,
  ...
  if (host_end != nullptr) ***REMOVED***
    ...
    if (inet_pton(AF_INET6, host_without_scope, &in6->sin6_addr) == 0) ***REMOVED***
      gpr_log(GPR_ERROR, "invalid ipv6 address: '%s'", host_without_scope);
      goto done;
    ***REMOVED***
    if (gpr_parse_bytes_to_uint32(host_end + 1,
                                  strlen(host) - host_without_scope_len - 1,
                                  &sin6_scope_id) == 0) ***REMOVED***
  ...
  ***REMOVED*** else ***REMOVED***
    if (inet_pton(AF_INET6, host, &in6->sin6_addr) == 0) ***REMOVED***
...
bool grpc_parse_ipv6(const grpc_uri* uri,
                     grpc_resolved_address* resolved_addr) ***REMOVED***
  if (strcmp("ipv6", uri->scheme) != 0) ***REMOVED***
...
bool grpc_parse_uri(const grpc_uri* uri, grpc_resolved_address* resolved_addr) ***REMOVED***
  if (strcmp("unix", uri->scheme) == 0) ***REMOVED***
    return grpc_parse_unix(uri, resolved_addr);
  ***REMOVED*** else if (strcmp("ipv4", uri->scheme) == 0) ***REMOVED***
    return grpc_parse_ipv4(uri, resolved_addr);
  ***REMOVED*** else if (strcmp("ipv6", uri->scheme) == 0) ***REMOVED***
