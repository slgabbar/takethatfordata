...
static bool count_serverlist(pb_istream_t* stream, const pb_field_t* field,
  ...
  if (!pb_decode(stream, grpc_lb_v1_Server_fields, &server)) ***REMOVED***
...
typedef struct decode_serverlist_arg ***REMOVED***
...
grpc_grpclb_request* grpc_grpclb_request_create(const char* lb_service_name) ***REMOVED***
...
static void populate_timestamp(gpr_timespec timestamp,
                               struct _grpc_lb_v1_Timestamp* timestamp_pb) ***REMOVED***
...
static bool encode_drops(pb_ostream_t* stream, const pb_field_t* field,
  ...
  for (size_t i = 0; i < drop_entries->num_entries; ++i) ***REMOVED***
    if (!pb_encode_tag_for_field(stream, field)) return false;
...
grpc_grpclb_request* grpc_grpclb_load_report_request_create_locked(
    grpc_grpclb_client_stats* client_stats) ***REMOVED***
  grpc_grpclb_request* req =
...
grpc_slice grpc_grpclb_request_encode(const grpc_grpclb_request* request) ***REMOVED***
...
void grpc_grpclb_request_destroy(grpc_grpclb_request* request) ***REMOVED***
  if (request->has_client_stats) ***REMOVED***
    grpc_grpclb_dropped_call_counts* drop_entries =
...
grpc_grpclb_initial_response* grpc_grpclb_initial_response_parse(
    grpc_slice encoded_grpc_grpclb_response) ***REMOVED***
  pb_istream_t stream =
      pb_istream_from_buffer(GRPC_SLICE_START_PTR(encoded_grpc_grpclb_response),
                             GRPC_SLICE_LENGTH(encoded_grpc_grpclb_response));
  grpc_grpclb_response res;
  memset(&res, 0, sizeof(grpc_grpclb_response));
  if (!pb_decode(&stream, grpc_lb_v1_LoadBalanceResponse_fields, &res)) ***REMOVED***
    gpr_log(GPR_ERROR, "nanopb error: %s", PB_GET_ERROR(&stream));
    return nullptr;
  ***REMOVED***
...
grpc_grpclb_serverlist* grpc_grpclb_response_parse_serverlist(
  ...
  ***REMOVED***
  ...
  if (sl->num_servers > 0) ***REMOVED***
    ...
    ***REMOVED***
  ***REMOVED***
  if (res.server_list.has_expiration_interval) ***REMOVED***
    sl->expiration_interval = res.server_list.expiration_interval;
  ***REMOVED***
  return sl;
***REMOVED***

void grpc_grpclb_destroy_serverlist(grpc_grpclb_serverlist* serverlist) ***REMOVED***
...
grpc_grpclb_serverlist* grpc_grpclb_serverlist_copy(
    const grpc_grpclb_serverlist* sl) ***REMOVED***
  ...
  copy->servers = (grpc_grpclb_server**)gpr_malloc(sizeof(grpc_grpclb_server*) *
                                                   sl->num_servers);
  for (size_t i = 0; i < sl->num_servers; i++) ***REMOVED***
    copy->servers[i] =
...
bool grpc_grpclb_serverlist_equals(const grpc_grpclb_serverlist* lhs,
                                   const grpc_grpclb_serverlist* rhs) ***REMOVED***
  if (lhs == nullptr || rhs == nullptr) ***REMOVED***
    return false;
  ***REMOVED***
  if (lhs->num_servers != rhs->num_servers) ***REMOVED***
    return false;
  ***REMOVED***
  if (grpc_grpclb_duration_compare(&lhs->expiration_interval,
                                   &rhs->expiration_interval) != 0) ***REMOVED***
    return false;
  ***REMOVED***
  for (size_t i = 0; i < lhs->num_servers; i++) ***REMOVED***
    if (!grpc_grpclb_server_equals(lhs->servers[i], rhs->servers[i])) ***REMOVED***
      return false;
    ***REMOVED***
  ***REMOVED***
  return true;
...
bool grpc_grpclb_server_equals(const grpc_grpclb_server* lhs,
                               const grpc_grpclb_server* rhs) ***REMOVED***
  return memcmp(lhs, rhs, sizeof(grpc_grpclb_server)) == 0;
***REMOVED***

int grpc_grpclb_duration_compare(const grpc_grpclb_duration* lhs,
                                 const grpc_grpclb_duration* rhs) ***REMOVED***
  GPR_ASSERT(lhs && rhs);
  if (lhs->has_seconds && rhs->has_seconds) ***REMOVED***
    if (lhs->seconds < rhs->seconds) return -1;
    if (lhs->seconds > rhs->seconds) return 1;
  ***REMOVED*** else if (lhs->has_seconds) ***REMOVED***
    return 1;
  ***REMOVED*** else if (rhs->has_seconds) ***REMOVED***
    return -1;
  ***REMOVED***

  GPR_ASSERT(lhs->seconds == rhs->seconds);
  if (lhs->has_nanos && rhs->has_nanos) ***REMOVED***
    if (lhs->nanos < rhs->nanos) return -1;
    if (lhs->nanos > rhs->nanos) return 1;
  ***REMOVED*** else if (lhs->has_nanos) ***REMOVED***
    return 1;
  ***REMOVED*** else if (rhs->has_nanos) ***REMOVED***
    return -1;
  ***REMOVED***
...
grpc_millis grpc_grpclb_duration_to_millis(grpc_grpclb_duration* duration_pb) ***REMOVED***
  return (grpc_millis)(
...
void grpc_grpclb_initial_response_destroy(
    grpc_grpclb_initial_response* response) ***REMOVED***
  gpr_free(response);
***REMOVED***
