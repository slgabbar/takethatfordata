...
static void mdelem_list_ensure_capacity(grpc_credentials_mdelem_array* list,
                                        size_t additional_space_needed) ***REMOVED***
  ...
  size_t new_size = 2;
  while (new_size < target_size) ***REMOVED***
    new_size *= 2;
  ***REMOVED***
  list->md =
...
void grpc_credentials_mdelem_array_append(grpc_credentials_mdelem_array* dst,
                                          grpc_credentials_mdelem_array* src) ***REMOVED***
  mdelem_list_ensure_capacity(dst, src->size);
  for (size_t i = 0; i < src->size; ++i) ***REMOVED***
    dst->md[dst->size++] = GRPC_MDELEM_REF(src->md[i]);
  ***REMOVED***
***REMOVED***

void grpc_credentials_mdelem_array_destroy(
    grpc_exec_ctx* exec_ctx, grpc_credentials_mdelem_array* list) ***REMOVED***
  for (size_t i = 0; i < list->size; ++i) ***REMOVED***
