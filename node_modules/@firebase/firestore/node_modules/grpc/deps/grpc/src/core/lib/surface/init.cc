...
static void do_basic_init(void) ***REMOVED***
...
static bool append_filter(grpc_exec_ctx* exec_ctx,
                          grpc_channel_stack_builder* builder, void* arg) ***REMOVED***
...
static void register_builtin_channel_init() ***REMOVED***
...
typedef struct grpc_plugin ***REMOVED***
  void (*init)();
  void (*destroy)();
***REMOVED*** grpc_plugin;
...
void grpc_register_plugin(void (*init)(void), void (*destroy)(void)) ***REMOVED***
...
void grpc_init(void) ***REMOVED***
  int i;
  gpr_once_init(&g_basic_init, do_basic_init);

  grpc_exec_ctx exec_ctx = GRPC_EXEC_CTX_INIT;
  gpr_mu_lock(&g_init_mu);
  if (++g_initializations == 1) ***REMOVED***
    gpr_time_init();
    gpr_thd_init();
    grpc_stats_init();
    ...
    grpc_security_init();
    for (i = 0; i < g_number_of_plugins; i++) ***REMOVED***
      if (g_all_of_the_plugins[i].init != nullptr) ***REMOVED***
...
void grpc_shutdown(void) ***REMOVED***
  int i;
  GRPC_API_TRACE("grpc_shutdown(void)", 0, ());
  grpc_exec_ctx exec_ctx =
      GRPC_EXEC_CTX_INITIALIZER(0, grpc_never_ready_to_finish, nullptr);
  gpr_mu_lock(&g_init_mu);
  if (--g_initializations == 0) ***REMOVED***
    grpc_executor_shutdown(&exec_ctx);
    grpc_timer_manager_set_threading(false);  // shutdown timer_manager thread
    for (i = g_number_of_plugins; i >= 0; i--) ***REMOVED***
      if (g_all_of_the_plugins[i].destroy != nullptr) ***REMOVED***
...
int grpc_is_initialized(void) ***REMOVED***
  int r;
  gpr_once_init(&g_basic_init, do_basic_init);
  gpr_mu_lock(&g_init_mu);
