...
static grpc_error* set_non_block(SOCKET sock) ***REMOVED***
  int status;
  uint32_t param = 1;
  DWORD ret;
...
static grpc_error* set_dualstack(SOCKET sock) ***REMOVED***
  int status;
...
grpc_error* grpc_tcp_prepare_socket(SOCKET sock) ***REMOVED***
  grpc_error* err;
...
typedef struct grpc_tcp ***REMOVED***
...
static void on_read(grpc_exec_ctx* exec_ctx, void* tcpp, grpc_error* error) ***REMOVED***
  ...
  if (error == GRPC_ERROR_NONE) ***REMOVED***
    if (info->wsa_error != 0 && !tcp->shutting_down) ***REMOVED***
      char* utf8_message = gpr_format_message(info->wsa_error);
      error = GRPC_ERROR_CREATE_FROM_COPIED_STRING(utf8_message);
      gpr_free(utf8_message);
      grpc_slice_unref_internal(exec_ctx, tcp->read_slice);
    ***REMOVED*** else ***REMOVED***
      if (info->bytes_transfered != 0 && !tcp->shutting_down) ***REMOVED***
...
static void win_read(grpc_exec_ctx* exec_ctx, grpc_endpoint* ep,
                     grpc_slice_buffer* read_slices, grpc_closure* cb) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (info->wsa_error != WSAEWOULDBLOCK) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (status != 0) ***REMOVED***
    ...
    if (wsa_error != WSA_IO_PENDING) ***REMOVED***
...
/* Asynchronous callback from the IOCP, or the background thread. */
static void on_write(grpc_exec_ctx* exec_ctx, void* tcpp, grpc_error* error) ***REMOVED***
  grpc_tcp* tcp = (grpc_tcp*)tcpp;
  grpc_winsocket* handle = tcp->socket;
...
static void win_write(grpc_exec_ctx* exec_ctx, grpc_endpoint* ep,
                      grpc_slice_buffer* slices, grpc_closure* cb) ***REMOVED***
  grpc_tcp* tcp = (grpc_tcp*)ep;
  grpc_winsocket* socket = tcp->socket;
  ...
  ***REMOVED***
  ...
  if (tcp->write_slices->count > GPR_ARRAY_SIZE(local_buffers)) ***REMOVED***
    buffers = (WSABUF*)gpr_malloc(sizeof(WSABUF) * tcp->write_slices->count);
    allocated = buffers;
  ***REMOVED***

  for (i = 0; i < tcp->write_slices->count; i++) ***REMOVED***
...
static void win_destroy(grpc_exec_ctx* exec_ctx, grpc_endpoint* ep) ***REMOVED***
  grpc_network_status_unregister_endpoint(ep);
  grpc_tcp* tcp = (grpc_tcp*)ep;
  TCP_UNREF(exec_ctx, tcp, "destroy");
***REMOVED***

static char* win_get_peer(grpc_endpoint* ep) ***REMOVED***
  grpc_tcp* tcp = (grpc_tcp*)ep;
  return gpr_strdup(tcp->peer_string);
***REMOVED***

static grpc_resource_user* win_get_resource_user(grpc_endpoint* ep) ***REMOVED***
  grpc_tcp* tcp = (grpc_tcp*)ep;
  return tcp->resource_user;
***REMOVED***

static int win_get_fd(grpc_endpoint* ep) ***REMOVED*** return -1; ***REMOVED***

static grpc_endpoint_vtable vtable = ***REMOVED***win_read,
                                      win_write,
                                      win_add_to_pollset,
