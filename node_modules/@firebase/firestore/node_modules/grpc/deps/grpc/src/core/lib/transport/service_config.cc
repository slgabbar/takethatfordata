...
struct grpc_service_config ***REMOVED***
  char* json_string;  // Underlying storage for json_tree.
  grpc_json* json_tree;
***REMOVED***;

grpc_service_config* grpc_service_config_create(const char* json_string) ***REMOVED***
  ...
  if (service_config->json_tree == nullptr) ***REMOVED***
...
void grpc_service_config_destroy(grpc_service_config* service_config) ***REMOVED***
...
void grpc_service_config_parse_global_params(
    ...
    void (*process_json)(const grpc_json* json, void* arg), void* arg) ***REMOVED***
...
static size_t count_names_in_method_config_json(grpc_json* json) ***REMOVED***
  size_t num_names = 0;
  for (grpc_json* field = json->child; field != nullptr; field = field->next) ***REMOVED***
    if (field->key != nullptr && strcmp(field->key, "name") == 0) ***REMOVED***
      if (field->type != GRPC_JSON_ARRAY) return -1;
      for (grpc_json* name = field->child; name != nullptr; name = name->next) ***REMOVED***
...
static bool parse_json_method_config(
    ...
    grpc_slice_hash_table_entry* entries, size_t* idx) ***REMOVED***
  // Construct value.
  ...
  for (grpc_json* child = json->child; child != nullptr; child = child->next) ***REMOVED***
    ...
    if (strcmp(child->key, "name") == 0) ***REMOVED***
      if (child->type != GRPC_JSON_ARRAY) goto done;
      for (grpc_json* name = child->child; name != nullptr; name = name->next) ***REMOVED***
  ...
  ***REMOVED***
  ...
  for (size_t i = 0; i < paths.count; ++i) ***REMOVED***
...
grpc_slice_hash_table* grpc_service_config_create_method_config_table(
    ...
    void* (*create_value)(const grpc_json* method_config_json),
    void* (*ref_value)(void* value),
    void (*unref_value)(grpc_exec_ctx* exec_ctx, void* value)) ***REMOVED***
  const grpc_json* json = service_config->json_tree;
      ...
      ***REMOVED***
      ...
      for (grpc_json* method = field->child; method != nullptr;
        ...
        if (!parse_json_method_config(exec_ctx, method, create_value, ref_value,
                                      unref_value, entries, &idx)) ***REMOVED***
          for (size_t i = 0; i < idx; ++i) ***REMOVED***
