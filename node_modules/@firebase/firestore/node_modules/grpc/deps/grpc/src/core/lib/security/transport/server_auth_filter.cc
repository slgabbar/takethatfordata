...
static grpc_metadata_array metadata_batch_to_md_array(
    const grpc_metadata_batch* batch) ***REMOVED***
  grpc_linked_mdelem* l;
  grpc_metadata_array result;
  grpc_metadata_array_init(&result);
  for (l = batch->list.head; l != nullptr; l = l->next) ***REMOVED***
    grpc_metadata* usr_md = nullptr;
    grpc_mdelem md = l->md;
    grpc_slice key = GRPC_MDKEY(md);
    grpc_slice value = GRPC_MDVALUE(md);
    if (result.count == result.capacity) ***REMOVED***
...
static grpc_filtered_mdelem remove_consumed_md(grpc_exec_ctx* exec_ctx,
  ...
  size_t i;
  for (i = 0; i < calld->num_consumed_md; i++) ***REMOVED***
...
static void on_md_processing_done_inner(grpc_exec_ctx* exec_ctx,
  ...
  if (response_md != nullptr && num_response_md > 0) ***REMOVED***
    gpr_log(GPR_INFO,
...
static void on_md_processing_done(
    ...
    grpc_status_code status, const char* error_details) ***REMOVED***
  grpc_call_element* elem = (grpc_call_element*)user_data;
  call_data* calld = (call_data*)elem->call_data;
  grpc_exec_ctx exec_ctx = GRPC_EXEC_CTX_INIT;
  // If the call was not cancelled while we were in flight, process the result.
  if (gpr_atm_full_cas(&calld->state, (gpr_atm)STATE_INIT,
                       (gpr_atm)STATE_DONE)) ***REMOVED***
    grpc_error* error = GRPC_ERROR_NONE;
    if (status != GRPC_STATUS_OK) ***REMOVED***
      if (error_details == nullptr) ***REMOVED***
  ...
  ***REMOVED***
  // Clean up.
  for (size_t i = 0; i < calld->md.count; i++) ***REMOVED***
...
static void cancel_call(grpc_exec_ctx* exec_ctx, void* arg, grpc_error* error) ***REMOVED***
  grpc_call_element* elem = (grpc_call_element*)arg;
  call_data* calld = (call_data*)elem->call_data;
  // If the result was not already processed, invoke the callback now.
  if (error != GRPC_ERROR_NONE &&
      gpr_atm_full_cas(&calld->state, (gpr_atm)STATE_INIT,
                       (gpr_atm)STATE_CANCELLED)) ***REMOVED***
...
static void recv_initial_metadata_ready(grpc_exec_ctx* exec_ctx, void* arg,
  ...
  grpc_transport_stream_op_batch* batch = calld->recv_initial_metadata_batch;
  if (error == GRPC_ERROR_NONE) ***REMOVED***
    if (chand->creds != nullptr && chand->creds->processor.process != nullptr) ***REMOVED***
...
static grpc_error* init_call_elem(grpc_exec_ctx* exec_ctx,
  ...
  if (args->context[GRPC_CONTEXT_SECURITY].value != nullptr) ***REMOVED***
...
const grpc_channel_filter grpc_server_auth_filter = ***REMOVED***
