...
void grpc_resolver_registry_init() ***REMOVED******REMOVED***

void grpc_resolver_registry_shutdown(void) ***REMOVED***
  for (int i = 0; i < g_number_of_resolvers; i++) ***REMOVED***
...
void grpc_resolver_registry_set_default_prefix(
    const char* default_resolver_prefix) ***REMOVED***
...
void grpc_register_resolver_type(grpc_resolver_factory* factory) ***REMOVED***
  int i;
  for (i = 0; i < g_number_of_resolvers; i++) ***REMOVED***
...
static grpc_resolver_factory* lookup_factory(const char* name) ***REMOVED***
  int i;

  for (i = 0; i < g_number_of_resolvers; i++) ***REMOVED***
    if (0 == strcmp(name, g_all_of_the_resolvers[i]->vtable->scheme)) ***REMOVED***
...
grpc_resolver_factory* grpc_resolver_factory_lookup(const char* name) ***REMOVED***
  grpc_resolver_factory* f = lookup_factory(name);
  if (f) grpc_resolver_factory_ref(f);
  return f;
***REMOVED***

static grpc_resolver_factory* lookup_factory_by_uri(grpc_uri* uri) ***REMOVED***
...
char* grpc_get_default_authority(grpc_exec_ctx* exec_ctx, const char* target) ***REMOVED***
  grpc_uri* uri = nullptr;
  char* canonical_target = nullptr;
  grpc_resolver_factory* factory =
      resolve_factory(exec_ctx, target, &uri, &canonical_target);
...
char* grpc_resolver_factory_add_default_prefix_if_needed(
    grpc_exec_ctx* exec_ctx, const char* target) ***REMOVED***
  grpc_uri* uri = nullptr;
  char* canonical_target = nullptr;
  resolve_factory(exec_ctx, target, &uri, &canonical_target);
  grpc_uri_destroy(uri);
