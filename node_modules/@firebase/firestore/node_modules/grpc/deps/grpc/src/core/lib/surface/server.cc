...
typedef struct listener ***REMOVED***
  void* arg;
...
typedef enum ***REMOVED*** BATCH_CALL, REGISTERED_CALL ***REMOVED*** requested_call_type;

grpc_core::TraceFlag grpc_server_channel_trace(false, "server_channel");

typedef struct requested_call ***REMOVED***
  ...
  union ***REMOVED***
    ...
    ***REMOVED*** registered;
  ***REMOVED*** data;
***REMOVED*** requested_call;

typedef struct channel_registered_method ***REMOVED***
  registered_method* server_registered_method;
  uint32_t flags;
  bool has_host;
  grpc_slice method;
  grpc_slice host;
***REMOVED*** channel_registered_method;

struct channel_data ***REMOVED***
  grpc_server* server;
...
typedef struct shutdown_tag ***REMOVED***
  void* tag;
  grpc_completion_queue* cq;
  grpc_cq_completion completion;
***REMOVED*** shutdown_tag;
...
struct request_matcher ***REMOVED***
  grpc_server* server;
...
struct registered_method ***REMOVED***
  char* method;
  char* host;
...
typedef struct ***REMOVED***
  ...
  size_t num_channels;
***REMOVED*** channel_broadcaster;

struct grpc_server ***REMOVED***
...
static void channel_broadcaster_init(grpc_server* s, channel_broadcaster* cb) ***REMOVED***
  channel_data* c;
  size_t count = 0;
  for (c = s->root_channel_data.next; c != &s->root_channel_data; c = c->next) ***REMOVED***
    count++;
  ***REMOVED***
...
struct shutdown_cleanup_args ***REMOVED***
  grpc_closure closure;
  grpc_slice slice;
***REMOVED***;
...
static void send_shutdown(grpc_exec_ctx* exec_ctx, grpc_channel* channel,
                          bool send_goaway, grpc_error* send_disconnect) ***REMOVED***
...
static void channel_broadcaster_shutdown(grpc_exec_ctx* exec_ctx,
                                         ...
                                         bool send_goaway,
                                         grpc_error* force_disconnect) ***REMOVED***
  size_t i;

  for (i = 0; i < cb->num_channels; i++) ***REMOVED***
...
static void request_matcher_init(request_matcher* rm, grpc_server* server) ***REMOVED***
  memset(rm, 0, sizeof(*rm));
  rm->server = server;
  rm->requests_per_cq = (gpr_locked_mpscq*)gpr_malloc(
      sizeof(*rm->requests_per_cq) * server->cq_count);
  for (size_t i = 0; i < server->cq_count; i++) ***REMOVED***
    gpr_locked_mpscq_init(&rm->requests_per_cq[i]);
  ***REMOVED***
***REMOVED***

static void request_matcher_destroy(request_matcher* rm) ***REMOVED***
  for (size_t i = 0; i < rm->server->cq_count; i++) ***REMOVED***
...
static void request_matcher_zombify_all_pending_calls(grpc_exec_ctx* exec_ctx,
                                                      request_matcher* rm) ***REMOVED***
  while (rm->pending_head) ***REMOVED***
...
static void request_matcher_kill_requests(grpc_exec_ctx* exec_ctx,
  ...
  for (size_t i = 0; i < server->cq_count; i++) ***REMOVED***
    while ((rc = (requested_call*)gpr_locked_mpscq_pop(
                &rm->requests_per_cq[i])) != nullptr) ***REMOVED***
...
static void server_ref(grpc_server* server) ***REMOVED***
  gpr_ref(&server->internal_refcount);
***REMOVED***

static void server_delete(grpc_exec_ctx* exec_ctx, grpc_server* server) ***REMOVED***
  ...
  while ((rm = server->registered_methods) != nullptr) ***REMOVED***
    server->registered_methods = rm->next;
    if (server->started) ***REMOVED***
  ...
  ***REMOVED***
  for (i = 0; i < server->cq_count; i++) ***REMOVED***
...
static void server_unref(grpc_exec_ctx* exec_ctx, grpc_server* server) ***REMOVED***
  if (gpr_unref(&server->internal_refcount)) ***REMOVED***
    server_delete(exec_ctx, server);
  ***REMOVED***
***REMOVED***

static int is_channel_orphaned(channel_data* chand) ***REMOVED***
  return chand->next == chand;
***REMOVED***

static void orphan_channel(channel_data* chand) ***REMOVED***
...
static void destroy_channel(grpc_exec_ctx* exec_ctx, channel_data* chand,
  ...
  if (grpc_server_channel_trace.enabled() && error != GRPC_ERROR_NONE) ***REMOVED***
...
static void done_request_event(grpc_exec_ctx* exec_ctx, void* req,
                               grpc_cq_completion* c) ***REMOVED***
  gpr_free(req);
***REMOVED***

static void publish_call(grpc_exec_ctx* exec_ctx, grpc_server* server,
                         call_data* calld, size_t cq_idx, requested_call* rc) ***REMOVED***
  grpc_call_set_completion_queue(exec_ctx, calld->call, rc->cq_bound_to_call);
  grpc_call* call = calld->call;
  *rc->call = call;
  calld->cq_new = server->cqs[cq_idx];
  GPR_SWAP(grpc_metadata_array, *rc->initial_metadata, calld->initial_metadata);
  switch (rc->type) ***REMOVED***
    case BATCH_CALL:
    ...
    case REGISTERED_CALL:
      *rc->data.registered.deadline =
          grpc_millis_to_timespec(calld->deadline, GPR_CLOCK_MONOTONIC);
      if (rc->data.registered.optional_payload) ***REMOVED***
...
static void publish_new_rpc(grpc_exec_ctx* exec_ctx, void* arg,
  ...
  if (error != GRPC_ERROR_NONE || gpr_atm_acq_load(&server->shutdown_flag)) ***REMOVED***
    gpr_atm_no_barrier_store(&calld->state, ZOMBIED);
    GRPC_CLOSURE_INIT(
        &calld->kill_zombie_closure, kill_zombie,
        grpc_call_stack_element(grpc_call_get_call_stack(calld->call), 0),
  ...
  ***REMOVED***
  ...
  if (rm->pending_head == nullptr) ***REMOVED***
...
static void finish_start_new_rpc(
    ...
    request_matcher* rm,
    grpc_server_register_method_payload_handling payload_handling) ***REMOVED***
  call_data* calld = (call_data*)elem->call_data;

  if (gpr_atm_acq_load(&server->shutdown_flag)) ***REMOVED***
    gpr_atm_no_barrier_store(&calld->state, ZOMBIED);
  ...
  ***REMOVED***
  ...
  switch (payload_handling) ***REMOVED***
    case GRPC_SRM_PAYLOAD_NONE:
      publish_new_rpc(exec_ctx, elem, GRPC_ERROR_NONE);
      break;
    case GRPC_SRM_PAYLOAD_READ_INITIAL_BYTE_BUFFER: ***REMOVED***
      grpc_op op;
...
static void start_new_rpc(grpc_exec_ctx* exec_ctx, grpc_call_element* elem) ***REMOVED***
  ...
  if (chand->registered_methods && calld->path_set && calld->host_set) ***REMOVED***
    /* TODO(ctiller): unify these two searches */
    /* check for an exact match with host */
    hash = GRPC_MDSTR_KV_HASH(grpc_slice_hash(calld->host),
                              grpc_slice_hash(calld->path));
    for (i = 0; i <= chand->registered_method_max_probes; i++) ***REMOVED***
      ...
      if ((rm->flags & GRPC_INITIAL_METADATA_IDEMPOTENT_REQUEST) &&
          0 == (calld->recv_initial_metadata_flags &
                GRPC_INITIAL_METADATA_IDEMPOTENT_REQUEST)) ***REMOVED***
        continue;
      ***REMOVED***
...
static int num_listeners(grpc_server* server) ***REMOVED***
  listener* l;
  int n = 0;
  for (l = server->listeners; l; l = l->next) ***REMOVED***
    n++;
  ***REMOVED***
  return n;
***REMOVED***
...
static int num_channels(grpc_server* server) ***REMOVED***
  channel_data* chand;
  int n = 0;
  for (chand = server->root_channel_data.next;
       chand != &server->root_channel_data; chand = chand->next) ***REMOVED***
    n++;
  ***REMOVED***
  return n;
***REMOVED***
...
static void kill_pending_work_locked(grpc_exec_ctx* exec_ctx,
                                     grpc_server* server, grpc_error* error) ***REMOVED***
  if (server->started) ***REMOVED***
...
static void maybe_finish_shutdown(grpc_exec_ctx* exec_ctx,
                                  grpc_server* server) ***REMOVED***
  size_t i;
  if (!gpr_atm_acq_load(&server->shutdown_flag) || server->shutdown_published) ***REMOVED***
    return;
  ***REMOVED***
  ...
  if (server->root_channel_data.next != &server->root_channel_data ||
      server->listeners_destroyed < num_listeners(server)) ***REMOVED***
    if (gpr_time_cmp(gpr_time_sub(gpr_now(GPR_CLOCK_REALTIME),
                                  server->last_shutdown_message_time),
                     gpr_time_from_seconds(1, GPR_TIMESPAN)) >= 0) ***REMOVED***
  ...
  ***REMOVED***
  ...
  for (i = 0; i < server->num_shutdown_tags; i++) ***REMOVED***
    server_ref(server);
...
static void server_on_recv_initial_metadata(grpc_exec_ctx* exec_ctx, void* ptr,
  ...
  ***REMOVED***
  ...
  if (op_deadline != GRPC_MILLIS_INF_FUTURE) ***REMOVED***
    calld->deadline = op_deadline;
  ***REMOVED***
  if (calld->host_set && calld->path_set) ***REMOVED***
    /* do nothing */
  ***REMOVED*** else ***REMOVED***
...
static void got_initial_metadata(grpc_exec_ctx* exec_ctx, void* ptr,
  ...
  ***REMOVED*** else ***REMOVED***
    if (gpr_atm_full_cas(&calld->state, NOT_STARTED, ZOMBIED)) ***REMOVED***
      GRPC_CLOSURE_INIT(&calld->kill_zombie_closure, kill_zombie, elem,
                        grpc_schedule_on_exec_ctx);
      GRPC_CLOSURE_SCHED(exec_ctx, &calld->kill_zombie_closure,
                         GRPC_ERROR_NONE);
    ***REMOVED*** else if (gpr_atm_full_cas(&calld->state, PENDING, ZOMBIED)) ***REMOVED***
...
static void accept_stream(grpc_exec_ctx* exec_ctx, void* cd,
                          ...
                          const void* transport_server_data) ***REMOVED***
  channel_data* chand = (channel_data*)cd;
  /* create a call */
...
static void channel_connectivity_changed(grpc_exec_ctx* exec_ctx, void* cd,
  ...
  channel_data* chand = (channel_data*)cd;
  grpc_server* server = chand->server;
  if (chand->connectivity_state != GRPC_CHANNEL_SHUTDOWN) ***REMOVED***
...
static void destroy_call_elem(grpc_exec_ctx* exec_ctx, grpc_call_element* elem,
                              ...
                              grpc_closure* ignored) ***REMOVED***
  channel_data* chand = (channel_data*)elem->channel_data;
  call_data* calld = (call_data*)elem->call_data;

  GPR_ASSERT(calld->state != PENDING);

  if (calld->host_set) ***REMOVED***
    grpc_slice_unref_internal(exec_ctx, calld->host);
  ***REMOVED***
  if (calld->path_set) ***REMOVED***
...
static void destroy_channel_elem(grpc_exec_ctx* exec_ctx,
  ...
  size_t i;
  channel_data* chand = (channel_data*)elem->channel_data;
  if (chand->registered_methods) ***REMOVED***
    for (i = 0; i < chand->registered_method_slots; i++) ***REMOVED***
      grpc_slice_unref_internal(exec_ctx, chand->registered_methods[i].method);
      if (chand->registered_methods[i].has_host) ***REMOVED***
        grpc_slice_unref_internal(exec_ctx, chand->registered_methods[i].host);
      ***REMOVED***
    ***REMOVED***
    gpr_free(chand->registered_methods);
  ***REMOVED***
  if (chand->server) ***REMOVED***
...
const grpc_channel_filter grpc_server_top_filter = ***REMOVED***
...
void grpc_server_register_completion_queue(grpc_server* server,
  ...
  if (grpc_get_cq_completion_type(cq) != GRPC_CQ_NEXT) ***REMOVED***
    gpr_log(GPR_INFO,
...
grpc_server* grpc_server_create(const grpc_channel_args* args, void* reserved) ***REMOVED***
...
static int streq(const char* a, const char* b) ***REMOVED***
...
void* grpc_server_register_method(
  ...
  if (!method) ***REMOVED***
    ...
    return nullptr;
  ***REMOVED***
  for (m = server->registered_methods; m; m = m->next) ***REMOVED***
    if (streq(m->method, method) && streq(m->host, host)) ***REMOVED***
      gpr_log(GPR_ERROR, "duplicate registration for %s@%s", method,
              host ? host : "*");
      return nullptr;
    ***REMOVED***
  ***REMOVED***
  if ((flags & ~GRPC_INITIAL_METADATA_USED_MASK) != 0) ***REMOVED***
...
static void start_listeners(grpc_exec_ctx* exec_ctx, void* s,
  ...
  for (listener* l = server->listeners; l; l = l->next) ***REMOVED***
...
void grpc_server_start(grpc_server* server) ***REMOVED***
  size_t i;
  ...
  for (i = 0; i < server->cq_count; i++) ***REMOVED***
    if (grpc_cq_can_listen(server->cqs[i])) ***REMOVED***
      server->pollsets[server->pollset_count++] =
          grpc_cq_pollset(server->cqs[i]);
    ***REMOVED***
  ***REMOVED***
  request_matcher_init(&server->unregistered_request_matcher, server);
  for (registered_method* rm = server->registered_methods; rm; rm = rm->next) ***REMOVED***
...
void grpc_server_setup_transport(grpc_exec_ctx* exec_ctx, grpc_server* s,
  ...
  size_t cq_idx;
  for (cq_idx = 0; cq_idx < s->cq_count; cq_idx++) ***REMOVED***
    if (grpc_cq_pollset(s->cqs[cq_idx]) == accepting_pollset) break;
  ***REMOVED***
  if (cq_idx == s->cq_count) ***REMOVED***
  ...
  ***REMOVED***
  ...
  for (rm = s->registered_methods; rm; rm = rm->next) ***REMOVED***
    num_registered_methods++;
  ***REMOVED***
  /* build a lookup table phrased in terms of mdstr's in this channels context
     to quickly find registered methods */
  if (num_registered_methods > 0) ***REMOVED***
  ...
  ***REMOVED***
  ...
  op->on_connectivity_state_change = &chand->channel_connectivity_changed;
  op->connectivity_state = &chand->connectivity_state;
  if (gpr_atm_acq_load(&s->shutdown_flag) != 0) ***REMOVED***
...
void grpc_server_shutdown_and_notify(grpc_server* server,
                                     grpc_completion_queue* cq, void* tag) ***REMOVED***
  listener* l;
  shutdown_tag* sdt;
  ...
  ***REMOVED***
  ...
  if (server->shutdown_published) ***REMOVED***
...
void grpc_server_cancel_all_calls(grpc_server* server) ***REMOVED***
  channel_broadcaster broadcaster;
  grpc_exec_ctx exec_ctx = GRPC_EXEC_CTX_INIT;
...
void grpc_server_destroy(grpc_server* server) ***REMOVED***
  listener* l;
  grpc_exec_ctx exec_ctx = GRPC_EXEC_CTX_INIT;
  ...
  while (server->listeners) ***REMOVED***
    l = server->listeners;
  ...
  ***REMOVED***
  switch (rc->type) ***REMOVED***
    ...
    case REGISTERED_CALL:
      ...
      break;
  ***REMOVED***
  if (gpr_locked_mpscq_push(&rm->requests_per_cq[cq_idx], &rc->request_link)) ***REMOVED***
    /* this was the first queued request: we need to lock and start
       matching calls */
    gpr_mu_lock(&server->mu_call);
    while ((calld = rm->pending_head) != nullptr) ***REMOVED***
      rc = (requested_call*)gpr_locked_mpscq_pop(&rm->requests_per_cq[cq_idx]);
      if (rc == nullptr) break;
      rm->pending_head = calld->pending_next;
      gpr_mu_unlock(&server->mu_call);
      if (!gpr_atm_full_cas(&calld->state, PENDING, ACTIVATED)) ***REMOVED***
        // Zombied Call
        GRPC_CLOSURE_INIT(
            &calld->kill_zombie_closure, kill_zombie,
            grpc_call_stack_element(grpc_call_get_call_stack(calld->call), 0),
...
grpc_call_error grpc_server_request_call(
    ...
    grpc_metadata_array* initial_metadata,
    grpc_completion_queue* cq_bound_to_call,
    grpc_completion_queue* cq_for_notification, void* tag) ***REMOVED***
  ...
  size_t cq_idx;
  for (cq_idx = 0; cq_idx < server->cq_count; cq_idx++) ***REMOVED***
    if (server->cqs[cq_idx] == cq_for_notification) ***REMOVED***
      break;
    ***REMOVED***
  ***REMOVED***
  if (cq_idx == server->cq_count) ***REMOVED***
    gpr_free(rc);
    error = GRPC_CALL_ERROR_NOT_SERVER_COMPLETION_QUEUE;
    goto done;
  ***REMOVED***
  if (grpc_cq_begin_op(cq_for_notification, tag) == false) ***REMOVED***
    gpr_free(rc);
...
grpc_call_error grpc_server_request_registered_call(
  ...
  ***REMOVED***
  if ((optional_payload == nullptr) !=
      (rm->payload_handling == GRPC_SRM_PAYLOAD_NONE)) ***REMOVED***
    gpr_free(rc);
...
static void fail_call(grpc_exec_ctx* exec_ctx, grpc_server* server,
                      size_t cq_idx, requested_call* rc, grpc_error* error) ***REMOVED***
  *rc->call = nullptr;
...
const grpc_channel_args* grpc_server_get_channel_args(grpc_server* server) ***REMOVED***
  return server->channel_args;
***REMOVED***

int grpc_server_has_open_connections(grpc_server* server) ***REMOVED***
  int r;
  gpr_mu_lock(&server->mu_global);
