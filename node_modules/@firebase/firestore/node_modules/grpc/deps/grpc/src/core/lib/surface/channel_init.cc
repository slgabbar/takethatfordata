...
typedef struct stage_slot ***REMOVED***
...
typedef struct stage_slots ***REMOVED***
  stage_slot* slots;
  size_t num_slots;
  size_t cap_slots;
***REMOVED*** stage_slots;
...
void grpc_channel_init_init(void) ***REMOVED***
  for (int i = 0; i < GRPC_NUM_CHANNEL_STACK_TYPES; i++) ***REMOVED***
...
void grpc_channel_init_register_stage(grpc_channel_stack_type type,
                                      ...
                                      void* stage_arg) ***REMOVED***
  GPR_ASSERT(!g_finalized);
  if (g_slots[type].cap_slots == g_slots[type].num_slots) ***REMOVED***
...
static int compare_slots(const void* a, const void* b) ***REMOVED***
...
void grpc_channel_init_finalize(void) ***REMOVED***
  GPR_ASSERT(!g_finalized);
  for (int i = 0; i < GRPC_NUM_CHANNEL_STACK_TYPES; i++) ***REMOVED***
...
void grpc_channel_init_shutdown(void) ***REMOVED***
  for (int i = 0; i < GRPC_NUM_CHANNEL_STACK_TYPES; i++) ***REMOVED***
...
bool grpc_channel_init_create_stack(grpc_exec_ctx* exec_ctx,
                                    ...
                                    grpc_channel_stack_type type) ***REMOVED***
  GPR_ASSERT(g_finalized);

  grpc_channel_stack_builder_set_name(builder,
                                      grpc_channel_stack_type_string(type));

  for (size_t i = 0; i < g_slots[type].num_slots; i++) ***REMOVED***
    const stage_slot* slot = &g_slots[type].slots[i];
    if (!slot->fn(exec_ctx, builder, slot->arg)) ***REMOVED***
      return false;
    ***REMOVED***
  ***REMOVED***
