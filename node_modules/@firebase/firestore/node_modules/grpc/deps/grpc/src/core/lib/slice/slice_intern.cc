...
typedef struct interned_slice_refcount ***REMOVED***
...
***REMOVED*** interned_slice_refcount;

typedef struct slice_shard ***REMOVED***
  gpr_mu mu;
...
typedef struct ***REMOVED***
  uint32_t hash;
  uint32_t idx;
***REMOVED*** static_metadata_hash_ent;
...
static void interned_slice_ref(void* p) ***REMOVED***
  interned_slice_refcount* s = (interned_slice_refcount*)p;
  GPR_ASSERT(gpr_atm_no_barrier_fetch_add(&s->refcnt, 1) > 0);
***REMOVED***

static void interned_slice_destroy(interned_slice_refcount* s) ***REMOVED***
...
static void interned_slice_unref(grpc_exec_ctx* exec_ctx, void* p) ***REMOVED***
  interned_slice_refcount* s = (interned_slice_refcount*)p;
  if (1 == gpr_atm_full_fetch_add(&s->refcnt, -1)) ***REMOVED***
    interned_slice_destroy(s);
  ***REMOVED***
***REMOVED***

static void interned_slice_sub_ref(void* p) ***REMOVED***
  interned_slice_ref(((char*)p) - offsetof(interned_slice_refcount, sub));
***REMOVED***

static void interned_slice_sub_unref(grpc_exec_ctx* exec_ctx, void* p) ***REMOVED***
  interned_slice_unref(exec_ctx,
                       ((char*)p) - offsetof(interned_slice_refcount, sub));
***REMOVED***

static uint32_t interned_slice_hash(grpc_slice slice) ***REMOVED***
  interned_slice_refcount* s = (interned_slice_refcount*)slice.refcount;
  return s->hash;
***REMOVED***

static int interned_slice_eq(grpc_slice a, grpc_slice b) ***REMOVED***
  return a.refcount == b.refcount;
***REMOVED***

static const grpc_slice_refcount_vtable interned_slice_vtable = ***REMOVED***
    interned_slice_ref, interned_slice_unref, interned_slice_eq,
    interned_slice_hash***REMOVED***;
static const grpc_slice_refcount_vtable interned_slice_sub_vtable = ***REMOVED***
    interned_slice_sub_ref, interned_slice_sub_unref,
    grpc_slice_default_eq_impl, grpc_slice_default_hash_impl***REMOVED***;

static void grow_shard(slice_shard* shard) ***REMOVED***
  ...
  for (i = 0; i < shard->capacity; i++) ***REMOVED***
    for (s = shard->strs[i]; s; s = next) ***REMOVED***
...
static grpc_slice materialize(interned_slice_refcount* s) ***REMOVED***
  grpc_slice slice;
...
uint32_t grpc_slice_default_hash_impl(grpc_slice s) ***REMOVED***
  return gpr_murmur_hash3(GRPC_SLICE_START_PTR(s), GRPC_SLICE_LENGTH(s),
                          g_hash_seed);
***REMOVED***

uint32_t grpc_static_slice_hash(grpc_slice s) ***REMOVED***
  return static_metadata_hash_values[GRPC_STATIC_METADATA_INDEX(s)];
***REMOVED***

int grpc_static_slice_eq(grpc_slice a, grpc_slice b) ***REMOVED***
  return GRPC_STATIC_METADATA_INDEX(a) == GRPC_STATIC_METADATA_INDEX(b);
***REMOVED***

uint32_t grpc_slice_hash(grpc_slice s) ***REMOVED***
  return s.refcount == nullptr ? grpc_slice_default_hash_impl(s)
                               : s.refcount->vtable->hash(s);
***REMOVED***

grpc_slice grpc_slice_maybe_static_intern(grpc_slice slice,
                                          bool* returned_slice_is_different) ***REMOVED***
  if (GRPC_IS_STATIC_METADATA_STRING(slice)) ***REMOVED***
    return slice;
  ***REMOVED***

  uint32_t hash = grpc_slice_hash(slice);
  for (uint32_t i = 0; i <= max_static_metadata_hash_probe; i++) ***REMOVED***
    static_metadata_hash_ent ent =
        static_metadata_hash[(hash + i) % GPR_ARRAY_SIZE(static_metadata_hash)];
    if (ent.hash == hash && ent.idx < GRPC_STATIC_MDSTR_COUNT &&
        grpc_slice_eq(grpc_static_slice_table[ent.idx], slice)) ***REMOVED***
...
bool grpc_slice_is_interned(grpc_slice slice) ***REMOVED***
  return (slice.refcount && slice.refcount->vtable == &interned_slice_vtable) ||
         GRPC_IS_STATIC_METADATA_STRING(slice);
***REMOVED***

grpc_slice grpc_slice_intern(grpc_slice slice) ***REMOVED***
  ...
  ***REMOVED***
  ...
  for (s = shard->strs[idx]; s; s = s->bucket_next) ***REMOVED***
    if (s->hash == hash && grpc_slice_eq(slice, materialize(s))) ***REMOVED***
      if (gpr_atm_no_barrier_fetch_add(&s->refcnt, 1) == 0) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (shard->count > shard->capacity * 2) ***REMOVED***
    grow_shard(shard);
  ***REMOVED***
...
void grpc_test_only_set_slice_hash_seed(uint32_t seed) ***REMOVED***
  g_hash_seed = seed;
  g_forced_hash_seed = 1;
***REMOVED***

void grpc_slice_intern_init(void) ***REMOVED***
  if (!g_forced_hash_seed) ***REMOVED***
    g_hash_seed = (uint32_t)gpr_now(GPR_CLOCK_REALTIME).tv_nsec;
  ***REMOVED***
  for (size_t i = 0; i < SHARD_COUNT; i++) ***REMOVED***
...
void grpc_slice_intern_shutdown(void) ***REMOVED***
  for (size_t i = 0; i < SHARD_COUNT; i++) ***REMOVED***
    slice_shard* shard = &g_shards[i];
    gpr_mu_destroy(&shard->mu);
    /* TODO(ctiller): GPR_ASSERT(shard->count == 0); */
    if (shard->count != 0) ***REMOVED***
      gpr_log(GPR_DEBUG, "WARNING: %" PRIuPTR " metadata strings were leaked",
              shard->count);
      for (size_t j = 0; j < shard->capacity; j++) ***REMOVED***
        for (interned_slice_refcount* s = shard->strs[j]; s;
             s = s->bucket_next) ***REMOVED***
          char* text =
        ...
        ***REMOVED***
      ***REMOVED***
      if (grpc_iomgr_abort_on_leaks()) ***REMOVED***
        abort();
      ***REMOVED***
    ***REMOVED***
    gpr_free(shard->strs);
