...
static void plugin_destruct(grpc_exec_ctx* exec_ctx,
  ...
  gpr_mu_destroy(&c->mu);
  if (c->plugin.state != nullptr && c->plugin.destroy != nullptr) ***REMOVED***
...
static void pending_request_remove_locked(
    ...
    grpc_plugin_credentials_pending_request* pending_request) ***REMOVED***
  if (pending_request->prev == nullptr) ***REMOVED***
    c->pending_requests = pending_request->next;
  ***REMOVED*** else ***REMOVED***
    pending_request->prev->next = pending_request->next;
  ***REMOVED***
  if (pending_request->next != nullptr) ***REMOVED***
...
static void pending_request_complete(
    grpc_exec_ctx* exec_ctx, grpc_plugin_credentials_pending_request* r) ***REMOVED***
...
static grpc_error* process_plugin_result(
    ...
    const char* error_details) ***REMOVED***
  ...
  ***REMOVED*** else ***REMOVED***
    ...
    for (size_t i = 0; i < num_md; ++i) ***REMOVED***
      if (!GRPC_LOG_IF_ERROR("validate_metadata_from_plugin",
                             grpc_validate_header_key_is_legal(md[i].key))) ***REMOVED***
        seen_illegal_header = true;
        break;
      ***REMOVED*** else if (!grpc_is_binary_header(md[i].key) &&
                 !GRPC_LOG_IF_ERROR(
                     "validate_metadata_from_plugin",
                     grpc_validate_header_nonbin_value_is_legal(md[i].value))) ***REMOVED***
        gpr_log(GPR_ERROR, "Plugin added invalid metadata value.");
        seen_illegal_header = true;
        break;
      ***REMOVED***
    ***REMOVED***
    if (seen_illegal_header) ***REMOVED***
...
static void plugin_md_request_metadata_ready(void* request,
  ...
  if (grpc_plugin_credentials_trace.enabled()) ***REMOVED***
    gpr_log(GPR_INFO,
...
static bool plugin_get_request_metadata(grpc_exec_ctx* exec_ctx,
  ...
  if (c->plugin.get_metadata != nullptr) ***REMOVED***
    ...
    gpr_mu_lock(&c->mu);
    if (c->pending_requests != nullptr) ***REMOVED***
    ...
    ***REMOVED***
    ...
    if (pending_request->cancelled) ***REMOVED***
      if (grpc_plugin_credentials_trace.enabled()) ***REMOVED***
        gpr_log(GPR_INFO,
    ...
    ***REMOVED***
    // Clean up.
    for (size_t i = 0; i < num_creds_md; ++i) ***REMOVED***
...
static void plugin_cancel_get_request_metadata(
  ...
  for (grpc_plugin_credentials_pending_request* pending_request =
           c->pending_requests;
       pending_request != nullptr; pending_request = pending_request->next) ***REMOVED***
...
static grpc_call_credentials_vtable plugin_vtable = ***REMOVED***
    plugin_destruct, plugin_get_request_metadata,
    plugin_cancel_get_request_metadata***REMOVED***;

grpc_call_credentials* grpc_metadata_credentials_create_from_plugin(
    grpc_metadata_credentials_plugin plugin, void* reserved) ***REMOVED***
