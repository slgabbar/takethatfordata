...
int grpc_auth_refresh_token_is_valid(
    const grpc_auth_refresh_token* refresh_token) ***REMOVED***
  return (refresh_token != nullptr) &&
         strcmp(refresh_token->type, GRPC_AUTH_JSON_TYPE_INVALID);
***REMOVED***

grpc_auth_refresh_token grpc_auth_refresh_token_create_from_json(
    const grpc_json* json) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (prop_value == nullptr ||
      strcmp(prop_value, GRPC_AUTH_JSON_TYPE_AUTHORIZED_USER)) ***REMOVED***
    goto end;
  ***REMOVED***
...
grpc_auth_refresh_token grpc_auth_refresh_token_create_from_string(
    const char* json_string) ***REMOVED***
...
void grpc_auth_refresh_token_destruct(grpc_auth_refresh_token* refresh_token) ***REMOVED***
  if (refresh_token == nullptr) return;
  refresh_token->type = GRPC_AUTH_JSON_TYPE_INVALID;
  if (refresh_token->client_id != nullptr) ***REMOVED***
    gpr_free(refresh_token->client_id);
    refresh_token->client_id = nullptr;
  ***REMOVED***
  if (refresh_token->client_secret != nullptr) ***REMOVED***
    gpr_free(refresh_token->client_secret);
    refresh_token->client_secret = nullptr;
  ***REMOVED***
  if (refresh_token->refresh_token != nullptr) ***REMOVED***
...
grpc_oauth2_token_fetcher_credentials_parse_server_response(
    ...
    grpc_mdelem* token_md, grpc_millis* token_lifetime) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (response->body_length > 0) ***REMOVED***
  ...
  ***REMOVED*** else ***REMOVED***
    ...
    if (json == nullptr) ***REMOVED***
      ...
      goto end;
    ***REMOVED***
    if (json->type != GRPC_JSON_OBJECT) ***REMOVED***
      gpr_log(GPR_ERROR, "Response should be a JSON object");
      status = GRPC_CREDENTIALS_ERROR;
      goto end;
    ***REMOVED***
    for (ptr = json->child; ptr; ptr = ptr->next) ***REMOVED***
      if (strcmp(ptr->key, "access_token") == 0) ***REMOVED***
        access_token = ptr;
      ***REMOVED*** else if (strcmp(ptr->key, "token_type") == 0) ***REMOVED***
        token_type = ptr;
      ***REMOVED*** else if (strcmp(ptr->key, "expires_in") == 0) ***REMOVED***
        expires_in = ptr;
      ***REMOVED***
    ***REMOVED***
    if (access_token == nullptr || access_token->type != GRPC_JSON_STRING) ***REMOVED***
      gpr_log(GPR_ERROR, "Missing or invalid access_token in JSON.");
      status = GRPC_CREDENTIALS_ERROR;
      goto end;
    ***REMOVED***
    if (token_type == nullptr || token_type->type != GRPC_JSON_STRING) ***REMOVED***
      gpr_log(GPR_ERROR, "Missing or invalid token_type in JSON.");
      status = GRPC_CREDENTIALS_ERROR;
      goto end;
    ***REMOVED***
    if (expires_in == nullptr || expires_in->type != GRPC_JSON_NUMBER) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (status != GRPC_CREDENTIALS_OK && !GRPC_MDISNULL(*token_md)) ***REMOVED***
...
static void on_oauth2_token_fetcher_http_response(grpc_exec_ctx* exec_ctx,
  ...
  while (pending_request != nullptr) ***REMOVED***
    if (status == GRPC_CREDENTIALS_OK) ***REMOVED***
...
static bool oauth2_token_fetcher_get_request_metadata(
  ...
  if (!GRPC_MDISNULL(c->access_token_md) &&
      (c->token_expiration - grpc_exec_ctx_now(exec_ctx) > refresh_threshold)) ***REMOVED***
    cached_access_token_md = GRPC_MDELEM_REF(c->access_token_md);
  ***REMOVED***
  if (!GRPC_MDISNULL(cached_access_token_md)) ***REMOVED***
    gpr_mu_unlock(&c->mu);
  ...
  ***REMOVED***
  ...
  if (!c->token_fetch_pending) ***REMOVED***
    c->token_fetch_pending = true;
    start_fetch = true;
  ***REMOVED***
  gpr_mu_unlock(&c->mu);
  if (start_fetch) ***REMOVED***
...
static void oauth2_token_fetcher_cancel_get_request_metadata(
  ...
  grpc_oauth2_pending_get_request_metadata* pending_request =
      c->pending_requests;
  while (pending_request != nullptr) ***REMOVED***
    if (pending_request->md_array == md_array) ***REMOVED***
...
static void init_oauth2_token_fetcher(grpc_oauth2_token_fetcher_credentials* c,
                                      grpc_fetch_oauth2_func fetch_func) ***REMOVED***
...
static grpc_call_credentials_vtable compute_engine_vtable = ***REMOVED***
    oauth2_token_fetcher_destruct, oauth2_token_fetcher_get_request_metadata,
    oauth2_token_fetcher_cancel_get_request_metadata***REMOVED***;

static void compute_engine_fetch_oauth2(
    grpc_exec_ctx* exec_ctx, grpc_credentials_metadata_request* metadata_req,
    grpc_httpcli_context* httpcli_context, grpc_polling_entity* pollent,
    grpc_iomgr_cb_func response_cb, grpc_millis deadline) ***REMOVED***
  grpc_http_header header = ***REMOVED***(char*)"Metadata-Flavor", (char*)"Google"***REMOVED***;
...
static grpc_call_credentials_vtable refresh_token_vtable = ***REMOVED***
...
static void refresh_token_fetch_oauth2(
  ...
  grpc_http_header header = ***REMOVED***(char*)"Content-Type",
                             (char*)"application/x-www-form-urlencoded"***REMOVED***;
  grpc_httpcli_request request;
  char* body = nullptr;
...
grpc_refresh_token_credentials_create_from_auth_refresh_token(
    grpc_auth_refresh_token refresh_token) ***REMOVED***
  grpc_google_refresh_token_credentials* c;
  if (!grpc_auth_refresh_token_is_valid(&refresh_token)) ***REMOVED***
...
static char* create_loggable_refresh_token(grpc_auth_refresh_token* token) ***REMOVED***
  if (strcmp(token->type, GRPC_AUTH_JSON_TYPE_INVALID) == 0) ***REMOVED***
...
grpc_call_credentials* grpc_google_refresh_token_credentials_create(
    const char* json_refresh_token, void* reserved) ***REMOVED***
  grpc_auth_refresh_token token =
      grpc_auth_refresh_token_create_from_string(json_refresh_token);
  if (grpc_api_trace.enabled()) ***REMOVED***
...
static grpc_call_credentials_vtable access_token_vtable = ***REMOVED***
    access_token_destruct, access_token_get_request_metadata,
    access_token_cancel_get_request_metadata***REMOVED***;

grpc_call_credentials* grpc_access_token_credentials_create(
    const char* access_token, void* reserved) ***REMOVED***
