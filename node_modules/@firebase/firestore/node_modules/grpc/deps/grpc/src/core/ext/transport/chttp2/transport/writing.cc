...
static void add_to_write_list(grpc_chttp2_write_cb** list,
                              grpc_chttp2_write_cb* cb) ***REMOVED***
  cb->next = *list;
  *list = cb;
***REMOVED***
...
static void maybe_initiate_ping(grpc_exec_ctx* exec_ctx,
  ...
  if (grpc_closure_list_empty(pq->lists[GRPC_CHTTP2_PCL_NEXT])) ***REMOVED***
    ...
    return;
  ***REMOVED***
  if (!grpc_closure_list_empty(pq->lists[GRPC_CHTTP2_PCL_INFLIGHT])) ***REMOVED***
    /* ping already in-flight: wait */
    if (grpc_http_trace.enabled() || grpc_bdp_estimator_trace.enabled()) ***REMOVED***
  ...
  ***REMOVED***
  if (t->ping_state.pings_before_data_required == 0 &&
      t->ping_policy.max_pings_without_data != 0) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (next_allowed_ping > now) ***REMOVED***
...
static bool update_list(grpc_exec_ctx* exec_ctx, grpc_chttp2_transport* t,
  ...
  *ctr += send_bytes;
  while (cb) ***REMOVED***
    ...
    if (cb->call_at_byte <= *ctr) ***REMOVED***
      sched_any = true;
...
static bool stream_ref_if_not_destroyed(gpr_refcount* r) ***REMOVED***
  gpr_atm count;
  do ***REMOVED***
    count = gpr_atm_acq_load(&r->count);
    if (count == 0) return false;
  ***REMOVED*** while (!gpr_atm_rel_cas(&r->count, count, count + 1));
  return true;
***REMOVED***

/* How many bytes would we like to put on the wire during a single syscall */
static uint32_t target_write_size(grpc_chttp2_transport* t) ***REMOVED***
  return 1024 * 1024;
***REMOVED***

// Returns true if initial_metadata contains only default headers.
static bool is_default_initial_metadata(grpc_metadata_batch* initial_metadata) ***REMOVED***
...
class WriteContext ***REMOVED***
 public:
  WriteContext(grpc_exec_ctx* exec_ctx, grpc_chttp2_transport* t) : t_(t) ***REMOVED***
    GRPC_STATS_INC_HTTP2_WRITES_BEGUN(exec_ctx);
    GPR_TIMER_BEGIN("grpc_chttp2_begin_write", 0);
  ***REMOVED***

  // TODO(ctiller): make this the destructor
  void FlushStats(grpc_exec_ctx* exec_ctx) ***REMOVED***
  ...
  ***REMOVED***
  ...
  void FlushSettings(grpc_exec_ctx* exec_ctx) ***REMOVED***
    if (t_->dirtied_local_settings && !t_->sent_local_settings) ***REMOVED***
  ...
  ***REMOVED***
  ...
  void FlushQueuedBuffers(grpc_exec_ctx* exec_ctx) ***REMOVED***
    /* simple writes are queued to qbuf, and flushed here */
    grpc_slice_buffer_move_into(&t_->qbuf, &t_->outbuf);
    GPR_ASSERT(t_->qbuf.count == 0);
  ***REMOVED***

  void FlushWindowUpdates(grpc_exec_ctx* exec_ctx) ***REMOVED***
    uint32_t transport_announce =
        t_->flow_control->MaybeSendUpdate(t_->outbuf.count > 0);
    if (transport_announce) ***REMOVED***
  ...
  ***REMOVED***
  ...
  void FlushPingAcks() ***REMOVED***
    for (size_t i = 0; i < t_->ping_ack_count; i++) ***REMOVED***
  ...
  ***REMOVED***
  ...
  void EnactHpackSettings(grpc_exec_ctx* exec_ctx) ***REMOVED***
  ...
  ***REMOVED***
  ...
  void UpdateStreamsNoLongerStalled() ***REMOVED***
    grpc_chttp2_stream* s;
    while (grpc_chttp2_list_pop_stalled_by_transport(t_, &s)) ***REMOVED***
      if (t_->closed_with_error == GRPC_ERROR_NONE &&
          grpc_chttp2_list_add_writable_stream(t_, s)) ***REMOVED***
        if (!stream_ref_if_not_destroyed(&s->refcount->refs)) ***REMOVED***
  ...
  ***REMOVED***
  ...
  grpc_chttp2_stream* NextStream() ***REMOVED***
    if (t_->outbuf.length > target_write_size(t_)) ***REMOVED***
      result_.partial = true;
      return nullptr;
    ***REMOVED***

    grpc_chttp2_stream* s;
    if (!grpc_chttp2_list_pop_writable_stream(t_, &s)) ***REMOVED***
      return nullptr;
    ***REMOVED***
  ...
  ***REMOVED***
  ...
  void ResetPingRecvClock() ***REMOVED***
    if (!t_->is_client) ***REMOVED***
      t_->ping_recv_state.last_ping_recv_time = GRPC_MILLIS_INF_PAST;
      t_->ping_recv_state.ping_strikes = 0;
    ***REMOVED***
  ***REMOVED***

  void IncInitialMetadataWrites() ***REMOVED*** ++initial_metadata_writes_; ***REMOVED***
  void IncWindowUpdateWrites() ***REMOVED*** ++flow_control_writes_; ***REMOVED***
  void IncMessageWrites() ***REMOVED*** ++message_writes_; ***REMOVED***
  void IncTrailingMetadataWrites() ***REMOVED*** ++trailing_metadata_writes_; ***REMOVED***

  void NoteScheduledResults() ***REMOVED*** result_.early_results_scheduled = true; ***REMOVED***

  grpc_chttp2_transport* transport() const ***REMOVED*** return t_; ***REMOVED***

  grpc_chttp2_begin_write_result Result() ***REMOVED***
 ...
 private:
  ...
  grpc_chttp2_begin_write_result result_ = ***REMOVED***false, false, false***REMOVED***;
***REMOVED***;

class DataSendContext ***REMOVED***
 public:
  DataSendContext(WriteContext* write_context, grpc_chttp2_transport* t,
      ...
      : write_context_(write_context),
        t_(t),
        s_(s),
        sending_bytes_before_(s_->sending_bytes) ***REMOVED******REMOVED***

  uint32_t stream_remote_window() const ***REMOVED***
  ...
  ***REMOVED***
  ...
  uint32_t max_outgoing() const ***REMOVED***
    return (uint32_t)GPR_MIN(
        t_->settings[GRPC_PEER_SETTINGS][GRPC_CHTTP2_SETTINGS_MAX_FRAME_SIZE],
        GPR_MIN(stream_remote_window(), t_->flow_control->remote_window()));
  ***REMOVED***

  bool AnyOutgoing() const ***REMOVED*** return max_outgoing() > 0; ***REMOVED***

  void FlushCompressedBytes() ***REMOVED***
    uint32_t send_bytes =
    ...
    ***REMOVED***
    ...
    if (s_->compressed_data_buffer.length == 0) ***REMOVED***
      s_->sending_bytes += s_->uncompressed_data_size;
    ***REMOVED***
  ***REMOVED***

  void CompressMoreBytes() ***REMOVED***
    if (s_->stream_compression_ctx == nullptr) ***REMOVED***
    ...
    ***REMOVED***
    ...
    if (!grpc_stream_compress(s_->stream_compression_ctx,
                              ...
                              &s_->compressed_data_buffer, nullptr, MAX_SIZE_T,
                              GRPC_STREAM_COMPRESSION_FLUSH_SYNC)) ***REMOVED***
      gpr_log(GPR_ERROR, "Stream compression failed.");
    ***REMOVED***
  ***REMOVED***

  bool is_last_frame() const ***REMOVED*** return is_last_frame_; ***REMOVED***

  void CallCallbacks(grpc_exec_ctx* exec_ctx) ***REMOVED***
    if (update_list(exec_ctx, t_, s_,
                    (int64_t)(s_->sending_bytes - sending_bytes_before_),
                    &s_->on_flow_controlled_cbs,
                    &s_->flow_controlled_bytes_flowed, GRPC_ERROR_NONE)) ***REMOVED***
...
class StreamWriteContext ***REMOVED***
 public:
  StreamWriteContext(WriteContext* write_context, grpc_chttp2_stream* s)
      : write_context_(write_context), t_(write_context->transport()), s_(s) ***REMOVED***
  ...
  ***REMOVED***
  ...
  void FlushInitialMetadata(grpc_exec_ctx* exec_ctx) ***REMOVED***
    ...
    if (!t_->is_client && s_->fetching_send_message == nullptr &&
        ...
        s_->send_trailing_metadata != nullptr &&
        is_default_initial_metadata(s_->send_initial_metadata)) ***REMOVED***
      ConvertInitialMetadataToTrailingMetadata();
    ***REMOVED*** else ***REMOVED***
      grpc_encode_header_options hopt = ***REMOVED***
          s_->id,  // stream_id
          false,   // is_eof
          t_->settings[GRPC_PEER_SETTINGS]
  ...
  ***REMOVED***
  ...
  void FlushData(grpc_exec_ctx* exec_ctx) ***REMOVED***
    if (!s_->sent_initial_metadata) return;

    if (s_->flow_controlled_buffer.length == 0 &&
        s_->compressed_data_buffer.length == 0) ***REMOVED***
      return;  // early out: nothing to do
    ***REMOVED***

    DataSendContext data_send_context(write_context_, t_, s_);

    if (!data_send_context.AnyOutgoing()) ***REMOVED***
      if (t_->flow_control->remote_window() <= 0) ***REMOVED***
        report_stall(t_, s_, "transport");
        grpc_chttp2_list_add_stalled_by_transport(t_, s_);
      ***REMOVED*** else if (data_send_context.stream_remote_window() <= 0) ***REMOVED***
    ...
    ***REMOVED***
    ...
    while ((s_->flow_controlled_buffer.length > 0 ||
            s_->compressed_data_buffer.length > 0) &&
           data_send_context.max_outgoing() > 0) ***REMOVED***
      if (s_->compressed_data_buffer.length > 0) ***REMOVED***
    ...
    ***REMOVED***
    write_context_->ResetPingRecvClock();
    if (data_send_context.is_last_frame()) ***REMOVED***
  ...
  ***REMOVED***
  ...
  void FlushTrailingMetadata(grpc_exec_ctx* exec_ctx) ***REMOVED***
    if (!s_->sent_initial_metadata) return;
    ...
    if (grpc_metadata_batch_is_empty(s_->send_trailing_metadata)) ***REMOVED***
  ...
  ***REMOVED***
  ...
  bool stream_became_writable() ***REMOVED*** return stream_became_writable_; ***REMOVED***

 private:
  void ConvertInitialMetadataToTrailingMetadata() ***REMOVED***
    GRPC_CHTTP2_IF_TRACING(
        gpr_log(GPR_INFO, "not sending initial_metadata (Trailers-Only)"));
    // When sending Trailers-Only, we need to move the :status and
    // content-type headers to the trailers.
    if (s_->send_initial_metadata->idx.named.status != nullptr) ***REMOVED***
      extra_headers_for_trailing_metadata_
          [num_extra_headers_for_trailing_metadata_++] =
              &s_->send_initial_metadata->idx.named.status->md;
    ***REMOVED***
    if (s_->send_initial_metadata->idx.named.content_type != nullptr) ***REMOVED***
      extra_headers_for_trailing_metadata_
          [num_extra_headers_for_trailing_metadata_++] =
  ...
  ***REMOVED***
  ...
  void SentLastFrame(grpc_exec_ctx* exec_ctx) ***REMOVED***
    s_->send_trailing_metadata = nullptr;
    s_->sent_trailing_metadata = true;

    if (!t_->is_client && !s_->read_closed) ***REMOVED***
      grpc_slice_buffer_add(
...
grpc_chttp2_begin_write_result grpc_chttp2_begin_write(
    grpc_exec_ctx* exec_ctx, grpc_chttp2_transport* t) ***REMOVED***
  ...
  if (t->flow_control->remote_window() > 0) ***REMOVED***
    ctx.UpdateStreamsNoLongerStalled();
  ***REMOVED***

  /* for each grpc_chttp2_stream that's become writable, frame it's data
     (according to available window sizes) and add to the output buffer */
  while (grpc_chttp2_stream* s = ctx.NextStream()) ***REMOVED***
    ...
    if (stream_ctx.stream_became_writable()) ***REMOVED***
      if (!grpc_chttp2_list_add_writing_stream(t, s)) ***REMOVED***
...
void grpc_chttp2_end_write(grpc_exec_ctx* exec_ctx, grpc_chttp2_transport* t,
  ...
  while (grpc_chttp2_list_pop_writing_stream(t, &s)) ***REMOVED***
    if (s->sending_bytes != 0) ***REMOVED***
