...
static void uv_close_callback(uv_handle_t* handle) ***REMOVED***
...
static void call_read_cb(grpc_exec_ctx* exec_ctx, grpc_tcp* tcp,
  ...
  if (grpc_tcp_trace.enabled()) ***REMOVED***
    ...
    for (i = 0; i < tcp->read_slices->count; i++) ***REMOVED***
...
static void read_callback(uv_stream_t* stream, ssize_t nread,
  ...
  ***REMOVED*** else if (nread > 0) ***REMOVED***
    // Successful read
    error = GRPC_ERROR_NONE;
    if ((size_t)nread < tcp->read_slices->length) ***REMOVED***
...
static void write_callback(uv_write_t* req, int status) ***REMOVED***
...
static void uv_endpoint_write(grpc_exec_ctx* exec_ctx, grpc_endpoint* ep,
  ...
  if (grpc_tcp_trace.enabled()) ***REMOVED***
    ...
    for (j = 0; j < write_slices->count; j++) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (tcp->write_slices->count == 0) ***REMOVED***
  ...
  ***REMOVED***
  ...
  for (i = 0; i < buffer_count; i++) ***REMOVED***
...
static void uv_add_to_pollset_set(grpc_exec_ctx* exec_ctx, grpc_endpoint* ep,
                                  grpc_pollset_set* pollset) ***REMOVED***
  // No-op. We're ignoring pollsets currently
  (void)exec_ctx;
  (void)ep;
  (void)pollset;
...
static void shutdown_callback(uv_shutdown_t* req, int status) ***REMOVED******REMOVED***
...
static void uv_destroy(grpc_exec_ctx* exec_ctx, grpc_endpoint* ep) ***REMOVED***
  grpc_network_status_unregister_endpoint(ep);
  grpc_tcp* tcp = (grpc_tcp*)ep;
  uv_close((uv_handle_t*)tcp->handle, uv_close_callback);
***REMOVED***

static char* uv_get_peer(grpc_endpoint* ep) ***REMOVED***
  grpc_tcp* tcp = (grpc_tcp*)ep;
  return gpr_strdup(tcp->peer_string);
***REMOVED***

static grpc_resource_user* uv_get_resource_user(grpc_endpoint* ep) ***REMOVED***
  grpc_tcp* tcp = (grpc_tcp*)ep;
  return tcp->resource_user;
***REMOVED***

static int uv_get_fd(grpc_endpoint* ep) ***REMOVED*** return -1; ***REMOVED***

static grpc_endpoint_vtable vtable = ***REMOVED***uv_endpoint_read,
                                      uv_endpoint_write,
                                      uv_add_to_pollset,
