...
grpc_error* grpc_tcp_server_create(grpc_exec_ctx* exec_ctx,
                                   ...
                                   grpc_tcp_server** server) ***REMOVED***
  gpr_once_init(&check_init, init);

  grpc_tcp_server* s = (grpc_tcp_server*)gpr_zalloc(sizeof(grpc_tcp_server));
  s->so_reuseport = has_so_reuseport;
  s->expand_wildcard_addrs = false;
  for (size_t i = 0; i < (args == nullptr ? 0 : args->num_args); i++) ***REMOVED***
    if (0 == strcmp(GRPC_ARG_ALLOW_REUSEPORT, args->args[i].key)) ***REMOVED***
      ...
      ***REMOVED*** else ***REMOVED***
        ...
        return GRPC_ERROR_CREATE_FROM_STATIC_STRING(GRPC_ARG_ALLOW_REUSEPORT
                                                    " must be an integer");
      ***REMOVED***
    ***REMOVED*** else if (0 == strcmp(GRPC_ARG_EXPAND_WILDCARD_ADDRS, args->args[i].key)) ***REMOVED***
      if (args->args[i].type == GRPC_ARG_INTEGER) ***REMOVED***
...
static void finish_shutdown(grpc_exec_ctx* exec_ctx, grpc_tcp_server* s) ***REMOVED***
  gpr_mu_lock(&s->mu);
  GPR_ASSERT(s->shutdown);
  gpr_mu_unlock(&s->mu);
  if (s->shutdown_complete != nullptr) ***REMOVED***
    GRPC_CLOSURE_SCHED(exec_ctx, s->shutdown_complete, GRPC_ERROR_NONE);
  ***REMOVED***

  gpr_mu_destroy(&s->mu);

  while (s->head) ***REMOVED***
...
static void destroyed_port(grpc_exec_ctx* exec_ctx, void* server,
  ...
  gpr_mu_lock(&s->mu);
  s->destroyed_ports++;
  if (s->destroyed_ports == s->nports) ***REMOVED***
    gpr_mu_unlock(&s->mu);
...
/* called when all listening endpoints have been shutdown, so no further
   ...
   things */
static void deactivated_all_ports(grpc_exec_ctx* exec_ctx, grpc_tcp_server* s) ***REMOVED***
  ...
  if (s->head) ***REMOVED***
    ...
    for (sp = s->head; sp; sp = sp->next) ***REMOVED***
...
static void tcp_server_destroy(grpc_exec_ctx* exec_ctx, grpc_tcp_server* s) ***REMOVED***
  gpr_mu_lock(&s->mu);
...
static void on_read(grpc_exec_ctx* exec_ctx, void* arg, grpc_error* err) ***REMOVED***
    ...
    if (fd < 0) ***REMOVED***
      switch (errno) ***REMOVED***
        ...
        default:
          ...
          if (!sp->server->shutdown_listeners) ***REMOVED***
...
error:
  gpr_mu_lock(&sp->server->mu);
  if (0 == --sp->server->active_ports && sp->server->shutdown) ***REMOVED***
    gpr_mu_unlock(&sp->server->mu);
...
static grpc_error* add_wildcard_addrs_to_server(grpc_tcp_server* s,
  ...
  *out_port = -1;
  ...
  if (grpc_tcp_server_have_ifaddrs() && s->expand_wildcard_addrs) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if ((v6_err = grpc_tcp_server_add_addr(s, &wild6, port_index, fd_index,
                                         &dsmode, &sp)) == GRPC_ERROR_NONE) ***REMOVED***
    ++fd_index;
    requested_port = *out_port = sp->port;
    if (dsmode == GRPC_DSMODE_DUALSTACK || dsmode == GRPC_DSMODE_IPV4) ***REMOVED***
      return GRPC_ERROR_NONE;
    ***REMOVED***
  ***REMOVED***
  ...
  if ((v4_err = grpc_tcp_server_add_addr(s, &wild4, port_index, fd_index,
                                         &dsmode, &sp2)) == GRPC_ERROR_NONE) ***REMOVED***
    *out_port = sp2->port;
    if (sp != nullptr) ***REMOVED***
      sp2->is_sibling = 1;
      sp->sibling = sp2;
    ***REMOVED***
  ***REMOVED***
  if (*out_port > 0) ***REMOVED***
    if (v6_err != GRPC_ERROR_NONE) ***REMOVED***
      gpr_log(GPR_INFO,
    ...
    ***REMOVED***
    if (v4_err != GRPC_ERROR_NONE) ***REMOVED***
      gpr_log(GPR_INFO,
...
static grpc_error* clone_port(grpc_tcp_listener* listener, unsigned count) ***REMOVED***
  grpc_tcp_listener* sp = nullptr;
  char* addr_str;
  char* name;
  grpc_error* err;

  for (grpc_tcp_listener* l = listener->next; l && l->is_sibling; l = l->next) ***REMOVED***
    l->fd_index += count;
  ***REMOVED***
  ...
  for (unsigned i = 0; i < count; i++) ***REMOVED***
    ...
    GPR_ASSERT(sp->emfd);
    while (listener->server->tail->next != nullptr) ***REMOVED***
...
grpc_error* grpc_tcp_server_add_port(grpc_tcp_server* s,
  ...
  ***REMOVED***
  ...
  if (requested_port == 0) ***REMOVED***
    for (sp = s->head; sp; sp = sp->next) ***REMOVED***
      sockname_temp.len = sizeof(struct sockaddr_storage);
      if (0 == getsockname(sp->fd, (struct sockaddr*)&sockname_temp.addr,
                           (socklen_t*)&sockname_temp.len)) ***REMOVED***
        int used_port = grpc_sockaddr_get_port(&sockname_temp);
        if (used_port > 0) ***REMOVED***
    ...
    ***REMOVED***
  ***REMOVED***
  if (grpc_sockaddr_is_wildcard(addr, &requested_port)) ***REMOVED***
    return add_wildcard_addrs_to_server(s, port_index, requested_port,
                                        out_port);
  ***REMOVED***
  if (grpc_sockaddr_to_v4mapped(addr, &addr6_v4mapped)) ***REMOVED***
    addr = &addr6_v4mapped;
  ***REMOVED***
...
static grpc_tcp_listener* get_port_index(grpc_tcp_server* s,
                                         unsigned port_index) ***REMOVED***
  unsigned num_ports = 0;
  grpc_tcp_listener* sp;
  for (sp = s->head; sp; sp = sp->next) ***REMOVED***
    if (!sp->is_sibling) ***REMOVED***
      if (++num_ports > port_index) ***REMOVED***
        return sp;
      ***REMOVED***
    ***REMOVED***
  ***REMOVED***
...
int grpc_tcp_server_port_fd(grpc_tcp_server* s, unsigned port_index,
                            unsigned fd_index) ***REMOVED***
  gpr_mu_lock(&s->mu);
  grpc_tcp_listener* sp = get_port_index(s, port_index);
  for (; sp; sp = sp->sibling, --fd_index) ***REMOVED***
    if (fd_index == 0) ***REMOVED***
      gpr_mu_unlock(&s->mu);
      return sp->fd;
    ***REMOVED***
  ***REMOVED***
...
void grpc_tcp_server_start(grpc_exec_ctx* exec_ctx, grpc_tcp_server* s,
  ...
  sp = s->head;
  while (sp != nullptr) ***REMOVED***
    if (s->so_reuseport && !grpc_is_unix_socket(&sp->addr) &&
        pollset_count > 1) ***REMOVED***
      GPR_ASSERT(GRPC_LOG_IF_ERROR(
          "clone_port", clone_port(sp, (unsigned)(pollset_count - 1))));
      for (i = 0; i < pollset_count; i++) ***REMOVED***
...
grpc_tcp_server* grpc_tcp_server_ref(grpc_tcp_server* s) ***REMOVED***
  gpr_ref_non_zero(&s->refs);
  return s;
***REMOVED***

void grpc_tcp_server_shutdown_starting_add(grpc_tcp_server* s,
                                           grpc_closure* shutdown_starting) ***REMOVED***
  gpr_mu_lock(&s->mu);
...
void grpc_tcp_server_unref(grpc_exec_ctx* exec_ctx, grpc_tcp_server* s) ***REMOVED***
  if (gpr_unref(&s->refs)) ***REMOVED***
