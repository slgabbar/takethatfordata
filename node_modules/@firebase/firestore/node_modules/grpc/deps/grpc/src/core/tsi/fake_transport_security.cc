...
***REMOVED*** tsi_fake_handshake_message;
...
***REMOVED*** tsi_fake_frame_protector;
...
***REMOVED*** tsi_fake_zero_copy_grpc_protector;

/* --- Utils. ---*/

static const char* tsi_fake_handshake_message_strings[] = ***REMOVED***
    "CLIENT_INIT", "SERVER_INIT", "CLIENT_FINISHED", "SERVER_FINISHED"***REMOVED***;

static const char* tsi_fake_handshake_message_to_string(int msg) ***REMOVED***
  if (msg < 0 || msg >= TSI_FAKE_HANDSHAKE_MESSAGE_MAX) ***REMOVED***
...
static tsi_result tsi_fake_handshake_message_from_string(
    const char* msg_string, tsi_fake_handshake_message* msg) ***REMOVED***
  for (int i = 0; i < TSI_FAKE_HANDSHAKE_MESSAGE_MAX; i++) ***REMOVED***
    if (strncmp(msg_string, tsi_fake_handshake_message_strings[i],
                strlen(tsi_fake_handshake_message_strings[i])) == 0) ***REMOVED***
...
static uint32_t load32_little_endian(const unsigned char* buf) ***REMOVED***
  return ((uint32_t)(buf[0]) | (uint32_t)(buf[1] << 8) |
          (uint32_t)(buf[2] << 16) | (uint32_t)(buf[3] << 24));
***REMOVED***

static void store32_little_endian(uint32_t value, unsigned char* buf) ***REMOVED***
...
static uint32_t read_frame_size(const grpc_slice_buffer* sb) ***REMOVED***
  ...
  for (size_t i = 0; i < sb->count; i++) ***REMOVED***
    size_t slice_length = GRPC_SLICE_LENGTH(sb->slices[i]);
    if (remaining <= slice_length) ***REMOVED***
...
static void tsi_fake_frame_reset(tsi_fake_frame* frame, int needs_draining) ***REMOVED***
  frame->offset = 0;
...
static void tsi_fake_frame_ensure_size(tsi_fake_frame* frame) ***REMOVED***
  if (frame->data == nullptr) ***REMOVED***
    frame->allocated_size = frame->size;
    frame->data = (unsigned char*)gpr_malloc(frame->allocated_size);
  ***REMOVED*** else if (frame->size > frame->allocated_size) ***REMOVED***
...
static tsi_result tsi_fake_frame_decode(const unsigned char* incoming_bytes,
  ...
  ***REMOVED***
  ...
  if (frame->offset < TSI_FAKE_FRAME_HEADER_SIZE) ***REMOVED***
    to_read_size = TSI_FAKE_FRAME_HEADER_SIZE - frame->offset;
    if (to_read_size > available_size) ***REMOVED***
...
static tsi_result tsi_fake_frame_encode(unsigned char* outgoing_bytes,
  ...
  if (!frame->needs_draining) return TSI_INTERNAL_ERROR;
  if (*outgoing_bytes_size < to_write_size) ***REMOVED***
...
static void tsi_fake_frame_destruct(tsi_fake_frame* frame) ***REMOVED***
...
static tsi_result fake_protector_protect(tsi_frame_protector* self,
  ...
  *num_bytes_written = 0;
  ...
  /* Try to drain first. */
  if (frame->needs_draining) ***REMOVED***
...
static void fake_protector_destroy(tsi_frame_protector* self) ***REMOVED***
  tsi_fake_frame_protector* impl = (tsi_fake_frame_protector*)self;
...
static tsi_result fake_zero_copy_grpc_protector_protect(
    ...
    grpc_slice_buffer* protected_slices) ***REMOVED***
  if (self == nullptr || unprotected_slices == nullptr ||
      protected_slices == nullptr) ***REMOVED***
    return TSI_INVALID_ARGUMENT;
  ***REMOVED***
  tsi_fake_zero_copy_grpc_protector* impl =
      (tsi_fake_zero_copy_grpc_protector*)self;
  /* Protects each frame. */
  while (unprotected_slices->length > 0) ***REMOVED***
    size_t frame_length =
...
static tsi_result fake_zero_copy_grpc_protector_unprotect(
    ...
    grpc_slice_buffer* unprotected_slices) ***REMOVED***
  if (self == nullptr || unprotected_slices == nullptr ||
      protected_slices == nullptr) ***REMOVED***
    return TSI_INVALID_ARGUMENT;
  ***REMOVED***
  ...
  while (impl->protected_sb.length >= TSI_FAKE_FRAME_HEADER_SIZE) ***REMOVED***
    if (impl->parsed_frame_size == 0) ***REMOVED***
      impl->parsed_frame_size = read_frame_size(&impl->protected_sb);
      if (impl->parsed_frame_size <= 4) ***REMOVED***
...
static void fake_zero_copy_grpc_protector_destroy(
    grpc_exec_ctx* exec_ctx, tsi_zero_copy_grpc_protector* self) ***REMOVED***
...
static const tsi_zero_copy_grpc_protector_vtable
    zero_copy_grpc_protector_vtable = ***REMOVED***
...
***REMOVED*** fake_handshaker_result;

static tsi_result fake_handshaker_result_extract_peer(
    const tsi_handshaker_result* self, tsi_peer* peer) ***REMOVED***
...
static tsi_result fake_handshaker_result_create_zero_copy_grpc_protector(
    ...
    tsi_zero_copy_grpc_protector** protector) ***REMOVED***
  *protector =
...
static void fake_handshaker_result_destroy(tsi_handshaker_result* self) ***REMOVED***
  fake_handshaker_result* result = (fake_handshaker_result*)self;
  gpr_free(result->unused_bytes);
  gpr_free(self);
***REMOVED***

static const tsi_handshaker_result_vtable handshaker_result_vtable = ***REMOVED***
...
static tsi_result fake_handshaker_result_create(
  ...
  if ((unused_bytes_size > 0 && unused_bytes == nullptr) ||
      handshaker_result == nullptr) ***REMOVED***
    return TSI_INVALID_ARGUMENT;
  ***REMOVED***
...
static tsi_result fake_handshaker_get_bytes_to_send_to_peer(
    tsi_handshaker* self, unsigned char* bytes, size_t* bytes_size) ***REMOVED***
  tsi_fake_handshaker* impl = (tsi_fake_handshaker*)self;
  tsi_result result = TSI_OK;
  if (impl->needs_incoming_message || impl->result == TSI_OK) ***REMOVED***
    *bytes_size = 0;
    return TSI_OK;
  ***REMOVED***
  if (!impl->outgoing_frame.needs_draining) ***REMOVED***
    tsi_fake_handshake_message next_message_to_send =
    ...
    if (result != TSI_OK) return result;
    if (next_message_to_send > TSI_FAKE_HANDSHAKE_MESSAGE_MAX) ***REMOVED***
      next_message_to_send = TSI_FAKE_HANDSHAKE_MESSAGE_MAX;
    ***REMOVED***
    if (tsi_tracing_enabled.enabled()) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (result != TSI_OK) return result;
  if (!impl->is_client &&
      impl->next_message_to_send == TSI_FAKE_HANDSHAKE_MESSAGE_MAX) ***REMOVED***
    /* We're done. */
    if (tsi_tracing_enabled.enabled()) ***REMOVED***
...
static tsi_result fake_handshaker_process_bytes_from_peer(
  ...
  ***REMOVED***
  ...
  if (result != TSI_OK) ***REMOVED***
    ...
    return result;
  ***REMOVED***
  if (received_msg != expected_msg) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (impl->next_message_to_send == TSI_FAKE_HANDSHAKE_MESSAGE_MAX) ***REMOVED***
    /* We're done. */
    if (tsi_tracing_enabled.enabled()) ***REMOVED***
...
static tsi_result fake_handshaker_get_result(tsi_handshaker* self) ***REMOVED***
  tsi_fake_handshaker* impl = (tsi_fake_handshaker*)self;
  return impl->result;
***REMOVED***

static void fake_handshaker_destroy(tsi_handshaker* self) ***REMOVED***
  tsi_fake_handshaker* impl = (tsi_fake_handshaker*)self;
...
static tsi_result fake_handshaker_next(
    ...
    tsi_handshaker_on_next_done_cb cb, void* user_data) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (received_bytes_size > 0) ***REMOVED***
  ...
  ***REMOVED***
  ...
  do ***REMOVED***
    ...
    ***REMOVED***
  ***REMOVED*** while (result == TSI_INCOMPLETE_DATA);
  if (result != TSI_OK) return result;
  *bytes_to_send = handshaker->outgoing_bytes_buffer;
  *bytes_to_send_size = offset;

  /* Check if the handshake was completed. */
  if (fake_handshaker_get_result(self) == TSI_HANDSHAKE_IN_PROGRESS) ***REMOVED***
...
tsi_handshaker* tsi_create_fake_handshaker(int is_client) ***REMOVED***
...
tsi_frame_protector* tsi_create_fake_frame_protector(
    size_t* max_protected_frame_size) ***REMOVED***
