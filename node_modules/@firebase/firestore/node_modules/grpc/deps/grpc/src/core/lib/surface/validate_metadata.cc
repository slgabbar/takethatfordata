...
static grpc_error* conforms_to(grpc_slice slice, const uint8_t* legal_bits,
  ...
  for (; p != e; p++) ***REMOVED***
    ...
    int byte = idx / 8;
    int bit = idx % 8;
    if ((legal_bits[byte] & (1 << bit)) == 0) ***REMOVED***
...
static int error2int(grpc_error* error) ***REMOVED***
  int r = (error == GRPC_ERROR_NONE);
  GRPC_ERROR_UNREF(error);
  return r;
***REMOVED***

grpc_error* grpc_validate_header_key_is_legal(grpc_slice slice) ***REMOVED***
  static const uint8_t legal_header_bits[256 / 8] = ***REMOVED***
      0x00, 0x00, 0x00, 0x00, 0x00, 0x60, 0xff, 0x03, 0x00, 0x00, 0x00,
      0x80, 0xfe, 0xff, 0xff, 0x07, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00***REMOVED***;
  if (GRPC_SLICE_LENGTH(slice) == 0) ***REMOVED***
    return GRPC_ERROR_CREATE_FROM_STATIC_STRING(
        "Metadata keys cannot be zero length");
  ***REMOVED***
  if (GRPC_SLICE_START_PTR(slice)[0] == ':') ***REMOVED***
    return GRPC_ERROR_CREATE_FROM_STATIC_STRING(
...
int grpc_header_key_is_legal(grpc_slice slice) ***REMOVED***
  return error2int(grpc_validate_header_key_is_legal(slice));
***REMOVED***

grpc_error* grpc_validate_header_nonbin_value_is_legal(grpc_slice slice) ***REMOVED***
  static const uint8_t legal_header_bits[256 / 8] = ***REMOVED***
...
int grpc_header_nonbin_value_is_legal(grpc_slice slice) ***REMOVED***
  return error2int(grpc_validate_header_nonbin_value_is_legal(slice));
***REMOVED***

int grpc_is_binary_header(grpc_slice slice) ***REMOVED***
