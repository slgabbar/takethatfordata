...
grpc_error* grpc_chttp2_perform_read(grpc_exec_ctx* exec_ctx,
  ...
  switch (t->deframe_state) ***REMOVED***
    ...
    case GRPC_DTS_CLIENT_PREFIX_23:
      while (cur != end && t->deframe_state != GRPC_DTS_FH_0) ***REMOVED***
        if (*cur != GRPC_CHTTP2_CLIENT_CONNECT_STRING[t->deframe_state]) ***REMOVED***
          char* msg;
          gpr_asprintf(
              &msg,
    ...
    case GRPC_DTS_FH_8:
      ...
      if (err != GRPC_ERROR_NONE) ***REMOVED***
        return err;
      ***REMOVED***
      if (t->incoming_frame_size == 0) ***REMOVED***
        ...
        ***REMOVED***
        goto dts_fh_0; /* loop */
      ***REMOVED*** else if (t->incoming_frame_size >
                 t->settings[GRPC_ACKED_SETTINGS]
                            [GRPC_CHTTP2_SETTINGS_MAX_FRAME_SIZE]) ***REMOVED***
        char* msg;
    ...
    case GRPC_DTS_FRAME:
      GPR_ASSERT(cur < end);
      if ((uint32_t)(end - cur) == t->incoming_frame_size) ***REMOVED***
        err =
        ...
        ***REMOVED***
        ...
        t->incoming_stream = nullptr;
        return GRPC_ERROR_NONE;
      ***REMOVED*** else if ((uint32_t)(end - cur) > t->incoming_frame_size) ***REMOVED***
...
static grpc_error* init_frame_parser(grpc_exec_ctx* exec_ctx,
  ...
  if (t->is_first_frame &&
      t->incoming_frame_type != GRPC_CHTTP2_FRAME_SETTINGS) ***REMOVED***
    char* msg;
    gpr_asprintf(
  ...
  ***REMOVED***
  ...
  if (t->expect_continuation_stream_id != 0) ***REMOVED***
    if (t->incoming_frame_type != GRPC_CHTTP2_FRAME_CONTINUATION) ***REMOVED***
      char* msg;
      ...
      return err;
    ***REMOVED***
    if (t->expect_continuation_stream_id != t->incoming_stream_id) ***REMOVED***
      char* msg;
      gpr_asprintf(
          &msg,
  ...
  ***REMOVED***
  switch (t->incoming_frame_type) ***REMOVED***
...
static void skip_header(grpc_exec_ctx* exec_ctx, void* tp, grpc_mdelem md) ***REMOVED***
  GRPC_MDELEM_UNREF(exec_ctx, md);
***REMOVED***

static grpc_error* init_skip_frame_parser(grpc_exec_ctx* exec_ctx,
                                          grpc_chttp2_transport* t,
                                          int is_header) ***REMOVED***
  if (is_header) ***REMOVED***
...
static grpc_error* init_data_frame_parser(grpc_exec_ctx* exec_ctx,
  ...
  ***REMOVED***
  ...
  if (err == GRPC_ERROR_NONE && s->read_closed) ***REMOVED***
    return init_skip_frame_parser(exec_ctx, t, 0);
  ***REMOVED***
  if (err == GRPC_ERROR_NONE) ***REMOVED***
...
error_handler:
  if (err == GRPC_ERROR_NONE) ***REMOVED***
    ...
    return GRPC_ERROR_NONE;
  ***REMOVED*** else if (grpc_error_get_int(err, GRPC_ERROR_INT_STREAM_ID, nullptr)) ***REMOVED***
...
static void free_timeout(void* p) ***REMOVED*** gpr_free(p); ***REMOVED***
...
static void on_initial_header(grpc_exec_ctx* exec_ctx, void* tp,
  ...
  ***REMOVED***
  ...
  if (grpc_slice_eq(GRPC_MDKEY(md), GRPC_MDSTR_GRPC_STATUS) &&
      !grpc_mdelem_eq(md, GRPC_MDELEM_GRPC_STATUS_0)) ***REMOVED***
    /* TODO(ctiller): check for a status like " 0" */
    s->seen_error = true;
  ***REMOVED***

  if (grpc_slice_eq(GRPC_MDKEY(md), GRPC_MDSTR_GRPC_TIMEOUT)) ***REMOVED***
    grpc_millis* cached_timeout =
        static_cast<grpc_millis*>(grpc_mdelem_get_user_data(md, free_timeout));
    grpc_millis timeout;
    if (cached_timeout != nullptr) ***REMOVED***
      timeout = *cached_timeout;
    ***REMOVED*** else ***REMOVED***
      if (!grpc_http2_decode_timeout(GRPC_MDVALUE(md), &timeout)) ***REMOVED***
        char* val = grpc_slice_to_c_string(GRPC_MDVALUE(md));
        gpr_log(GPR_ERROR, "Ignoring bad timeout value '%s'", val);
        gpr_free(val);
        timeout = GRPC_MILLIS_INF_FUTURE;
      ***REMOVED***
      if (GRPC_MDELEM_IS_INTERNED(md)) ***REMOVED***
        /* store the result */
      ...
      ***REMOVED***
    ***REMOVED***
    if (timeout != GRPC_MILLIS_INF_FUTURE) ***REMOVED***
  ...
  ***REMOVED*** else ***REMOVED***
    ...
    if (new_size > metadata_size_limit) ***REMOVED***
      gpr_log(GPR_DEBUG,
...
static grpc_error* init_header_frame_parser(grpc_exec_ctx* exec_ctx,
    ...
    ***REMOVED***
    if (t->is_client) ***REMOVED***
      if ((t->incoming_stream_id & 1) &&
          t->incoming_stream_id < t->next_stream_id) ***REMOVED***
      ...
      ***REMOVED***
      ...
      if (t->incoming_frame_flags & GRPC_CHTTP2_FLAG_HAS_PRIORITY) ***REMOVED***
        grpc_chttp2_hpack_parser_set_has_priority(&t->hpack_parser);
      ***REMOVED***
      return err;
    ***REMOVED*** else if (t->last_new_stream_id >= t->incoming_stream_id) ***REMOVED***
      GRPC_CHTTP2_IF_TRACING(gpr_log(
          GPR_ERROR,
      ...
      return init_skip_frame_parser(exec_ctx, t, 1);
    ***REMOVED*** else if ((t->incoming_stream_id & 1) == 0) ***REMOVED***
      GRPC_CHTTP2_IF_TRACING(gpr_log(
          GPR_ERROR,
          "ignoring grpc_chttp2_stream with non-client generated index %d",
          t->incoming_stream_id));
      return init_skip_frame_parser(exec_ctx, t, 1);
    ***REMOVED*** else if (grpc_chttp2_stream_map_size(&t->stream_map) >=
               t->settings[GRPC_ACKED_SETTINGS]
                          [GRPC_CHTTP2_SETTINGS_MAX_CONCURRENT_STREAMS]) ***REMOVED***
  ...
  ***REMOVED***
  t->parser = grpc_chttp2_header_parser_parse;
  t->parser_data = &t->hpack_parser;
  switch (s->header_frames_received) ***REMOVED***
    case 0:
      if (t->is_client && t->header_eof) ***REMOVED***
        GRPC_CHTTP2_IF_TRACING(gpr_log(GPR_INFO, "parsing Trailers-Only"));
        if (s->trailing_metadata_available != nullptr) ***REMOVED***
  ...
  ***REMOVED***
  ...
  t->hpack_parser.is_eof = (uint8_t)(is_eoh ? t->header_eof : 0);
  if (!is_continuation &&
      (t->incoming_frame_flags & GRPC_CHTTP2_FLAG_HAS_PRIORITY)) ***REMOVED***
    grpc_chttp2_hpack_parser_set_has_priority(&t->hpack_parser);
  ***REMOVED***
  return GRPC_ERROR_NONE;
***REMOVED***
...
static grpc_error* init_window_update_frame_parser(grpc_exec_ctx* exec_ctx,
  ...
  if (t->incoming_stream_id != 0) ***REMOVED***
    grpc_chttp2_stream* s = t->incoming_stream =
        grpc_chttp2_parsing_lookup_stream(t, t->incoming_stream_id);
    if (s == nullptr) ***REMOVED***
      return init_skip_frame_parser(exec_ctx, t, 0);
...
static grpc_error* init_settings_frame_parser(grpc_exec_ctx* exec_ctx,
  ...
  ***REMOVED***
  ...
  if (err != GRPC_ERROR_NONE) ***REMOVED***
    return err;
  ***REMOVED***
  if (t->incoming_frame_flags & GRPC_CHTTP2_FLAG_ACK) ***REMOVED***
