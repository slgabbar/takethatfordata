...
enum e_op_result ***REMOVED***
  ACTION_TAKEN_WITH_CALLBACK,
  ACTION_TAKEN_NO_CALLBACK,
  NO_ACTION_POSSIBLE
***REMOVED***;

enum e_op_id ***REMOVED***
...
static bidirectional_stream_callback cronet_callbacks = ***REMOVED***
    on_stream_ready,
...
struct grpc_cronet_transport ***REMOVED***
...
struct write_state ***REMOVED***
  char* write_buffer;
***REMOVED***;

/* track state of one stream op */
struct op_state ***REMOVED***
...
struct op_storage ***REMOVED***
  int num_pending_ops;
  struct op_and_state* head;
***REMOVED***;

struct stream_obj ***REMOVED***
  gpr_arena* arena;
...
void grpc_cronet_stream_ref(stream_obj* s, const char* reason) ***REMOVED***
...
void grpc_cronet_stream_ref(stream_obj* s) ***REMOVED*** grpc_stream_ref(s->refcount); ***REMOVED***
void grpc_cronet_stream_unref(grpc_exec_ctx* exec_ctx, stream_obj* s) ***REMOVED***
...
*/
static const char* op_result_string(enum e_op_result i) ***REMOVED***
  switch (i) ***REMOVED***
...
static const char* op_id_string(enum e_op_id i) ***REMOVED***
  switch (i) ***REMOVED***
...
static void null_and_maybe_free_read_buffer(stream_obj* s) ***REMOVED***
  if (s->state.rs.read_buffer &&
      s->state.rs.read_buffer != s->state.rs.grpc_header_bytes) ***REMOVED***
    gpr_free(s->state.rs.read_buffer);
  ***REMOVED***
  s->state.rs.read_buffer = nullptr;
***REMOVED***

static void maybe_flush_read(stream_obj* s) ***REMOVED***
  ...
  if (s->state.pending_recv_trailing_metadata && s->state.fail_state) ***REMOVED***
    if (!s->state.flush_read && !s->state.rs.read_stream_closed) ***REMOVED***
      ...
      if (!s->state.pending_read_from_cronet) ***REMOVED***
...
static grpc_error* make_error_with_desc(int error_code, const char* desc) ***REMOVED***
...
static void remove_from_storage(struct stream_obj* s,
                                struct op_and_state* oas) ***REMOVED***
  struct op_and_state* curr;
  if (s->storage.head == nullptr || oas == nullptr) ***REMOVED***
    return;
  ***REMOVED***
  if (s->storage.head == oas) ***REMOVED***
  ...
  ***REMOVED*** else ***REMOVED***
    for (curr = s->storage.head; curr != nullptr; curr = curr->next) ***REMOVED***
      if (curr->next == oas) ***REMOVED***
        curr->next = oas->next;
        s->storage.num_pending_ops--;
        CRONET_LOG(GPR_DEBUG, "Freed %p. Now %d in the queue", oas,
                   s->storage.num_pending_ops);
        gpr_free(oas);
        break;
      ***REMOVED*** else if (curr->next == nullptr) ***REMOVED***
...
*/
static void execute_from_storage(grpc_exec_ctx* exec_ctx, stream_obj* s) ***REMOVED***
  gpr_mu_lock(&s->mu);
  for (struct op_and_state* curr = s->storage.head; curr != nullptr;) ***REMOVED***
    ...
    ***REMOVED***
    ...
    if (result == NO_ACTION_POSSIBLE) ***REMOVED***
      curr = curr->next;
    ***REMOVED*** else if (result == ACTION_TAKEN_WITH_CALLBACK) ***REMOVED***
      break;
    ***REMOVED***
  ***REMOVED***
  gpr_mu_unlock(&s->mu);
...
/*
  Cronet callback
*/
static void on_failed(bidirectional_stream* stream, int net_error) ***REMOVED***
  ...
  s->cbs = nullptr;
  if (s->header_array.headers) ***REMOVED***
    gpr_free(s->header_array.headers);
    s->header_array.headers = nullptr;
  ***REMOVED***
  if (s->state.ws.write_buffer) ***REMOVED***
...
/*
  Cronet callback
*/
static void on_canceled(bidirectional_stream* stream) ***REMOVED***
...
/*
  Cronet callback
*/
static void on_succeeded(bidirectional_stream* stream) ***REMOVED***
...
/*
  Cronet callback
*/
static void on_stream_ready(bidirectional_stream* stream) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (t->use_packet_coalescing) ***REMOVED***
    if (s->state.flush_cronet_when_ready) ***REMOVED***
...
static void on_response_headers_received(
    ...
    const char* negotiated_protocol) ***REMOVED***
  grpc_exec_ctx exec_ctx = GRPC_EXEC_CTX_INIT;
   ...
   */
  for (size_t i = 0; i < headers->count; i++) ***REMOVED***
    if (0 == strcmp("grpc-status", headers->headers[i].key)) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (!(s->state.state_op_done[OP_CANCEL_ERROR] ||
        s->state.state_callback_received[OP_FAILED])) ***REMOVED***
...
/*
  Cronet callback
*/
static void on_write_completed(bidirectional_stream* stream, const char* data) ***REMOVED***
  grpc_exec_ctx exec_ctx = GRPC_EXEC_CTX_INIT;
  stream_obj* s = (stream_obj*)stream->annotation;
...
static void on_read_completed(bidirectional_stream* stream, char* data,
  ...
  if (count > 0 && s->state.flush_read) ***REMOVED***
    CRONET_LOG(GPR_DEBUG, "bidirectional_stream_read(%p)", s->cbs);
    bidirectional_stream_read(s->cbs, s->state.rs.read_buffer,
                              GRPC_FLUSH_READ_SIZE);
    s->state.pending_read_from_cronet = true;
  ...
  ***REMOVED*** else if (count > 0) ***REMOVED***
    ...
    if (s->state.rs.remaining_bytes > 0) ***REMOVED***
      CRONET_LOG(GPR_DEBUG, "bidirectional_stream_read(%p)", s->cbs);
...
static void on_response_trailers_received(
    bidirectional_stream* stream,
    const bidirectional_stream_header_array* trailers) ***REMOVED***
  grpc_exec_ctx exec_ctx = GRPC_EXEC_CTX_INIT;
  ...
  grpc_chttp2_incoming_metadata_buffer_init(&s->state.rs.trailing_metadata,
                                            s->arena);
  for (size_t i = 0; i < trailers->count; i++) ***REMOVED***
    ...
    if (0 == strcmp(trailers->headers[i].key, "grpc-status") &&
        0 != strcmp(trailers->headers[i].value, "0")) ***REMOVED***
...
static void create_grpc_frame(grpc_exec_ctx* exec_ctx,
                              ...
                              size_t* p_write_buffer_size, uint32_t flags) ***REMOVED***
...
static void convert_metadata_to_cronet_headers(
  ...
  *pp_headers = headers;
  ...
  while (num_headers < num_headers_available) ***REMOVED***
    ...
    if (grpc_slice_eq(GRPC_MDKEY(mdelem), GRPC_MDSTR_SCHEME) ||
        grpc_slice_eq(GRPC_MDKEY(mdelem), GRPC_MDSTR_AUTHORITY)) ***REMOVED***
      /* Cronet populates these fields on its own */
      gpr_free(key);
      gpr_free(value);
      continue;
    ***REMOVED***
    if (grpc_slice_eq(GRPC_MDKEY(mdelem), GRPC_MDSTR_METHOD)) ***REMOVED***
      if (grpc_slice_eq(GRPC_MDVALUE(mdelem), GRPC_MDSTR_PUT)) ***REMOVED***
        *method = "PUT";
      ***REMOVED*** else ***REMOVED***
      ...
      ***REMOVED***
      ...
      continue;
    ***REMOVED***
    if (grpc_slice_eq(GRPC_MDKEY(mdelem), GRPC_MDSTR_PATH)) ***REMOVED***
...
static bool header_has_authority(grpc_linked_mdelem* head) ***REMOVED***
  while (head != nullptr) ***REMOVED***
    if (grpc_slice_eq(GRPC_MDKEY(head->md), GRPC_MDSTR_AUTHORITY)) ***REMOVED***
      return true;
    ***REMOVED***
    head = head->next;
  ***REMOVED***
...
static bool op_can_be_run(grpc_transport_stream_op_batch* curr_op,
  ...
  */
  ...
  if (is_canceled_or_failed) ***REMOVED***
    if (op_id == OP_SEND_INITIAL_METADATA) ***REMOVED***
      CRONET_LOG(GPR_DEBUG, "Because");
      result = false;
    ***REMOVED***
    if (op_id == OP_SEND_MESSAGE) ***REMOVED***
      CRONET_LOG(GPR_DEBUG, "Because");
      result = false;
    ***REMOVED***
    if (op_id == OP_SEND_TRAILING_METADATA) ***REMOVED***
      CRONET_LOG(GPR_DEBUG, "Because");
      result = false;
    ***REMOVED***
    if (op_id == OP_CANCEL_ERROR) ***REMOVED***
      CRONET_LOG(GPR_DEBUG, "Because");
      result = false;
    ***REMOVED***
    /* already executed */
    if (op_id == OP_RECV_INITIAL_METADATA &&
        stream_state->state_op_done[OP_RECV_INITIAL_METADATA]) ***REMOVED***
      CRONET_LOG(GPR_DEBUG, "Because");
      result = false;
    ***REMOVED***
    if (op_id == OP_RECV_MESSAGE && op_state->state_op_done[OP_RECV_MESSAGE]) ***REMOVED***
      CRONET_LOG(GPR_DEBUG, "Because");
      result = false;
    ***REMOVED***
    if (op_id == OP_RECV_TRAILING_METADATA &&
        stream_state->state_op_done[OP_RECV_TRAILING_METADATA]) ***REMOVED***
      CRONET_LOG(GPR_DEBUG, "Because");
      result = false;
    ***REMOVED***
    ...
    if (op_id == OP_ON_COMPLETE &&
        !(stream_state->state_callback_received[OP_FAILED] ||
          ...
          !stream_state->state_op_done[OP_SEND_INITIAL_METADATA])) ***REMOVED***
      CRONET_LOG(GPR_DEBUG, "Because");
      result = false;
    ***REMOVED***
  ***REMOVED*** else if (op_id == OP_SEND_INITIAL_METADATA) ***REMOVED***
    /* already executed */
    if (stream_state->state_op_done[OP_SEND_INITIAL_METADATA]) result = false;
  ***REMOVED*** else if (op_id == OP_RECV_INITIAL_METADATA) ***REMOVED***
    /* already executed */
    ...
    /* we haven't received headers yet. */
      ...
      result = false;
  ***REMOVED*** else if (op_id == OP_SEND_MESSAGE) ***REMOVED***
    ...
    /* we haven't sent headers yet. */
      ...
      result = false;
  ***REMOVED*** else if (op_id == OP_RECV_MESSAGE) ***REMOVED***
    /* already executed */
    ...
    /* we haven't received headers yet. */
             ...
             !stream_state->state_op_done[OP_RECV_TRAILING_METADATA])
      result = false;
  ***REMOVED*** else if (op_id == OP_RECV_TRAILING_METADATA) ***REMOVED***
    /* already executed */
    ...
    /* we haven't received on_succeeded  yet. */
      ...
      result = false;
  ***REMOVED*** else if (op_id == OP_SEND_TRAILING_METADATA) ***REMOVED***
    /* already executed */
    ...
    /* we haven't got on_write_completed for the send yet */
      ...
      result = false;
  ***REMOVED*** else if (op_id == OP_CANCEL_ERROR) ***REMOVED***
    /* already executed */
    if (stream_state->state_op_done[OP_CANCEL_ERROR]) result = false;
  ***REMOVED*** else if (op_id == OP_ON_COMPLETE) ***REMOVED***
    /* already executed (note we're checking op specific state, not stream
    state) */
    if (op_state->state_op_done[OP_ON_COMPLETE]) ***REMOVED***
      CRONET_LOG(GPR_DEBUG, "Because");
      result = false;
    ***REMOVED***
    /* Check if every op that was asked for is done. */
    else if (curr_op->send_initial_metadata &&
             !stream_state->state_callback_received[OP_SEND_INITIAL_METADATA]) ***REMOVED***
      CRONET_LOG(GPR_DEBUG, "Because");
      result = false;
    ***REMOVED*** else if (curr_op->send_message &&
               !op_state->state_op_done[OP_SEND_MESSAGE]) ***REMOVED***
      CRONET_LOG(GPR_DEBUG, "Because");
      result = false;
    ***REMOVED*** else if (curr_op->send_message &&
               !stream_state->state_callback_received[OP_SEND_MESSAGE]) ***REMOVED***
      CRONET_LOG(GPR_DEBUG, "Because");
      result = false;
    ***REMOVED*** else if (curr_op->send_trailing_metadata &&
               !stream_state->state_op_done[OP_SEND_TRAILING_METADATA]) ***REMOVED***
      CRONET_LOG(GPR_DEBUG, "Because");
      result = false;
    ***REMOVED*** else if (curr_op->recv_initial_metadata &&
               !stream_state->state_op_done[OP_RECV_INITIAL_METADATA]) ***REMOVED***
      CRONET_LOG(GPR_DEBUG, "Because");
      result = false;
    ***REMOVED*** else if (curr_op->recv_message &&
               !op_state->state_op_done[OP_RECV_MESSAGE]) ***REMOVED***
      CRONET_LOG(GPR_DEBUG, "Because");
      result = false;
    ***REMOVED*** else if (curr_op->cancel_stream &&
               !stream_state->state_callback_received[OP_CANCELED]) ***REMOVED***
      CRONET_LOG(GPR_DEBUG, "Because");
      result = false;
    ***REMOVED*** else if (curr_op->recv_trailing_metadata) ***REMOVED***
      /* We aren't done with trailing metadata yet */
      if (!stream_state->state_op_done[OP_RECV_TRAILING_METADATA]) ***REMOVED***
        CRONET_LOG(GPR_DEBUG, "Because");
        result = false;
      ***REMOVED***
      /* We've asked for actual message in an earlier op, and it hasn't been
        delivered yet. */
      else if (stream_state->state_op_done[OP_READ_REQ_MADE]) ***REMOVED***
        ...
        if (!curr_op->recv_message &&
            !stream_state->state_callback_received[OP_SUCCEEDED]) ***REMOVED***
          CRONET_LOG(GPR_DEBUG, "Because");
          result = false;
        ***REMOVED***
      ***REMOVED***
...
static enum e_op_result execute_stream_op(grpc_exec_ctx* exec_ctx,
  ...
  if (stream_op->send_initial_metadata &&
      op_can_be_run(stream_op, s, &oas->state, OP_SEND_INITIAL_METADATA)) ***REMOVED***
    ...
    ***REMOVED***
    ...
    for (header_index = 0; header_index < s->header_array.count;
         header_index++) ***REMOVED***
      gpr_free((void*)s->header_array.headers[header_index].key);
      gpr_free((void*)s->header_array.headers[header_index].value);
    ***REMOVED***
    stream_state->state_op_done[OP_SEND_INITIAL_METADATA] = true;
    if (t->use_packet_coalescing) ***REMOVED***
      if (!stream_op->send_message && !stream_op->send_trailing_metadata) ***REMOVED***
        s->state.flush_cronet_when_ready = true;
      ***REMOVED***
    ***REMOVED***
    result = ACTION_TAKEN_WITH_CALLBACK;
  ***REMOVED*** else if (stream_op->send_message &&
             op_can_be_run(stream_op, s, &oas->state, OP_SEND_MESSAGE)) ***REMOVED***
    CRONET_LOG(GPR_DEBUG, "running: %p  OP_SEND_MESSAGE", oas);
    stream_state->pending_send_message = false;
    if (stream_state->state_callback_received[OP_FAILED]) ***REMOVED***
  ...
  ***REMOVED*** else if (stream_op->send_trailing_metadata &&
             op_can_be_run(stream_op, s, &oas->state,
                           OP_SEND_TRAILING_METADATA)) ***REMOVED***
  ...
  ***REMOVED*** else if (stream_op->recv_initial_metadata &&
             op_can_be_run(stream_op, s, &oas->state,
                           OP_RECV_INITIAL_METADATA)) ***REMOVED***
    CRONET_LOG(GPR_DEBUG, "running: %p  OP_RECV_INITIAL_METADATA", oas);
    if (stream_state->state_op_done[OP_CANCEL_ERROR]) ***REMOVED***
      GRPC_CLOSURE_SCHED(
          exec_ctx,
          stream_op->payload->recv_initial_metadata.recv_initial_metadata_ready,
          GRPC_ERROR_NONE);
    ***REMOVED*** else if (stream_state->state_callback_received[OP_FAILED]) ***REMOVED***
      GRPC_CLOSURE_SCHED(
          exec_ctx,
          stream_op->payload->recv_initial_metadata.recv_initial_metadata_ready,
          GRPC_ERROR_NONE);
    ***REMOVED*** else if (stream_state->state_op_done[OP_RECV_TRAILING_METADATA]) ***REMOVED***
      GRPC_CLOSURE_SCHED(
          exec_ctx,
          stream_op->payload->recv_initial_metadata.recv_initial_metadata_ready,
          GRPC_ERROR_NONE);
    ...
    ***REMOVED***
    ...
    result = ACTION_TAKEN_NO_CALLBACK;
  ***REMOVED*** else if (stream_op->recv_message &&
             op_can_be_run(stream_op, s, &oas->state, OP_RECV_MESSAGE)) ***REMOVED***
    ...
    ***REMOVED*** else if (stream_state->state_callback_received[OP_FAILED]) ***REMOVED***
      ...
      oas->state.state_op_done[OP_RECV_MESSAGE] = true;
      result = ACTION_TAKEN_NO_CALLBACK;
    ***REMOVED*** else if (stream_state->rs.read_stream_closed == true) ***REMOVED***
      ...
      oas->state.state_op_done[OP_RECV_MESSAGE] = true;
      result = ACTION_TAKEN_NO_CALLBACK;
    ***REMOVED*** else if (stream_state->flush_read) ***REMOVED***
      ...
      oas->state.state_op_done[OP_RECV_MESSAGE] = true;
      result = ACTION_TAKEN_NO_CALLBACK;
    ***REMOVED*** else if (stream_state->rs.length_field_received == false) ***REMOVED***
      if (stream_state->rs.received_bytes == GRPC_HEADER_SIZE_IN_BYTES &&
          stream_state->rs.remaining_bytes == 0) ***REMOVED***
        ...
        CRONET_LOG(GPR_DEBUG, "length field = %d",
                   stream_state->rs.length_field);
        if (stream_state->rs.length_field > 0) ***REMOVED***
        ...
        ***REMOVED*** else ***REMOVED***
          ...
          if (stream_state->rs.compressed) ***REMOVED***
          ...
          *((grpc_byte_buffer**)stream_op->payload->recv_message.recv_message) =
          ...
          result = ACTION_TAKEN_NO_CALLBACK;
        ***REMOVED***
      ***REMOVED*** else if (stream_state->rs.remaining_bytes == 0) ***REMOVED***
    ...
    ***REMOVED*** else if (stream_state->rs.remaining_bytes == 0) ***REMOVED***
      ...
      *((grpc_byte_buffer**)stream_op->payload->recv_message.recv_message) =
      ...
      result = ACTION_TAKEN_NO_CALLBACK;
    ***REMOVED***
  ***REMOVED*** else if (stream_op->recv_trailing_metadata &&
             op_can_be_run(stream_op, s, &oas->state,
                           OP_RECV_TRAILING_METADATA)) ***REMOVED***
    CRONET_LOG(GPR_DEBUG, "running: %p  OP_RECV_TRAILING_METADATA", oas);
    if (oas->s->state.rs.trailing_metadata_valid) ***REMOVED***
      grpc_chttp2_incoming_metadata_buffer_publish(
    ...
    ***REMOVED***
    ...
    result = ACTION_TAKEN_NO_CALLBACK;
  ***REMOVED*** else if (stream_op->cancel_stream &&
             op_can_be_run(stream_op, s, &oas->state, OP_CANCEL_ERROR)) ***REMOVED***
    ...
    ***REMOVED***
    ...
    if (!stream_state->cancel_error) ***REMOVED***
      stream_state->cancel_error =
          GRPC_ERROR_REF(stream_op->payload->cancel_stream.cancel_error);
    ***REMOVED***
  ***REMOVED*** else if (stream_op->on_complete &&
             op_can_be_run(stream_op, s, &oas->state, OP_ON_COMPLETE)) ***REMOVED***
...
static void set_pollset_do_nothing(grpc_exec_ctx* exec_ctx, grpc_transport* gt,
                                   grpc_stream* gs, grpc_pollset* pollset) ***REMOVED******REMOVED***

static void set_pollset_set_do_nothing(grpc_exec_ctx* exec_ctx,
                                       grpc_transport* gt, grpc_stream* gs,
                                       grpc_pollset_set* pollset_set) ***REMOVED******REMOVED***
...
static void perform_stream_op(grpc_exec_ctx* exec_ctx, grpc_transport* gt,
  ...
  if (op->send_initial_metadata &&
      header_has_authority(op->payload->send_initial_metadata
                               .send_initial_metadata->list.head)) ***REMOVED***
...
static void destroy_transport(grpc_exec_ctx* exec_ctx, grpc_transport* gt) ***REMOVED******REMOVED***

static grpc_endpoint* get_endpoint(grpc_exec_ctx* exec_ctx,
                                   grpc_transport* gt) ***REMOVED***
  return nullptr;
***REMOVED***

static void perform_op(grpc_exec_ctx* exec_ctx, grpc_transport* gt,
                       grpc_transport_op* op) ***REMOVED******REMOVED***

static const grpc_transport_vtable grpc_cronet_vtable = ***REMOVED***
    sizeof(stream_obj),
    "cronet_http",
    init_stream,
...
grpc_transport* grpc_create_cronet_transport(void* engine, const char* target,
  ...
  ***REMOVED***
  ...
  if (args) ***REMOVED***
    for (size_t i = 0; i < args->num_args; i++) ***REMOVED***
      if (0 ==
          strcmp(args->args[i].key, GRPC_ARG_USE_CRONET_PACKET_COALESCING)) ***REMOVED***
        if (args->args[i].type != GRPC_ARG_INTEGER) ***REMOVED***
