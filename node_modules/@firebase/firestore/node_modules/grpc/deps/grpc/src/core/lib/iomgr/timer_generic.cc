...
static grpc_timer* g_timer_ht[NUM_HASH_BUCKETS] = ***REMOVED***nullptr***REMOVED***;
...
static void remove_from_ht(grpc_timer* t) ***REMOVED***
  ...
  if (g_timer_ht[i] == t) ***REMOVED***
    ...
    removed = true;
  ***REMOVED*** else if (g_timer_ht[i] != nullptr) ***REMOVED***
    grpc_timer* p = g_timer_ht[i];
    while (p->hash_table_next != nullptr && p->hash_table_next != t) ***REMOVED***
      p = p->hash_table_next;
    ***REMOVED***
...
void grpc_timer_list_init(grpc_exec_ctx* exec_ctx) ***REMOVED***
  uint32_t i;
  ...
  for (i = 0; i < g_num_shards; i++) ***REMOVED***
...
void grpc_timer_init(grpc_exec_ctx* exec_ctx, grpc_timer* timer,
                     grpc_millis deadline, grpc_closure* closure) ***REMOVED***
  ...
  if (grpc_timer_trace.enabled()) ***REMOVED***
    gpr_log(GPR_DEBUG,
  ...
  ***REMOVED***
  ...
  if (deadline < shard->queue_deadline_cap) ***REMOVED***
  ...
  if (is_first_timer) ***REMOVED***
    ...
    if (grpc_timer_trace.enabled()) ***REMOVED***
      gpr_log(GPR_DEBUG, "  .. old shard min_deadline=%" PRIdPTR,
              shard->min_deadline);
    ***REMOVED***
    if (deadline < shard->min_deadline) ***REMOVED***
      gpr_atm old_min_deadline = g_shard_queue[0]->min_deadline;
      shard->min_deadline = deadline;
      note_deadline_change(shard);
      if (shard->shard_queue_index == 0 && deadline < old_min_deadline) ***REMOVED***
...
static int refill_heap(timer_shard* shard, gpr_atm now) ***REMOVED***
  ...
  if (grpc_timer_check_trace.enabled()) ***REMOVED***
