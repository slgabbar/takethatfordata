...
static void destroy_user_agent_md(void* user_agent_md) ***REMOVED***
  gpr_free(user_agent_md);
***REMOVED***

grpc_workaround_user_agent_md* grpc_parse_user_agent(grpc_mdelem md) ***REMOVED***
  ...
  if (nullptr != user_agent_md) ***REMOVED***
    return user_agent_md;
  ***REMOVED***
  user_agent_md = (grpc_workaround_user_agent_md*)gpr_malloc(
      sizeof(grpc_workaround_user_agent_md));
  for (int i = 0; i < GRPC_MAX_WORKAROUND_ID; i++) ***REMOVED***
    if (ua_parser[i]) ***REMOVED***
...
void grpc_register_workaround(uint32_t id, user_agent_parser parser) ***REMOVED***
