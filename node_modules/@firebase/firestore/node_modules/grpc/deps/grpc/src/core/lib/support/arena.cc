...
typedef struct zone ***REMOVED***
  size_t size_begin;
  size_t size_end;
  gpr_atm next_atm;
***REMOVED*** zone;

struct gpr_arena ***REMOVED***
  gpr_atm size_so_far;
  zone initial_zone;
***REMOVED***;

gpr_arena* gpr_arena_create(size_t initial_size) ***REMOVED***
...
size_t gpr_arena_destroy(gpr_arena* arena) ***REMOVED***
...
void* gpr_arena_alloc(gpr_arena* arena, size_t size) ***REMOVED***
  size = ROUND_UP_TO_ALIGNMENT_SIZE(size);
  size_t start =
      (size_t)gpr_atm_no_barrier_fetch_add(&arena->size_so_far, size);
  zone* z = &arena->initial_zone;
  while (start > z->size_end) ***REMOVED***
    zone* next_z = (zone*)gpr_atm_acq_load(&z->next_atm);
    if (next_z == nullptr) ***REMOVED***
      ...
      if (!gpr_atm_rel_cas(&z->next_atm, (gpr_atm)NULL, (gpr_atm)next_z)) ***REMOVED***
        gpr_free(next_z);
    ...
    ***REMOVED***
    z = next_z;
  ***REMOVED***
  if (start + size > z->size_end) ***REMOVED***
