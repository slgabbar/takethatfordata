...
struct grpc_udp_listener ***REMOVED***
  int fd;
  grpc_fd* emfd;
...
struct shutdown_fd_args ***REMOVED***
  grpc_fd* fd;
  gpr_mu* server_mu;
***REMOVED***;

/* the overall server */
struct grpc_udp_server ***REMOVED***
  gpr_mu mu;
...
static grpc_socket_factory* get_socket_factory(const grpc_channel_args* args) ***REMOVED***
  if (args) ***REMOVED***
...
static void dummy_cb(grpc_exec_ctx* exec_ctx, void* arg, grpc_error* error) ***REMOVED***
  // No-op.
***REMOVED***

static void finish_shutdown(grpc_exec_ctx* exec_ctx, grpc_udp_server* s) ***REMOVED***
...
/* called when all listening endpoints have been shutdown, so no further
   ...
   things */
static void deactivated_all_ports(grpc_exec_ctx* exec_ctx, grpc_udp_server* s) ***REMOVED***
...
static int bind_socket(grpc_socket_factory* socket_factory, int sockfd,
                       const grpc_resolved_address* addr) ***REMOVED***
...
static int prepare_socket(grpc_socket_factory* socket_factory, int fd,
  ...
  ***REMOVED***
  ...
  if (grpc_set_socket_nonblocking(fd, 1) != GRPC_ERROR_NONE) ***REMOVED***
    gpr_log(GPR_ERROR, "Unable to set nonblocking %d: %s", fd, strerror(errno));
    goto error;
  ***REMOVED***
  if (grpc_set_socket_cloexec(fd, 1) != GRPC_ERROR_NONE) ***REMOVED***
    gpr_log(GPR_ERROR, "Unable to set cloexec %d: %s", fd, strerror(errno));
    goto error;
  ***REMOVED***

  if (grpc_set_socket_ip_pktinfo_if_possible(fd) != GRPC_ERROR_NONE) ***REMOVED***
    gpr_log(GPR_ERROR, "Unable to set ip_pktinfo.");
    goto error;
  ***REMOVED*** else if (addr_ptr->sa_family == AF_INET6) ***REMOVED***
    if (grpc_set_socket_ipv6_recvpktinfo_if_possible(fd) != GRPC_ERROR_NONE) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (bind_socket(socket_factory, fd, addr) < 0) ***REMOVED***
    char* addr_str;
  ...
  ***REMOVED***
  ...
  if (getsockname(fd, (struct sockaddr*)sockname_temp.addr,
                  (socklen_t*)&sockname_temp.len) < 0) ***REMOVED***
    goto error;
  ***REMOVED***

  if (grpc_set_socket_sndbuf(fd, buffer_size_bytes) != GRPC_ERROR_NONE) ***REMOVED***
    gpr_log(GPR_ERROR, "Failed to set send buffer size to %d bytes",
            buffer_size_bytes);
    goto error;
  ***REMOVED***

  if (grpc_set_socket_rcvbuf(fd, buffer_size_bytes) != GRPC_ERROR_NONE) ***REMOVED***
...
static void on_read(grpc_exec_ctx* exec_ctx, void* arg, grpc_error* error) ***REMOVED***
...
static void on_write(grpc_exec_ctx* exec_ctx, void* arg, grpc_error* error) ***REMOVED***
  grpc_udp_listener* sp = (grpc_udp_listener*)arg;
...
static int add_socket_to_server(grpc_udp_server* s, int fd,
                                ...
                                grpc_udp_server_read_cb read_cb,
                                grpc_udp_server_write_cb write_cb,
                                grpc_udp_server_orphan_cb orphan_cb) ***REMOVED***
  grpc_udp_listener* sp;
  int port;
  char* addr_str;
  char* name;
...
int grpc_udp_server_add_port(grpc_udp_server* s,
  ...
  if (grpc_sockaddr_get_port(addr) == 0) ***REMOVED***
    for (sp = s->head; sp; sp = sp->next) ***REMOVED***
      sockname_temp.len = sizeof(struct sockaddr_storage);
  ...
  ***REMOVED***
  ...
  if (grpc_sockaddr_is_wildcard(addr, &port)) ***REMOVED***
    ...
    if (fd >= 0 && dsmode == GRPC_DSMODE_DUALSTACK) ***REMOVED***
      goto done;
    ***REMOVED***

    /* If we didn't get a dualstack socket, also listen on 0.0.0.0. */
    if (port == 0 && allocated_port1 > 0) ***REMOVED***
  ...
  ***REMOVED***
  if (dsmode == GRPC_DSMODE_IPV4 &&
      grpc_sockaddr_is_v4mapped(addr, &addr4_copy)) ***REMOVED***
    addr = &addr4_copy;
  ***REMOVED***
  allocated_port2 =
      add_socket_to_server(s, fd, addr, read_cb, write_cb, orphan_cb);
...
int grpc_udp_server_get_fd(grpc_udp_server* s, unsigned port_index) ***REMOVED***
  grpc_udp_listener* sp;
  if (port_index >= s->nports) ***REMOVED***
    return -1;
  ***REMOVED***

  for (sp = s->head; sp && port_index != 0; sp = sp->next) ***REMOVED***
    --port_index;
  ***REMOVED***
