...
struct grpc_grpclb_client_stats ***REMOVED***
  gpr_refcount refs;
...
grpc_grpclb_client_stats* grpc_grpclb_client_stats_create() ***REMOVED***
...
void grpc_grpclb_client_stats_unref(grpc_grpclb_client_stats* client_stats) ***REMOVED***
  if (gpr_unref(&client_stats->refs)) ***REMOVED***
...
void grpc_grpclb_client_stats_add_call_finished(
  ...
  if (finished_with_client_failed_to_send) ***REMOVED***
    gpr_atm_full_fetch_add(
        &client_stats->num_calls_finished_with_client_failed_to_send,
        (gpr_atm)1);
  ***REMOVED***
  if (finished_known_received) ***REMOVED***
...
void grpc_grpclb_client_stats_add_call_dropped_locked(
    char* token, grpc_grpclb_client_stats* client_stats) ***REMOVED***
  // Increment num_calls_started and num_calls_finished.
  gpr_atm_full_fetch_add(&client_stats->num_calls_started, (gpr_atm)1);
  gpr_atm_full_fetch_add(&client_stats->num_calls_finished, (gpr_atm)1);
  // Record the drop.
  if (client_stats->drop_token_counts == nullptr) ***REMOVED***
  ...
  ***REMOVED***
  grpc_grpclb_dropped_call_counts* drop_token_counts =
      client_stats->drop_token_counts;
  for (size_t i = 0; i < drop_token_counts->num_entries; ++i) ***REMOVED***
    if (strcmp(drop_token_counts->token_counts[i].token, token) == 0) ***REMOVED***
  ...
  ***REMOVED***
  ...
  while (new_num_entries < drop_token_counts->num_entries + 1) ***REMOVED***
    new_num_entries *= 2;
  ***REMOVED***
...
static void atomic_get_and_reset_counter(int64_t* value, gpr_atm* counter) ***REMOVED***
...
void grpc_grpclb_client_stats_get_locked(
    ...
    grpc_grpclb_dropped_call_counts** drop_token_counts) ***REMOVED***
...
void grpc_grpclb_dropped_call_counts_destroy(
    grpc_grpclb_dropped_call_counts* drop_entries) ***REMOVED***
  if (drop_entries != nullptr) ***REMOVED***
