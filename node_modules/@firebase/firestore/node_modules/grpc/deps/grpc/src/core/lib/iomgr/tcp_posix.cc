...
typedef struct backup_poller ***REMOVED***
  gpr_mu* pollset_mu;
  grpc_closure run_poller;
***REMOVED*** backup_poller;
...
static void done_poller(grpc_exec_ctx* exec_ctx, void* bp,
  ...
  if (grpc_tcp_trace.enabled()) ***REMOVED***
...
static void run_poller(grpc_exec_ctx* exec_ctx, void* bp,
  ...
  ***REMOVED***
  ...
  if (gpr_atm_no_barrier_load(&g_uncovered_notifications_pending) == 1 &&
      gpr_atm_full_cas(&g_uncovered_notifications_pending, 1, 0)) ***REMOVED***
    gpr_mu_lock(p->pollset_mu);
...
static void drop_uncovered(grpc_exec_ctx* exec_ctx, grpc_tcp* tcp) ***REMOVED***
...
static void cover_self(grpc_exec_ctx* exec_ctx, grpc_tcp* tcp) ***REMOVED***
  backup_poller* p;
  gpr_atm old_count =
...
static void notify_on_read(grpc_exec_ctx* exec_ctx, grpc_tcp* tcp) ***REMOVED***
  if (grpc_tcp_trace.enabled()) ***REMOVED***
...
static void notify_on_write(grpc_exec_ctx* exec_ctx, grpc_tcp* tcp) ***REMOVED***
  if (grpc_tcp_trace.enabled()) ***REMOVED***
...
static void add_to_estimate(grpc_tcp* tcp, size_t bytes) ***REMOVED***
  tcp->bytes_read_this_round += (double)bytes;
***REMOVED***

static void finish_estimate(grpc_tcp* tcp) ***REMOVED***
  /* If we read >80% of the target buffer in one read loop, increase the size
     ...
     value */
  if (tcp->bytes_read_this_round > tcp->target_length * 0.8) ***REMOVED***
    tcp->target_length =
...
static size_t get_target_read_size(grpc_tcp* tcp) ***REMOVED***
  ...
  if (sz > rqmax / 16 && rqmax > 1024) ***REMOVED***
    sz = rqmax / 16;
  ***REMOVED***
  return sz;
***REMOVED***

static grpc_error* tcp_annotate_error(grpc_error* src_error, grpc_tcp* tcp) ***REMOVED***
...
static void tcp_free(grpc_exec_ctx* exec_ctx, grpc_tcp* tcp) ***REMOVED***
...
static void tcp_unref(grpc_exec_ctx* exec_ctx, grpc_tcp* tcp,
  ...
  if (grpc_tcp_trace.enabled()) ***REMOVED***
    ...
    gpr_log(file, line, GPR_LOG_SEVERITY_DEBUG,
            ...
            val - 1);
  ***REMOVED***
  if (gpr_unref(&tcp->refcount)) ***REMOVED***
...
static void tcp_unref(grpc_exec_ctx* exec_ctx, grpc_tcp* tcp) ***REMOVED***
  if (gpr_unref(&tcp->refcount)) ***REMOVED***
    tcp_free(exec_ctx, tcp);
  ***REMOVED***
***REMOVED***

static void tcp_ref(grpc_tcp* tcp) ***REMOVED*** gpr_ref(&tcp->refcount); ***REMOVED***
#endif

static void tcp_destroy(grpc_exec_ctx* exec_ctx, grpc_endpoint* ep) ***REMOVED***
...
static void call_read_cb(grpc_exec_ctx* exec_ctx, grpc_tcp* tcp,
  ...
  if (grpc_tcp_trace.enabled()) ***REMOVED***
    ...
    for (i = 0; i < tcp->incoming_buffer->count; i++) ***REMOVED***
...
static void tcp_do_read(grpc_exec_ctx* exec_ctx, grpc_tcp* tcp) ***REMOVED***
  struct msghdr msg;
  ...
  ***REMOVED*** while (read_bytes < 0 && errno == EINTR);
  GPR_TIMER_END("recvmsg", read_bytes >= 0);

  if (read_bytes < 0) ***REMOVED***
    /* NB: After calling call_read_cb a parallel call of the read handler may
     * be running. */
    if (errno == EAGAIN) ***REMOVED***
      finish_estimate(tcp);
    ...
    ***REMOVED***
  ***REMOVED*** else if (read_bytes == 0) ***REMOVED***
  ...
  ***REMOVED*** else ***REMOVED***
    ...
    if ((size_t)read_bytes < tcp->incoming_buffer->length) ***REMOVED***
...
static void tcp_continue_read(grpc_exec_ctx* exec_ctx, grpc_tcp* tcp) ***REMOVED***
  size_t target_read_size = get_target_read_size(tcp);
  if (tcp->incoming_buffer->length < target_read_size &&
      tcp->incoming_buffer->count < MAX_READ_IOVEC) ***REMOVED***
    if (grpc_tcp_trace.enabled()) ***REMOVED***
...
static void tcp_read(grpc_exec_ctx* exec_ctx, grpc_endpoint* ep,
                     grpc_slice_buffer* incoming_buffer, grpc_closure* cb) ***REMOVED***
  grpc_tcp* tcp = (grpc_tcp*)ep;
...
static bool tcp_flush(grpc_exec_ctx* exec_ctx, grpc_tcp* tcp,
  ...
  for (;;) ***REMOVED***
    ...
    ***REMOVED*** while (sent_length < 0 && errno == EINTR);
    GPR_TIMER_END("sendmsg", 0);

    if (sent_length < 0) ***REMOVED***
      if (errno == EAGAIN) ***REMOVED***
        tcp->outgoing_byte_idx = unwind_byte_idx;
        // unref all and forget about all slices that have been written to this
        // point
        for (size_t idx = 0; idx < unwind_slice_idx; ++idx) ***REMOVED***
          grpc_slice_unref_internal(
              exec_ctx, grpc_slice_buffer_take_first(tcp->outgoing_buffer));
        ***REMOVED***
        return false;
      ***REMOVED*** else if (errno == EPIPE) ***REMOVED***
    ...
    ***REMOVED***
    ...
    while (trailing > 0) ***REMOVED***
      size_t slice_length;

      outgoing_slice_idx--;
      slice_length =
          GRPC_SLICE_LENGTH(tcp->outgoing_buffer->slices[outgoing_slice_idx]);
      if (slice_length > trailing) ***REMOVED***
    ...
    ***REMOVED***
    ...
    if (outgoing_slice_idx == tcp->outgoing_buffer->count) ***REMOVED***
...
static void tcp_handle_write(grpc_exec_ctx* exec_ctx, void* arg /* grpc_tcp */,
  ...
  ***REMOVED***
  ...
  if (!tcp_flush(exec_ctx, tcp, &error)) ***REMOVED***
    if (grpc_tcp_trace.enabled()) ***REMOVED***
...
static void tcp_write(grpc_exec_ctx* exec_ctx, grpc_endpoint* ep,
                      grpc_slice_buffer* buf, grpc_closure* cb) ***REMOVED***
  grpc_tcp* tcp = (grpc_tcp*)ep;
...
static char* tcp_get_peer(grpc_endpoint* ep) ***REMOVED***
  grpc_tcp* tcp = (grpc_tcp*)ep;
  return gpr_strdup(tcp->peer_string);
***REMOVED***

static int tcp_get_fd(grpc_endpoint* ep) ***REMOVED***
  grpc_tcp* tcp = (grpc_tcp*)ep;
  return tcp->fd;
***REMOVED***

static grpc_resource_user* tcp_get_resource_user(grpc_endpoint* ep) ***REMOVED***
  grpc_tcp* tcp = (grpc_tcp*)ep;
  return tcp->resource_user;
***REMOVED***

static const grpc_endpoint_vtable vtable = ***REMOVED***tcp_read,
                                            tcp_write,
                                            tcp_add_to_pollset,
...
grpc_endpoint* grpc_tcp_create(grpc_exec_ctx* exec_ctx, grpc_fd* em_fd,
                               ...
                               const char* peer_string) ***REMOVED***
  int tcp_read_chunk_size = GRPC_TCP_DEFAULT_READ_SLICE_SIZE;
  int tcp_max_read_chunk_size = 4 * 1024 * 1024;
  int tcp_min_read_chunk_size = 256;
  grpc_resource_quota* resource_quota = grpc_resource_quota_create(nullptr);
  if (channel_args != nullptr) ***REMOVED***
    for (size_t i = 0; i < channel_args->num_args; i++) ***REMOVED***
      if (0 ==
          strcmp(channel_args->args[i].key, GRPC_ARG_TCP_READ_CHUNK_SIZE)) ***REMOVED***
        grpc_integer_options options = ***REMOVED***(int)tcp_read_chunk_size, 1,
                                        MAX_CHUNK_SIZE***REMOVED***;
        tcp_read_chunk_size =
            grpc_channel_arg_get_integer(&channel_args->args[i], options);
      ***REMOVED*** else if (0 == strcmp(channel_args->args[i].key,
                             GRPC_ARG_TCP_MIN_READ_CHUNK_SIZE)) ***REMOVED***
        grpc_integer_options options = ***REMOVED***(int)tcp_read_chunk_size, 1,
                                        MAX_CHUNK_SIZE***REMOVED***;
        tcp_min_read_chunk_size =
            grpc_channel_arg_get_integer(&channel_args->args[i], options);
      ***REMOVED*** else if (0 == strcmp(channel_args->args[i].key,
                             GRPC_ARG_TCP_MAX_READ_CHUNK_SIZE)) ***REMOVED***
        grpc_integer_options options = ***REMOVED***(int)tcp_read_chunk_size, 1,
                                        MAX_CHUNK_SIZE***REMOVED***;
        tcp_max_read_chunk_size =
            grpc_channel_arg_get_integer(&channel_args->args[i], options);
      ***REMOVED*** else if (0 ==
                 strcmp(channel_args->args[i].key, GRPC_ARG_RESOURCE_QUOTA)) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (tcp_min_read_chunk_size > tcp_max_read_chunk_size) ***REMOVED***
...
int grpc_tcp_fd(grpc_endpoint* ep) ***REMOVED***
  grpc_tcp* tcp = (grpc_tcp*)ep;
...
void grpc_tcp_destroy_and_release_fd(grpc_exec_ctx* exec_ctx, grpc_endpoint* ep,
                                     int* fd, grpc_closure* done) ***REMOVED***
  grpc_network_status_unregister_endpoint(ep);
  grpc_tcp* tcp = (grpc_tcp*)ep;
  GPR_ASSERT(ep->vtable == &vtable);
  tcp->release_fd = fd;
