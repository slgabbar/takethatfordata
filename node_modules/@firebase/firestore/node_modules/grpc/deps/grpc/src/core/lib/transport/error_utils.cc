...
static grpc_error* recursively_find_error_with_field(grpc_error* error,
                                                     grpc_error_ints which) ***REMOVED***
  // If the error itself has a status code, return it.
  if (grpc_error_get_int(error, which, nullptr)) ***REMOVED***
    return error;
  ***REMOVED***
...
void grpc_error_get_status(grpc_exec_ctx* exec_ctx, grpc_error* error,
  ...
  if (found_error == nullptr) ***REMOVED***
  ...
  ***REMOVED***
  ...
  intptr_t integer;
  if (grpc_error_get_int(found_error, GRPC_ERROR_INT_GRPC_STATUS, &integer)) ***REMOVED***
    status = (grpc_status_code)integer;
  ***REMOVED*** else if (grpc_error_get_int(found_error, GRPC_ERROR_INT_HTTP2_ERROR,
                                &integer)) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (error_string != NULL && status != GRPC_STATUS_OK) ***REMOVED***
    *error_string = gpr_strdup(grpc_error_string(error));
  ***REMOVED***

  if (http_error != nullptr) ***REMOVED***
    if (grpc_error_get_int(found_error, GRPC_ERROR_INT_HTTP2_ERROR, &integer)) ***REMOVED***
      *http_error = (grpc_http2_error_code)integer;
    ***REMOVED*** else if (grpc_error_get_int(found_error, GRPC_ERROR_INT_GRPC_STATUS,
                                  &integer)) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (slice != nullptr) ***REMOVED***
    if (!grpc_error_get_str(found_error, GRPC_ERROR_STR_GRPC_MESSAGE, slice)) ***REMOVED***
      if (!grpc_error_get_str(found_error, GRPC_ERROR_STR_DESCRIPTION, slice)) ***REMOVED***
...
bool grpc_error_has_clear_grpc_status(grpc_error* error) ***REMOVED***
  if (grpc_error_get_int(error, GRPC_ERROR_INT_GRPC_STATUS, nullptr)) ***REMOVED***
    return true;
  ***REMOVED***
  uint8_t slot = error->first_err;
  while (slot != UINT8_MAX) ***REMOVED***
    grpc_linked_error* lerr = (grpc_linked_error*)(error->arena + slot);
    if (grpc_error_has_clear_grpc_status(lerr->err)) ***REMOVED***
      return true;
    ***REMOVED***
    slot = lerr->next;
  ***REMOVED***
