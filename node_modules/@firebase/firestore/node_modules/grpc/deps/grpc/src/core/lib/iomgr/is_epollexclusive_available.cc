...
bool grpc_is_epollexclusive_available(void) ***REMOVED***
  static bool logged_why_not = false;

  int fd = epoll_create1(EPOLL_CLOEXEC);
  if (fd < 0) ***REMOVED***
    if (!logged_why_not) ***REMOVED***
      gpr_log(GPR_ERROR,
  ...
  ***REMOVED***
  ...
  ev.data.ptr = nullptr;
  if (epoll_ctl(fd, EPOLL_CTL_ADD, evfd, &ev) != 0) ***REMOVED***
    if (errno != EINVAL) ***REMOVED***
      if (!logged_why_not) ***REMOVED***
        gpr_log(
            GPR_ERROR,
...
bool grpc_is_epollexclusive_available(void) ***REMOVED*** return false; ***REMOVED***
