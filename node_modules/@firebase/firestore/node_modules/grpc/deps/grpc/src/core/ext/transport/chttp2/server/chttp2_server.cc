...
***REMOVED*** server_connection_state;

static void server_connection_state_unref(
    grpc_exec_ctx* exec_ctx, server_connection_state* connection_state) ***REMOVED***
  if (gpr_unref(&connection_state->refs)) ***REMOVED***
    if (connection_state->transport != nullptr) ***REMOVED***
...
static void on_timeout(grpc_exec_ctx* exec_ctx, void* arg, grpc_error* error) ***REMOVED***
...
static void on_handshake_done(grpc_exec_ctx* exec_ctx, void* arg,
  ...
  if (error != GRPC_ERROR_NONE || connection_state->svr_state->shutdown) ***REMOVED***
    const char* error_str = grpc_error_string(error);
    gpr_log(GPR_DEBUG, "Handshaking failed: %s", error_str);
    if (error == GRPC_ERROR_NONE && args->endpoint != nullptr) ***REMOVED***
  ...
  ***REMOVED*** else ***REMOVED***
    ...
    if (args->endpoint != nullptr) ***REMOVED***
...
static void on_accept(grpc_exec_ctx* exec_ctx, void* arg, grpc_endpoint* tcp,
                      grpc_pollset* accepting_pollset,
                      grpc_tcp_server_acceptor* acceptor) ***REMOVED***
  server_state* state = (server_state*)arg;
  gpr_mu_lock(&state->mu);
  if (state->shutdown) ***REMOVED***
  ...
  ***REMOVED***
  ...
  connection_state->deadline =
      ...
      grpc_channel_arg_get_integer(timeout_arg,
                                   ***REMOVED***120 * GPR_MS_PER_SEC, 1, INT_MAX***REMOVED***);
...
static void tcp_server_shutdown_complete(grpc_exec_ctx* exec_ctx, void* arg,
  ...
  if (destroy_done != nullptr) ***REMOVED***
...
static void server_destroy_listener(grpc_exec_ctx* exec_ctx,
                                    ...
                                    grpc_closure* destroy_done) ***REMOVED***
  server_state* state = (server_state*)arg;
  gpr_mu_lock(&state->mu);
  state->shutdown = true;
...
grpc_error* grpc_chttp2_server_add_port(grpc_exec_ctx* exec_ctx,
  ...
  ***REMOVED***
  ...
  for (i = 0; i < naddrs; i++) ***REMOVED***
    errors[i] =
        grpc_tcp_server_add_port(tcp_server, &resolved->addrs[i], &port_temp);
    if (errors[i] == GRPC_ERROR_NONE) ***REMOVED***
      if (*port_num == -1) ***REMOVED***
        *port_num = port_temp;
      ***REMOVED*** else ***REMOVED***
  ...
  ***REMOVED***
  if (count == 0) ***REMOVED***
    ...
    gpr_free(msg);
    goto error;
  ***REMOVED*** else if (count != naddrs) ***REMOVED***
    char* msg;
    gpr_asprintf(&msg,
