...
#define GRPC_COMBINER_TRACE(fn)          \
  do ***REMOVED***                                   \
    if (grpc_combiner_trace.enabled()) ***REMOVED*** \
      fn;                                \
    ***REMOVED***                                    \
  ***REMOVED*** while (0)
...
static const grpc_closure_scheduler_vtable scheduler = ***REMOVED***
    combiner_exec, combiner_exec, "combiner:immediately"***REMOVED***;
static const grpc_closure_scheduler_vtable finally_scheduler = ***REMOVED***
    combiner_finally_exec, combiner_finally_exec, "combiner:finally"***REMOVED***;

static void offload(grpc_exec_ctx* exec_ctx, void* arg, grpc_error* error);

grpc_combiner* grpc_combiner_create(void) ***REMOVED***
...
static void really_destroy(grpc_exec_ctx* exec_ctx, grpc_combiner* lock) ***REMOVED***
...
static void start_destroy(grpc_exec_ctx* exec_ctx, grpc_combiner* lock) ***REMOVED***
  gpr_atm old_state = gpr_atm_full_fetch_add(&lock->state, -STATE_UNORPHANED);
  GRPC_COMBINER_TRACE(gpr_log(
      GPR_DEBUG, "C:%p really_destroy old_state=%" PRIdPTR, lock, old_state));
  if (old_state == 1) ***REMOVED***
...
void grpc_combiner_unref(grpc_exec_ctx* exec_ctx,
                         grpc_combiner* lock GRPC_COMBINER_DEBUG_ARGS) ***REMOVED***
  GRPC_COMBINER_DEBUG_SPAM("UNREF", -1);
  if (gpr_unref(&lock->refs)) ***REMOVED***
    start_destroy(exec_ctx, lock);
  ***REMOVED***
***REMOVED***

grpc_combiner* grpc_combiner_ref(grpc_combiner* lock GRPC_COMBINER_DEBUG_ARGS) ***REMOVED***
...
static void push_last_on_exec_ctx(grpc_exec_ctx* exec_ctx,
                                  grpc_combiner* lock) ***REMOVED***
  lock->next_combiner_on_this_exec_ctx = nullptr;
  if (exec_ctx->active_combiner == nullptr) ***REMOVED***
...
static void push_first_on_exec_ctx(grpc_exec_ctx* exec_ctx,
  ...
  if (lock->next_combiner_on_this_exec_ctx == nullptr) ***REMOVED***
    exec_ctx->last_combiner = lock;
  ***REMOVED***
***REMOVED***
...
static void combiner_exec(grpc_exec_ctx* exec_ctx, grpc_closure* cl,
  ...
  ***REMOVED*** else ***REMOVED***
    ...
    if (initiator != 0 && initiator != (gpr_atm)exec_ctx) ***REMOVED***
...
static void move_next(grpc_exec_ctx* exec_ctx) ***REMOVED***
...
static void offload(grpc_exec_ctx* exec_ctx, void* arg, grpc_error* error) ***REMOVED***
  grpc_combiner* lock = (grpc_combiner*)arg;
  push_last_on_exec_ctx(exec_ctx, lock);
***REMOVED***

static void queue_offload(grpc_exec_ctx* exec_ctx, grpc_combiner* lock) ***REMOVED***
...
bool grpc_combiner_continue_exec_ctx(grpc_exec_ctx* exec_ctx) ***REMOVED***
  GPR_TIMER_BEGIN("combiner.continue_exec_ctx", 0);
  grpc_combiner* lock = exec_ctx->active_combiner;
  if (lock == nullptr) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (contended && grpc_exec_ctx_ready_to_finish(exec_ctx) &&
      grpc_executor_is_threaded()) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (!lock->time_to_execute_final_list ||
      ...
      // priority
      (gpr_atm_acq_load(&lock->state) >> 1) > 1) ***REMOVED***
...
#define OLD_STATE_WAS(orphaned, elem_count) \
  ...
  switch (old_state) ***REMOVED***
    ...
    case OLD_STATE_WAS(true, 2):
      ...
      if (!grpc_closure_list_empty(lock->final_list)) ***REMOVED***
...
static void combiner_finally_exec(grpc_exec_ctx* exec_ctx,
  ...
  if (exec_ctx->active_combiner != lock) ***REMOVED***
...
grpc_closure_scheduler* grpc_combiner_scheduler(grpc_combiner* combiner) ***REMOVED***
