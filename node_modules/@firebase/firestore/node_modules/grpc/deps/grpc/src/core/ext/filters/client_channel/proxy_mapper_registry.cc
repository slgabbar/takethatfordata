...
static bool grpc_proxy_mapper_list_map_name(grpc_exec_ctx* exec_ctx,
  ...
  for (size_t i = 0; i < list->num_mappers; ++i) ***REMOVED***
    if (grpc_proxy_mapper_map_name(exec_ctx, list->list[i], server_uri, args,
                                   name_to_resolve, new_args)) ***REMOVED***
      return true;
    ***REMOVED***
  ***REMOVED***
  return false;
...
static bool grpc_proxy_mapper_list_map_address(
    ...
    grpc_resolved_address** new_address, grpc_channel_args** new_args) ***REMOVED***
  for (size_t i = 0; i < list->num_mappers; ++i) ***REMOVED***
    if (grpc_proxy_mapper_map_address(exec_ctx, list->list[i], address, args,
                                      new_address, new_args)) ***REMOVED***
      return true;
    ***REMOVED***
  ***REMOVED***
  return false;
...
static void grpc_proxy_mapper_list_destroy(grpc_proxy_mapper_list* list) ***REMOVED***
  for (size_t i = 0; i < list->num_mappers; ++i) ***REMOVED***
...
void grpc_proxy_mapper_registry_init() ***REMOVED******REMOVED***

void grpc_proxy_mapper_registry_shutdown() ***REMOVED***
  grpc_proxy_mapper_list_destroy(&g_proxy_mapper_list);
***REMOVED***

void grpc_proxy_mapper_register(bool at_start, grpc_proxy_mapper* mapper) ***REMOVED***
