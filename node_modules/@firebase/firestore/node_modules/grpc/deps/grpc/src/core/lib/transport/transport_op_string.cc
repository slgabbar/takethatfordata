...
static void put_metadata_list(gpr_strvec* b, grpc_metadata_batch md) ***REMOVED***
  ...
  for (m = md.list.head; m != nullptr; m = m->next) ***REMOVED***
    if (m != md.list.head) gpr_strvec_add(b, gpr_strdup(", "));
    put_metadata(b, m->md);
  ***REMOVED***
  if (md.deadline != GRPC_MILLIS_INF_FUTURE) ***REMOVED***
    char* tmp;
...
char* grpc_transport_op_string(grpc_transport_op* op) ***REMOVED***
  ...
  if (op->on_connectivity_state_change != nullptr) ***REMOVED***
    ...
    first = false;
    if (op->connectivity_state != nullptr) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (op->bind_pollset_set != nullptr) ***REMOVED***
    if (!first) gpr_strvec_add(&b, gpr_strdup(" "));
    first = false;
