 ...
 */
typedef struct epoll_set ***REMOVED***
  int epfd;
...
static bool epoll_set_init() ***REMOVED***
  g_epoll_set.epfd = epoll_create1(EPOLL_CLOEXEC);
  if (g_epoll_set.epfd < 0) ***REMOVED***
...
static void epoll_set_shutdown() ***REMOVED***
  if (g_epoll_set.epfd >= 0) ***REMOVED***
...
typedef enum ***REMOVED*** UNKICKED, KICKED, DESIGNATED_POLLER ***REMOVED*** kick_state;

static const char* kick_state_string(kick_state st) ***REMOVED***
  switch (st) ***REMOVED***
    case UNKICKED:
      return "UNKICKED";
    case KICKED:
...
typedef struct pollset_neighborhood ***REMOVED***
  gpr_mu mu;
  grpc_pollset* active_root;
  char pad[GPR_CACHELINE_SIZE];
***REMOVED*** pollset_neighborhood;
...
static grpc_fd* fd_create(int fd, const char* name) ***REMOVED***
  ...
  ***REMOVED***
  ...
  ev.data.ptr = new_fd;
  if (epoll_ctl(g_epoll_set.epfd, EPOLL_CTL_ADD, fd, &ev) != 0) ***REMOVED***
...
static int fd_wrapped_fd(grpc_fd* fd) ***REMOVED*** return fd->fd; ***REMOVED***
...
static void fd_orphan(grpc_exec_ctx* exec_ctx, grpc_fd* fd,
  ...
  if (!fd->read_closure->IsShutdown()) ***REMOVED***
...
static bool worker_insert(grpc_pollset* pollset, grpc_pollset_worker* worker) ***REMOVED***
...
static worker_remove_result worker_remove(grpc_pollset* pollset,
  ...
  if (worker == pollset->root_worker) ***REMOVED***
    if (worker == worker->next) ***REMOVED***
...
static size_t choose_neighborhood(void) ***REMOVED***
...
static grpc_error* pollset_global_init(void) ***REMOVED***
  ...
  ***REMOVED***
  ...
  g_neighborhoods = (pollset_neighborhood*)gpr_zalloc(sizeof(*g_neighborhoods) *
                                                      g_num_neighborhoods);
  for (size_t i = 0; i < g_num_neighborhoods; i++) ***REMOVED***
...
static void pollset_destroy(grpc_exec_ctx* exec_ctx, grpc_pollset* pollset) ***REMOVED***
  ...
  if (!pollset->seen_inactive) ***REMOVED***
  ...
  retry_lock_neighborhood:
    ...
    gpr_mu_lock(&pollset->mu);
    if (!pollset->seen_inactive) ***REMOVED***
      if (pollset->neighborhood != neighborhood) ***REMOVED***
      ...
      ***REMOVED***
      ...
      if (pollset == pollset->neighborhood->active_root) ***REMOVED***
...
static grpc_error* process_epoll_events(grpc_exec_ctx* exec_ctx,
  ...
  for (int idx = 0;
    ...
    if (data_ptr == &global_wakeup_fd) ***REMOVED***
...
static bool begin_worker(grpc_exec_ctx* exec_ctx, grpc_pollset* pollset,
  ...
  ***REMOVED***
  ...
  if (pollset->seen_inactive) ***REMOVED***
    ...
    if (!pollset->reassigning_neighborhood) ***REMOVED***
      is_reassigning = true;
  ...
  retry_lock_neighborhood:
    ...
    ***REMOVED***
    if (pollset->seen_inactive) ***REMOVED***
      if (neighborhood != pollset->neighborhood) ***REMOVED***
        gpr_mu_unlock(&neighborhood->mu);
        neighborhood = pollset->neighborhood;
        gpr_mu_unlock(&pollset->mu);
        goto retry_lock_neighborhood;
      ***REMOVED***
      ...
      if (worker->state == UNKICKED) ***REMOVED***
        pollset->seen_inactive = false;
        if (neighborhood->active_root == nullptr) ***REMOVED***
          ...
          if (worker->state == UNKICKED &&
              gpr_atm_no_barrier_cas(&g_active_poller, 0, (gpr_atm)worker)) ***REMOVED***
  ...
  ***REMOVED***
  ...
  pollset->begin_refs--;
  if (worker->state == UNKICKED && !pollset->kicked_without_poller) ***REMOVED***
    GPR_ASSERT(gpr_atm_no_barrier_load(&g_active_poller) != (gpr_atm)worker);
    worker->initialized_cv = true;
    gpr_cv_init(&worker->cv);
    while (worker->state == UNKICKED && !pollset->shutting_down) ***REMOVED***
      if (grpc_polling_trace.enabled()) ***REMOVED***
  ...
  do ***REMOVED***
    ...
    ***REMOVED***
    ...
    if (inspect_worker != nullptr) ***REMOVED***
      ...
      ***REMOVED*** while (!found_worker && inspect_worker != inspect->root_worker);
    ***REMOVED***
    if (!found_worker) ***REMOVED***
      if (grpc_polling_trace.enabled()) ***REMOVED***
        gpr_log(GPR_DEBUG, " .. mark pollset %p inactive", inspect);
      ***REMOVED***
      inspect->seen_inactive = true;
      if (inspect == neighborhood->active_root) ***REMOVED***
...
static void end_worker(grpc_exec_ctx* exec_ctx, grpc_pollset* pollset,
  ...
  ***REMOVED***
  ...
  if (gpr_atm_no_barrier_load(&g_active_poller) == (gpr_atm)worker) ***REMOVED***
    if (worker->next != worker && worker->next->state == UNKICKED) ***REMOVED***
      if (grpc_polling_trace.enabled()) ***REMOVED***
    ...
    ***REMOVED*** else ***REMOVED***
      ...
      for (size_t i = 0; !found_worker && i < g_num_neighborhoods; i++) ***REMOVED***
        pollset_neighborhood* neighborhood =
            &g_neighborhoods[(poller_neighborhood_idx + i) %
                             g_num_neighborhoods];
        if (gpr_mu_trylock(&neighborhood->mu)) ***REMOVED***
          found_worker =
      ...
      ***REMOVED***
      ...
      gpr_mu_lock(&pollset->mu);
    ***REMOVED***
  ***REMOVED*** else if (grpc_exec_ctx_has_work(exec_ctx)) ***REMOVED***
    gpr_mu_unlock(&pollset->mu);
    grpc_exec_ctx_flush(exec_ctx);
    gpr_mu_lock(&pollset->mu);
  ***REMOVED***
  ...
  ***REMOVED***
  if (EMPTIED == worker_remove(pollset, worker)) ***REMOVED***
    pollset_maybe_finish_shutdown(exec_ctx, pollset);
  ***REMOVED***
  GPR_ASSERT(gpr_atm_no_barrier_load(&g_active_poller) != (gpr_atm)worker);
...
static grpc_error* pollset_work(grpc_exec_ctx* exec_ctx, grpc_pollset* ps,
  ...
  if (ps->kicked_without_poller) ***REMOVED***
    ps->kicked_without_poller = false;
    GPR_TIMER_END("pollset_work", 0);
    return GRPC_ERROR_NONE;
  ***REMOVED***

  if (begin_worker(exec_ctx, ps, &worker, worker_hdl, deadline)) ***REMOVED***
    ...
    if (gpr_atm_acq_load(&g_epoll_set.cursor) ==
        gpr_atm_acq_load(&g_epoll_set.num_events)) ***REMOVED***
...
static grpc_error* pollset_kick(grpc_exec_ctx* exec_ctx, grpc_pollset* pollset,
  ...
  if (grpc_polling_trace.enabled()) ***REMOVED***
    ...
    if (pollset->root_worker != nullptr) ***REMOVED***
      gpr_asprintf(&tmp, " ***REMOVED***kick_state=%s next=%p ***REMOVED***kick_state=%s***REMOVED******REMOVED***",
  ...
  if (specific_worker == nullptr) ***REMOVED***
    if (gpr_tls_get(&g_current_thread_pollset) != (intptr_t)pollset) ***REMOVED***
      ...
      ***REMOVED***
      ...
      if (root_worker->state == KICKED) ***REMOVED***
        GRPC_STATS_INC_POLLSET_KICKED_AGAIN(exec_ctx);
        if (grpc_polling_trace.enabled()) ***REMOVED***
        ...
        ***REMOVED***
        ...
        goto done;
      ***REMOVED*** else if (next_worker->state == KICKED) ***REMOVED***
        GRPC_STATS_INC_POLLSET_KICKED_AGAIN(exec_ctx);
        if (grpc_polling_trace.enabled()) ***REMOVED***
      ...
      ***REMOVED*** else if (root_worker ==
        ...
        ***REMOVED***
        ...
        goto done;
      ***REMOVED*** else if (next_worker->state == UNKICKED) ***REMOVED***
        GRPC_STATS_INC_POLLSET_KICK_WAKEUP_CV(exec_ctx);
        if (grpc_polling_trace.enabled()) ***REMOVED***
        ...
        ***REMOVED***
        ...
        goto done;
      ***REMOVED*** else if (next_worker->state == DESIGNATED_POLLER) ***REMOVED***
        if (root_worker->state != DESIGNATED_POLLER) ***REMOVED***
          if (grpc_polling_trace.enabled()) ***REMOVED***
            gpr_log(
                GPR_ERROR,
          ...
          ***REMOVED***
          SET_KICK_STATE(root_worker, KICKED);
          if (root_worker->initialized_cv) ***REMOVED***
            GRPC_STATS_INC_POLLSET_KICK_WAKEUP_CV(exec_ctx);
  ...
  ***REMOVED***
  ...
  if (specific_worker->state == KICKED) ***REMOVED***
    if (grpc_polling_trace.enabled()) ***REMOVED***
  ...
  ***REMOVED*** else if (gpr_tls_get(&g_current_thread_worker) ==
    ...
    ***REMOVED***
    ...
    goto done;
  ***REMOVED*** else if (specific_worker ==
             (grpc_pollset_worker*)gpr_atm_no_barrier_load(&g_active_poller)) ***REMOVED***
    GRPC_STATS_INC_POLLSET_KICK_WAKEUP_FD(exec_ctx);
    if (grpc_polling_trace.enabled()) ***REMOVED***
    ...
    ***REMOVED***
    ...
    ret_err = grpc_wakeup_fd_wakeup(&global_wakeup_fd);
    goto done;
  ***REMOVED*** else if (specific_worker->initialized_cv) ***REMOVED***
    GRPC_STATS_INC_POLLSET_KICK_WAKEUP_CV(exec_ctx);
    if (grpc_polling_trace.enabled()) ***REMOVED***
...
static void pollset_set_destroy(grpc_exec_ctx* exec_ctx,
                                grpc_pollset_set* pss) ***REMOVED******REMOVED***
...
const grpc_event_engine_vtable* grpc_init_epoll1_linux(bool explicit_request) ***REMOVED***
  if (!grpc_has_wakeup_fd()) ***REMOVED***
    gpr_log(GPR_ERROR, "Skipping epoll1 because of no wakeup fd.");
    return nullptr;
  ***REMOVED***

  if (!epoll_set_init()) ***REMOVED***
    return nullptr;
  ***REMOVED***
