...
void grpc_set_ssl_roots_override_callback(grpc_ssl_roots_override_callback cb) ***REMOVED***
...
static void init_cipher_suites(void) ***REMOVED***
  char* overridden = gpr_getenv("GRPC_SSL_CIPHER_SUITES");
  cipher_suites = overridden != nullptr ? overridden : GRPC_SSL_CIPHER_SUITES;
***REMOVED***

static const char* ssl_cipher_suites(void) ***REMOVED***
...
const tsi_peer_property* tsi_peer_get_property_by_name(const tsi_peer* peer,
  ...
  for (i = 0; i < peer->property_count; i++) ***REMOVED***
    ...
    if (name == nullptr && property->name == nullptr) ***REMOVED***
      return property;
    ***REMOVED***
    if (name != nullptr && property->name != nullptr &&
        strcmp(property->name, name) == 0) ***REMOVED***
      return property;
    ***REMOVED***
  ***REMOVED***
  return nullptr;
...
void grpc_security_connector_check_peer(grpc_exec_ctx* exec_ctx,
                                        ...
                                        grpc_closure* on_peer_checked) ***REMOVED***
  if (sc == nullptr) ***REMOVED***
...
int grpc_security_connector_cmp(grpc_security_connector* sc,
                                grpc_security_connector* other) ***REMOVED***
...
int grpc_channel_security_connector_cmp(grpc_channel_security_connector* sc1,
                                        grpc_channel_security_connector* sc2) ***REMOVED***
...
int grpc_server_security_connector_cmp(grpc_server_security_connector* sc1,
                                       grpc_server_security_connector* sc2) ***REMOVED***
...
bool grpc_channel_security_connector_check_call_host(
    ...
    grpc_closure* on_call_host_checked, grpc_error** error) ***REMOVED***
  if (sc == nullptr || sc->check_call_host == nullptr) ***REMOVED***
...
void grpc_channel_security_connector_cancel_check_call_host(
    grpc_exec_ctx* exec_ctx, grpc_channel_security_connector* sc,
    grpc_closure* on_call_host_checked, grpc_error* error) ***REMOVED***
  if (sc == nullptr || sc->cancel_check_call_host == nullptr) ***REMOVED***
...
grpc_security_connector* grpc_security_connector_ref(
  ...
  if (grpc_trace_security_connector_refcount.enabled()) ***REMOVED***
...
#else
grpc_security_connector* grpc_security_connector_ref(
    grpc_security_connector* sc) ***REMOVED***
...
static void connector_arg_destroy(grpc_exec_ctx* exec_ctx, void* p) ***REMOVED***
  GRPC_SECURITY_CONNECTOR_UNREF(exec_ctx, (grpc_security_connector*)p,
                                "connector_arg_destroy");
***REMOVED***

static void* connector_arg_copy(void* p) ***REMOVED***
  return GRPC_SECURITY_CONNECTOR_REF((grpc_security_connector*)p,
                                     "connector_arg_copy");
***REMOVED***

static int connector_cmp(void* a, void* b) ***REMOVED***
  return grpc_security_connector_cmp((grpc_security_connector*)a,
                                     (grpc_security_connector*)b);
***REMOVED***

static const grpc_arg_pointer_vtable connector_arg_vtable = ***REMOVED***
    connector_arg_copy, connector_arg_destroy, connector_cmp***REMOVED***;

grpc_arg grpc_security_connector_to_arg(grpc_security_connector* sc) ***REMOVED***
  return grpc_channel_arg_pointer_create((char*)GRPC_ARG_SECURITY_CONNECTOR, sc,
                                         &connector_arg_vtable);
***REMOVED***

grpc_security_connector* grpc_security_connector_from_arg(const grpc_arg* arg) ***REMOVED***
...
get_tsi_client_certificate_request_type(
    grpc_ssl_client_certificate_request_type grpc_request_type) ***REMOVED***
  switch (grpc_request_type) ***REMOVED***
...
typedef struct ***REMOVED***
  ...
  bool is_lb_channel;
***REMOVED*** grpc_fake_channel_security_connector;
...
static bool fake_check_target(const char* target_type, const char* target,
  ...
  bool found = false;
  for (size_t i = 0; i < set_size; ++i) ***REMOVED***
...
static void fake_secure_name_check(const char* target,
  ...
  if (lbs_and_backends_size > 2 || lbs_and_backends_size == 0) ***REMOVED***
    gpr_log(GPR_ERROR, "Invalid expected targets arg value: '%s'",
            expected_targets);
    goto done;
  ***REMOVED***
  if (is_lb_channel) ***REMOVED***
    if (lbs_and_backends_size != 2) ***REMOVED***
      gpr_log(GPR_ERROR,
      ...
      goto done;
    ***REMOVED***
    if (!fake_check_target("LB", target, lbs_and_backends[1])) ***REMOVED***
    ...
    ***REMOVED***
    success = true;
  ***REMOVED*** else ***REMOVED***
    if (!fake_check_target("Backend", target, lbs_and_backends[0])) ***REMOVED***
  ...
  ***REMOVED***
  ...
  for (size_t i = 0; i < lbs_and_backends_size; ++i) ***REMOVED***
...
static void fake_check_peer(grpc_exec_ctx* exec_ctx,
  ...
  *auth_context = nullptr;
  if (peer.property_count != 1) ***REMOVED***
    error = GRPC_ERROR_CREATE_FROM_STATIC_STRING(
  ...
  ***REMOVED***
  ...
  if (prop_name == nullptr ||
      strcmp(prop_name, TSI_CERTIFICATE_TYPE_PEER_PROPERTY)) ***REMOVED***
    char* msg;
  ...
  ***REMOVED***
  if (strncmp(peer.properties[0].value.data, TSI_FAKE_CERTIFICATE_TYPE,
              peer.properties[0].value.length)) ***REMOVED***
    error = GRPC_ERROR_CREATE_FROM_STATIC_STRING(
...
static int fake_channel_cmp(grpc_security_connector* sc1,
                            grpc_security_connector* sc2) ***REMOVED***
  ...
  if (c != 0) return c;
  if (c1->expected_targets == nullptr || c2->expected_targets == nullptr) ***REMOVED***
...
static grpc_security_connector_vtable fake_channel_vtable = ***REMOVED***
    fake_channel_destroy, fake_channel_check_peer, fake_channel_cmp***REMOVED***;

static grpc_security_connector_vtable fake_server_vtable = ***REMOVED***
    fake_server_destroy, fake_server_check_peer, fake_server_cmp***REMOVED***;
...
grpc_server_security_connector* grpc_fake_server_security_connector_create(
    grpc_server_credentials* server_creds) ***REMOVED***
...
***REMOVED*** grpc_ssl_channel_security_connector;

typedef struct ***REMOVED***
  grpc_server_security_connector base;
  tsi_ssl_server_handshaker_factory* server_handshaker_factory;
***REMOVED*** grpc_ssl_server_security_connector;

static bool server_connector_has_cert_config_fetcher(
    grpc_ssl_server_security_connector* c) ***REMOVED***
...
static const char** fill_alpn_protocol_strings(size_t* num_alpn_protocols) ***REMOVED***
  GPR_ASSERT(num_alpn_protocols != nullptr);
  *num_alpn_protocols = grpc_chttp2_num_alpn_versions();
  const char** alpn_protocol_strings =
      (const char**)gpr_malloc(sizeof(const char*) * (*num_alpn_protocols));
  for (size_t i = 0; i < *num_alpn_protocols; i++) ***REMOVED***
...
static bool try_replace_server_handshaker_factory(
    grpc_ssl_server_security_connector* sc,
    const grpc_ssl_server_certificate_config* config) ***REMOVED***
  if (config == nullptr) ***REMOVED***
    gpr_log(GPR_ERROR,
...
static bool try_fetch_ssl_server_credentials(
    grpc_ssl_server_security_connector* sc) ***REMOVED***
  ...
  grpc_ssl_certificate_config_reload_status cb_result =
      server_creds->certificate_config_fetcher.cb(
          ...
          &certificate_config);
  if (cb_result == GRPC_SSL_CERTIFICATE_CONFIG_RELOAD_UNCHANGED) ***REMOVED***
    gpr_log(GPR_DEBUG, "No change in SSL server credentials.");
    status = false;
  ***REMOVED*** else if (cb_result == GRPC_SSL_CERTIFICATE_CONFIG_RELOAD_NEW) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (certificate_config != nullptr) ***REMOVED***
...
static int ssl_host_matches_name(const tsi_peer* peer, const char* peer_name) ***REMOVED***
  char* allocated_name = nullptr;
  int r;

  if (strchr(peer_name, ':') != nullptr) ***REMOVED***
    char* ignored_port;
...
grpc_auth_context* tsi_ssl_peer_to_auth_context(const tsi_peer* peer) ***REMOVED***
  size_t i;
  ...
  for (i = 0; i < peer->property_count; i++) ***REMOVED***
    ...
    if (strcmp(prop->name, TSI_X509_SUBJECT_COMMON_NAME_PEER_PROPERTY) == 0) ***REMOVED***
      /* If there is no subject alt name, have the CN as the identity. */
      if (peer_identity_property_name == nullptr) ***REMOVED***
        peer_identity_property_name = GRPC_X509_CN_PROPERTY_NAME;
      ***REMOVED***
      grpc_auth_context_add_property(ctx, GRPC_X509_CN_PROPERTY_NAME,
                                     prop->value.data, prop->value.length);
    ***REMOVED*** else if (strcmp(prop->name,
      ...
      grpc_auth_context_add_property(ctx, GRPC_X509_SAN_PROPERTY_NAME,
                                     prop->value.data, prop->value.length);
    ***REMOVED*** else if (strcmp(prop->name, TSI_X509_PEM_CERT_PROPERTY) == 0) ***REMOVED***
...
static grpc_error* ssl_check_peer(grpc_security_connector* sc,
                                  ...
                                  grpc_auth_context** auth_context) ***REMOVED***
  /* Check the ALPN. */
  ...
  ***REMOVED***
  if (!grpc_chttp2_is_alpn_version_supported(p->value.data, p->value.length)) ***REMOVED***
    return GRPC_ERROR_CREATE_FROM_STATIC_STRING(
        "Cannot check peer: invalid ALPN value.");
  ***REMOVED***

  /* Check the peer name if specified. */
  if (peer_name != nullptr && !ssl_host_matches_name(peer, peer_name)) ***REMOVED***
    char* msg;
...
static void add_shallow_auth_property_to_peer(tsi_peer* peer,
                                              ...
                                              const char* tsi_prop_name) ***REMOVED***
...
tsi_peer tsi_shallow_peer_from_ssl_auth_context(
    const grpc_auth_context* auth_context) ***REMOVED***
  ...
  if (max_num_props > 0) ***REMOVED***
    peer.properties = (tsi_peer_property*)gpr_malloc(max_num_props *
                                                     sizeof(tsi_peer_property));
    it = grpc_auth_context_property_iterator(auth_context);
    while ((prop = grpc_auth_property_iterator_next(&it)) != nullptr) ***REMOVED***
      if (strcmp(prop->name, GRPC_X509_SAN_PROPERTY_NAME) == 0) ***REMOVED***
        add_shallow_auth_property_to_peer(
            &peer, prop, TSI_X509_SUBJECT_ALTERNATIVE_NAME_PEER_PROPERTY);
      ***REMOVED*** else if (strcmp(prop->name, GRPC_X509_CN_PROPERTY_NAME) == 0) ***REMOVED***
        add_shallow_auth_property_to_peer(
            &peer, prop, TSI_X509_SUBJECT_COMMON_NAME_PEER_PROPERTY);
      ***REMOVED*** else if (strcmp(prop->name, GRPC_X509_PEM_CERT_PROPERTY_NAME) == 0) ***REMOVED***
...
void tsi_shallow_peer_destruct(tsi_peer* peer) ***REMOVED***
...
static bool ssl_channel_check_call_host(grpc_exec_ctx* exec_ctx,
  ...
  if (c->overridden_target_name != nullptr &&
      strcmp(host, c->target_name) == 0) ***REMOVED***
    status = GRPC_SECURITY_OK;
  ***REMOVED***
  if (status != GRPC_SECURITY_OK) ***REMOVED***
    *error = GRPC_ERROR_CREATE_FROM_STATIC_STRING(
...
static grpc_security_connector_vtable ssl_channel_vtable = ***REMOVED***
    ssl_channel_destroy, ssl_channel_check_peer, ssl_channel_cmp***REMOVED***;

static grpc_security_connector_vtable ssl_server_vtable = ***REMOVED***
    ssl_server_destroy, ssl_server_check_peer, ssl_server_cmp***REMOVED***;

/* returns a NULL terminated slice. */
static grpc_slice compute_default_pem_root_certs_once(void) ***REMOVED***
  grpc_slice result = grpc_empty_slice();

  /* First try to load the roots from the environment. */
  char* default_root_certs_path =
      gpr_getenv(GRPC_DEFAULT_SSL_ROOTS_FILE_PATH_ENV_VAR);
  if (default_root_certs_path != nullptr) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (GRPC_SLICE_IS_EMPTY(result) && ssl_roots_override_cb != nullptr) ***REMOVED***
    char* pem_root_certs = nullptr;
    ovrd_res = ssl_roots_override_cb(&pem_root_certs);
    if (ovrd_res == GRPC_SSL_ROOTS_OVERRIDE_OK) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (GRPC_SLICE_IS_EMPTY(result) &&
      ovrd_res != GRPC_SSL_ROOTS_OVERRIDE_FAIL_PERMANENTLY) ***REMOVED***
    GRPC_LOG_IF_ERROR("load_file",
...
static void init_default_pem_root_certs(void) ***REMOVED***
  default_pem_root_certs = compute_default_pem_root_certs_once();
***REMOVED***

grpc_slice grpc_get_default_ssl_roots_for_testing(void) ***REMOVED***
  return compute_default_pem_root_certs_once();
***REMOVED***

const char* grpc_get_default_ssl_roots(void) ***REMOVED***
...
grpc_security_status grpc_ssl_channel_security_connector_create(
    ...
    const char* overridden_target_name, grpc_channel_security_connector** sc) ***REMOVED***
  size_t num_alpn_protocols = 0;
  const char** alpn_protocol_strings =
      fill_alpn_protocol_strings(&num_alpn_protocols);
  ...
  if (config == nullptr || target_name == nullptr) ***REMOVED***
    gpr_log(GPR_ERROR, "An ssl channel needs a config and a target name.");
    goto error;
  ***REMOVED***
  if (config->pem_root_certs == nullptr) ***REMOVED***
    pem_root_certs = grpc_get_default_ssl_roots();
    if (pem_root_certs == nullptr) ***REMOVED***
  ...
  ***REMOVED***
  ...
  gpr_free(port);
  if (overridden_target_name != nullptr) ***REMOVED***
...
grpc_security_status grpc_ssl_server_security_connector_create(
    ...
    grpc_server_security_connector** sc) ***REMOVED***
  tsi_result result = TSI_OK;
  ...
  if (server_connector_has_cert_config_fetcher(c)) ***REMOVED***
    // Load initial credentials from certificate_config_fetcher:
    if (!try_fetch_ssl_server_credentials(c)) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (retval == GRPC_SECURITY_OK) ***REMOVED***
    *sc = &c->base;
  ***REMOVED*** else ***REMOVED***
