...
void grpc_backoff_init(grpc_backoff* backoff, grpc_millis initial_backoff,
                       ...
                       grpc_millis max_backoff) ***REMOVED***
...
grpc_backoff_result grpc_backoff_begin(grpc_exec_ctx* exec_ctx,
  ...
  const grpc_backoff_result result = ***REMOVED***now + initial_timeout,
                                      now + backoff->current_backoff***REMOVED***;
  return result;
***REMOVED***

/* Generate a random number between 0 and 1. */
static double generate_uniform_random_number(uint32_t* rng_state) ***REMOVED***
...
grpc_backoff_result grpc_backoff_step(grpc_exec_ctx* exec_ctx,
  ...
  const grpc_millis now = grpc_exec_ctx_now(exec_ctx);
  const grpc_backoff_result result = ***REMOVED***now + current_timeout,
                                      now + next_timeout***REMOVED***;
  return result;
***REMOVED***

void grpc_backoff_reset(grpc_backoff* backoff) ***REMOVED***
  backoff->current_backoff = backoff->initial_backoff;
***REMOVED***
