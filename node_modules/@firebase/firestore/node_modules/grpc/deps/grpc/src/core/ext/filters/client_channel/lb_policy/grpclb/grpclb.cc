 ...
 *
 * This policy takes as input a set of resolved addresses ***REMOVED***a1..an***REMOVED*** for which the
 * LB set was set (it's the resolver's responsibility to ensure this). That is
 * to say, ***REMOVED***a1..an***REMOVED*** represent a collection of LB servers.
 *
 * An internal channel (\a glb_lb_policy.lb_channel) is created over ***REMOVED***a1..an***REMOVED***.
 ...
 * server (whichever address from ***REMOVED***a1..an***REMOVED*** pick-first chose). This call is
 ...
 * applies, continuing to pick from the list ***REMOVED***a1..an***REMOVED***.
 *
...
static grpc_error* initial_metadata_add_lb_token(
    ...
    grpc_linked_mdelem* lb_token_mdelem_storage, grpc_mdelem lb_token) ***REMOVED***
...
static void destroy_client_stats(void* arg) ***REMOVED***
  grpc_grpclb_client_stats_unref((grpc_grpclb_client_stats*)arg);
***REMOVED***

typedef struct wrapped_rr_closure_arg ***REMOVED***
  ...
  void* free_when_done;
***REMOVED*** wrapped_rr_closure_arg;
...
static void wrapped_rr_closure(grpc_exec_ctx* exec_ctx, void* arg,
  ...
  if (wc_arg->rr_policy != nullptr) ***REMOVED***
    /* if *target is NULL, no pick has been made by the RR policy (eg, all
     * addresses failed to connect). There won't be any user_data/token
     * available */
    if (*wc_arg->target != nullptr) ***REMOVED***
      if (!GRPC_MDISNULL(wc_arg->lb_token)) ***REMOVED***
    ...
    ***REMOVED***
    if (grpc_lb_glb_trace.enabled()) ***REMOVED***
...
typedef struct pending_pick ***REMOVED***
...
typedef struct pending_ping ***REMOVED***
  struct pending_ping* next;

  /* args for wrapped_notify */
  wrapped_rr_closure_arg wrapped_notify_arg;
***REMOVED*** pending_ping;

static void add_pending_ping(pending_ping** root, grpc_closure* notify) ***REMOVED***
...
typedef struct glb_lb_policy ***REMOVED***
...
***REMOVED*** glb_lb_policy;

/* Keeps track and reacts to changes in connectivity of the RR instance */
struct rr_connectivity_data ***REMOVED***
...
static bool is_server_valid(const grpc_grpclb_server* server, size_t idx,
  ...
  if (server->port >> 16 != 0) ***REMOVED***
    ...
    ***REMOVED***
    return false;
  ***REMOVED***
  if (ip->size != 4 && ip->size != 16) ***REMOVED***
    if (log) ***REMOVED***
      gpr_log(GPR_ERROR,
...
static void* lb_token_copy(void* token) ***REMOVED***
  return token == nullptr
             ? nullptr
             : (void*)GRPC_MDELEM_REF(grpc_mdelem***REMOVED***(uintptr_t)token***REMOVED***).payload;
***REMOVED***
static void lb_token_destroy(grpc_exec_ctx* exec_ctx, void* token) ***REMOVED***
  if (token != nullptr) ***REMOVED***
    GRPC_MDELEM_UNREF(exec_ctx, grpc_mdelem***REMOVED***(uintptr_t)token***REMOVED***);
  ***REMOVED***
***REMOVED***
static int lb_token_cmp(void* token1, void* token2) ***REMOVED***
  if (token1 > token2) return 1;
  if (token1 < token2) return -1;
  return 0;
***REMOVED***
static const grpc_lb_user_data_vtable lb_token_vtable = ***REMOVED***
    lb_token_copy, lb_token_destroy, lb_token_cmp***REMOVED***;

static void parse_server(const grpc_grpclb_server* server,
                         grpc_resolved_address* addr) ***REMOVED***
  ...
  const grpc_grpclb_ip_address* ip = &server->ip_address;
  if (ip->size == 4) ***REMOVED***
    addr->len = sizeof(struct sockaddr_in);
    struct sockaddr_in* addr4 = (struct sockaddr_in*)&addr->addr;
    addr4->sin_family = AF_INET;
    memcpy(&addr4->sin_addr, ip->bytes, ip->size);
    addr4->sin_port = netorder_port;
  ***REMOVED*** else if (ip->size == 16) ***REMOVED***
...
static grpc_lb_addresses* process_serverlist_locked(
    grpc_exec_ctx* exec_ctx, const grpc_grpclb_serverlist* serverlist) ***REMOVED***
  size_t num_valid = 0;
  /* first pass: count how many are valid in order to allocate the necessary
   * memory in a single block */
  for (size_t i = 0; i < serverlist->num_servers; ++i) ***REMOVED***
  ...
  ***REMOVED***
  ...
  size_t addr_idx = 0;
  for (size_t sl_idx = 0; sl_idx < serverlist->num_servers; ++sl_idx) ***REMOVED***
    ...
    /* lb token processing */
    void* user_data;
    if (server->has_load_balance_token) ***REMOVED***
...
static grpc_lb_addresses* extract_backend_addresses_locked(
    grpc_exec_ctx* exec_ctx, const grpc_lb_addresses* addresses) ***REMOVED***
...
static void update_lb_connectivity_status_locked(
    ...
    grpc_connectivity_state rr_state, grpc_error* rr_state_error) ***REMOVED***
...
static bool pick_from_internal_rr_locked(
    ...
    grpc_connected_subchannel** target, wrapped_rr_closure_arg* wc_arg) ***REMOVED***
  // Check for drops if we are not using fallback backend addresses.
  if (glb_policy->serverlist != nullptr) ***REMOVED***
    // Look at the index into the serverlist to see if we should drop this call.
    grpc_grpclb_server* server =
        glb_policy->serverlist->servers[glb_policy->serverlist_index++];
    if (glb_policy->serverlist_index == glb_policy->serverlist->num_servers) ***REMOVED***
      glb_policy->serverlist_index = 0;  // Wrap-around.
    ***REMOVED***
    if (server->drop) ***REMOVED***
...
static grpc_lb_policy_args* lb_policy_args_create(grpc_exec_ctx* exec_ctx,
                                                  glb_lb_policy* glb_policy) ***REMOVED***
  ...
  ***REMOVED***
  ...
  // the subchannel.
  static const char* keys_to_remove[] = ***REMOVED***GRPC_ARG_LB_ADDRESSES***REMOVED***;
...
static void lb_policy_args_destroy(grpc_exec_ctx* exec_ctx,
                                   grpc_lb_policy_args* args) ***REMOVED***
...
static void create_rr_locked(grpc_exec_ctx* exec_ctx, glb_lb_policy* glb_policy,
  ...
  if (new_rr_policy == nullptr) ***REMOVED***
    gpr_log(GPR_ERROR,
  ...
  ***REMOVED***
  ...
  pending_pick* pp;
  while ((pp = glb_policy->pending_picks)) ***REMOVED***
  ...
  ***REMOVED***
  ...
  pending_ping* pping;
  while ((pping = glb_policy->pending_pings)) ***REMOVED***
...
static void rr_handover_locked(grpc_exec_ctx* exec_ctx,
  ...
  if (glb_policy->rr_policy != nullptr) ***REMOVED***
    if (grpc_lb_glb_trace.enabled()) ***REMOVED***
...
static void glb_rr_connectivity_changed_locked(grpc_exec_ctx* exec_ctx,
  ...
  if (glb_policy->shutting_down) ***REMOVED***
    GRPC_LB_POLICY_WEAK_UNREF(exec_ctx, &glb_policy->base,
                              "glb_rr_connectivity_cb");
    gpr_free(rr_connectivity);
    return;
  ***REMOVED***
  if (rr_connectivity->state == GRPC_CHANNEL_SHUTDOWN) ***REMOVED***
...
static void destroy_balancer_name(grpc_exec_ctx* exec_ctx,
                                  void* balancer_name) ***REMOVED***
  gpr_free(balancer_name);
***REMOVED***

static grpc_slice_hash_table_entry targets_info_entry_create(
    const char* address, const char* balancer_name) ***REMOVED***
...
static int balancer_name_cmp_fn(void* a, void* b) ***REMOVED***
...
static grpc_channel_args* build_lb_channel_args(
  ...
  ***REMOVED***
  ...
  for (size_t i = 0; i < addresses->num_addresses; ++i) ***REMOVED***
    if (!addresses->addresses[i].is_balancer) continue;
    if (addresses->addresses[i].user_data != nullptr) ***REMOVED***
      gpr_log(GPR_ERROR,
...
static void glb_destroy(grpc_exec_ctx* exec_ctx, grpc_lb_policy* pol) ***REMOVED***
  glb_lb_policy* glb_policy = (glb_lb_policy*)pol;
  GPR_ASSERT(glb_policy->pending_picks == nullptr);
  GPR_ASSERT(glb_policy->pending_pings == nullptr);
  gpr_free((void*)glb_policy->server_name);
  grpc_channel_args_destroy(exec_ctx, glb_policy->args);
  if (glb_policy->client_stats != nullptr) ***REMOVED***
  ...
  ***REMOVED***
  if (glb_policy->fallback_backend_addresses != nullptr) ***REMOVED***
...
static void glb_shutdown_locked(grpc_exec_ctx* exec_ctx, grpc_lb_policy* pol) ***REMOVED***
  glb_lb_policy* glb_policy = (glb_lb_policy*)pol;
  ...
  if (lb_call != nullptr) ***REMOVED***
    grpc_call_cancel(lb_call, nullptr);
    /* lb_on_server_status_received will pick up the cancel and clean up */
  ***REMOVED***
  if (glb_policy->retry_timer_active) ***REMOVED***
    grpc_timer_cancel(exec_ctx, &glb_policy->lb_call_retry_timer);
    glb_policy->retry_timer_active = false;
  ***REMOVED***
  if (glb_policy->fallback_timer_active) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (glb_policy->lb_channel != nullptr) ***REMOVED***
...
static void glb_cancel_picks_locked(grpc_exec_ctx* exec_ctx,
  ...
  while (pp != nullptr) ***REMOVED***
    ...
    if ((pp->pick_args.initial_metadata_flags & initial_metadata_flags_mask) ==
        initial_metadata_flags_eq) ***REMOVED***
      GRPC_CLOSURE_SCHED(exec_ctx, &pp->wrapped_on_complete_arg.wrapper_closure,
                         GRPC_ERROR_CREATE_REFERENCING_FROM_STATIC_STRING(
                             "Pick Cancelled", &error, 1));
    ***REMOVED*** else ***REMOVED***
...
static void start_picking_locked(grpc_exec_ctx* exec_ctx,
  ...
  if (glb_policy->lb_fallback_timeout_ms > 0 &&
      glb_policy->serverlist == nullptr && !glb_policy->fallback_timer_active) ***REMOVED***
...
static void glb_exit_idle_locked(grpc_exec_ctx* exec_ctx, grpc_lb_policy* pol) ***REMOVED***
  glb_lb_policy* glb_policy = (glb_lb_policy*)pol;
  if (!glb_policy->started_picking) ***REMOVED***
...
static int glb_pick_locked(grpc_exec_ctx* exec_ctx, grpc_lb_policy* pol,
                           ...
                           grpc_closure* on_complete) ***REMOVED***
  if (pick_args->lb_token_mdelem_storage == nullptr) ***REMOVED***
    *target = nullptr;
  ...
  ***REMOVED***
  ...
  if (glb_policy->rr_policy != nullptr) ***REMOVED***
    ...
    // that's in shutdown.
    if (rr_connectivity_state == GRPC_CHANNEL_SHUTDOWN) ***REMOVED***
      if (grpc_lb_glb_trace.enabled()) ***REMOVED***
        gpr_log(GPR_INFO,
      ...
      ***REMOVED***
      add_pending_pick(&glb_policy->pending_picks, pick_args, target, context,
                       on_complete);
      pick_done = false;
    ***REMOVED*** else ***REMOVED***  // RR not in shutdown
      if (grpc_lb_glb_trace.enabled()) ***REMOVED***
    ...
    ***REMOVED***
  ***REMOVED*** else ***REMOVED***  // glb_policy->rr_policy == NULL
    if (grpc_lb_glb_trace.enabled()) ***REMOVED***
      gpr_log(GPR_DEBUG,
...
static void glb_ping_one_locked(grpc_exec_ctx* exec_ctx, grpc_lb_policy* pol,
  ...
  glb_lb_policy* glb_policy = (glb_lb_policy*)pol;
  if (glb_policy->rr_policy) ***REMOVED***
...
static void maybe_restart_lb_call(grpc_exec_ctx* exec_ctx,
                                  glb_lb_policy* glb_policy) ***REMOVED***
  if (glb_policy->started_picking && glb_policy->updating_lb_call) ***REMOVED***
    if (glb_policy->retry_timer_active) ***REMOVED***
      grpc_timer_cancel(exec_ctx, &glb_policy->lb_call_retry_timer);
    ***REMOVED***
    if (!glb_policy->shutting_down) start_picking_locked(exec_ctx, glb_policy);
    glb_policy->updating_lb_call = false;
  ***REMOVED*** else if (!glb_policy->shutting_down) ***REMOVED***
...
static void client_load_report_done_locked(grpc_exec_ctx* exec_ctx, void* arg,
  ...
  if (error != GRPC_ERROR_NONE || glb_policy->lb_call == nullptr) ***REMOVED***
    glb_policy->client_load_report_timer_pending = false;
    GRPC_LB_POLICY_WEAK_UNREF(exec_ctx, &glb_policy->base,
                              "client_load_report");
    if (glb_policy->lb_call == nullptr) ***REMOVED***
...
static bool load_report_counters_are_zero(grpc_grpclb_request* request) ***REMOVED***
...
static void send_client_load_report_locked(grpc_exec_ctx* exec_ctx, void* arg,
                                           grpc_error* error) ***REMOVED***
  glb_lb_policy* glb_policy = (glb_lb_policy*)arg;
  if (error == GRPC_ERROR_CANCELLED || glb_policy->lb_call == nullptr) ***REMOVED***
    glb_policy->client_load_report_timer_pending = false;
    GRPC_LB_POLICY_WEAK_UNREF(exec_ctx, &glb_policy->base,
                              "client_load_report");
    if (glb_policy->lb_call == nullptr) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (load_report_counters_are_zero(request)) ***REMOVED***
    if (glb_policy->last_client_load_report_counters_were_zero) ***REMOVED***
  ...
  ***REMOVED***
  ...
  grpc_call_error call_error = grpc_call_start_batch_and_execute(
      ...
      &glb_policy->client_load_report_closure);
  if (call_error != GRPC_CALL_OK) ***REMOVED***
...
static void lb_call_destroy_locked(grpc_exec_ctx* exec_ctx,
  ...
  if (glb_policy->client_load_report_timer_pending) ***REMOVED***
...
static void lb_on_response_received_locked(grpc_exec_ctx* exec_ctx, void* arg,
  ...
  memset(ops, 0, sizeof(ops));
  grpc_op* op = ops;
  if (glb_policy->lb_response_payload != nullptr) ***REMOVED***
    grpc_backoff_reset(&glb_policy->lb_call_backoff_state);
    ...
    if (!glb_policy->seen_initial_response &&
        (response = grpc_grpclb_initial_response_parse(response_slice)) !=
            nullptr) ***REMOVED***
      if (response->has_client_stats_report_interval) ***REMOVED***
        ...
        ***REMOVED***
        ...
        schedule_next_client_load_report(exec_ctx, glb_policy);
      ***REMOVED*** else if (grpc_lb_glb_trace.enabled()) ***REMOVED***
        gpr_log(GPR_INFO,
    ...
    ***REMOVED*** else ***REMOVED***
      ...
      if (serverlist != nullptr) ***REMOVED***
        ...
        ***REMOVED***
        /* update serverlist */
        if (serverlist->num_servers > 0) ***REMOVED***
          ...
          ***REMOVED*** else ***REMOVED*** /* new serverlist */
            if (glb_policy->serverlist != nullptr) ***REMOVED***
        ...
        ***REMOVED*** else ***REMOVED***
          ...
          ***REMOVED***
          grpc_grpclb_destroy_serverlist(serverlist);
        ***REMOVED***
      ***REMOVED*** else ***REMOVED*** /* serverlist == NULL */
        gpr_log(GPR_ERROR,
    ...
    ***REMOVED***
  ***REMOVED*** else ***REMOVED*** /* empty payload: call cancelled. */
...
static void lb_on_fallback_timer_locked(grpc_exec_ctx* exec_ctx, void* arg,
  ...
  if (glb_policy->serverlist == nullptr) ***REMOVED***
    if (!glb_policy->shutting_down && error == GRPC_ERROR_NONE) ***REMOVED***
      if (grpc_lb_glb_trace.enabled()) ***REMOVED***
        gpr_log(GPR_INFO,
...
static void fallback_update_locked(grpc_exec_ctx* exec_ctx,
                                   glb_lb_policy* glb_policy,
                                   const grpc_lb_addresses* addresses) ***REMOVED***
  GPR_ASSERT(glb_policy->fallback_backend_addresses != nullptr);
  grpc_lb_addresses_destroy(exec_ctx, glb_policy->fallback_backend_addresses);
  glb_policy->fallback_backend_addresses =
      extract_backend_addresses_locked(exec_ctx, addresses);
  if (glb_policy->started_picking && glb_policy->lb_fallback_timeout_ms > 0 &&
      !glb_policy->fallback_timer_active) ***REMOVED***
    rr_handover_locked(exec_ctx, glb_policy);
  ***REMOVED***
***REMOVED***

static void glb_update_locked(grpc_exec_ctx* exec_ctx, grpc_lb_policy* policy,
                              const grpc_lb_policy_args* args) ***REMOVED***
  glb_lb_policy* glb_policy = (glb_lb_policy*)policy;
  const grpc_arg* arg =
      grpc_channel_args_find(args->args, GRPC_ARG_LB_ADDRESSES);
  if (arg == nullptr || arg->type != GRPC_ARG_POINTER) ***REMOVED***
    if (glb_policy->lb_channel == nullptr) ***REMOVED***
  ...
  ***REMOVED***
  ...
  // already doing so.
  if (!glb_policy->watching_lb_channel) ***REMOVED***
...
static void glb_lb_channel_on_connectivity_changed_cb(grpc_exec_ctx* exec_ctx,
  ...
  switch (glb_policy->lb_channel_connectivity) ***REMOVED***
    case GRPC_CHANNEL_CONNECTING:
    case GRPC_CHANNEL_TRANSIENT_FAILURE: ***REMOVED***
      /* resub. */
    ...
    ***REMOVED***
    ...
    case GRPC_CHANNEL_READY:
      if (glb_policy->lb_call != nullptr) ***REMOVED***
        ...
        // lb_call.
      ***REMOVED*** else if (glb_policy->started_picking && !glb_policy->shutting_down) ***REMOVED***
        if (glb_policy->retry_timer_active) ***REMOVED***
          grpc_timer_cancel(exec_ctx, &glb_policy->lb_call_retry_timer);
          glb_policy->retry_timer_active = false;
        ***REMOVED***
...
static const grpc_lb_policy_vtable glb_lb_policy_vtable = ***REMOVED***
    glb_destroy,
    glb_shutdown_locked,
    glb_pick_locked,
...
static grpc_lb_policy* glb_create(grpc_exec_ctx* exec_ctx,
  ...
  ***REMOVED***
  ...
  glb_policy->lb_call_timeout_ms =
      grpc_channel_arg_get_integer(arg, ***REMOVED***0, 0, INT_MAX***REMOVED***);

  arg = grpc_channel_args_find(args->args, GRPC_ARG_GRPCLB_FALLBACK_TIMEOUT_MS);
  glb_policy->lb_fallback_timeout_ms = grpc_channel_arg_get_integer(
      arg, ***REMOVED***GRPC_GRPCLB_DEFAULT_FALLBACK_TIMEOUT_MS, 0, INT_MAX***REMOVED***);

  // Make sure that GRPC_ARG_LB_POLICY_NAME is set in channel args,
  // since we use this to trigger the client_load_reporting filter.
  grpc_arg new_arg = grpc_channel_arg_string_create(
      (char*)GRPC_ARG_LB_POLICY_NAME, (char*)"grpclb");
  static const char* args_to_remove[] = ***REMOVED***GRPC_ARG_LB_POLICY_NAME***REMOVED***;
...
static void glb_factory_ref(grpc_lb_policy_factory* factory) ***REMOVED******REMOVED***

static void glb_factory_unref(grpc_lb_policy_factory* factory) ***REMOVED******REMOVED***

static const grpc_lb_policy_factory_vtable glb_factory_vtable = ***REMOVED***
    glb_factory_ref, glb_factory_unref, glb_create, "grpclb"***REMOVED***;

static grpc_lb_policy_factory glb_lb_policy_factory = ***REMOVED***&glb_factory_vtable***REMOVED***;

grpc_lb_policy_factory* grpc_glb_lb_factory_create() ***REMOVED***
...
static bool maybe_add_client_load_reporting_filter(
    grpc_exec_ctx* exec_ctx, grpc_channel_stack_builder* builder, void* arg) ***REMOVED***
  const grpc_channel_args* args =
      grpc_channel_stack_builder_get_channel_arguments(builder);
  const grpc_arg* channel_arg =
      grpc_channel_args_find(args, GRPC_ARG_LB_POLICY_NAME);
  if (channel_arg != nullptr && channel_arg->type == GRPC_ARG_STRING &&
      strcmp(channel_arg->value.string, "grpclb") == 0) ***REMOVED***
...
extern "C" void grpc_lb_policy_grpclb_init() ***REMOVED***
...
extern "C" void grpc_lb_policy_grpclb_shutdown() ***REMOVED******REMOVED***
