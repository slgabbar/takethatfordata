...
typedef struct ***REMOVED***
  ...
  size_t string_len;
  size_t allocated;
***REMOVED*** json_writer_userdata;

/* This function checks if there's enough space left in the output buffer,
 * and will enlarge it if necessary. We're only allocating chunks of 256
 * bytes at a time (or multiples thereof).
 */
static void json_writer_output_check(void* userdata, size_t needed) ***REMOVED***
...
static void json_writer_output_char(void* userdata, char c) ***REMOVED***
  json_writer_userdata* state = (json_writer_userdata*)userdata;
...
static void json_writer_output_string(void* userdata, const char* str) ***REMOVED***
 ...
 */
static void json_reader_string_clear(void* userdata) ***REMOVED***
  json_reader_userdata* state = (json_reader_userdata*)userdata;
  if (state->string) ***REMOVED***
...
static void json_reader_string_add_char(void* userdata, uint32_t c) ***REMOVED***
  json_reader_userdata* state = (json_reader_userdata*)userdata;
  GPR_ASSERT(state->string_ptr < state->input);
  GPR_ASSERT(c <= 0xff);
 ...
 */
static void json_reader_string_add_utf32(void* userdata, uint32_t c) ***REMOVED***
  if (c <= 0x7f) ***REMOVED***
    json_reader_string_add_char(userdata, c);
  ***REMOVED*** else if (c <= 0x7ff) ***REMOVED***
  ...
  ***REMOVED*** else if (c <= 0xffff) ***REMOVED***
  ...
  ***REMOVED*** else if (c <= 0x1fffff) ***REMOVED***
 ...
 */
static uint32_t json_reader_read_char(void* userdata) ***REMOVED***
  uint32_t r;
  json_reader_userdata* state = (json_reader_userdata*)userdata;
 ...
 */
static grpc_json* json_create_and_link(void* userdata, grpc_json_type type) ***REMOVED***
  json_reader_userdata* state = (json_reader_userdata*)userdata;
  ...
  if (json->prev) ***REMOVED***
    json->prev->next = json;
  ***REMOVED***
  if (json->parent) ***REMOVED***
    if (!json->parent->child) ***REMOVED***
      json->parent->child = json;
    ***REMOVED***
    if (json->parent->type == GRPC_JSON_OBJECT) ***REMOVED***
      json->key = (char*)state->key;
    ***REMOVED***
  ***REMOVED***
  if (!state->top) ***REMOVED***
    state->top = json;
  ***REMOVED***
...
static void json_reader_container_begins(void* userdata, grpc_json_type type) ***REMOVED***
  json_reader_userdata* state = (json_reader_userdata*)userdata;
  grpc_json* container;
 ...
 */
static grpc_json_type json_reader_container_ends(void* userdata) ***REMOVED***
  ...
  if (state->current_container) ***REMOVED***
 ...
 */
static void json_reader_set_key(void* userdata) ***REMOVED***
  json_reader_userdata* state = (json_reader_userdata*)userdata;
  state->key = state->string;
***REMOVED***

static void json_reader_set_string(void* userdata) ***REMOVED***
  json_reader_userdata* state = (json_reader_userdata*)userdata;
  grpc_json* json = json_create_and_link(userdata, GRPC_JSON_STRING);
  json->value = (char*)state->string;
***REMOVED***

static int json_reader_set_number(void* userdata) ***REMOVED***
  json_reader_userdata* state = (json_reader_userdata*)userdata;
 ...
 */
static void json_reader_set_true(void* userdata) ***REMOVED***
  json_create_and_link(userdata, GRPC_JSON_TRUE);
***REMOVED***

static void json_reader_set_false(void* userdata) ***REMOVED***
  json_create_and_link(userdata, GRPC_JSON_FALSE);
***REMOVED***

static void json_reader_set_null(void* userdata) ***REMOVED***
  json_create_and_link(userdata, GRPC_JSON_NULL);
***REMOVED***

static grpc_json_reader_vtable reader_vtable = ***REMOVED***
    ...
    json_reader_set_false,        json_reader_set_null***REMOVED***;

/* And finally, let's define our public API. */
grpc_json* grpc_json_parse_string_with_len(char* input, size_t size) ***REMOVED***
  ...
  if ((status != GRPC_JSON_DONE) && json) ***REMOVED***
...
static grpc_json_writer_vtable writer_vtable = ***REMOVED***
    json_writer_output_char, json_writer_output_string,
    json_writer_output_string_with_len***REMOVED***;

char* grpc_json_dump_to_string(grpc_json* json, int indent) ***REMOVED***
