...
typedef struct grpc_stream_compression_context_gzip ***REMOVED***
  grpc_stream_compression_context base;

  z_stream zs;
  int (*flate)(z_stream* zs, int flush);
***REMOVED*** grpc_stream_compression_context_gzip;
...
static bool gzip_flate(grpc_stream_compression_context_gzip* ctx,
  ...
  while (max_output_size > 0 && (in->length > 0 || flush) && !eoc) ***REMOVED***
    ...
    while (ctx->zs.avail_out > 0 && in->length > 0 && !eoc) ***REMOVED***
      ...
      if (r < 0 && r != Z_BUF_ERROR) ***REMOVED***
        gpr_log(GPR_ERROR, "zlib error (%d)", r);
        grpc_slice_unref_internal(&exec_ctx, slice_out);
        grpc_exec_ctx_finish(&exec_ctx);
        return false;
      ***REMOVED*** else if (r == Z_STREAM_END && ctx->flate == inflate) ***REMOVED***
        eoc = true;
      ***REMOVED***
      if (ctx->zs.avail_in > 0) ***REMOVED***
      ...
      if (flush == Z_SYNC_FLUSH) ***REMOVED***
        switch (r) ***REMOVED***
          ...
          default:
            ...
            return false;
        ***REMOVED***
      ***REMOVED*** else if (flush == Z_FINISH) ***REMOVED***
        switch (r) ***REMOVED***
          case Z_OK:
          case Z_BUF_ERROR:
    ...
    ***REMOVED*** else if (ctx->zs.avail_out < slice_size) ***REMOVED***
...
static bool grpc_stream_compress_gzip(grpc_stream_compression_context* ctx,
                                      ...
                                      grpc_stream_compression_flush flush) ***REMOVED***
...
grpc_stream_compression_context_create_gzip(
    grpc_stream_compression_method method) ***REMOVED***
  ...
  int r;
  if (gzip_ctx == nullptr) ***REMOVED***
    return nullptr;
  ***REMOVED***
  if (method == GRPC_STREAM_COMPRESSION_GZIP_DECOMPRESS) ***REMOVED***
...
static void grpc_stream_compression_context_destroy_gzip(
    grpc_stream_compression_context* ctx) ***REMOVED***
  if (ctx == nullptr) ***REMOVED***
    return;
  ***REMOVED***
  grpc_stream_compression_context_gzip* gzip_ctx =
      (grpc_stream_compression_context_gzip*)ctx;
  if (gzip_ctx->flate == inflate) ***REMOVED***
...
const grpc_stream_compression_vtable grpc_stream_compression_gzip_vtable = ***REMOVED***
    grpc_stream_compress_gzip, grpc_stream_decompress_gzip,
    grpc_stream_compression_context_create_gzip,
    grpc_stream_compression_context_destroy_gzip***REMOVED***;
