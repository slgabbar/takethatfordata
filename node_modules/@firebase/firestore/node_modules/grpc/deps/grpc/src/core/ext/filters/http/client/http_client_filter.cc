...
typedef struct call_data ***REMOVED***
...
typedef struct channel_data ***REMOVED***
...
static grpc_error* client_filter_incoming_metadata(grpc_exec_ctx* exec_ctx,
  ...
  if (b->idx.named.status != nullptr) ***REMOVED***
    if (grpc_mdelem_eq(b->idx.named.status->md, GRPC_MDELEM_STATUS_200)) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (b->idx.named.grpc_message != nullptr) ***REMOVED***
    ...
    if (grpc_slice_is_equivalent(pct_decoded_msg,
                                 GRPC_MDVALUE(b->idx.named.grpc_message->md))) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (b->idx.named.content_type != nullptr) ***REMOVED***
    if (!grpc_mdelem_eq(b->idx.named.content_type->md,
                        GRPC_MDELEM_CONTENT_TYPE_APPLICATION_SLASH_GRPC)) ***REMOVED***
...
static void recv_initial_metadata_ready(grpc_exec_ctx* exec_ctx,
                                        void* user_data, grpc_error* error) ***REMOVED***
...
static grpc_error* read_all_available_send_message_data(grpc_exec_ctx* exec_ctx,
  ...
  while (grpc_byte_stream_next(exec_ctx,
                               ...
                               ~(size_t)0, &calld->on_send_message_next_done)) ***REMOVED***
    grpc_error* error = pull_slice_from_send_message(exec_ctx, calld);
    if (error != GRPC_ERROR_NONE) return error;
    if (calld->send_message_bytes_read ==
        calld->send_message_caching_stream.base.length) ***REMOVED***
      break;
    ***REMOVED***
  ***REMOVED***
  return GRPC_ERROR_NONE;
...
static char* slice_buffer_to_string(grpc_slice_buffer* slice_buffer) ***REMOVED***
  char* payload_bytes = (char*)gpr_malloc(slice_buffer->length + 1);
  size_t offset = 0;
  for (size_t i = 0; i < slice_buffer->count; ++i) ***REMOVED***
...
static void remove_if_present(grpc_exec_ctx* exec_ctx,
                              ...
                              grpc_metadata_batch_callouts_index idx) ***REMOVED***
  if (batch->idx.array[idx] != nullptr) ***REMOVED***
...
static void hc_start_transport_stream_op_batch(
  ...
  if (batch->recv_initial_metadata) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (batch->send_initial_metadata) ***REMOVED***
    ...
    if (batch->send_message &&
        ...
        batch->payload->send_message.send_message->length <
            channeld->max_payload_size_for_get) ***REMOVED***
      ...
      ***REMOVED***
    ***REMOVED*** else if (batch->payload->send_initial_metadata
  ...
  ***REMOVED***
  ...
  if (error != GRPC_ERROR_NONE) ***REMOVED***
    grpc_transport_stream_op_batch_finish_with_failure(
        exec_ctx, calld->send_message_batch, error, calld->call_combiner);
  ***REMOVED*** else if (!batch_will_be_handled_asynchronously) ***REMOVED***
...
static void destroy_call_elem(grpc_exec_ctx* exec_ctx, grpc_call_element* elem,
                              ...
                              grpc_closure* ignored) ***REMOVED******REMOVED***

static grpc_mdelem scheme_from_args(const grpc_channel_args* args) ***REMOVED***
  unsigned i;
  size_t j;
  grpc_mdelem valid_schemes[] = ***REMOVED***GRPC_MDELEM_SCHEME_HTTP,
                                 GRPC_MDELEM_SCHEME_HTTPS***REMOVED***;
  if (args != nullptr) ***REMOVED***
    for (i = 0; i < args->num_args; ++i) ***REMOVED***
      if (args->args[i].type == GRPC_ARG_STRING &&
          strcmp(args->args[i].key, GRPC_ARG_HTTP2_SCHEME) == 0) ***REMOVED***
        for (j = 0; j < GPR_ARRAY_SIZE(valid_schemes); j++) ***REMOVED***
          if (0 == grpc_slice_str_cmp(GRPC_MDVALUE(valid_schemes[j]),
                                      args->args[i].value.string)) ***REMOVED***
...
static size_t max_payload_size_from_args(const grpc_channel_args* args) ***REMOVED***
  if (args != nullptr) ***REMOVED***
    for (size_t i = 0; i < args->num_args; ++i) ***REMOVED***
      if (0 == strcmp(args->args[i].key, GRPC_ARG_MAX_PAYLOAD_SIZE_FOR_GET)) ***REMOVED***
...
static grpc_slice user_agent_from_args(const grpc_channel_args* args,
                                       const char* transport_name) ***REMOVED***
  gpr_strvec v;
  size_t i;
  int is_first = 1;
  char* tmp;
  ...
  for (i = 0; args && i < args->num_args; i++) ***REMOVED***
    if (0 == strcmp(args->args[i].key, GRPC_ARG_PRIMARY_USER_AGENT_STRING)) ***REMOVED***
  ...
  ***REMOVED***
  ...
  for (i = 0; args && i < args->num_args; i++) ***REMOVED***
    if (0 == strcmp(args->args[i].key, GRPC_ARG_SECONDARY_USER_AGENT_STRING)) ***REMOVED***
      if (args->args[i].type != GRPC_ARG_STRING) ***REMOVED***
        gpr_log(GPR_ERROR, "Channel argument '%s' should be a string",
...
const grpc_channel_filter grpc_http_client_filter = ***REMOVED***
