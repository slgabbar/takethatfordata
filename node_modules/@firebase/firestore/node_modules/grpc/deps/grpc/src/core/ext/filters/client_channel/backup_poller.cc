...
static void backup_poller_shutdown_unref(grpc_exec_ctx* exec_ctx,
                                         backup_poller* p) ***REMOVED***
  if (gpr_unref(&p->shutdown_refs)) ***REMOVED***
...
static void done_poller(grpc_exec_ctx* exec_ctx, void* arg, grpc_error* error) ***REMOVED***
  backup_poller_shutdown_unref(exec_ctx, (backup_poller*)arg);
***REMOVED***

static void g_poller_unref(grpc_exec_ctx* exec_ctx) ***REMOVED***
  if (gpr_unref(&g_poller->refs)) ***REMOVED***
...
static void run_poller(grpc_exec_ctx* exec_ctx, void* arg, grpc_error* error) ***REMOVED***
...
void grpc_client_channel_start_backup_polling(
    grpc_exec_ctx* exec_ctx, grpc_pollset_set* interested_parties) ***REMOVED***
  gpr_once_init(&g_once, init_globals);
  if (g_poll_interval_ms == 0) ***REMOVED***
    return;
  ***REMOVED***
  gpr_mu_lock(&g_poller_mu);
  if (g_poller == nullptr) ***REMOVED***
