...
***REMOVED*** chttp2_connector;

static void chttp2_connector_ref(grpc_connector* con) ***REMOVED***
  chttp2_connector* c = (chttp2_connector*)con;
  gpr_ref(&c->refs);
***REMOVED***

static void chttp2_connector_unref(grpc_exec_ctx* exec_ctx,
                                   grpc_connector* con) ***REMOVED***
  chttp2_connector* c = (chttp2_connector*)con;
  if (gpr_unref(&c->refs)) ***REMOVED***
    gpr_mu_destroy(&c->mu);
...
static void chttp2_connector_shutdown(grpc_exec_ctx* exec_ctx,
                                      grpc_connector* con, grpc_error* why) ***REMOVED***
  chttp2_connector* c = (chttp2_connector*)con;
  gpr_mu_lock(&c->mu);
  c->shutdown = true;
  if (c->handshake_mgr != nullptr) ***REMOVED***
    grpc_handshake_manager_shutdown(exec_ctx, c->handshake_mgr,
                                    GRPC_ERROR_REF(why));
  ***REMOVED***
  // If handshaking is not yet in progress, shutdown the endpoint.
  // Otherwise, the handshaker will do this for us.
  if (!c->connecting && c->endpoint != nullptr) ***REMOVED***
...
static void on_handshake_done(grpc_exec_ctx* exec_ctx, void* arg,
  ...
  gpr_mu_lock(&c->mu);
  if (error != GRPC_ERROR_NONE || c->shutdown) ***REMOVED***
...
static void connected(grpc_exec_ctx* exec_ctx, void* arg, grpc_error* error) ***REMOVED***
...
static const grpc_connector_vtable chttp2_connector_vtable = ***REMOVED***
    chttp2_connector_ref, chttp2_connector_unref, chttp2_connector_shutdown,
    chttp2_connector_connect***REMOVED***;

grpc_connector* grpc_chttp2_connector_create() ***REMOVED***
