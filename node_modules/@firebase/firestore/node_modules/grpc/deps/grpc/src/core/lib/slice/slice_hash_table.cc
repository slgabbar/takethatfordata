...
struct grpc_slice_hash_table ***REMOVED***
  gpr_refcount refs;
...
grpc_slice_hash_table* grpc_slice_hash_table_create(
    ...
    void (*destroy_value)(grpc_exec_ctx* exec_ctx, void* value),
    int (*value_cmp)(void* a, void* b)) ***REMOVED***
  ...
  for (size_t i = 0; i < num_entries; ++i) ***REMOVED***
...
grpc_slice_hash_table* grpc_slice_hash_table_ref(grpc_slice_hash_table* table) ***REMOVED***
  if (table != nullptr) gpr_ref(&table->refs);
  return table;
***REMOVED***

void grpc_slice_hash_table_unref(grpc_exec_ctx* exec_ctx,
                                 grpc_slice_hash_table* table) ***REMOVED***
  if (table != nullptr && gpr_unref(&table->refs)) ***REMOVED***
    for (size_t i = 0; i < table->size; ++i) ***REMOVED***
      grpc_slice_hash_table_entry* entry = &table->entries[i];
      if (!is_empty(entry)) ***REMOVED***
...
void* grpc_slice_hash_table_get(const grpc_slice_hash_table* table,
                                const grpc_slice key) ***REMOVED***
  const size_t hash = grpc_slice_hash(key);
  // We cap the number of probes at the max number recorded when
  // populating the table.
  for (size_t offset = 0; offset <= table->max_num_probes; ++offset) ***REMOVED***
    const size_t idx = (hash + offset) % table->size;
    if (is_empty(&table->entries[idx])) break;
    if (grpc_slice_eq(table->entries[idx].key, key)) ***REMOVED***
...
static int pointer_cmp(void* a, void* b) ***REMOVED*** return GPR_ICMP(a, b); ***REMOVED***
int grpc_slice_hash_table_cmp(const grpc_slice_hash_table* a,
                              const grpc_slice_hash_table* b) ***REMOVED***
  ...
  if (a->size > b->size) return 1;
  // Compare rows.
  for (size_t i = 0; i < a->size; ++i) ***REMOVED***
    if (is_empty(&a->entries[i])) ***REMOVED***
      if (!is_empty(&b->entries[i])) ***REMOVED***
        return -1;  // a empty but b non-empty
      ***REMOVED***
      continue;  // both empty, no need to check key or value
    ***REMOVED*** else if (is_empty(&b->entries[i])) ***REMOVED***
