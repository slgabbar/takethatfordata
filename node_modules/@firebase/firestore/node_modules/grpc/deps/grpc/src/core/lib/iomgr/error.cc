...
static const char* error_int_name(grpc_error_ints key) ***REMOVED***
  switch (key) ***REMOVED***
...
static const char* error_str_name(grpc_error_strs key) ***REMOVED***
  switch (key) ***REMOVED***
...
static const char* error_time_name(grpc_error_times key) ***REMOVED***
  switch (key) ***REMOVED***
...
bool grpc_error_is_special(grpc_error* err) ***REMOVED***
  return err == GRPC_ERROR_NONE || err == GRPC_ERROR_OOM ||
         err == GRPC_ERROR_CANCELLED;
***REMOVED***

#ifndef NDEBUG
grpc_error* grpc_error_ref(grpc_error* err, const char* file, int line) ***REMOVED***
  if (grpc_error_is_special(err)) return err;
  if (grpc_trace_error_refcount.enabled()) ***REMOVED***
...
***REMOVED***
#else
grpc_error* grpc_error_ref(grpc_error* err) ***REMOVED***
  if (grpc_error_is_special(err)) return err;
  gpr_ref(&err->atomics.refs);
  return err;
***REMOVED***
...
static void unref_errs(grpc_error* err) ***REMOVED***
  uint8_t slot = err->first_err;
  while (slot != UINT8_MAX) ***REMOVED***
...
static void unref_slice(grpc_slice slice) ***REMOVED***
  grpc_exec_ctx exec_ctx = GRPC_EXEC_CTX_INIT;
  grpc_slice_unref_internal(&exec_ctx, slice);
  grpc_exec_ctx_finish(&exec_ctx);
***REMOVED***

static void unref_strs(grpc_error* err) ***REMOVED***
  for (size_t which = 0; which < GRPC_ERROR_STR_MAX; ++which) ***REMOVED***
    uint8_t slot = err->strs[which];
    if (slot != UINT8_MAX) ***REMOVED***
      unref_slice(*(grpc_slice*)(err->arena + slot));
    ***REMOVED***
  ***REMOVED***
***REMOVED***

static void error_destroy(grpc_error* err) ***REMOVED***
...
#ifndef NDEBUG
void grpc_error_unref(grpc_error* err, const char* file, int line) ***REMOVED***
  ...
  if (grpc_trace_error_refcount.enabled()) ***REMOVED***
    gpr_log(GPR_DEBUG, "%p: %" PRIdPTR " -> %" PRIdPTR " [%s:%d]", err,
            ...
            gpr_atm_no_barrier_load(&err->atomics.refs.count) - 1, file, line);
  ***REMOVED***
  if (gpr_unref(&err->atomics.refs)) ***REMOVED***
    error_destroy(err);
  ***REMOVED***
***REMOVED***
#else
void grpc_error_unref(grpc_error* err) ***REMOVED***
  if (grpc_error_is_special(err)) return;
  if (gpr_unref(&err->atomics.refs)) ***REMOVED***
    error_destroy(err);
  ***REMOVED***
...
static uint8_t get_placement(grpc_error** err, size_t size) ***REMOVED***
  GPR_ASSERT(*err);
  uint8_t slots = (uint8_t)(size / sizeof(intptr_t));
  if ((*err)->arena_size + slots > (*err)->arena_capacity) ***REMOVED***
    (*err)->arena_capacity =
...
static void internal_set_int(grpc_error** err, grpc_error_ints which,
                             intptr_t value) ***REMOVED***
  uint8_t slot = (*err)->ints[which];
  if (slot == UINT8_MAX) ***REMOVED***
    slot = get_placement(err, sizeof(value));
    if (slot == UINT8_MAX) ***REMOVED***
      gpr_log(GPR_ERROR, "Error %p is full, dropping int ***REMOVED***\"%s\":%" PRIiPTR "***REMOVED***",
...
static void internal_set_str(grpc_error** err, grpc_error_strs which,
                             grpc_slice value) ***REMOVED***
  uint8_t slot = (*err)->strs[which];
  if (slot == UINT8_MAX) ***REMOVED***
    slot = get_placement(err, sizeof(value));
    if (slot == UINT8_MAX) ***REMOVED***
      const char* str = grpc_slice_to_c_string(value);
      gpr_log(GPR_ERROR, "Error %p is full, dropping string ***REMOVED***\"%s\":\"%s\"***REMOVED***",
...
static void internal_set_time(grpc_error** err, grpc_error_times which,
                              gpr_timespec value) ***REMOVED***
  uint8_t slot = (*err)->times[which];
  if (slot == UINT8_MAX) ***REMOVED***
    slot = get_placement(err, sizeof(value));
    if (slot == UINT8_MAX) ***REMOVED***
      const char* time_str = fmt_time(value);
      gpr_log(GPR_ERROR, "Error %p is full, dropping \"%s\":\"%s\"***REMOVED***", *err,
...
static void internal_add_error(grpc_error** err, grpc_error* new_err) ***REMOVED***
  grpc_linked_error new_last = ***REMOVED***new_err, UINT8_MAX***REMOVED***;
  ...
  if (slot == UINT8_MAX) ***REMOVED***
    ...
    return;
  ***REMOVED***
  if ((*err)->first_err == UINT8_MAX) ***REMOVED***
...
grpc_error* grpc_error_create(const char* file, int line, grpc_slice desc,
                              ...
                              size_t num_referencing) ***REMOVED***
  ...
  if (err == nullptr) ***REMOVED***  // TODO(ctiller): make gpr_malloc return NULL
  ...
  ***REMOVED***
  ...
  for (size_t i = 0; i < num_referencing; ++i) ***REMOVED***
...
static void ref_strs(grpc_error* err) ***REMOVED***
  for (size_t i = 0; i < GRPC_ERROR_STR_MAX; ++i) ***REMOVED***
...
static void ref_errs(grpc_error* err) ***REMOVED***
  uint8_t slot = err->first_err;
  while (slot != UINT8_MAX) ***REMOVED***
    grpc_linked_error* lerr = (grpc_linked_error*)(err->arena + slot);
...
static grpc_error* copy_error_and_unref(grpc_error* in) ***REMOVED***
  GPR_TIMER_BEGIN("copy_error_and_unref", 0);
  grpc_error* out;
  if (grpc_error_is_special(in)) ***REMOVED***
    out = GRPC_ERROR_CREATE_FROM_STATIC_STRING("unknown");
    if (in == GRPC_ERROR_NONE) ***REMOVED***
    ...
    ***REMOVED*** else if (in == GRPC_ERROR_OOM) ***REMOVED***
      internal_set_str(&out, GRPC_ERROR_STR_DESCRIPTION,
                       grpc_slice_from_static_string("oom"));
    ***REMOVED*** else if (in == GRPC_ERROR_CANCELLED) ***REMOVED***
      internal_set_str(&out, GRPC_ERROR_STR_DESCRIPTION,
                       grpc_slice_from_static_string("cancelled"));
      internal_set_int(&out, GRPC_ERROR_INT_GRPC_STATUS, GRPC_STATUS_CANCELLED);
    ***REMOVED***
  ***REMOVED*** else if (gpr_ref_is_unique(&in->atomics.refs)) ***REMOVED***
    out = in;
  ***REMOVED*** else ***REMOVED***
    ...
    if (in->arena_capacity - in->arena_size < (uint8_t)SLOTS_PER_STR) ***REMOVED***
...
typedef struct ***REMOVED***
  ...
  const char* msg;
***REMOVED*** special_error_status_map;
static special_error_status_map error_status_map[] = ***REMOVED***
    ***REMOVED***GRPC_ERROR_NONE, GRPC_STATUS_OK, ""***REMOVED***,
    ***REMOVED***GRPC_ERROR_CANCELLED, GRPC_STATUS_CANCELLED, "Cancelled"***REMOVED***,
    ***REMOVED***GRPC_ERROR_OOM, GRPC_STATUS_RESOURCE_EXHAUSTED, "Out of memory"***REMOVED***,
***REMOVED***;

bool grpc_error_get_int(grpc_error* err, grpc_error_ints which, intptr_t* p) ***REMOVED***
  GPR_TIMER_BEGIN("grpc_error_get_int", 0);
  if (grpc_error_is_special(err)) ***REMOVED***
    if (which == GRPC_ERROR_INT_GRPC_STATUS) ***REMOVED***
      for (size_t i = 0; i < GPR_ARRAY_SIZE(error_status_map); i++) ***REMOVED***
        if (error_status_map[i].error == err) ***REMOVED***
...
bool grpc_error_get_str(grpc_error* err, grpc_error_strs which,
                        grpc_slice* str) ***REMOVED***
  if (grpc_error_is_special(err)) ***REMOVED***
    if (which == GRPC_ERROR_STR_GRPC_MESSAGE) ***REMOVED***
      for (size_t i = 0; i < GPR_ARRAY_SIZE(error_status_map); i++) ***REMOVED***
        if (error_status_map[i].error == err) ***REMOVED***
...
grpc_error* grpc_error_add_child(grpc_error* src, grpc_error* child) ***REMOVED***
...
static void append_chr(char c, char** s, size_t* sz, size_t* cap) ***REMOVED***
  if (*sz == *cap) ***REMOVED***
...
static void append_str(const char* str, char** s, size_t* sz, size_t* cap) ***REMOVED***
  for (const char* c = str; *c; c++) ***REMOVED***
...
static void append_esc_str(const uint8_t* str, size_t len, char** s, size_t* sz,
  ...
  append_chr('"', s, sz, cap);
  for (size_t i = 0; i < len; i++, str++) ***REMOVED***
    if (*str < 32 || *str >= 127) ***REMOVED***
      append_chr('\\', s, sz, cap);
      switch (*str) ***REMOVED***
        case '\b':
...
static void append_kv(kv_pairs* kvs, char* key, char* value) ***REMOVED***
  if (kvs->num_kvs == kvs->cap_kvs) ***REMOVED***
...
static char* key_int(grpc_error_ints which) ***REMOVED***
  return gpr_strdup(error_int_name(which));
***REMOVED***

static char* fmt_int(intptr_t p) ***REMOVED***
  char* s;
  gpr_asprintf(&s, "%" PRIdPTR, p);
  return s;
***REMOVED***

static void collect_ints_kvs(grpc_error* err, kv_pairs* kvs) ***REMOVED***
  for (size_t which = 0; which < GRPC_ERROR_INT_MAX; ++which) ***REMOVED***
    uint8_t slot = err->ints[which];
    if (slot != UINT8_MAX) ***REMOVED***
...
static char* key_str(grpc_error_strs which) ***REMOVED***
  return gpr_strdup(error_str_name(which));
***REMOVED***

static char* fmt_str(grpc_slice slice) ***REMOVED***
  char* s = nullptr;
  size_t sz = 0;
  size_t cap = 0;
...
static void collect_strs_kvs(grpc_error* err, kv_pairs* kvs) ***REMOVED***
  for (size_t which = 0; which < GRPC_ERROR_STR_MAX; ++which) ***REMOVED***
    uint8_t slot = err->strs[which];
    if (slot != UINT8_MAX) ***REMOVED***
...
static char* key_time(grpc_error_times which) ***REMOVED***
  return gpr_strdup(error_time_name(which));
***REMOVED***

static char* fmt_time(gpr_timespec tm) ***REMOVED***
  char* out;
  const char* pfx = "!!";
  switch (tm.clock_type) ***REMOVED***
...
static void collect_times_kvs(grpc_error* err, kv_pairs* kvs) ***REMOVED***
  for (size_t which = 0; which < GRPC_ERROR_TIME_MAX; ++which) ***REMOVED***
    uint8_t slot = err->times[which];
    if (slot != UINT8_MAX) ***REMOVED***
...
static void add_errs(grpc_error* err, char** s, size_t* sz, size_t* cap) ***REMOVED***
  uint8_t slot = err->first_err;
  bool first = true;
  while (slot != UINT8_MAX) ***REMOVED***
    grpc_linked_error* lerr = (grpc_linked_error*)(err->arena + slot);
...
static char* errs_string(grpc_error* err) ***REMOVED***
  char* s = nullptr;
  size_t sz = 0;
  size_t cap = 0;
  append_chr('[', &s, &sz, &cap);
...
static int cmp_kvs(const void* a, const void* b) ***REMOVED***
  const kv_pair* ka = (const kv_pair*)a;
  const kv_pair* kb = (const kv_pair*)b;
  return strcmp(ka->key, kb->key);
***REMOVED***

static char* finish_kvs(kv_pairs* kvs) ***REMOVED***
  char* s = nullptr;
  size_t sz = 0;
  size_t cap = 0;

  append_chr('***REMOVED***', &s, &sz, &cap);
  for (size_t i = 0; i < kvs->num_kvs; i++) ***REMOVED***
...
const char* grpc_error_string(grpc_error* err) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (!gpr_atm_rel_cas(&err->atomics.error_string, 0, (gpr_atm)out)) ***REMOVED***
    gpr_free(out);
