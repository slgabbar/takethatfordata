...
static void plaintext_handshake(grpc_exec_ctx* exec_ctx, void* arg,
                                ...
                                void (*on_done)(grpc_exec_ctx* exec_ctx,
                                                void* arg,
                                                grpc_endpoint* endpoint)) ***REMOVED***
  on_done(exec_ctx, arg, endpoint);
***REMOVED***

const grpc_httpcli_handshaker grpc_httpcli_plaintext = ***REMOVED***"http",
                                                        plaintext_handshake***REMOVED***;

void grpc_httpcli_context_init(grpc_httpcli_context* context) ***REMOVED***
  context->pollset_set = grpc_pollset_set_create();
***REMOVED***

void grpc_httpcli_context_destroy(grpc_exec_ctx* exec_ctx,
                                  grpc_httpcli_context* context) ***REMOVED***
...
static void finish(grpc_exec_ctx* exec_ctx, internal_request* req,
  ...
  if (req->addresses != nullptr) ***REMOVED***
    grpc_resolved_addresses_destroy(req->addresses);
  ***REMOVED***
  if (req->ep != nullptr) ***REMOVED***
...
static void append_error(internal_request* req, grpc_error* error) ***REMOVED***
  if (req->overall_error == GRPC_ERROR_NONE) ***REMOVED***
    req->overall_error =
...
static void do_read(grpc_exec_ctx* exec_ctx, internal_request* req) ***REMOVED***
...
static void on_read(grpc_exec_ctx* exec_ctx, void* user_data,
  ...
  for (i = 0; i < req->incoming.count; i++) ***REMOVED***
    if (GRPC_SLICE_LENGTH(req->incoming.slices[i])) ***REMOVED***
  ...
  ***REMOVED*** else if (!req->have_read_byte) ***REMOVED***
...
static void on_written(grpc_exec_ctx* exec_ctx, internal_request* req) ***REMOVED***
  do_read(exec_ctx, req);
***REMOVED***

static void done_write(grpc_exec_ctx* exec_ctx, void* arg, grpc_error* error) ***REMOVED***
...
static void start_write(grpc_exec_ctx* exec_ctx, internal_request* req) ***REMOVED***
...
static void next_address(grpc_exec_ctx* exec_ctx, internal_request* req,
  ...
  ***REMOVED***
  if (req->next_address == req->addresses->naddrs) ***REMOVED***
    finish(exec_ctx, req,
  ...
  ***REMOVED***
  ...
  grpc_channel_args args = ***REMOVED***1, &arg***REMOVED***;
  grpc_tcp_client_connect(exec_ctx, &req->connected, &req->ep,
                          req->context->pollset_set, &args, addr,
                          req->deadline);
***REMOVED***

static void on_resolved(grpc_exec_ctx* exec_ctx, void* arg, grpc_error* error) ***REMOVED***
  internal_request* req = (internal_request*)arg;
  if (error != GRPC_ERROR_NONE) ***REMOVED***
...
static void internal_request_begin(grpc_exec_ctx* exec_ctx,
                                   ...
                                   const char* name, grpc_slice request_text) ***REMOVED***
...
void grpc_httpcli_get(grpc_exec_ctx* exec_ctx, grpc_httpcli_context* context,
                      ...
                      grpc_closure* on_done, grpc_httpcli_response* response) ***REMOVED***
  char* name;
  if (g_get_override &&
      g_get_override(exec_ctx, request, deadline, on_done, response)) ***REMOVED***
    return;
  ***REMOVED***
...
void grpc_httpcli_post(grpc_exec_ctx* exec_ctx, grpc_httpcli_context* context,
                       ...
                       grpc_millis deadline, grpc_closure* on_done,
                       grpc_httpcli_response* response) ***REMOVED***
  char* name;
  if (g_post_override &&
      g_post_override(exec_ctx, request, body_bytes, body_size, deadline,
                      on_done, response)) ***REMOVED***
    return;
  ***REMOVED***
...
void grpc_httpcli_set_override(grpc_httpcli_get_override get,
                               grpc_httpcli_post_override post) ***REMOVED***
  g_get_override = get;
