...
typedef struct external_state_watcher ***REMOVED***
  grpc_subchannel* subchannel;
...
***REMOVED*** external_state_watcher;

struct grpc_subchannel ***REMOVED***
...
struct grpc_subchannel_call ***REMOVED***
...
grpc_connected_subchannel* grpc_connected_subchannel_ref(
    grpc_connected_subchannel* c GRPC_SUBCHANNEL_REF_EXTRA_ARGS) ***REMOVED***
...
void grpc_connected_subchannel_unref(grpc_exec_ctx* exec_ctx,
                                     grpc_connected_subchannel* c
                                         GRPC_SUBCHANNEL_REF_EXTRA_ARGS) ***REMOVED***
...
#ifndef NDEBUG
  if (grpc_trace_stream_refcount.enabled()) ***REMOVED***
...
grpc_subchannel* grpc_subchannel_ref(
    grpc_subchannel* c GRPC_SUBCHANNEL_REF_EXTRA_ARGS) ***REMOVED***
  gpr_atm old_refs;
...
grpc_subchannel* grpc_subchannel_ref_from_weak_ref(
  ...
  for (;;) ***REMOVED***
    ...
    if (old_refs >= (1 << INTERNAL_REF_BITS)) ***REMOVED***
      gpr_atm new_refs = old_refs + (1 << INTERNAL_REF_BITS);
      if (gpr_atm_rel_cas(&c->ref_pair, old_refs, new_refs)) ***REMOVED***
        return c;
      ***REMOVED***
...
static void disconnect(grpc_exec_ctx* exec_ctx, grpc_subchannel* c) ***REMOVED***
...
void grpc_subchannel_unref(grpc_exec_ctx* exec_ctx,
  ...
  if ((old_refs & STRONG_REF_MASK) == (1 << INTERNAL_REF_BITS)) ***REMOVED***
...
grpc_subchannel* grpc_subchannel_create(grpc_exec_ctx* exec_ctx,
  ...
  ***REMOVED***
  ...
  if (grpc_proxy_mappers_map_address(exec_ctx, addr, args->args, &new_address,
    ...
    addr = new_address;
  ***REMOVED***
  static const char* keys_to_remove[] = ***REMOVED***GRPC_ARG_SUBCHANNEL_ADDRESS***REMOVED***;
  ...
  if (c->args) ***REMOVED***
    for (size_t i = 0; i < c->args->num_args; i++) ***REMOVED***
      if (0 == strcmp(c->args->args[i].key,
                      "grpc.testing.fixed_reconnect_backoff_ms")) ***REMOVED***
        fixed_reconnect_backoff = true;
        initial_backoff_ms = min_backoff_ms = max_backoff_ms =
            grpc_channel_arg_get_integer(&c->args->args[i],
                                         ***REMOVED***initial_backoff_ms, 100, INT_MAX***REMOVED***);
      ***REMOVED*** else if (0 == strcmp(c->args->args[i].key,
                             GRPC_ARG_MIN_RECONNECT_BACKOFF_MS)) ***REMOVED***
        fixed_reconnect_backoff = false;
        min_backoff_ms = grpc_channel_arg_get_integer(
            &c->args->args[i], ***REMOVED***min_backoff_ms, 100, INT_MAX***REMOVED***);
      ***REMOVED*** else if (0 == strcmp(c->args->args[i].key,
                             GRPC_ARG_MAX_RECONNECT_BACKOFF_MS)) ***REMOVED***
        fixed_reconnect_backoff = false;
        max_backoff_ms = grpc_channel_arg_get_integer(
            &c->args->args[i], ***REMOVED***max_backoff_ms, 100, INT_MAX***REMOVED***);
      ***REMOVED*** else if (0 == strcmp(c->args->args[i].key,
                             GRPC_ARG_INITIAL_RECONNECT_BACKOFF_MS)) ***REMOVED***
        fixed_reconnect_backoff = false;
        initial_backoff_ms = grpc_channel_arg_get_integer(
            &c->args->args[i], ***REMOVED***initial_backoff_ms, 100, INT_MAX***REMOVED***);
      ***REMOVED***
    ***REMOVED***
  ***REMOVED***
  grpc_backoff_init(
...
static void on_external_state_watcher_done(grpc_exec_ctx* exec_ctx, void* arg,
  ...
  if (w->pollset_set != nullptr) ***REMOVED***
...
static void on_alarm(grpc_exec_ctx* exec_ctx, void* arg, grpc_error* error) ***REMOVED***
  grpc_subchannel* c = (grpc_subchannel*)arg;
  gpr_mu_lock(&c->mu);
  c->have_alarm = false;
  if (c->disconnected) ***REMOVED***
...
static void maybe_start_connecting_locked(grpc_exec_ctx* exec_ctx,
  ...
  ***REMOVED***
  ...
  if (c->connecting) ***REMOVED***
    /* Already connecting: don't restart */
    return;
  ***REMOVED***

  if (GET_CONNECTED_SUBCHANNEL(c, no_barrier) != nullptr) ***REMOVED***
    /* Already connected: don't restart */
    return;
  ***REMOVED***

  if (!grpc_connectivity_state_has_watchers(&c->state_tracker)) ***REMOVED***
...
void grpc_subchannel_notify_on_state_change(
  ...
  if (state == nullptr) ***REMOVED***
    gpr_mu_lock(&c->mu);
    for (w = c->root_external_state_watcher.next;
         w != &c->root_external_state_watcher; w = w->next) ***REMOVED***
      if (w->notify == notify) ***REMOVED***
  ...
  ***REMOVED*** else ***REMOVED***
    ...
    GRPC_CLOSURE_INIT(&w->closure, on_external_state_watcher_done, w,
                      grpc_schedule_on_exec_ctx);
    if (interested_parties != nullptr) ***REMOVED***
...
static void subchannel_on_child_state_changed(grpc_exec_ctx* exec_ctx, void* p,
  ...
  if (sw->connectivity_state == GRPC_CHANNEL_TRANSIENT_FAILURE) ***REMOVED***
  ...
  ***REMOVED***
  grpc_connectivity_state_set(exec_ctx, &c->state_tracker,
                              ...
                              "reflect_child");
  if (sw->connectivity_state != GRPC_CHANNEL_SHUTDOWN) ***REMOVED***
...
static bool publish_transport_locked(grpc_exec_ctx* exec_ctx,
  ...
  if (!grpc_channel_init_create_stack(exec_ctx, builder,
                                      GRPC_CLIENT_SUBCHANNEL)) ***REMOVED***
...
static void subchannel_connected(grpc_exec_ctx* exec_ctx, void* arg,
  ...
  if (c->connecting_result.transport != nullptr &&
      publish_transport_locked(exec_ctx, c)) ***REMOVED***
    /* do nothing, transport was published */
  ***REMOVED*** else if (c->disconnected) ***REMOVED***
    GRPC_SUBCHANNEL_WEAK_UNREF(exec_ctx, c, "connecting");
  ***REMOVED*** else ***REMOVED***
...
void grpc_subchannel_call_ref(
    grpc_subchannel_call* c GRPC_SUBCHANNEL_REF_EXTRA_ARGS) ***REMOVED***
...
const grpc_subchannel_key* grpc_subchannel_get_key(
    const grpc_subchannel* subchannel) ***REMOVED***
...
grpc_call_stack* grpc_subchannel_call_get_call_stack(
    grpc_subchannel_call* subchannel_call) ***REMOVED***
...
void grpc_get_subchannel_address_arg(grpc_exec_ctx* exec_ctx,
  ...
  if (*addr_uri_str != '\0') ***REMOVED***
    grpc_uri_to_sockaddr(exec_ctx, addr_uri_str, addr);
  ***REMOVED***
***REMOVED***

const char* grpc_get_subchannel_address_uri_arg(const grpc_channel_args* args) ***REMOVED***
...
grpc_arg grpc_create_subchannel_address_arg(const grpc_resolved_address* addr) ***REMOVED***
