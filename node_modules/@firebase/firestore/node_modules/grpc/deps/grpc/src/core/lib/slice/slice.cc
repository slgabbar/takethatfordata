...
char* grpc_slice_to_c_string(grpc_slice slice) ***REMOVED***
...
grpc_slice grpc_empty_slice(void) ***REMOVED***
  grpc_slice out;
...
grpc_slice grpc_slice_copy(grpc_slice s) ***REMOVED***
...
grpc_slice grpc_slice_ref_internal(grpc_slice slice) ***REMOVED***
  if (slice.refcount) ***REMOVED***
    slice.refcount->vtable->ref(slice.refcount);
  ***REMOVED***
  return slice;
***REMOVED***

void grpc_slice_unref_internal(grpc_exec_ctx* exec_ctx, grpc_slice slice) ***REMOVED***
  if (slice.refcount) ***REMOVED***
...
/* Public API */
grpc_slice grpc_slice_ref(grpc_slice slice) ***REMOVED***
  return grpc_slice_ref_internal(slice);
***REMOVED***

/* Public API */
void grpc_slice_unref(grpc_slice slice) ***REMOVED***
...
/* grpc_slice_from_static_string support structure - a refcount that does
   nothing */
static void noop_ref(void* unused) ***REMOVED******REMOVED***
static void noop_unref(grpc_exec_ctx* exec_ctx, void* unused) ***REMOVED******REMOVED***

static const grpc_slice_refcount_vtable noop_refcount_vtable = ***REMOVED***
    noop_ref, noop_unref, grpc_slice_default_eq_impl,
    grpc_slice_default_hash_impl***REMOVED***;
static grpc_slice_refcount noop_refcount = ***REMOVED***&noop_refcount_vtable,
                                            &noop_refcount***REMOVED***;

grpc_slice grpc_slice_from_static_buffer(const void* s, size_t len) ***REMOVED***
  grpc_slice slice;
...
grpc_slice grpc_slice_from_static_string(const char* s) ***REMOVED***
...
typedef struct new_slice_refcount ***REMOVED***
  grpc_slice_refcount rc;
  gpr_refcount refs;
  void (*user_destroy)(void*);
  void* user_data;
***REMOVED*** new_slice_refcount;

static void new_slice_ref(void* p) ***REMOVED***
  new_slice_refcount* r = (new_slice_refcount*)p;
  gpr_ref(&r->refs);
***REMOVED***

static void new_slice_unref(grpc_exec_ctx* exec_ctx, void* p) ***REMOVED***
  new_slice_refcount* r = (new_slice_refcount*)p;
  if (gpr_unref(&r->refs)) ***REMOVED***
    r->user_destroy(r->user_data);
    gpr_free(r);
  ***REMOVED***
***REMOVED***

static const grpc_slice_refcount_vtable new_slice_vtable = ***REMOVED***
...
grpc_slice grpc_slice_new(void* p, size_t len, void (*destroy)(void*)) ***REMOVED***
...
typedef struct new_with_len_slice_refcount ***REMOVED***
  grpc_slice_refcount rc;
  gpr_refcount refs;
  void* user_data;
  size_t user_length;
  void (*user_destroy)(void*, size_t);
***REMOVED*** new_with_len_slice_refcount;

static void new_with_len_ref(void* p) ***REMOVED***
  new_with_len_slice_refcount* r = (new_with_len_slice_refcount*)p;
  gpr_ref(&r->refs);
***REMOVED***

static void new_with_len_unref(grpc_exec_ctx* exec_ctx, void* p) ***REMOVED***
  new_with_len_slice_refcount* r = (new_with_len_slice_refcount*)p;
  if (gpr_unref(&r->refs)) ***REMOVED***
...
static const grpc_slice_refcount_vtable new_with_len_vtable = ***REMOVED***
    new_with_len_ref, new_with_len_unref, grpc_slice_default_eq_impl,
    grpc_slice_default_hash_impl***REMOVED***;

grpc_slice grpc_slice_new_with_len(void* p, size_t len,
                                   void (*destroy)(void*, size_t)) ***REMOVED***
  grpc_slice slice;
...
grpc_slice grpc_slice_from_copied_buffer(const char* source, size_t length) ***REMOVED***
...
grpc_slice grpc_slice_from_copied_string(const char* source) ***REMOVED***
...
typedef struct ***REMOVED***
  ...
  gpr_refcount refs;
***REMOVED*** malloc_refcount;

static void malloc_ref(void* p) ***REMOVED***
  malloc_refcount* r = (malloc_refcount*)p;
  gpr_ref(&r->refs);
***REMOVED***

static void malloc_unref(grpc_exec_ctx* exec_ctx, void* p) ***REMOVED***
  malloc_refcount* r = (malloc_refcount*)p;
  if (gpr_unref(&r->refs)) ***REMOVED***
    gpr_free(r);
  ***REMOVED***
...
static const grpc_slice_refcount_vtable malloc_vtable = ***REMOVED***
    malloc_ref, malloc_unref, grpc_slice_default_eq_impl,
    grpc_slice_default_hash_impl***REMOVED***;

grpc_slice grpc_slice_malloc_large(size_t length) ***REMOVED***
  grpc_slice slice;
...
grpc_slice grpc_slice_malloc(size_t length) ***REMOVED***
  grpc_slice slice;

  if (length > sizeof(slice.data.inlined.bytes)) ***REMOVED***
...
grpc_slice grpc_slice_sub_no_ref(grpc_slice source, size_t begin, size_t end) ***REMOVED***
  grpc_slice subset;

  GPR_ASSERT(end >= begin);

  if (source.refcount) ***REMOVED***
...
grpc_slice grpc_slice_sub(grpc_slice source, size_t begin, size_t end) ***REMOVED***
  grpc_slice subset;

  if (end - begin <= sizeof(subset.data.inlined.bytes)) ***REMOVED***
    subset.refcount = nullptr;
    subset.data.inlined.length = (uint8_t)(end - begin);
...
grpc_slice grpc_slice_split_tail_maybe_ref(grpc_slice* source, size_t split,
                                           grpc_slice_ref_whom ref_whom) ***REMOVED***
  grpc_slice tail;

  if (source->refcount == nullptr) ***REMOVED***
  ...
  ***REMOVED*** else ***REMOVED***
    ...
    if (tail_length < sizeof(tail.data.inlined.bytes) &&
        ref_whom != GRPC_SLICE_REF_TAIL) ***REMOVED***
...
grpc_slice grpc_slice_split_tail(grpc_slice* source, size_t split) ***REMOVED***
  return grpc_slice_split_tail_maybe_ref(source, split, GRPC_SLICE_REF_BOTH);
***REMOVED***

grpc_slice grpc_slice_split_head(grpc_slice* source, size_t split) ***REMOVED***
  grpc_slice head;
  ...
  if (source->refcount == nullptr) ***REMOVED***
    ...
    memmove(source->data.inlined.bytes, source->data.inlined.bytes + split,
            source->data.inlined.length);
  ***REMOVED*** else if (split < sizeof(head.data.inlined.bytes)) ***REMOVED***
    GPR_ASSERT(source->data.refcounted.length >= split);
...
int grpc_slice_default_eq_impl(grpc_slice a, grpc_slice b) ***REMOVED***
...
int grpc_slice_eq(grpc_slice a, grpc_slice b) ***REMOVED***
  if (a.refcount && b.refcount && a.refcount->vtable == b.refcount->vtable) ***REMOVED***
    return a.refcount->vtable->eq(a, b);
  ***REMOVED***
  return grpc_slice_default_eq_impl(a, b);
***REMOVED***

int grpc_slice_cmp(grpc_slice a, grpc_slice b) ***REMOVED***
...
int grpc_slice_str_cmp(grpc_slice a, const char* b) ***REMOVED***
...
int grpc_slice_is_equivalent(grpc_slice a, grpc_slice b) ***REMOVED***
  if (a.refcount == nullptr || b.refcount == nullptr) ***REMOVED***
...
int grpc_slice_buf_start_eq(grpc_slice a, const void* b, size_t len) ***REMOVED***
  if (GRPC_SLICE_LENGTH(a) < len) return 0;
  return 0 == memcmp(GRPC_SLICE_START_PTR(a), b, len);
***REMOVED***

int grpc_slice_rchr(grpc_slice s, char c) ***REMOVED***
...
int grpc_slice_chr(grpc_slice s, char c) ***REMOVED***
  const char* b = (const char*)GRPC_SLICE_START_PTR(s);
  const char* p = (const char*)memchr(b, c, GRPC_SLICE_LENGTH(s));
  return p == nullptr ? -1 : (int)(p - b);
***REMOVED***

int grpc_slice_slice(grpc_slice haystack, grpc_slice needle) ***REMOVED***
  ...
  for (const uint8_t* cur = haystack_bytes; cur != last; ++cur) ***REMOVED***
    if (0 == memcmp(cur, needle_bytes, needle_len)) ***REMOVED***
...
grpc_slice grpc_slice_dup(grpc_slice a) ***REMOVED***
