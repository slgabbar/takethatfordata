...
typedef struct completed_thread ***REMOVED***
  gpr_thd_id t;
  struct completed_thread* next;
***REMOVED*** completed_thread;
...
static void gc_completed_threads(void) ***REMOVED***
  if (g_completed_threads != nullptr) ***REMOVED***
    completed_thread* to_gc = g_completed_threads;
    g_completed_threads = nullptr;
    gpr_mu_unlock(&g_mu);
    while (to_gc != nullptr) ***REMOVED***
...
static void start_timer_thread_and_unlock(void) ***REMOVED***
...
void grpc_timer_manager_tick() ***REMOVED***
...
static void run_some_timers(grpc_exec_ctx* exec_ctx) ***REMOVED***
  // if there's something to execute...
  gpr_mu_lock(&g_mu);
  // remove a waiter from the pool, and start another thread if necessary
  --g_waiter_count;
  if (g_waiter_count == 0 && g_threaded) ***REMOVED***
    start_timer_thread_and_unlock();
  ***REMOVED*** else ***REMOVED***
    // if there's no thread waiting with a timeout, kick an existing
    // waiter so that the next deadline is not missed
    if (!g_has_timed_waiter) ***REMOVED***
      if (grpc_timer_check_trace.enabled()) ***REMOVED***
...
// shutdown)
static bool wait_until(grpc_exec_ctx* exec_ctx, grpc_millis next) ***REMOVED***
  gpr_mu_lock(&g_mu);
  // if we're not threaded anymore, leave
  if (!g_threaded) ***REMOVED***
    gpr_mu_unlock(&g_mu);
    return false;
  ***REMOVED***
  ...
  if (!g_kicked) ***REMOVED***
    ...
    /* If there's no timed waiter, we should become one: that waiter waits only
       ...
       waiter) */
    if (next != GRPC_MILLIS_INF_FUTURE) ***REMOVED***
      if (!g_has_timed_waiter || (next < g_timed_waiter_deadline)) ***REMOVED***
        ...
        if (grpc_timer_check_trace.enabled()) ***REMOVED***
          ...
          gpr_log(GPR_DEBUG, "sleep for a %" PRIdPTR " milliseconds",
                  wait_time);
        ***REMOVED***
      ***REMOVED*** else ***REMOVED***  // g_timed_waiter == true && next >= g_timed_waiter_deadline
        next = GRPC_MILLIS_INF_FUTURE;
      ***REMOVED***
    ***REMOVED***

    if (grpc_timer_check_trace.enabled() && next == GRPC_MILLIS_INF_FUTURE) ***REMOVED***
    ...
    ***REMOVED***
    ...
    if (my_timed_waiter_generation == g_timed_waiter_generation) ***REMOVED***
...
static void timer_main_loop(grpc_exec_ctx* exec_ctx) ***REMOVED***
  for (;;) ***REMOVED***
    grpc_millis next = GRPC_MILLIS_INF_FUTURE;
    grpc_exec_ctx_invalidate_now(exec_ctx);

    // check timer state, updates next to the next time to run a check
    switch (grpc_timer_check(exec_ctx, &next)) ***REMOVED***
      ...
      case GRPC_TIMERS_CHECKED_AND_EMPTY:
        if (!wait_until(exec_ctx, next)) ***REMOVED***
          return;
        ***REMOVED***
        break;
    ***REMOVED***
...
static void timer_thread(void* completed_thread_ptr) ***REMOVED***
...
static void start_threads(void) ***REMOVED***
  gpr_mu_lock(&g_mu);
  if (!g_threaded) ***REMOVED***
    g_threaded = true;
    start_timer_thread_and_unlock();
...
void grpc_timer_manager_init(void) ***REMOVED***
  gpr_mu_init(&g_mu);
...
static void stop_threads(void) ***REMOVED***
  gpr_mu_lock(&g_mu);
  if (grpc_timer_check_trace.enabled()) ***REMOVED***
  ...
  ***REMOVED***
  if (g_threaded) ***REMOVED***
    ...
    ***REMOVED***
    while (g_thread_count > 0) ***REMOVED***
...
void grpc_timer_manager_shutdown(void) ***REMOVED***
  stop_threads();
...
void grpc_timer_manager_set_threading(bool threaded) ***REMOVED***
  if (threaded) ***REMOVED***
    start_threads();
  ***REMOVED*** else ***REMOVED***
    stop_threads();
...
void grpc_kick_poller(void) ***REMOVED***
  gpr_mu_lock(&g_mu);
  g_kicked = true;
  g_has_timed_waiter = false;
  g_timed_waiter_deadline = GRPC_MILLIS_INF_FUTURE;
