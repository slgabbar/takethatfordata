...
error occurred,  and a `loc`  object that contains a  `***REMOVED***line, column***REMOVED***`
...
- **onInsertedSemicolon**: If given a callback, that callback will be
  ...
  `***REMOVED***line, column***REMOVED***` object representing this position.
...
- **locations**: When `true`, each node has a `loc` object attached
  ...
  one-based line and zero-based column numbers in `***REMOVED***line, column***REMOVED***`
...
- **onComment**: If a function is passed for this option, whenever a
  ...
  When the `locations` options is on, the `***REMOVED***line, column***REMOVED***` locations
  ...
  ***REMOVED***
    ...
    "loc": ***REMOVED***
      "start": ***REMOVED***line: Number, column: Number***REMOVED***
      "end": ***REMOVED***line: Number, column: Number***REMOVED***
    ***REMOVED***,
...
**getLineInfo**`(input, offset)` can be used to get a `***REMOVED***line,
column***REMOVED***` object for a given program string and character offset.

**tokenizer**`(input, options)` returns an object with a `getToken`
method that can be called repeatedly to get the next token, a `***REMOVED***start,
end, type, value***REMOVED***` object (with added `loc` property when the
...
```javascript
for (let token of acorn.tokenizer(str)) ***REMOVED***
...
var ast = acorn.parse('var x = 42; // answer', ***REMOVED***
...
// generate code
console.log(escodegen.generate(ast, ***REMOVED***comment: true***REMOVED***));
// > 'var x = 42;    // answer'
```
...
walk.simple(acorn.parse("let x = 10"), ***REMOVED***
  Literal(node) ***REMOVED***
    console.log(`Found a literal: $***REMOVED***node.value***REMOVED***`)
  ***REMOVED***
***REMOVED***)
```
...
walk.ancestor(acorn.parse("foo('hi')"), ***REMOVED***
  Literal(_, ancestors) ***REMOVED***
...
walk.full(acorn.parse("1 + 1"), node => ***REMOVED***
  console.log(`There's a $***REMOVED***node.type***REMOVED*** node at $***REMOVED***node.ch***REMOVED***`)
***REMOVED***)
```
...
- `--locations`: Attaches a "loc" object to each node with "start" and
  ...
  zero-based column numbers in `***REMOVED***line, column***REMOVED***` form.
...
```javascript
parser.extend("readToken", function(nextMethod) ***REMOVED***
  return function(code) ***REMOVED***
...
```javascript
looseParser.extend("readToken", function(nextMethod) ***REMOVED***
