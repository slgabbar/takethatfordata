/*
	MIT License http://www.opensource.org/licenses/mit-license.php
	Author Tobias Koppers @sokra
*/
"use strict";
const Template = require("./Template");
const ModuleHotAcceptDependency = require("./dependencies/ModuleHotAcceptDependency");
const ModuleHotDeclineDependency = require("./dependencies/ModuleHotDeclineDependency");
const RawSource = require("webpack-sources").RawSource;
const ConstDependency = require("./dependencies/ConstDependency");
const NullFactory = require("./NullFactory");
const ParserHelpers = require("./ParserHelpers");

module.exports = class HotModuleReplacementPlugin ***REMOVED***
	constructor(options) ***REMOVED***
		this.options = options || ***REMOVED******REMOVED***;
		this.multiStep = this.options.multiStep;
		this.fullBuildTimeout = this.options.fullBuildTimeout || 200;
		this.requestTimeout = this.options.requestTimeout || 10000;
	***REMOVED***

	apply(compiler) ***REMOVED***
		const multiStep = this.multiStep;
		const fullBuildTimeout = this.fullBuildTimeout;
		const requestTimeout = this.requestTimeout;
		const hotUpdateChunkFilename = compiler.options.output.hotUpdateChunkFilename;
		const hotUpdateMainFilename = compiler.options.output.hotUpdateMainFilename;
		compiler.plugin("additional-pass", callback => ***REMOVED***
			if(multiStep)
				return setTimeout(callback, fullBuildTimeout);
			return callback();
		***REMOVED***);
		compiler.plugin("compilation", (compilation, params) => ***REMOVED***
			const hotUpdateChunkTemplate = compilation.hotUpdateChunkTemplate;
			if(!hotUpdateChunkTemplate) return;

			const normalModuleFactory = params.normalModuleFactory;

			compilation.dependencyFactories.set(ConstDependency, new NullFactory());
			compilation.dependencyTemplates.set(ConstDependency, new ConstDependency.Template());

			compilation.dependencyFactories.set(ModuleHotAcceptDependency, normalModuleFactory);
			compilation.dependencyTemplates.set(ModuleHotAcceptDependency, new ModuleHotAcceptDependency.Template());

			compilation.dependencyFactories.set(ModuleHotDeclineDependency, normalModuleFactory);
			compilation.dependencyTemplates.set(ModuleHotDeclineDependency, new ModuleHotDeclineDependency.Template());

			compilation.plugin("record", function(compilation, records) ***REMOVED***
				if(records.hash === this.hash) return;
				records.hash = compilation.hash;
				records.moduleHashs = ***REMOVED******REMOVED***;
				this.modules.forEach(module => ***REMOVED***
					const identifier = module.identifier();
					const hash = require("crypto").createHash("md5");
					module.updateHash(hash);
					records.moduleHashs[identifier] = hash.digest("hex");
				***REMOVED***);
				records.chunkHashs = ***REMOVED******REMOVED***;
				this.chunks.forEach(chunk => ***REMOVED***
					records.chunkHashs[chunk.id] = chunk.hash;
				***REMOVED***);
				records.chunkModuleIds = ***REMOVED******REMOVED***;
				this.chunks.forEach(chunk => ***REMOVED***
					records.chunkModuleIds[chunk.id] = chunk.mapModules(m => m.id);
				***REMOVED***);
			***REMOVED***);
			let initialPass = false;
			let recompilation = false;
			compilation.plugin("after-hash", function() ***REMOVED***
				let records = this.records;
				if(!records) ***REMOVED***
					initialPass = true;
					return;
				***REMOVED***
				if(!records.hash)
					initialPass = true;
				const preHash = records.preHash || "x";
				const prepreHash = records.prepreHash || "x";
				if(preHash === this.hash) ***REMOVED***
					recompilation = true;
					this.modifyHash(prepreHash);
					return;
				***REMOVED***
				records.prepreHash = records.hash || "x";
				records.preHash = this.hash;
				this.modifyHash(records.prepreHash);
			***REMOVED***);
			compilation.plugin("should-generate-chunk-assets", () => ***REMOVED***
				if(multiStep && !recompilation && !initialPass)
					return false;
			***REMOVED***);
			compilation.plugin("need-additional-pass", () => ***REMOVED***
				if(multiStep && !recompilation && !initialPass)
					return true;
			***REMOVED***);
			compilation.plugin("additional-chunk-assets", function() ***REMOVED***
				const records = this.records;
				if(records.hash === this.hash) return;
				if(!records.moduleHashs || !records.chunkHashs || !records.chunkModuleIds) return;
				this.modules.forEach(module => ***REMOVED***
					const identifier = module.identifier();
					let hash = require("crypto").createHash("md5");
					module.updateHash(hash);
					hash = hash.digest("hex");
					module.hotUpdate = records.moduleHashs[identifier] !== hash;
				***REMOVED***);
				const hotUpdateMainContent = ***REMOVED***
					h: this.hash,
					c: ***REMOVED******REMOVED***,
				***REMOVED***;
				Object.keys(records.chunkHashs).forEach(function(chunkId) ***REMOVED***
					chunkId = isNaN(+chunkId) ? chunkId : +chunkId;
					const currentChunk = this.chunks.find(chunk => chunk.id === chunkId);
					if(currentChunk) ***REMOVED***
						const newModules = currentChunk.getModules().filter(module => module.hotUpdate);
						const allModules = ***REMOVED******REMOVED***;
						currentChunk.forEachModule(module => ***REMOVED***
							allModules[module.id] = true;
						***REMOVED***);
						const removedModules = records.chunkModuleIds[chunkId].filter(id => !allModules[id]);
						if(newModules.length > 0 || removedModules.length > 0) ***REMOVED***
							const source = hotUpdateChunkTemplate.render(chunkId, newModules, removedModules, this.hash, this.moduleTemplate, this.dependencyTemplates);
							const filename = this.getPath(hotUpdateChunkFilename, ***REMOVED***
								hash: records.hash,
								chunk: currentChunk
							***REMOVED***);
							this.additionalChunkAssets.push(filename);
							this.assets[filename] = source;
							hotUpdateMainContent.c[chunkId] = true;
							currentChunk.files.push(filename);
							this.applyPlugins("chunk-asset", currentChunk, filename);
						***REMOVED***
					***REMOVED*** else ***REMOVED***
						hotUpdateMainContent.c[chunkId] = false;
					***REMOVED***
				***REMOVED***, this);
				const source = new RawSource(JSON.stringify(hotUpdateMainContent));
				const filename = this.getPath(hotUpdateMainFilename, ***REMOVED***
					hash: records.hash
				***REMOVED***);
				this.assets[filename] = source;
			***REMOVED***);

			compilation.mainTemplate.plugin("hash", hash => ***REMOVED***
				hash.update("HotMainTemplateDecorator");
			***REMOVED***);

			compilation.mainTemplate.plugin("module-require", (_, chunk, hash, varModuleId) => ***REMOVED***
				return `hotCreateRequire($***REMOVED***varModuleId***REMOVED***)`;
			***REMOVED***);

			compilation.mainTemplate.plugin("require-extensions", function(source) ***REMOVED***
				const buf = [source];
				buf.push("");
				buf.push("// __webpack_hash__");
				buf.push(this.requireFn + ".h = function() ***REMOVED*** return hotCurrentHash; ***REMOVED***;");
				return this.asString(buf);
			***REMOVED***);

			compilation.mainTemplate.plugin("bootstrap", function(source, chunk, hash) ***REMOVED***
				source = this.applyPluginsWaterfall("hot-bootstrap", source, chunk, hash);
				return this.asString([
					source,
					"",
					hotInitCode
					.replace(/\$require\$/g, this.requireFn)
					.replace(/\$hash\$/g, JSON.stringify(hash))
					.replace(/\$requestTimeout\$/g, requestTimeout)
					.replace(/\/\*foreachInstalledChunks\*\//g, chunk.chunks.length > 0 ? "for(var chunkId in installedChunks)" : `var chunkId = $***REMOVED***JSON.stringify(chunk.id)***REMOVED***;`)
				]);
			***REMOVED***);

			compilation.mainTemplate.plugin("global-hash", () => true);

			compilation.mainTemplate.plugin("current-hash", (_, length) => ***REMOVED***
				if(isFinite(length))
					return `hotCurrentHash.substr(0, $***REMOVED***length***REMOVED***)`;
				else
					return "hotCurrentHash";
			***REMOVED***);

			compilation.mainTemplate.plugin("module-obj", function(source, chunk, hash, varModuleId) ***REMOVED***
				return this.asString([
					`$***REMOVED***source***REMOVED***,`,
					`hot: hotCreateModule($***REMOVED***varModuleId***REMOVED***),`,
					"parents: (hotCurrentParentsTemp = hotCurrentParents, hotCurrentParents = [], hotCurrentParentsTemp),",
					"children: []"
				]);
			***REMOVED***);

			params.normalModuleFactory.plugin("parser", (parser, parserOptions) => ***REMOVED***
				parser.plugin("expression __webpack_hash__", ParserHelpers.toConstantDependency("__webpack_require__.h()"));
				parser.plugin("evaluate typeof __webpack_hash__", ParserHelpers.evaluateToString("string"));
				parser.plugin("evaluate Identifier module.hot", function(expr) ***REMOVED***
					return ParserHelpers.evaluateToIdentifier("module.hot", !!this.state.compilation.hotUpdateChunkTemplate)(expr);
				***REMOVED***);
				parser.plugin("call module.hot.accept", function(expr) ***REMOVED***
					if(!this.state.compilation.hotUpdateChunkTemplate) return false;
					if(expr.arguments.length >= 1) ***REMOVED***
						const arg = this.evaluateExpression(expr.arguments[0]);
						let params = [];
						let requests = [];
						if(arg.isString()) ***REMOVED***
							params = [arg];
						***REMOVED*** else if(arg.isArray()) ***REMOVED***
							params = arg.items.filter(param => param.isString());
						***REMOVED***
						if(params.length > 0) ***REMOVED***
							params.forEach((param, idx) => ***REMOVED***
								const request = param.string;
								const dep = new ModuleHotAcceptDependency(request, param.range);
								dep.optional = true;
								dep.loc = Object.create(expr.loc);
								dep.loc.index = idx;
								this.state.module.addDependency(dep);
								requests.push(request);
							***REMOVED***);
							if(expr.arguments.length > 1)
								this.applyPluginsBailResult("hot accept callback", expr.arguments[1], requests);
							else
								this.applyPluginsBailResult("hot accept without callback", expr, requests);
						***REMOVED***
					***REMOVED***
				***REMOVED***);
				parser.plugin("call module.hot.decline", function(expr) ***REMOVED***
					if(!this.state.compilation.hotUpdateChunkTemplate) return false;
					if(expr.arguments.length === 1) ***REMOVED***
						const arg = this.evaluateExpression(expr.arguments[0]);
						let params = [];
						if(arg.isString()) ***REMOVED***
							params = [arg];
						***REMOVED*** else if(arg.isArray()) ***REMOVED***
							params = arg.items.filter(param => param.isString());
						***REMOVED***
						params.forEach((param, idx) => ***REMOVED***
							const dep = new ModuleHotDeclineDependency(param.string, param.range);
							dep.optional = true;
							dep.loc = Object.create(expr.loc);
							dep.loc.index = idx;
							this.state.module.addDependency(dep);
						***REMOVED***);
					***REMOVED***
				***REMOVED***);
				parser.plugin("expression module.hot", ParserHelpers.skipTraversal);
			***REMOVED***);
		***REMOVED***);
	***REMOVED***

***REMOVED***;

const hotInitCode = Template.getFunctionContent(require("./HotModuleReplacement.runtime.js"));
