...
It **is recommended** that you customize this property. This is an extra path to use for publishing and finding remote tarballs. The default value for `remote_path` is `""` meaning that if you do not provide it then all packages will be published at the base of the `host`. It is recommended to provide a value like `./***REMOVED***name***REMOVED***/v***REMOVED***version***REMOVED***` to help organize remote packages in the case that you choose to publish multiple node addons to the same `host`.

Note: This property supports variables based on [Versioning](#versioning).

###### package_name

It is **not recommended** to override this property unless you are also overriding the `remote_path`. This is the versioned name of the remote tarball containing the binary `.node` module and any supporting files you've placed inside the `module_path` directory. Unless you specify `package_name` in your `package.json` then it defaults to `***REMOVED***module_name***REMOVED***-v***REMOVED***version***REMOVED***-***REMOVED***node_abi***REMOVED***-***REMOVED***platform***REMOVED***-***REMOVED***arch***REMOVED***.tar.gz` which allows your binary to work across node versions, platforms, and architectures. If you are using `remote_path` that is also versioned by `./***REMOVED***module_name***REMOVED***/v***REMOVED***version***REMOVED***` then you could remove these variables from the `package_name` and just use: `***REMOVED***node_abi***REMOVED***-***REMOVED***platform***REMOVED***-***REMOVED***arch***REMOVED***.tar.gz`. Then your remote tarball will be looked up at, for example, `https://example.com/your-module/v0.1.0/node-v11-linux-x64.tar.gz`.
...
If your commit message contains special characters (e.g. `&`) this method might fail. An alternative is to use PowerShell, which gives you additional possibilities, like ignoring case by using `ToLower()`:
    ...
    ps: if($env:APPVEYOR_REPO_COMMIT_MESSAGE.ToLower().Contains('[publish binary]')) ***REMOVED*** node-pre-gyp --msvs_version=2013 publish ***REMOVED***
...
Use `travis-encrypt` like:
    ...
    travis encrypt node_pre_gyp_accessKeyId=$***REMOVED***node_pre_gyp_accessKeyId***REMOVED***
    travis encrypt node_pre_gyp_secretAccessKey=$***REMOVED***node_pre_gyp_secretAccessKey***REMOVED***
...
```sh
export PATH=$(pwd)/node_modules/.bin:$***REMOVED***PATH***REMOVED***
```
...
You might wish to publish binaries only on a specific commit. To do this you could borrow from the [Travis CI idea of commit keywords](http://about.travis-ci.org/docs/user/how-to-skip-a-build/) and add special handling for commit messages with `[publish binary]`:
    ...
    if [[ $***REMOVED***COMMIT_MESSAGE***REMOVED*** =~ "[publish binary]" ]]; then node-pre-gyp publish; fi;
...
Using the `npm` config argument: `--***REMOVED***module_name***REMOVED***_binary_host_mirror` can download binary files through a mirror.
