...
int HKDF(uint8_t *out_key, size_t out_len, const EVP_MD *digest,
         ...
         size_t salt_len, const uint8_t *info, size_t info_len) ***REMOVED***
  ...
  if (!HKDF_extract(prk, &prk_len, digest, secret, secret_len, salt,
                    salt_len) ||
      !HKDF_expand(out_key, out_len, digest, prk, prk_len, info, info_len)) ***REMOVED***
    return 0;
  ***REMOVED***
...
int HKDF_expand(uint8_t *out_key, size_t out_len, const EVP_MD *digest,
  ...
  if (out_len + digest_len < out_len || n > 255) ***REMOVED***
    OPENSSL_PUT_ERROR(HKDF, HKDF_R_OUTPUT_TOO_LARGE);
    return 0;
  ***REMOVED***

  HMAC_CTX_init(&hmac);
  if (!HMAC_Init_ex(&hmac, prk, prk_len, digest, NULL)) ***REMOVED***
    goto out;
  ***REMOVED***
  ...
  for (i = 0; i < n; i++) ***REMOVED***
    ...
    if (i != 0 && (!HMAC_Init_ex(&hmac, NULL, 0, NULL, NULL) ||
                   !HMAC_Update(&hmac, previous, digest_len))) ***REMOVED***
      goto out;
    ***REMOVED***
    if (!HMAC_Update(&hmac, info, info_len) ||
        !HMAC_Update(&hmac, &ctr, 1) ||
        !HMAC_Final(&hmac, previous, NULL)) ***REMOVED***
      goto out;
    ***REMOVED***

    todo = digest_len;
    if (done + todo > out_len) ***REMOVED***
      todo = out_len - done;
    ***REMOVED***
