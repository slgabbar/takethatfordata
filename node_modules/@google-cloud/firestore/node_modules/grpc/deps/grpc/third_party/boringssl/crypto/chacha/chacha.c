...
static const uint8_t sigma[16] = ***REMOVED*** 'e', 'x', 'p', 'a', 'n', 'd', ' ', '3',
                                   '2', '-', 'b', 'y', 't', 'e', ' ', 'k' ***REMOVED***;
...
static void chacha_core(uint8_t output[64], const uint32_t input[16]) ***REMOVED***
  uint32_t x[16];
  int i;

  OPENSSL_memcpy(x, input, sizeof(uint32_t) * 16);
  for (i = 20; i > 0; i -= 2) ***REMOVED***
