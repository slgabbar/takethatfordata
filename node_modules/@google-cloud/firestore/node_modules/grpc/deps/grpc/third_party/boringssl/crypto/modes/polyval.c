...
static void byte_reverse(polyval_block *b) ***REMOVED***
...
static void reverse_and_mulX_ghash(polyval_block *b) ***REMOVED***
  uint64_t hi = b->u[0];
  uint64_t lo = b->u[1];
...
void CRYPTO_POLYVAL_init(struct polyval_ctx *ctx, const uint8_t key[16]) ***REMOVED***
  polyval_block H;
...
void CRYPTO_POLYVAL_update_blocks(struct polyval_ctx *ctx, const uint8_t *in,
  ...
  while (in_len > 0) ***REMOVED***
    size_t todo = in_len;
    if (todo > sizeof(reversed)) ***REMOVED***
    ...
    ***REMOVED***
    ...
    for (size_t i = 0; i < blocks; i++) ***REMOVED***
...
void CRYPTO_POLYVAL_finish(const struct polyval_ctx *ctx, uint8_t out[16]) ***REMOVED***
