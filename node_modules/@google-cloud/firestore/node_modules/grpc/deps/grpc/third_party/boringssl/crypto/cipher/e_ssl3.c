...
static int ssl3_mac(AEAD_SSL3_CTX *ssl3_ctx, uint8_t *out, unsigned *out_len,
  ...
  if (!EVP_MD_CTX_copy_ex(&md_ctx, &ssl3_ctx->md_ctx) ||
      ...
      !EVP_DigestFinal_ex(&md_ctx, tmp, NULL)) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (!EVP_MD_CTX_copy_ex(&md_ctx, &ssl3_ctx->md_ctx) ||
      ...
      !EVP_DigestFinal_ex(&md_ctx, out, out_len)) ***REMOVED***
    EVP_MD_CTX_cleanup(&md_ctx);
    return 0;
  ***REMOVED***
  EVP_MD_CTX_cleanup(&md_ctx);
...
static void aead_ssl3_cleanup(EVP_AEAD_CTX *ctx) ***REMOVED***
...
static int aead_ssl3_init(EVP_AEAD_CTX *ctx, const uint8_t *key, size_t key_len,
                          ...
                          const EVP_CIPHER *cipher, const EVP_MD *md) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (!EVP_CipherInit_ex(&ssl3_ctx->cipher_ctx, cipher, NULL, &key[mac_key_len],
      ...
      !EVP_DigestUpdate(&ssl3_ctx->md_ctx, key, mac_key_len)) ***REMOVED***
...
static int aead_ssl3_seal(const EVP_AEAD_CTX *ctx, uint8_t *out,
  ...
  if (!ssl3_ctx->cipher_ctx.encrypt) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (nonce_len != 0) ***REMOVED***
    OPENSSL_PUT_ERROR(CIPHER, CIPHER_R_IV_TOO_LARGE);
    return 0;
  ***REMOVED***

  if (ad_len != 11 - 2 /* length bytes */) ***REMOVED***
  ...
  ***REMOVED***
  ...
  unsigned mac_len;
  if (!ssl3_mac(ssl3_ctx, mac, &mac_len, ad, ad_len, in, in_len)) ***REMOVED***
    return 0;
  ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (!EVP_EncryptFinal_ex(&ssl3_ctx->cipher_ctx, out + total, &len)) ***REMOVED***
    return 0;
  ***REMOVED***
  total += len;
...
static int aead_ssl3_open(const EVP_AEAD_CTX *ctx, uint8_t *out,
  ...
  ***REMOVED***
  ...
  size_t total = 0;
  int len;
  if (!EVP_DecryptUpdate(&ssl3_ctx->cipher_ctx, out, &len, in, (int)in_len)) ***REMOVED***
    return 0;
  ***REMOVED***
  total += len;
  if (!EVP_DecryptFinal_ex(&ssl3_ctx->cipher_ctx, out + total, &len)) ***REMOVED***
    return 0;
  ***REMOVED***
  total += len;
  assert(total == in_len);
  ...
  size_t data_len;
  if (EVP_CIPHER_CTX_mode(&ssl3_ctx->cipher_ctx) == EVP_CIPH_CBC_MODE) ***REMOVED***
    unsigned padding_length = out[total - 1];
    if (total < padding_length + 1 + mac_len) ***REMOVED***
      OPENSSL_PUT_ERROR(CIPHER, CIPHER_R_BAD_DECRYPT);
      return 0;
    ***REMOVED***
    /* The padding must be minimal. */
    if (padding_length + 1 > EVP_CIPHER_CTX_block_size(&ssl3_ctx->cipher_ctx)) ***REMOVED***
      OPENSSL_PUT_ERROR(CIPHER, CIPHER_R_BAD_DECRYPT);
      return 0;
    ***REMOVED***
  ...
  ***REMOVED***
  ...
  uint8_t mac[EVP_MAX_MD_SIZE];
  if (!ssl3_mac(ssl3_ctx, mac, NULL, ad, ad_len, out, data_len)) ***REMOVED***
    return 0;
  ***REMOVED***
  if (CRYPTO_memcmp(&out[data_len], mac, mac_len) != 0) ***REMOVED***
    OPENSSL_PUT_ERROR(CIPHER, CIPHER_R_BAD_DECRYPT);
    return 0;
  ***REMOVED***
...
static const EVP_AEAD aead_aes_128_cbc_sha1_ssl3 = ***REMOVED***
...
static const EVP_AEAD aead_aes_256_cbc_sha1_ssl3 = ***REMOVED***
...
static const EVP_AEAD aead_des_ede3_cbc_sha1_ssl3 = ***REMOVED***
...
static const EVP_AEAD aead_null_sha1_ssl3 = ***REMOVED***
...
const EVP_AEAD *EVP_aead_aes_128_cbc_sha1_ssl3(void) ***REMOVED***
  return &aead_aes_128_cbc_sha1_ssl3;
***REMOVED***

const EVP_AEAD *EVP_aead_aes_256_cbc_sha1_ssl3(void) ***REMOVED***
  return &aead_aes_256_cbc_sha1_ssl3;
***REMOVED***

const EVP_AEAD *EVP_aead_des_ede3_cbc_sha1_ssl3(void) ***REMOVED***
  return &aead_des_ede3_cbc_sha1_ssl3;
***REMOVED***

const EVP_AEAD *EVP_aead_null_sha1_ssl3(void) ***REMOVED*** return &aead_null_sha1_ssl3; ***REMOVED***
