...
static const uint8_t kOCSPResponse[] = ***REMOVED***0x01, 0x02, 0x03, 0x04***REMOVED***;
static const uint8_t kSCT[] = ***REMOVED***0x05, 0x06, 0x07, 0x08***REMOVED***;

static int ALPNSelectCallback(SSL *ssl, const uint8_t **out, uint8_t *out_len,
                              const uint8_t *in, unsigned in_len, void *arg) ***REMOVED***
  static const uint8_t kProtocol[] = ***REMOVED***'a', 'a'***REMOVED***;
  *out = kProtocol;
...
static int NPNAdvertiseCallback(SSL *ssl, const uint8_t **out,
                                unsigned *out_len, void *arg) ***REMOVED***
  static const uint8_t kProtocols[] = ***REMOVED***
...
struct GlobalState ***REMOVED***
  GlobalState()
      : ctx(SSL_CTX_new(SSLv23_method())) ***REMOVED***
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *buf, size_t len) ***REMOVED***
  ...
  if (SSL_do_handshake(server) == 1) ***REMOVED***
    // Keep reading application data until error or EOF.
    uint8_t tmp[1024];
    for (;;) ***REMOVED***
      if (SSL_read(server, tmp, sizeof(tmp)) <= 0) ***REMOVED***
        break;
      ***REMOVED***
    ***REMOVED***
  ***REMOVED***
