...
// algorithms.
var supportedSignatureAlgorithms = []signatureAlgorithm***REMOVED***
...
// sessions.
type ClientSessionState struct ***REMOVED***
...
// goroutines.
type ClientSessionCache interface ***REMOVED***
...
// implementations should expect to be called concurrently from different
// goroutines.
type ServerSessionCache interface ***REMOVED***
...
type ProtocolBugs struct ***REMOVED***
	...
	// ChangeCipherSpec records instead of ***REMOVED***1***REMOVED***.
...
func (c *Config) serverInit() ***REMOVED***
	if c.SessionTicketsDisabled ***REMOVED***
		return
	***REMOVED***

	// If the key has already been set then we have nothing to do.
	for _, b := range c.SessionTicketKey ***REMOVED***
		if b != 0 ***REMOVED***
			return
		***REMOVED***
	***REMOVED***

	if _, err := io.ReadFull(c.rand(), c.SessionTicketKey[:]); err != nil ***REMOVED***
		c.SessionTicketsDisabled = true
	***REMOVED***
***REMOVED***

func (c *Config) rand() io.Reader ***REMOVED***
	r := c.Rand
	if r == nil ***REMOVED***
		return rand.Reader
	***REMOVED***
...
func (c *Config) time() time.Time ***REMOVED***
	t := c.Time
	if t == nil ***REMOVED***
		t = time.Now
	***REMOVED***
...
func (c *Config) cipherSuites() []uint16 ***REMOVED***
	s := c.CipherSuites
	if s == nil ***REMOVED***
...
func (c *Config) minVersion(isDTLS bool) uint16 ***REMOVED***
	ret := uint16(minVersion)
	if c != nil && c.MinVersion != 0 ***REMOVED***
		ret = c.MinVersion
	***REMOVED***
	if isDTLS ***REMOVED***
		// The lowest version of DTLS is 1.0. There is no DSSL 3.0.
		if ret < VersionTLS10 ***REMOVED***
			return VersionTLS10
		***REMOVED***
		// There is no such thing as DTLS 1.1.
		if ret == VersionTLS11 ***REMOVED***
			return VersionTLS12
		***REMOVED***
	***REMOVED***
	return ret
...
func (c *Config) maxVersion(isDTLS bool) uint16 ***REMOVED***
	ret := uint16(maxVersion)
	if c != nil && c.MaxVersion != 0 ***REMOVED***
		ret = c.MaxVersion
	***REMOVED***
	if isDTLS ***REMOVED***
		// We only implement up to DTLS 1.2.
		if ret > VersionTLS12 ***REMOVED***
			return VersionTLS12
		***REMOVED***
		// There is no such thing as DTLS 1.1.
		if ret == VersionTLS11 ***REMOVED***
...
var defaultCurvePreferences = []CurveID***REMOVED***CurveX25519, CurveP256, CurveP384, CurveP521***REMOVED***

func (c *Config) curvePreferences() []CurveID ***REMOVED***
	if c == nil || len(c.CurvePreferences) == 0 ***REMOVED***
		return defaultCurvePreferences
	***REMOVED***
	return c.CurvePreferences
***REMOVED***

func (c *Config) defaultCurves() map[CurveID]bool ***REMOVED***
	defaultCurves := make(map[CurveID]bool)
	curves := c.DefaultCurves
	if c == nil || c.DefaultCurves == nil ***REMOVED***
		curves = c.curvePreferences()
	***REMOVED***
	for _, curveID := range curves ***REMOVED***
...
// acceptable.
func (c *Config) isSupportedVersion(vers uint16, isDTLS bool) bool ***REMOVED***
...
// options.
func (c *Config) getCertificateForName(name string) *Certificate ***REMOVED***
	if len(c.Certificates) == 1 || c.NameToCertificate == nil ***REMOVED***
		// There's only one choice, so no point doing any work.
		return &c.Certificates[0]
	***REMOVED***

	name = strings.ToLower(name)
	for len(name) > 0 && name[len(name)-1] == '.' ***REMOVED***
		name = name[:len(name)-1]
	***REMOVED***

	if cert, ok := c.NameToCertificate[name]; ok ***REMOVED***
		return cert
	***REMOVED***
	...
	for i := range labels ***REMOVED***
		labels[i] = "*"
		candidate := strings.Join(labels, ".")
		if cert, ok := c.NameToCertificate[candidate]; ok ***REMOVED***
			return cert
		***REMOVED***
	***REMOVED***
...
func (c *Config) signSignatureAlgorithms() []signatureAlgorithm ***REMOVED***
	if c != nil && c.SignSignatureAlgorithms != nil ***REMOVED***
		return c.SignSignatureAlgorithms
	***REMOVED***
	return supportedSignatureAlgorithms
***REMOVED***

func (c *Config) verifySignatureAlgorithms() []signatureAlgorithm ***REMOVED***
	if c != nil && c.VerifySignatureAlgorithms != nil ***REMOVED***
...
// certificates.
func (c *Config) BuildNameToCertificate() ***REMOVED***
	c.NameToCertificate = make(map[string]*Certificate)
	for i := range c.Certificates ***REMOVED***
		cert := &c.Certificates[i]
		x509Cert, err := x509.ParseCertificate(cert.Certificate[0])
		if err != nil ***REMOVED***
			continue
		***REMOVED***
		if len(x509Cert.Subject.CommonName) > 0 ***REMOVED***
			c.NameToCertificate[x509Cert.Subject.CommonName] = cert
		***REMOVED***
		for _, san := range x509Cert.DNSNames ***REMOVED***
...
type handshakeMessage interface ***REMOVED***
	marshal() []byte
	unmarshal([]byte) bool
***REMOVED***
...
type lruSessionCache struct ***REMOVED***
	sync.Mutex
...
type lruSessionCacheEntry struct ***REMOVED***
	sessionKey string
	state      interface***REMOVED******REMOVED***
***REMOVED***

// Put adds the provided (sessionKey, cs) pair to the cache.
func (c *lruSessionCache) Put(sessionKey string, cs interface***REMOVED******REMOVED***) ***REMOVED***
	c.Lock()
	defer c.Unlock()

	if elem, ok := c.m[sessionKey]; ok ***REMOVED***
	...
	***REMOVED***
	...
	if c.q.Len() < c.capacity ***REMOVED***
		entry := &lruSessionCacheEntry***REMOVED***sessionKey, cs***REMOVED***
...
func (c *lruSessionCache) Get(sessionKey string) (interface***REMOVED******REMOVED***, bool) ***REMOVED***
	c.Lock()
	defer c.Unlock()
...
type lruClientSessionCache struct ***REMOVED***
	lruSessionCache
***REMOVED***

func (c *lruClientSessionCache) Put(sessionKey string, cs *ClientSessionState) ***REMOVED***
	c.lruSessionCache.Put(sessionKey, cs)
***REMOVED***

func (c *lruClientSessionCache) Get(sessionKey string) (*ClientSessionState, bool) ***REMOVED***
...
// uses an LRU caching strategy.
type lruServerSessionCache struct ***REMOVED***
	lruSessionCache
***REMOVED***

func (c *lruServerSessionCache) Put(sessionId string, session *sessionState) ***REMOVED***
	c.lruSessionCache.Put(sessionId, session)
***REMOVED***

func (c *lruServerSessionCache) Get(sessionId string) (*sessionState, bool) ***REMOVED***
...
// is used instead.
func NewLRUClientSessionCache(capacity int) ClientSessionCache ***REMOVED***
	const defaultSessionCacheCapacity = 64

	if capacity < 1 ***REMOVED***
		capacity = defaultSessionCacheCapacity
	***REMOVED***
	return &lruClientSessionCache***REMOVED***
		lruSessionCache***REMOVED***
...
// capacity that uses an LRU strategy. If capacity is < 1, a default capacity
// is used instead.
func NewLRUServerSessionCache(capacity int) ServerSessionCache ***REMOVED***
	const defaultSessionCacheCapacity = 64

	if capacity < 1 ***REMOVED***
		capacity = defaultSessionCacheCapacity
	***REMOVED***
	return &lruServerSessionCache***REMOVED***
		lruSessionCache***REMOVED***
			m:        make(map[string]*list.Element),
			q:        list.New(),
			capacity: capacity,
...
func defaultCipherSuites() []uint16 ***REMOVED***
	once.Do(initDefaultCipherSuites)
	return varDefaultCipherSuites
***REMOVED***

func initDefaultCipherSuites() ***REMOVED***
	for _, suite := range cipherSuites ***REMOVED***
		if suite.flags&suitePSK == 0 ***REMOVED***
			varDefaultCipherSuites = append(varDefaultCipherSuites, suite.id)
		***REMOVED***
	***REMOVED***
***REMOVED***

func unexpectedMessageError(wanted, got interface***REMOVED******REMOVED***) error ***REMOVED***
	return fmt.Errorf("tls: received unexpected handshake message of type %T when waiting for %T", got, wanted)
***REMOVED***

func isSupportedSignatureAlgorithm(sigAlg signatureAlgorithm, sigAlgs []signatureAlgorithm) bool ***REMOVED***
	for _, s := range sigAlgs ***REMOVED***
		if s == sigAlg ***REMOVED***
			return true
		***REMOVED***
	***REMOVED***
...
var (
	...
	downgradeTLS13 = []byte***REMOVED***0x44, 0x4f, 0x57, 0x4e, 0x47, 0x52, 0x44, 0x01***REMOVED***
	downgradeTLS12 = []byte***REMOVED***0x44, 0x4f, 0x57, 0x4e, 0x47, 0x52, 0x44, 0x00***REMOVED***
)
