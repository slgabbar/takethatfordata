...
func leftRotate(a uint32, n uint) uint32 ***REMOVED***
	return (a << n) | (a >> (32 - n))
***REMOVED***

func chaChaQuarterRound(state *[16]uint32, a, b, c, d int) ***REMOVED***
	state[a] += state[b]
...
func chaCha20Block(state *[16]uint32, out []byte) ***REMOVED***
...
func chaCha20(out, in, key, nonce []byte, counter uint64) ***REMOVED***
	var state [16]uint32
	state[0] = 0x61707865
	state[1] = 0x3320646e
	state[2] = 0x79622d32
	...
	***REMOVED***
	...
	for i := 0; i < len(in); i += 64 ***REMOVED***
		state[12] = uint32(counter)

		var tmp [64]byte
		chaCha20Block(&state, tmp[:])
		count := 64
		if len(in)-i < count ***REMOVED***
			count = len(in) - i
		***REMOVED***
...
func newChaCha20Poly1305(key []byte) (cipher.AEAD, error) ***REMOVED***
	if len(key) != 32 ***REMOVED***
...
func (c *chaCha20Poly1305) Overhead() int ***REMOVED*** return 16 ***REMOVED***

func (c *chaCha20Poly1305) poly1305(tag *[16]byte, nonce, ciphertext, additionalData []byte) ***REMOVED***
	input := make([]byte, 0, len(additionalData)+15+len(ciphertext)+15+8+8)
	input = append(input, additionalData...)
	var zeros [15]byte
	if pad := len(input) % 16; pad != 0 ***REMOVED***
...
func (c *chaCha20Poly1305) Seal(dst, nonce, plaintext, additionalData []byte) []byte ***REMOVED***
	if len(nonce) != c.NonceSize() ***REMOVED***
		panic("Bad nonce length")
	***REMOVED***
...
func (c *chaCha20Poly1305) Open(dst, nonce, ciphertext, additionalData []byte) ([]byte, error) ***REMOVED***
	...
	***REMOVED***
	...
	if subtle.ConstantTimeCompare(tag[:], ciphertext[plaintextLen:]) != 1 ***REMOVED***
		return nil, errors.New("chacha20: message authentication failed")
	***REMOVED***
