 ...
 * using the Chrome UI. */
static const uint8_t kPKCS7NSS[] = ***REMOVED***
...
static const uint8_t kPKCS7Windows[] = ***REMOVED***
...
static const uint8_t kOpenSSLCRL[] = ***REMOVED***
...
static int test_cert_reparse(const uint8_t *der_bytes, size_t der_len) ***REMOVED***
  CBS pkcs7;
  CBB cbb;
  ...
  if (!PKCS7_get_certificates(certs, &pkcs7)) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (!PKCS7_bundle_certificates(&cbb, certs) ||
      !CBB_finish(&cbb, &result_data, &result_len)) ***REMOVED***
    fprintf(stderr, "PKCS7_bundle_certificates failed.\n");
    return 0;
  ***REMOVED***

  CBS_init(&pkcs7, result_data, result_len);
  if (!PKCS7_get_certificates(certs2, &pkcs7)) ***REMOVED***
    fprintf(stderr, "PKCS7_get_certificates reparse failed.\n");
    return 0;
  ***REMOVED***

  if (sk_X509_num(certs) != sk_X509_num(certs2)) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (!PKCS7_bundle_certificates(&cbb, certs2) ||
      !CBB_finish(&cbb, &result2_data, &result2_len)) ***REMOVED***
    fprintf(stderr,
  ...
  ***REMOVED***
  ...
  if (result_len != result2_len ||
      OPENSSL_memcmp(result_data, result2_data, result_len) != 0) ***REMOVED***
...
static int test_crl_reparse(const uint8_t *der_bytes, size_t der_len) ***REMOVED***
  CBS pkcs7;
  CBB cbb;
  ...
  CBS_init(&pkcs7, der_bytes, der_len);
  if (!PKCS7_get_CRLs(crls, &pkcs7)) ***REMOVED***
  ...
  ***REMOVED***
  ...
  CBS_init(&pkcs7, result_data, result_len);
  if (!PKCS7_get_CRLs(crls2, &pkcs7)) ***REMOVED***
    fprintf(stderr, "PKCS7_get_CRLs reparse failed.\n");
    return 0;
  ***REMOVED***

  if (sk_X509_CRL_num(crls) != sk_X509_CRL_num(crls)) ***REMOVED***
...
static int test_pem_certs(const char *pem) ***REMOVED***
  BIO *bio = BIO_new_mem_buf(pem, strlen(pem));
  STACK_OF(X509) *certs = sk_X509_new_null();

  if (!PKCS7_get_PEM_certificates(certs, bio)) ***REMOVED***
    fprintf(stderr, "PKCS7_get_PEM_certificates failed.\n");
    return 0;
  ***REMOVED***

  if (sk_X509_num(certs) != 1) ***REMOVED***
    fprintf(stderr,
...
static int test_pem_crls(const char *pem) ***REMOVED***
  BIO *bio = BIO_new_mem_buf(pem, strlen(pem));
  STACK_OF(X509_CRL) *crls = sk_X509_CRL_new_null();

  if (!PKCS7_get_PEM_CRLs(crls, bio)) ***REMOVED***
    fprintf(stderr, "PKCS7_get_PEM_CRLs failed.\n");
    return 0;
  ***REMOVED***

  if (sk_X509_CRL_num(crls) != 1) ***REMOVED***
