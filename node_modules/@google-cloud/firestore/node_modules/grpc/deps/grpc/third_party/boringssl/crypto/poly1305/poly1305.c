...
static uint32_t U8TO32_LE(const uint8_t *m) ***REMOVED***
  uint32_t r;
  OPENSSL_memcpy(&r, m, sizeof(r));
  return r;
***REMOVED***

static void U32TO8_LE(uint8_t *m, uint32_t v) ***REMOVED***
  OPENSSL_memcpy(m, &v, sizeof(v));
***REMOVED***

static uint64_t mul32x32_64(uint32_t a, uint32_t b) ***REMOVED*** return (uint64_t)a * b; ***REMOVED***
...
void CRYPTO_poly1305_init(poly1305_state *statep, const uint8_t key[32]) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (state->buf_used) ***REMOVED***
    ...
    ***REMOVED***
    ...
    if (state->buf_used == 16) ***REMOVED***
      poly1305_update(state, state->buf, 16);
      state->buf_used = 0;
    ***REMOVED***
  ***REMOVED***

  if (in_len >= 16) ***REMOVED***
...
void CRYPTO_poly1305_finish(poly1305_state *statep, uint8_t mac[16]) ***REMOVED***
  struct poly1305_state_st *state = poly1305_aligned_state(statep);
