...
uint64_t bn_mont_n0(const BIGNUM *n) ***REMOVED***
...
static uint64_t bn_neg_inv_mod_r_u64(uint64_t n) ***REMOVED***
  assert(n % 2 == 1);
  ...
  for (size_t i = 0; i < LG_LITTLE_R; ++i) ***REMOVED***
 ...
 * odd. */
int bn_mod_exp_base_2_vartime(BIGNUM *r, unsigned p, const BIGNUM *n) ***REMOVED***
  assert(!BN_is_zero(n));
  assert(!BN_is_negative(n));
  assert(BN_is_odd(n));
