...
int tls13_init_key_schedule(SSL_HANDSHAKE *hs) ***REMOVED***
  if (!SSL_TRANSCRIPT_init_hash(&hs->transcript, ssl3_protocol_version(hs->ssl),
                                hs->new_cipher->algorithm_prf)) ***REMOVED***
    return 0;
  ***REMOVED***
...
static int hkdf_expand_label(uint8_t *out, const EVP_MD *digest,
                             ...
                             const uint8_t *hash, size_t hash_len, size_t len) ***REMOVED***
  ...
  if (!CBB_init(&cbb, 2 + 1 + strlen(kTLS13LabelVersion) + label_len + 1 +
      ...
      !CBB_finish(&cbb, &hkdf_label, &hkdf_label_len)) ***REMOVED***
    CBB_cleanup(&cbb);
    return 0;
  ***REMOVED***
...
static int derive_secret(SSL_HANDSHAKE *hs, uint8_t *out, size_t len,
                         const uint8_t *label, size_t label_len) ***REMOVED***
  uint8_t context_hash[EVP_MAX_MD_SIZE];
  size_t context_hash_len;
  if (!SSL_TRANSCRIPT_get_hash(&hs->transcript, context_hash,
                               &context_hash_len)) ***REMOVED***
    return 0;
  ***REMOVED***
...
int tls13_set_traffic_key(SSL *ssl, enum evp_aead_direction_t direction,
                          ...
                          size_t traffic_secret_len) ***REMOVED***
  if (traffic_secret_len > 0xff) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (!hkdf_expand_label(key, digest, traffic_secret, traffic_secret_len,
                         (const uint8_t *)"key", 3, NULL, 0, key_len)) ***REMOVED***
    return 0;
  ***REMOVED***
  ...
  if (!hkdf_expand_label(iv, digest, traffic_secret, traffic_secret_len,
                         (const uint8_t *)"iv", 2, NULL, 0, iv_len)) ***REMOVED***
    return 0;
  ***REMOVED***
  ...
  if (traffic_aead == NULL) ***REMOVED***
    return 0;
  ***REMOVED***

  if (direction == evp_aead_open) ***REMOVED***
    if (!ssl->method->set_read_state(ssl, traffic_aead)) ***REMOVED***
      return 0;
    ***REMOVED***
  ***REMOVED*** else ***REMOVED***
    if (!ssl->method->set_write_state(ssl, traffic_aead)) ***REMOVED***
      return 0;
    ***REMOVED***
  ***REMOVED***
...
int tls13_derive_handshake_secrets(SSL_HANDSHAKE *hs) ***REMOVED***
...
int tls13_derive_application_secrets(SSL_HANDSHAKE *hs) ***REMOVED***
  SSL *const ssl = hs->ssl;
...
int tls13_rotate_traffic_key(SSL *ssl, enum evp_aead_direction_t direction) ***REMOVED***
  const EVP_MD *digest = ssl_get_handshake_digest(
      SSL_get_session(ssl)->cipher->algorithm_prf, ssl3_protocol_version(ssl));
...
int tls13_derive_resumption_secret(SSL_HANDSHAKE *hs) ***REMOVED***
  if (hs->hash_len > SSL_MAX_MASTER_KEY_LENGTH) ***REMOVED***
...
static int tls13_verify_data(const EVP_MD *digest, uint8_t *out,
  ...
  if (!hkdf_expand_label(key, digest, secret, hash_len,
      ...
      HMAC(digest, key, hash_len, context, context_len, out, &len) == NULL) ***REMOVED***
    return 0;
  ***REMOVED***
  *out_len = len;
  return 1;
...
int tls13_finished_mac(SSL_HANDSHAKE *hs, uint8_t *out, size_t *out_len,
                       int is_server) ***REMOVED***
  SSL *const ssl = hs->ssl;

  const uint8_t *traffic_secret;
  if (is_server == ssl->server) ***REMOVED***
...
int tls13_export_keying_material(SSL *ssl, uint8_t *out, size_t out_len,
                                 ...
                                 int use_context) ***REMOVED***
  const EVP_MD *digest = ssl_get_handshake_digest(
      SSL_get_session(ssl)->cipher->algorithm_prf, ssl3_protocol_version(ssl));

  const uint8_t *hash = NULL;
  size_t hash_len = 0;
  if (use_context) ***REMOVED***
    hash = context;
...
static int tls13_psk_binder(uint8_t *out, const EVP_MD *digest, uint8_t *psk,
                            ...
                            size_t context_len, size_t hash_len) ***REMOVED***
  uint8_t binder_context[EVP_MAX_MD_SIZE];
  unsigned binder_context_len;
  if (!EVP_Digest(NULL, 0, binder_context, &binder_context_len, digest, NULL)) ***REMOVED***
    return 0;
  ***REMOVED***

  uint8_t early_secret[EVP_MAX_MD_SIZE] = ***REMOVED***0***REMOVED***;
  ...
  ***REMOVED***
  ...
  uint8_t binder_key[EVP_MAX_MD_SIZE] = ***REMOVED***0***REMOVED***;
  size_t len;
...
int tls13_write_psk_binder(SSL_HANDSHAKE *hs, uint8_t *msg, size_t len) ***REMOVED***
  SSL *const ssl = hs->ssl;
  const EVP_MD *digest = SSL_SESSION_get_digest(ssl->session, ssl);
  if (digest == NULL) ***REMOVED***
    OPENSSL_PUT_ERROR(SSL, ERR_R_INTERNAL_ERROR);
    return 0;
  ***REMOVED***
  size_t hash_len = EVP_MD_size(digest);

  if (len < hash_len + 3) ***REMOVED***
    OPENSSL_PUT_ERROR(SSL, ERR_R_INTERNAL_ERROR);
    return 0;
  ***REMOVED***
  ...
  if (!EVP_DigestInit_ex(&ctx, digest, NULL) ||
      ...
      !EVP_DigestFinal_ex(&ctx, context, &context_len)) ***REMOVED***
  ...
  ***REMOVED***
  ...
  uint8_t verify_data[EVP_MAX_MD_SIZE] = ***REMOVED***0***REMOVED***;
...
int tls13_verify_psk_binder(SSL_HANDSHAKE *hs, SSL_SESSION *session,
  ...
  if (CBS_len(&message) < CBS_len(binders) + 2) ***REMOVED***
    OPENSSL_PUT_ERROR(SSL, ERR_R_INTERNAL_ERROR);
    return 0;
  ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (!tls13_psk_binder(verify_data, SSL_TRANSCRIPT_md(&hs->transcript),
      ...
      !CBS_get_u8_length_prefixed(binders, &binder)) ***REMOVED***
    OPENSSL_PUT_ERROR(SSL, ERR_R_INTERNAL_ERROR);
    return 0;
  ***REMOVED***
