...
static bool TestBasic() ***REMOVED***
  ...
  static const uint8_t kDER[] = ***REMOVED***
  ...
  ***REMOVED***;
  ...
  if (OBJ_cbs2nid(&cbs) != kNID ||
      ...
      OBJ_txt2nid(kText) != kNID) ***REMOVED***
    return false;
  ***REMOVED***

  if (strcmp(kShortName, OBJ_nid2sn(kNID)) != 0 ||
      strcmp(kLongName, OBJ_nid2ln(kNID)) != 0) ***REMOVED***
    return false;
  ***REMOVED***

  if (OBJ_sn2nid("this is not an OID") != NID_undef ||
      OBJ_ln2nid("this is not an OID") != NID_undef ||
      OBJ_txt2nid("this is not an OID") != NID_undef) ***REMOVED***
    return false;
  ***REMOVED***

  CBS_init(&cbs, NULL, 0);
  if (OBJ_cbs2nid(&cbs) != NID_undef) ***REMOVED***
    return false;
  ***REMOVED***

  // 1.2.840.113554.4.1.72585.2 (https://davidben.net/oid).
  static const uint8_t kUnknownDER[] = ***REMOVED***
...
static bool TestSignatureAlgorithms() ***REMOVED***
  int digest_nid, pkey_nid;
  if (!OBJ_find_sigid_algs(NID_sha256WithRSAEncryption, &digest_nid,
                           &pkey_nid) ||
      digest_nid != NID_sha256 || pkey_nid != NID_rsaEncryption) ***REMOVED***
    return false;
  ***REMOVED***

  if (OBJ_find_sigid_algs(NID_sha256, &digest_nid, &pkey_nid)) ***REMOVED***
    return false;
  ***REMOVED***

  int sign_nid;
  if (!OBJ_find_sigid_by_algs(&sign_nid, NID_sha256, NID_rsaEncryption) ||
      sign_nid != NID_sha256WithRSAEncryption) ***REMOVED***
    return false;
  ***REMOVED***

  if (OBJ_find_sigid_by_algs(&sign_nid, NID_dsa, NID_rsaEncryption)) ***REMOVED***
    return false;
  ***REMOVED***
...
static bool ExpectObj2Txt(const uint8_t *der, size_t der_len,
                          bool always_return_oid, const char *expected) ***REMOVED***
  ASN1_OBJECT obj;
  ...
  ***REMOVED***
  ...
  if (OPENSSL_memchr(short_buf, '\0', sizeof(short_buf)) == nullptr) ***REMOVED***
    fprintf(stderr,
  ...
  ***REMOVED***
  ...
  if (strcmp(buf, expected) != 0) ***REMOVED***
...
static bool TestObj2Txt() ***REMOVED***
  ...
  ***REMOVED***;
  ...
  if (!ExpectObj2Txt(kSHA256WithRSAEncryption, sizeof(kSHA256WithRSAEncryption),
      ...
      !ExpectObj2Txt(nullptr, 0, true /* don't return name */, "")) ***REMOVED***
    return false;
  ***REMOVED***
  ...
  // encoded.
  static const uint8_t kNonMinimalOID[] = ***REMOVED***
      0x55, 0x1d, 0x80, 0x13,
  ***REMOVED***;
  obj.data = kNonMinimalOID;
  obj.length = sizeof(kNonMinimalOID);
  if (OBJ_obj2txt(NULL, 0, &obj, 0) != -1) ***REMOVED***
  ...
  ***REMOVED***
  ...
  static const uint8_t kOverflowOID[] = ***REMOVED***
  ...
  ***REMOVED***
  ...
  static const uint8_t kInvalidOID[] = ***REMOVED***
      0x55, 0x1d, 0x93,
  ***REMOVED***;
