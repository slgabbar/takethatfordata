...
.Lloop:
___
for($i=0;$i<20;$i++)	***REMOVED*** &BODY_00_19($i,@V); unshift(@V,pop(@V)); ***REMOVED***
...
___
if ($shaext) ***REMOVED******REMOVED******REMOVED***
######################################################################
...
$code.=<<___;
	...
	vmovdqu	@X[3],96(%rsp)
___
for (;$Xi<8;$Xi++) ***REMOVED***	# Xupdate_avx2_16_31
    use integer;
...
.Loop_avx2:
	...
	xor	$t0,$F
___
sub bodyx_00_19 () ***REMOVED***	# 8 instructions, 3 cycles critical path
...
sub bodyx_20_39 () ***REMOVED***	# 7 instructions, 2 cycles critical path
...
sub bodyx_40_59 () ***REMOVED***	# 10 instructions, 3 cycles critical path
...
sub sha1rnds4 ***REMOVED***
    if (@_[0] =~ /\$([x0-9a-f]+),\s*%xmm([0-7]),\s*%xmm([0-7])/) ***REMOVED***
...
sub sha1op38 ***REMOVED***
    ...
    if (defined($opcodelet***REMOVED***$instr***REMOVED***) && @_[0] =~ /%xmm([0-9]+),\s*%xmm([0-9]+)/) ***REMOVED***
      my @opcode=(0x0f,0x38);
      my $rex=0;
	$rex|=0x04			if ($2>=8);
	$rex|=0x01			if ($1>=8);
