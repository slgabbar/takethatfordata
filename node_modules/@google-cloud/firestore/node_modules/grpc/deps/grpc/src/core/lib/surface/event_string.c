...
static void addhdr(gpr_strvec *buf, grpc_event *ev) ***REMOVED***
  char *tmp;
  gpr_asprintf(&tmp, "tag:%p", ev->tag);
  gpr_strvec_add(buf, tmp);
***REMOVED***

static const char *errstr(int success) ***REMOVED*** return success ? "OK" : "ERROR"; ***REMOVED***

static void adderr(gpr_strvec *buf, int success) ***REMOVED***
  char *tmp;
  gpr_asprintf(&tmp, " %s", errstr(success));
  gpr_strvec_add(buf, tmp);
***REMOVED***

char *grpc_event_string(grpc_event *ev) ***REMOVED***
  char *out;
  gpr_strvec buf;
