...
typedef struct census_log_block_list_struct ***REMOVED***
  struct census_log_block_list_struct *next;
  struct census_log_block_list_struct *prev;
  struct census_log_block *block;
***REMOVED*** cl_block_list_struct;

typedef struct census_log_block ***REMOVED***
...
typedef struct census_log_core_local_block ***REMOVED***
  gpr_atm block;
...
#endif
***REMOVED*** cl_core_local_block;

struct census_log ***REMOVED***
...
static int cl_try_lock(gpr_atm *lock) ***REMOVED*** return gpr_atm_acq_cas(lock, 0, 1); ***REMOVED***

static void cl_unlock(gpr_atm *lock) ***REMOVED*** gpr_atm_rel_store(lock, 0); ***REMOVED***

/* Functions that operate on cl_core_local_block's */

static void cl_core_local_block_set_block(cl_core_local_block *clb,
                                          cl_block *block) ***REMOVED***
  gpr_atm_rel_store(&clb->block, (gpr_atm)block);
***REMOVED***

static cl_block *cl_core_local_block_get_block(cl_core_local_block *clb) ***REMOVED***
...
static void cl_block_list_initialize(cl_block_list *list) ***REMOVED***
  list->count = 0;
  cl_block_list_struct_initialize(&list->ht, NULL);
***REMOVED***

/* Returns head of *this, or NULL if empty. */
static cl_block *cl_block_list_head(cl_block_list *list) ***REMOVED***
...
static void cl_block_list_insert(cl_block_list *list, cl_block_list_struct *pos,
                                 cl_block_list_struct *e) ***REMOVED***
  list->count++;
  e->next = pos->next;
  e->prev = pos;
  e->next->prev = e;
...
static void cl_block_list_insert_at_head(cl_block_list *list, cl_block *block) ***REMOVED***
  cl_block_list_insert(list, &list->ht, &block->link);
***REMOVED***

/* Insert block at the tail of the list */
static void cl_block_list_insert_at_tail(cl_block_list *list, cl_block *block) ***REMOVED***
  cl_block_list_insert(list, list->ht.prev, &block->link);
***REMOVED***

/* Removes block *b. Requires *b be in the list. */
static void cl_block_list_remove(cl_block_list *list, cl_block *b) ***REMOVED***
  list->count--;
...
static void cl_block_initialize(cl_block *block, char *buffer) ***REMOVED***
  block->buffer = buffer;
...
static void cl_block_set_bytes_committed(cl_block *block,
                                         int32_t bytes_committed) ***REMOVED***
  gpr_atm_rel_store(&block->bytes_committed, bytes_committed);
***REMOVED***

static int32_t cl_block_get_bytes_committed(cl_block *block) ***REMOVED***
...
static int cl_block_try_disable_access(cl_block *block, int discard_data) ***REMOVED***
  if (!cl_try_lock(&block->writer_lock)) ***REMOVED***
    return 0;
  ***REMOVED***
  if (!cl_try_lock(&block->reader_lock)) ***REMOVED***
    cl_unlock(&block->writer_lock);
    return 0;
  ***REMOVED***
  if (!discard_data &&
      (block->bytes_read != cl_block_get_bytes_committed(block))) ***REMOVED***
...
static void cl_block_enable_access(cl_block *block) ***REMOVED***
  cl_unlock(&block->reader_lock);
  cl_unlock(&block->writer_lock);
***REMOVED***

/* Returns with writer_lock held. */
static void *cl_block_start_write(cl_block *block, size_t size) ***REMOVED***
  int32_t bytes_committed;
  if (!cl_try_lock(&block->writer_lock)) ***REMOVED***
    return NULL;
  ***REMOVED***
  bytes_committed = cl_block_get_bytes_committed(block);
  if (bytes_committed + size > CENSUS_LOG_MAX_RECORD_SIZE) ***REMOVED***
    cl_unlock(&block->writer_lock);
    return NULL;
  ***REMOVED***
...
static void cl_block_end_write(cl_block *block, size_t bytes_written) ***REMOVED***
...
/* Returns a pointer to the first unread byte in buffer. The number of bytes
   ...
   - read in progress
   - no data available */
static void *cl_block_start_read(cl_block *block, size_t *bytes_available) ***REMOVED***
  void *record;
  if (!cl_try_lock(&block->reader_lock)) ***REMOVED***
    return NULL;
  ***REMOVED***
...
static void cl_block_end_read(cl_block *block) ***REMOVED***
  cl_unlock(&block->reader_lock);
***REMOVED***
...
static cl_block *cl_allocate_block(void) ***REMOVED***
  ...
  if (block != NULL) ***REMOVED***
    ...
    return block;
  ***REMOVED***
  if (!g_log.discard_old_records) ***REMOVED***
  ...
  ***REMOVED***
  ...
  for (block = cl_block_list_head(&g_log.dirty_block_list); block != NULL;
       block = block->link.next->block) ***REMOVED***
    if (cl_block_try_disable_access(block, 1 /* discard data */)) ***REMOVED***
...
static int cl_allocate_core_local_block(int32_t core_id, cl_block *old_block) ***REMOVED***
  ...
  if ((block != NULL) && (block != old_block)) ***REMOVED***
    return 1;
  ***REMOVED***
  if (block != NULL) ***REMOVED***
...
static cl_block *cl_get_block(void *record) ***REMOVED***
...
static cl_block *cl_next_block_to_read(cl_block *prev) ***REMOVED***
  cl_block *block = NULL;
  if (g_log.read_iterator_state == g_log.num_cores) ***REMOVED***
    ...
    if (prev != NULL) ***REMOVED***
      ...
      if (cl_block_try_disable_access(prev, 0 /* do not discard data */)) ***REMOVED***
  ...
  ***REMOVED***
  while (g_log.read_iterator_state > 0) ***REMOVED***
...
void census_log_initialize(size_t size_in_mb, int discard_old_records) ***REMOVED***
  int32_t ix;
  ...
  for (ix = 0; ix < g_log.num_blocks; ++ix) ***REMOVED***
...
void census_log_shutdown(void) ***REMOVED***
  GPR_ASSERT(g_log.initialized);
...
void *census_log_start_write(size_t size) ***REMOVED***
  ...
  GPR_ASSERT(g_log.initialized);
  if (size > CENSUS_LOG_MAX_RECORD_SIZE) ***REMOVED***
    return NULL;
  ***REMOVED***
  do ***REMOVED***
    int allocated;
    ...
    if (block && (record = cl_block_start_write(block, size))) ***REMOVED***
      return record;
    ***REMOVED***
    ...
    if (gpr_atm_acq_load(&g_log.is_full)) ***REMOVED***
    ...
    ***REMOVED***
    ...
    if (!allocated) ***REMOVED***
      gpr_atm_no_barrier_fetch_add(&g_log.out_of_space_count, 1);
      return NULL;
    ***REMOVED***
  ***REMOVED*** while (attempts_remaining--);
  /* Give up. */
  gpr_atm_no_barrier_fetch_add(&g_log.out_of_space_count, 1);
  return NULL;
***REMOVED***

void census_log_end_write(void *record, size_t bytes_written) ***REMOVED***
  GPR_ASSERT(g_log.initialized);
  cl_block_end_write(cl_get_block(record), bytes_written);
***REMOVED***

void census_log_init_reader(void) ***REMOVED***
  GPR_ASSERT(g_log.initialized);
  gpr_mu_lock(&g_log.lock);
  /* If a block is locked for reading unlock it. */
  if (g_log.block_being_read != NULL) ***REMOVED***
...
const void *census_log_read_next(size_t *bytes_available) ***REMOVED***
  GPR_ASSERT(g_log.initialized);
  gpr_mu_lock(&g_log.lock);
  if (g_log.block_being_read != NULL) ***REMOVED***
    cl_block_end_read(g_log.block_being_read);
  ***REMOVED***
  do ***REMOVED***
    ...
    if (g_log.block_being_read != NULL) ***REMOVED***
      ...
      if (record != NULL) ***REMOVED***
        gpr_mu_unlock(&g_log.lock);
        return record;
      ***REMOVED***
    ***REMOVED***
  ***REMOVED*** while (g_log.block_being_read != NULL);
  gpr_mu_unlock(&g_log.lock);
  return NULL;
***REMOVED***

size_t census_log_remaining_space(void) ***REMOVED***
  size_t space;
  GPR_ASSERT(g_log.initialized);
  gpr_mu_lock(&g_log.lock);
  if (g_log.discard_old_records) ***REMOVED***
...
int census_log_out_of_space_count(void) ***REMOVED***
  GPR_ASSERT(g_log.initialized);
