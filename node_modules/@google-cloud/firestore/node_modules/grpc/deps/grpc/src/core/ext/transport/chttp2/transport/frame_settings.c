...
static uint8_t *fill_header(uint8_t *out, uint32_t length, uint8_t flags) ***REMOVED***
...
grpc_slice grpc_chttp2_settings_create(uint32_t *old_settings,
                                       ...
                                       uint32_t force_mask, size_t count) ***REMOVED***
  size_t i;
  uint32_t n = 0;
  grpc_slice output;
  uint8_t *p;
  ...
  ***REMOVED***
  ...
  for (i = 0; i < count; i++) ***REMOVED***
    if (new_settings[i] != old_settings[i] || (force_mask & (1u << i)) != 0) ***REMOVED***
...
grpc_slice grpc_chttp2_settings_ack_create(void) ***REMOVED***
...
grpc_error *grpc_chttp2_settings_parser_begin_frame(
    ...
    uint32_t *settings) ***REMOVED***
  parser->target_settings = settings;
  memcpy(parser->incoming_settings, settings,
         GRPC_CHTTP2_NUM_SETTINGS * sizeof(uint32_t));
  parser->is_ack = 0;
  parser->state = GRPC_CHTTP2_SPS_ID0;
  if (flags == GRPC_CHTTP2_FLAG_ACK) ***REMOVED***
    parser->is_ack = 1;
    if (length != 0) ***REMOVED***
  ...
  ***REMOVED*** else if (flags != 0) ***REMOVED***
    return GRPC_ERROR_CREATE_FROM_STATIC_STRING(
        "invalid flags on settings frame");
  ***REMOVED*** else if (length % 6 != 0) ***REMOVED***
    return GRPC_ERROR_CREATE_FROM_STATIC_STRING(
    ...
    switch (parser->state) ***REMOVED***
      ...
      case GRPC_CHTTP2_SPS_VAL3:
        ...
        ***REMOVED***
        ...
        if (grpc_wire_id_to_setting_id(parser->id, &id)) ***REMOVED***
          const grpc_chttp2_setting_parameters *sp =
              &grpc_chttp2_settings_parameters[id];
          if (parser->value < sp->min_value || parser->value > sp->max_value) ***REMOVED***
            switch (sp->invalid_value_behavior) ***REMOVED***
          ...
          ***REMOVED***
          ...
          if (GRPC_TRACER_ON(grpc_http_trace)) ***REMOVED***
            gpr_log(GPR_DEBUG, "CHTTP2:%s:%s: got setting %s = %d",
                    ...
                    parser->value);
          ***REMOVED***
        ***REMOVED*** else if (GRPC_TRACER_ON(grpc_http_trace)) ***REMOVED***
