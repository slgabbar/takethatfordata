...
void grpc_use_signal(int signum) ***REMOVED***
  grpc_wakeup_signal = signum;
  is_grpc_wakeup_signal_initialized = true;

  if (grpc_wakeup_signal < 0) ***REMOVED***
    gpr_log(GPR_INFO,
...
***REMOVED*** poll_obj_type;

typedef struct poll_obj ***REMOVED***
#ifndef NDEBUG
...
const char *poll_obj_string(poll_obj_type po_type) ***REMOVED***
  switch (po_type) ***REMOVED***
    case POLL_OBJ_FD:
      return "fd";
...
typedef struct polling_island ***REMOVED***
  gpr_mu mu;
...
static void pi_add_ref(polling_island *pi) ***REMOVED***
  gpr_atm_no_barrier_fetch_add(&pi->ref_count, 1);
***REMOVED***

static void pi_unref(grpc_exec_ctx *exec_ctx, polling_island *pi) ***REMOVED***
   ...
   */
  if (1 == gpr_atm_full_fetch_add(&pi->ref_count, -1)) ***REMOVED***
...
static void polling_island_add_fds_locked(polling_island *pi, grpc_fd **fds,
  ...
  for (i = 0; i < fd_count; i++) ***REMOVED***
    ev.events = (uint32_t)(EPOLLIN | EPOLLOUT | EPOLLET);
    ev.data.ptr = fds[i];
    err = epoll_ctl(pi->epoll_fd, EPOLL_CTL_ADD, fds[i]->fd, &ev);

    if (err < 0) ***REMOVED***
      if (errno != EEXIST) ***REMOVED***
        gpr_asprintf(
            &err_msg,
    ...
    ***REMOVED***
    ...
    if (pi->fd_cnt == pi->fd_capacity) ***REMOVED***
...
static void polling_island_remove_all_fds_locked(polling_island *pi,
  ...
  for (i = 0; i < pi->fd_cnt; i++) ***REMOVED***
    err = epoll_ctl(pi->epoll_fd, EPOLL_CTL_DEL, pi->fds[i]->fd, NULL);
    if (err < 0 && errno != ENOENT) ***REMOVED***
      gpr_asprintf(&err_msg,
...
static void polling_island_delete(grpc_exec_ctx *exec_ctx, polling_island *pi) ***REMOVED***
...
static polling_island *polling_island_maybe_get_latest(polling_island *pi) ***REMOVED***
  polling_island *next = (polling_island *)gpr_atm_acq_load(&pi->merged_to);
...
static polling_island *polling_island_lock(polling_island *pi) ***REMOVED***
...
*/
static void polling_island_lock_pair(polling_island **p, polling_island **q) ***REMOVED***
  ...
  while (true) ***REMOVED***
    ...
    while (next_1 != NULL) ***REMOVED***
      pi_1 = next_1;
      next_1 = (polling_island *)gpr_atm_acq_load(&pi_1->merged_to);
    ***REMOVED***

    next_2 = (polling_island *)gpr_atm_acq_load(&pi_2->merged_to);
    while (next_2 != NULL) ***REMOVED***
      pi_2 = next_2;
      next_2 = (polling_island *)gpr_atm_acq_load(&pi_2->merged_to);
    ***REMOVED***

    if (pi_1 == pi_2) ***REMOVED***
    ...
    ***REMOVED***
    ...
    next_1 = (polling_island *)gpr_atm_acq_load(&pi_1->merged_to);
    next_2 = (polling_island *)gpr_atm_acq_load(&pi_2->merged_to);
    if (next_1 == NULL && next_2 == NULL) ***REMOVED***
      break;
    ***REMOVED***
...
static void polling_island_unlock_pair(polling_island *p, polling_island *q) ***REMOVED***
  if (p == q) ***REMOVED***
    gpr_mu_unlock(&p->mu);
  ***REMOVED*** else ***REMOVED***
    gpr_mu_unlock(&p->mu);
...
static grpc_error *polling_island_global_init() ***REMOVED***
...
static void polling_island_global_shutdown() ***REMOVED***
...
static void sig_handler(int sig_num) ***REMOVED***
#ifdef GRPC_EPOLL_DEBUG
  gpr_log(GPR_INFO, "Received signal %d", sig_num);
#endif
***REMOVED***

static void poller_kick_init() ***REMOVED*** signal(grpc_wakeup_signal, sig_handler); ***REMOVED***
...
static grpc_error *pollset_worker_kick(grpc_pollset_worker *worker) ***REMOVED***
  grpc_error *err = GRPC_ERROR_NONE;

  /* Kick the worker only if it was not already kicked */
  if (gpr_atm_no_barrier_cas(&worker->is_kicked, (gpr_atm)0, (gpr_atm)1)) ***REMOVED***
    GRPC_POLLING_TRACE(
        "pollset_worker_kick: Kicking worker: %p (thread id: %ld)",
        (void *)worker, (long int)worker->pt_id);
    int err_num = pthread_kill(worker->pt_id, grpc_wakeup_signal);
    if (err_num != 0) ***REMOVED***
 ...
 * be locked) */
static int pollset_has_workers(grpc_pollset *p) ***REMOVED***
...
static grpc_error *pollset_kick(grpc_exec_ctx *exec_ctx, grpc_pollset *p,
  ...
  if (worker != NULL) ***REMOVED***
    if (worker == GRPC_POLLSET_KICK_BROADCAST) ***REMOVED***
      if (pollset_has_workers(p)) ***REMOVED***
        GPR_TIMER_BEGIN("pollset_kick.broadcast", 0);
        for (worker = p->root_worker.next; worker != &p->root_worker;
             worker = worker->next) ***REMOVED***
          if (gpr_tls_get(&g_current_thread_worker) != (intptr_t)worker) ***REMOVED***
    ...
    ***REMOVED*** else ***REMOVED***
      ...
      ***REMOVED***
    ***REMOVED***
  ***REMOVED*** else if (gpr_tls_get(&g_current_thread_pollset) != (intptr_t)p) ***REMOVED***
...
static void pollset_work_and_unlock(grpc_exec_ctx *exec_ctx,
                                    ...
                                    sigset_t *sig_mask, grpc_error **error) ***REMOVED***
  ...
  ***REMOVED***
  ...
  for (int i = 0; i < ep_rv; ++i) ***REMOVED***
    void *data_ptr = ep_ev[i].data.ptr;
    if (data_ptr == &polling_island_wakeup_fd) ***REMOVED***
      GRPC_POLLING_TRACE(
...
static grpc_error *pollset_work(grpc_exec_ctx *exec_ctx, grpc_pollset *pollset,
  ...
  ***REMOVED*** else if (!pollset->shutting_down) ***REMOVED***
    ...
    if (!g_initialized_sigmask) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (pollset->shutting_down && !pollset_has_workers(pollset) &&
      !pollset->finish_shutdown_called) ***REMOVED***
...
static void add_poll_object(grpc_exec_ctx *exec_ctx, poll_obj *bag,
                            ...
                            poll_obj_type item_type) ***REMOVED***
...
retry:
   ...
   * orphaned */
  if (item_type == POLL_OBJ_FD && (FD_FROM_PO(item))->orphaned) ***REMOVED***
    gpr_mu_unlock(&item->mu);
    gpr_mu_unlock(&bag->mu);
    return;
  ***REMOVED***

  if (item->pi == bag->pi) ***REMOVED***
    pi_new = item->pi;
    if (pi_new == NULL) ***REMOVED***
      ...
      if (item_type == POLL_OBJ_FD) ***REMOVED***
    ...
    ***REMOVED***
  ***REMOVED*** else if (item->pi == NULL) ***REMOVED***
    ...
    ***REMOVED***
    ...
    GRPC_POLLING_TRACE(
        ...
        (void *)pi_new, poll_obj_string(item_type), (void *)item,
        poll_obj_string(bag_type), (void *)bag);
  ***REMOVED*** else if (bag->pi == NULL) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (bag->pi != pi_new) ***REMOVED***
    PI_ADD_REF(pi_new, poll_obj_string(bag_type));
    if (bag->pi != NULL) ***REMOVED***
 ...
 * */
void *grpc_fd_get_polling_island(grpc_fd *fd) ***REMOVED***
  polling_island *pi;
...
void *grpc_pollset_get_polling_island(grpc_pollset *ps) ***REMOVED***
  polling_island *pi;
...
bool grpc_are_polling_islands_equal(void *p, void *q) ***REMOVED***
...
static bool is_epoll_available() ***REMOVED***
...
const grpc_event_engine_vtable *grpc_init_epollsig_linux(
    bool explicit_request) ***REMOVED***
  /* If use of signals is disabled, we cannot use epoll engine*/
  if (is_grpc_wakeup_signal_initialized && grpc_wakeup_signal < 0) ***REMOVED***
    return NULL;
  ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (!is_epoll_available()) ***REMOVED***
    return NULL;
  ***REMOVED***

  if (!is_grpc_wakeup_signal_initialized) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (!GRPC_LOG_IF_ERROR("polling_island_global_init",
                         polling_island_global_init())) ***REMOVED***
    return NULL;
  ***REMOVED***
...
void grpc_use_signal(int signum) ***REMOVED******REMOVED***
