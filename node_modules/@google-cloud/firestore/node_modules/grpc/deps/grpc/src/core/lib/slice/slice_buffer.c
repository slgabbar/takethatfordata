...
static void maybe_embiggen(grpc_slice_buffer *sb) ***REMOVED***
  /* How far away from sb->base_slices is sb->slices pointer */
  size_t slice_offset = (size_t)(sb->slices - sb->base_slices);
  size_t slice_count = sb->count + slice_offset;

  if (slice_count == sb->capacity) ***REMOVED***
    if (sb->base_slices != sb->slices) ***REMOVED***
    ...
    ***REMOVED*** else ***REMOVED***
      ...
      if (sb->base_slices == sb->inlined) ***REMOVED***
        sb->base_slices =
...
void grpc_slice_buffer_init(grpc_slice_buffer *sb) ***REMOVED***
  sb->count = 0;
  sb->length = 0;
...
void grpc_slice_buffer_destroy(grpc_slice_buffer *sb) ***REMOVED***
  grpc_exec_ctx exec_ctx = GRPC_EXEC_CTX_INIT;
  grpc_slice_buffer_destroy_internal(&exec_ctx, sb);
  grpc_exec_ctx_finish(&exec_ctx);
***REMOVED***

uint8_t *grpc_slice_buffer_tiny_add(grpc_slice_buffer *sb, size_t n) ***REMOVED***
  grpc_slice *back;
  uint8_t *out;
...
size_t grpc_slice_buffer_add_indexed(grpc_slice_buffer *sb, grpc_slice s) ***REMOVED***
  size_t out = sb->count;
  maybe_embiggen(sb);
  sb->slices[out] = s;
...
void grpc_slice_buffer_add(grpc_slice_buffer *sb, grpc_slice s) ***REMOVED***
  size_t n = sb->count;
  /* if both the last slice in the slice buffer and the slice being added
     ...
     writes */
  if (!s.refcount && n) ***REMOVED***
    grpc_slice *back = &sb->slices[n - 1];
    if (!back->refcount &&
        back->data.inlined.length < GRPC_SLICE_INLINED_SIZE) ***REMOVED***
      if (s.data.inlined.length + back->data.inlined.length <=
          GRPC_SLICE_INLINED_SIZE) ***REMOVED***
...
void grpc_slice_buffer_addn(grpc_slice_buffer *sb, grpc_slice *s, size_t n) ***REMOVED***
  size_t i;
...
void grpc_slice_buffer_pop(grpc_slice_buffer *sb) ***REMOVED***
  if (sb->count != 0) ***REMOVED***
...
void grpc_slice_buffer_reset_and_unref(grpc_slice_buffer *sb) ***REMOVED***
  grpc_exec_ctx exec_ctx = GRPC_EXEC_CTX_INIT;
  grpc_slice_buffer_reset_and_unref_internal(&exec_ctx, sb);
  grpc_exec_ctx_finish(&exec_ctx);
***REMOVED***

void grpc_slice_buffer_swap(grpc_slice_buffer *a, grpc_slice_buffer *b) ***REMOVED***
  ...
  if (a->base_slices == a->inlined) ***REMOVED***
    if (b->base_slices == b->inlined) ***REMOVED***
    ...
    ***REMOVED***
  ***REMOVED*** else if (b->base_slices == b->inlined) ***REMOVED***
...
void grpc_slice_buffer_move_into(grpc_slice_buffer *src,
                                 grpc_slice_buffer *dst) ***REMOVED***
  /* anything to move? */
  if (src->count == 0) ***REMOVED***
    return;
  ***REMOVED***
  /* anything in dst? */
  if (dst->count == 0) ***REMOVED***
...
void grpc_slice_buffer_trim_end(grpc_slice_buffer *sb, size_t n,
                                grpc_slice_buffer *garbage) ***REMOVED***
...
grpc_slice grpc_slice_buffer_take_first(grpc_slice_buffer *sb) ***REMOVED***
  grpc_slice slice;
