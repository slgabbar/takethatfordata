...
static grpc_timer *g_timer_ht[NUM_HASH_BUCKETS] = ***REMOVED***NULL***REMOVED***;

static void init_timer_ht() ***REMOVED***
  for (int i = 0; i < NUM_HASH_BUCKETS; i++) ***REMOVED***
    gpr_mu_init(&g_hash_mu[i]);
  ***REMOVED***
***REMOVED***

static bool is_in_ht(grpc_timer *t) ***REMOVED***
  size_t i = GPR_HASH_POINTER(t, NUM_HASH_BUCKETS);

  gpr_mu_lock(&g_hash_mu[i]);
  grpc_timer *p = g_timer_ht[i];
  while (p != NULL && p != t) ***REMOVED***
    p = p->hash_table_next;
  ***REMOVED***
...
static void add_to_ht(grpc_timer *t) ***REMOVED***
...
static void remove_from_ht(grpc_timer *t) ***REMOVED***
  size_t i = GPR_HASH_POINTER(t, NUM_HASH_BUCKETS);
  bool removed = false;

  gpr_mu_lock(&g_hash_mu[i]);
  if (g_timer_ht[i] == t) ***REMOVED***
    g_timer_ht[i] = g_timer_ht[i]->hash_table_next;
    removed = true;
  ***REMOVED*** else if (g_timer_ht[i] != NULL) ***REMOVED***
    grpc_timer *p = g_timer_ht[i];
    while (p->hash_table_next != NULL && p->hash_table_next != t) ***REMOVED***
      p = p->hash_table_next;
    ***REMOVED***

    if (p->hash_table_next == t) ***REMOVED***
...
static void validate_non_pending_timer(grpc_timer *t) ***REMOVED***
  if (!t->pending && is_in_ht(t)) ***REMOVED***
    grpc_closure *c = t->closure;
    gpr_log(GPR_ERROR,
...
struct shared_mutables ***REMOVED***
  ...
  gpr_mu mu;
***REMOVED*** GPR_ALIGN_STRUCT(GPR_CACHELINE_SIZE);
...
static gpr_atm saturating_add(gpr_atm a, gpr_atm b) ***REMOVED***
  if (a > GPR_ATM_MAX - b) ***REMOVED***
    return GPR_ATM_MAX;
  ***REMOVED***
  return a + b;
***REMOVED***
...
static gpr_timespec dbl_to_ts(double d) ***REMOVED***
  gpr_timespec ts;
  ts.tv_sec = (int64_t)d;
...
static gpr_atm timespec_to_atm_round_up(gpr_timespec ts) ***REMOVED***
...
static gpr_atm timespec_to_atm_round_down(gpr_timespec ts) ***REMOVED***
  ts = gpr_time_sub(ts, g_start_time);
  double x =
...
static gpr_timespec atm_to_timespec(gpr_atm x) ***REMOVED***
  return gpr_time_add(g_start_time, dbl_to_ts((double)x / 1000.0));
***REMOVED***

static gpr_atm compute_min_deadline(timer_shard *shard) ***REMOVED***
  return grpc_timer_heap_is_empty(&shard->heap)
             ? saturating_add(shard->queue_deadline_cap, 1)
             : grpc_timer_heap_top(&shard->heap)->deadline;
***REMOVED***

void grpc_timer_list_init(gpr_timespec now) ***REMOVED***
  uint32_t i;
  ...
  for (i = 0; i < NUM_SHARDS; i++) ***REMOVED***
...
void grpc_timer_list_shutdown(grpc_exec_ctx *exec_ctx) ***REMOVED***
  int i;
...
static double ts_to_dbl(gpr_timespec ts) ***REMOVED***
  return (double)ts.tv_sec + 1e-9 * ts.tv_nsec;
***REMOVED***

/* returns true if the first element in the list */
static void list_join(grpc_timer *head, grpc_timer *timer) ***REMOVED***
  timer->next = head;
  timer->prev = head->prev;
  timer->next->prev = timer->prev->next = timer;
***REMOVED***

static void list_remove(grpc_timer *timer) ***REMOVED***
  timer->next->prev = timer->prev;
  timer->prev->next = timer->next;
***REMOVED***

static void swap_adjacent_shards_in_queue(uint32_t first_shard_queue_index) ***REMOVED***
  timer_shard *temp;
...
static void note_deadline_change(timer_shard *shard) ***REMOVED***
  while (shard->shard_queue_index > 0 &&
         shard->min_deadline <
             g_shard_queue[shard->shard_queue_index - 1]->min_deadline) ***REMOVED***
...
void grpc_timer_init_unset(grpc_timer *timer) ***REMOVED*** timer->pending = false; ***REMOVED***
...
void grpc_timer_init(grpc_exec_ctx *exec_ctx, grpc_timer *timer,
  ...
  if (GRPC_TRACER_ON(grpc_timer_trace)) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (!g_shared_mutables.initialized) ***REMOVED***
    timer->pending = false;
  ...
  ***REMOVED***
  ...
  if (deadline_atm < shard->queue_deadline_cap) ***REMOVED***
...
void grpc_timer_consume_kick(void) ***REMOVED***
  /* force re-evaluation of last seeen min */
  gpr_tls_set(&g_last_seen_min_timer, 0);
***REMOVED***

void grpc_timer_cancel(grpc_exec_ctx *exec_ctx, grpc_timer *timer) ***REMOVED***
  if (!g_shared_mutables.initialized) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (timer->pending) ***REMOVED***
    REMOVE_FROM_HASH_TABLE(timer);

    GRPC_CLOSURE_SCHED(exec_ctx, timer->closure, GRPC_ERROR_CANCELLED);
    timer->pending = false;
    if (timer->heap_index == INVALID_HEAP_INDEX) ***REMOVED***
      list_remove(timer);
    ***REMOVED*** else ***REMOVED***
...
static int refill_heap(timer_shard *shard, gpr_atm now) ***REMOVED***
  ...
  if (GRPC_TRACER_ON(grpc_timer_check_trace)) ***REMOVED***
    gpr_log(GPR_DEBUG, "  .. shard[%d]->queue_deadline_cap --> %" PRIdPTR,
            (int)(shard - g_shards), shard->queue_deadline_cap);
  ***REMOVED***
  for (timer = shard->list.next; timer != &shard->list; timer = next) ***REMOVED***
    next = timer->next;

    if (timer->deadline < shard->queue_deadline_cap) ***REMOVED***
      if (GRPC_TRACER_ON(grpc_timer_check_trace)) ***REMOVED***
...
/* This pops the next non-cancelled timer with deadline <= now from the
   ...
   REQUIRES: shard->mu locked */
static grpc_timer *pop_one(timer_shard *shard, gpr_atm now) ***REMOVED***
  grpc_timer *timer;
  for (;;) ***REMOVED***
    if (GRPC_TRACER_ON(grpc_timer_check_trace)) ***REMOVED***
    ...
    ***REMOVED***
    if (grpc_timer_heap_is_empty(&shard->heap)) ***REMOVED***
...
static size_t pop_timers(grpc_exec_ctx *exec_ctx, timer_shard *shard,
  ...
  grpc_timer *timer;
  gpr_mu_lock(&shard->mu);
  while ((timer = pop_one(shard, now))) ***REMOVED***
    REMOVE_FROM_HASH_TABLE(timer);
...
static grpc_timer_check_result run_some_expired_timers(grpc_exec_ctx *exec_ctx,
  ...
  if (now < min_timer) ***REMOVED***
    if (next != NULL) *next = GPR_MIN(*next, min_timer);
    return GRPC_TIMERS_CHECKED_AND_EMPTY;
  ***REMOVED***

  if (gpr_spinlock_trylock(&g_shared_mutables.checker_mu)) ***REMOVED***
    gpr_mu_lock(&g_shared_mutables.mu);
    ...
    ***REMOVED***
    ...
    while (g_shard_queue[0]->min_deadline < now ||
           (now != GPR_ATM_MAX && g_shard_queue[0]->min_deadline == now)) ***REMOVED***
...
grpc_timer_check_result grpc_timer_check(grpc_exec_ctx *exec_ctx,
                                         gpr_timespec now, gpr_timespec *next) ***REMOVED***
  // prelude
  GPR_ASSERT(now.clock_type == g_clock_type);
