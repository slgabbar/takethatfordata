...
const uint8_t grpc_url_percent_encoding_unreserved_bytes[256 / 8] = ***REMOVED***
    0x00, 0x00, 0x00, 0x00, 0x00, 0x60, 0xff, 0x03, 0xfe, 0xff, 0xff,
    0x87, 0xfe, 0xff, 0xff, 0x47, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00***REMOVED***;
const uint8_t grpc_compatible_percent_encoding_unreserved_bytes[256 / 8] = ***REMOVED***
    0x00, 0x00, 0x00, 0x00, 0xdf, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0x7f, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00***REMOVED***;

static bool is_unreserved_character(uint8_t c,
                                    const uint8_t *unreserved_bytes) ***REMOVED***
...
grpc_slice grpc_percent_encode_slice(grpc_slice slice,
  ...
  for (p = slice_start; p < slice_end; p++) ***REMOVED***
  ...
  ***REMOVED***
  ...
  for (p = slice_start; p < slice_end; p++) ***REMOVED***
    if (is_unreserved_character(*p, unreserved_bytes)) ***REMOVED***
      *q++ = *p;
...
static bool valid_hex(const uint8_t *p, const uint8_t *end) ***REMOVED***
  if (p >= end) return false;
  return (*p >= '0' && *p <= '9') || (*p >= 'a' && *p <= 'f') ||
         (*p >= 'A' && *p <= 'F');
***REMOVED***

static uint8_t dehex(uint8_t c) ***REMOVED***
...
bool grpc_strict_percent_decode_slice(grpc_slice slice_in,
  ...
  while (p != in_end) ***REMOVED***
    ...
    ***REMOVED*** else if (is_unreserved_character(*p, unreserved_bytes)) ***REMOVED***
      p++;
      out_length++;
    ***REMOVED*** else ***REMOVED***
      return false;
    ***REMOVED***
  ***REMOVED***
  if (!any_percent_encoded_stuff) ***REMOVED***
...
grpc_slice grpc_permissive_percent_decode_slice(grpc_slice slice_in) ***REMOVED***
  const uint8_t *p = GRPC_SLICE_START_PTR(slice_in);
  const uint8_t *in_end = GRPC_SLICE_END_PTR(slice_in);
  size_t out_length = 0;
  bool any_percent_encoded_stuff = false;
  while (p != in_end) ***REMOVED***
    if (*p == '%') ***REMOVED***
      if (!valid_hex(p + 1, in_end) || !valid_hex(p + 2, in_end)) ***REMOVED***
        p++;
        out_length++;
      ***REMOVED*** else ***REMOVED***
        p += 3;
