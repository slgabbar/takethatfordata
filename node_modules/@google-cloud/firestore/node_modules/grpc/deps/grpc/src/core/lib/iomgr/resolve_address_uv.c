...
typedef struct request ***REMOVED***
...
static int retry_named_port_failure(int status, request *r,
                                    uv_getaddrinfo_cb getaddrinfo_cb) ***REMOVED***
  if (status != 0) ***REMOVED***
    // This loop is copied from resolve_address_posix.c
    char *svc[][2] = ***REMOVED******REMOVED***"http", "80"***REMOVED***, ***REMOVED***"https", "443"***REMOVED******REMOVED***;
    for (size_t i = 0; i < GPR_ARRAY_SIZE(svc); i++) ***REMOVED***
      if (strcmp(r->port, svc[i][0]) == 0) ***REMOVED***
        int retry_status;
        ...
        if (retry_status < 0 || getaddrinfo_cb == NULL) ***REMOVED***
...
static grpc_error *handle_addrinfo_result(int status, struct addrinfo *result,
                                          grpc_resolved_addresses **addresses) ***REMOVED***
  ...
  ***REMOVED***
  ...
  (*addresses)->naddrs = 0;
  for (resp = result; resp != NULL; resp = resp->ai_next) ***REMOVED***
    (*addresses)->naddrs++;
  ***REMOVED***
  (*addresses)->addrs =
  ...
  ***REMOVED***
    for (i = 0; i < (*addresses)->naddrs; i++) ***REMOVED***
      char *buf;
...
static grpc_error *blocking_resolve_address_impl(
  ...
  ***REMOVED***
  ...
  request r = ***REMOVED***
      .addresses = addresses, .hints = &hints, .host = host, .port = port***REMOVED***;
...
void grpc_resolved_addresses_destroy(grpc_resolved_addresses *addrs) ***REMOVED***
  if (addrs != NULL) ***REMOVED***
