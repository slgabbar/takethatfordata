...
static void fill_common_header(const grpc_httpcli_request *request,
                               gpr_strvec *buf, bool connection_close) ***REMOVED***
  size_t i;
  ...
  for (i = 0; i < request->http.hdr_count; i++) ***REMOVED***
...
grpc_slice grpc_httpcli_format_get_request(
    const grpc_httpcli_request *request) ***REMOVED***
  gpr_strvec out;
  char *flat;
  size_t flat_len;
...
grpc_slice grpc_httpcli_format_post_request(const grpc_httpcli_request *request,
  ...
  fill_common_header(request, &out, true);
  if (body_bytes) ***REMOVED***
    uint8_t has_content_type = 0;
    for (i = 0; i < request->http.hdr_count; i++) ***REMOVED***
      if (strcmp(request->http.hdrs[i].key, "Content-Type") == 0) ***REMOVED***
        has_content_type = 1;
        break;
      ***REMOVED***
    ***REMOVED***
    if (!has_content_type) ***REMOVED***
