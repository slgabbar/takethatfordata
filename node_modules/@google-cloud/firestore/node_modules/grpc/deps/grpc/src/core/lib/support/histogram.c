...
static size_t bucket_for_unchecked(gpr_histogram *h, double x) ***REMOVED***
  return (size_t)(log(x) * h->one_on_log_multiplier);
***REMOVED***

/* bounds checked version of the above */
static size_t bucket_for(gpr_histogram *h, double x) ***REMOVED***
...
static double bucket_start(gpr_histogram *h, double x) ***REMOVED***
  return pow(h->multiplier, x);
***REMOVED***

gpr_histogram *gpr_histogram_create(double resolution,
                                    double max_bucket_start) ***REMOVED***
...
void gpr_histogram_destroy(gpr_histogram *h) ***REMOVED***
  gpr_free(h->buckets);
  gpr_free(h);
***REMOVED***

void gpr_histogram_add(gpr_histogram *h, double x) ***REMOVED***
  h->sum += x;
  h->sum_of_squares += x * x;
  h->count++;
  if (x < h->min_seen) ***REMOVED***
    h->min_seen = x;
  ***REMOVED***
  if (x > h->max_seen) ***REMOVED***
    h->max_seen = x;
  ***REMOVED***
  h->buckets[bucket_for(h, x)]++;
***REMOVED***

int gpr_histogram_merge(gpr_histogram *dst, const gpr_histogram *src) ***REMOVED***
  if ((dst->num_buckets != src->num_buckets) ||
      (dst->multiplier != src->multiplier)) ***REMOVED***
...
void gpr_histogram_merge_contents(gpr_histogram *dst, const uint32_t *data,
                                  ...
                                  double sum_of_squares, double count) ***REMOVED***
  size_t i;
  GPR_ASSERT(dst->num_buckets == data_count);
  dst->sum += sum;
  dst->sum_of_squares += sum_of_squares;
  dst->count += count;
  if (min_seen < dst->min_seen) ***REMOVED***
    dst->min_seen = min_seen;
  ***REMOVED***
  if (max_seen > dst->max_seen) ***REMOVED***
    dst->max_seen = max_seen;
  ***REMOVED***
  for (i = 0; i < dst->num_buckets; i++) ***REMOVED***
    dst->buckets[i] += data[i];
  ***REMOVED***
***REMOVED***

static double threshold_for_count_below(gpr_histogram *h, double count_below) ***REMOVED***
  double count_so_far;
  double lower_bound;
  double upper_bound;
  size_t lower_idx;
  ...
  if (h->count == 0) ***REMOVED***
    return 0.0;
  ***REMOVED***

  if (count_below <= 0) ***REMOVED***
    return h->min_seen;
  ***REMOVED***
  if (count_below >= h->count) ***REMOVED***
    return h->max_seen;
  ***REMOVED***

  /* find the lowest bucket that gets us above count_below */
  count_so_far = 0.0;
  for (lower_idx = 0; lower_idx < h->num_buckets; lower_idx++) ***REMOVED***
  ...
  ***REMOVED***
  if (count_so_far == count_below) ***REMOVED***
    ...
    for (upper_idx = lower_idx + 1; upper_idx < h->num_buckets; upper_idx++) ***REMOVED***
      if (h->buckets[upper_idx]) ***REMOVED***
        break;
      ***REMOVED***
    ***REMOVED***
...
double gpr_histogram_percentile(gpr_histogram *h, double percentile) ***REMOVED***
  return threshold_for_count_below(h, h->count * percentile / 100.0);
***REMOVED***

double gpr_histogram_mean(gpr_histogram *h) ***REMOVED***
  GPR_ASSERT(h->count != 0);
  return h->sum / h->count;
***REMOVED***

double gpr_histogram_stddev(gpr_histogram *h) ***REMOVED***
  return sqrt(gpr_histogram_variance(h));
***REMOVED***

double gpr_histogram_variance(gpr_histogram *h) ***REMOVED***
  if (h->count == 0) return 0.0;
  return (h->sum_of_squares * h->count - h->sum * h->sum) /
         (h->count * h->count);
***REMOVED***

double gpr_histogram_maximum(gpr_histogram *h) ***REMOVED*** return h->max_seen; ***REMOVED***

double gpr_histogram_minimum(gpr_histogram *h) ***REMOVED*** return h->min_seen; ***REMOVED***

double gpr_histogram_count(gpr_histogram *h) ***REMOVED*** return h->count; ***REMOVED***

double gpr_histogram_sum(gpr_histogram *h) ***REMOVED*** return h->sum; ***REMOVED***

double gpr_histogram_sum_of_squares(gpr_histogram *h) ***REMOVED***
  return h->sum_of_squares;
***REMOVED***

const uint32_t *gpr_histogram_get_contents(gpr_histogram *h, size_t *size) ***REMOVED***
  *size = h->num_buckets;
  return h->buckets;
***REMOVED***
