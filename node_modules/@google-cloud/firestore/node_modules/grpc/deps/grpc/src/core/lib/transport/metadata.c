 ...
 * Static instances are declared in static_metadata.***REMOVED***h,c***REMOVED*** and
...
typedef struct interned_metadata ***REMOVED***
...
***REMOVED*** interned_metadata;

/* Shadow structure for grpc_mdelem_data for allocated elements */
typedef struct allocated_metadata ***REMOVED***
  /* must be byte compatible with grpc_mdelem_data */
  grpc_slice key;
  grpc_slice value;
  ...
  /* private only data */
  gpr_atm refcnt;
***REMOVED*** allocated_metadata;

typedef struct mdtab_shard ***REMOVED***
  gpr_mu mu;
...
void grpc_mdctx_global_init(void) ***REMOVED***
  /* initialize shards */
...
void grpc_mdctx_global_shutdown(grpc_exec_ctx *exec_ctx) ***REMOVED***
  for (size_t i = 0; i < SHARD_COUNT; i++) ***REMOVED***
    mdtab_shard *shard = &g_shards[i];
...
static int is_mdelem_static(grpc_mdelem e) ***REMOVED***
...
static void ref_md_locked(mdtab_shard *shard,
                          interned_metadata *md DEBUG_ARGS) ***REMOVED***
#ifndef NDEBUG
  if (GRPC_TRACER_ON(grpc_trace_metadata)) ***REMOVED***
  ...
  ***REMOVED***
#endif
  if (0 == gpr_atm_no_barrier_fetch_add(&md->refcnt, 1)) ***REMOVED***
    gpr_atm_no_barrier_fetch_add(&shard->free_estimate, -1);
  ***REMOVED***
***REMOVED***

static void gc_mdtab(grpc_exec_ctx *exec_ctx, mdtab_shard *shard) ***REMOVED***
  size_t i;
  ...
  for (i = 0; i < shard->capacity; i++) ***REMOVED***
    ...
    for (md = shard->elems[i]; md; md = next) ***REMOVED***
      void *user_data = (void *)gpr_atm_no_barrier_load(&md->user_data);
      next = md->bucket_next;
      if (gpr_atm_acq_load(&md->refcnt) == 0) ***REMOVED***
        grpc_slice_unref_internal(exec_ctx, md->key);
        grpc_slice_unref_internal(exec_ctx, md->value);
        if (md->user_data) ***REMOVED***
...
static void grow_mdtab(mdtab_shard *shard) ***REMOVED***
...
static void rehash_mdtab(grpc_exec_ctx *exec_ctx, mdtab_shard *shard) ***REMOVED***
  if (gpr_atm_no_barrier_load(&shard->free_estimate) >
      (gpr_atm)(shard->capacity / 4)) ***REMOVED***
    gc_mdtab(exec_ctx, shard);
  ***REMOVED*** else ***REMOVED***
    grow_mdtab(shard);
  ***REMOVED***
...
grpc_mdelem grpc_mdelem_create(
    ...
    grpc_mdelem_data *compatible_external_backing_store) ***REMOVED***
  if (!grpc_slice_is_interned(key) || !grpc_slice_is_interned(value)) ***REMOVED***
    if (compatible_external_backing_store != NULL) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (GRPC_IS_STATIC_METADATA_STRING(key) &&
      GRPC_IS_STATIC_METADATA_STRING(value)) ***REMOVED***
    ...
    if (!GRPC_MDISNULL(static_elem)) ***REMOVED***
      return static_elem;
    ***REMOVED***
  ***REMOVED***
  ...
  for (md = shard->elems[idx]; md; md = md->bucket_next) ***REMOVED***
    if (grpc_slice_eq(key, md->key) && grpc_slice_eq(value, md->value)) ***REMOVED***
...
grpc_mdelem grpc_mdelem_from_grpc_metadata(grpc_exec_ctx *exec_ctx,
                                           grpc_metadata *metadata) ***REMOVED***
  bool changed = false;
...
static size_t get_base64_encoded_size(size_t raw_length) ***REMOVED***
  static const uint8_t tail_xtra[3] = ***REMOVED***0, 2, 3***REMOVED***;
  return raw_length / 3 * 4 + tail_xtra[raw_length % 3];
***REMOVED***

size_t grpc_mdelem_get_size_in_hpack_table(grpc_mdelem elem) ***REMOVED***
...
grpc_mdelem grpc_mdelem_ref(grpc_mdelem gmd DEBUG_ARGS) ***REMOVED***
  switch (GRPC_MDELEM_STORAGE(gmd)) ***REMOVED***
    case GRPC_MDELEM_STORAGE_EXTERNAL:
    case GRPC_MDELEM_STORAGE_STATIC:
      break;
    case GRPC_MDELEM_STORAGE_INTERNED: ***REMOVED***
      ...
      ***REMOVED***
      ...
      break;
    ***REMOVED***
    case GRPC_MDELEM_STORAGE_ALLOCATED: ***REMOVED***
...
void grpc_mdelem_unref(grpc_exec_ctx *exec_ctx, grpc_mdelem gmd DEBUG_ARGS) ***REMOVED***
  switch (GRPC_MDELEM_STORAGE(gmd)) ***REMOVED***
    case GRPC_MDELEM_STORAGE_EXTERNAL:
    case GRPC_MDELEM_STORAGE_STATIC:
      break;
...
void *grpc_mdelem_get_user_data(grpc_mdelem md, void (*destroy_func)(void *)) ***REMOVED***
  switch (GRPC_MDELEM_STORAGE(md)) ***REMOVED***
    case GRPC_MDELEM_STORAGE_EXTERNAL:
    ...
    case GRPC_MDELEM_STORAGE_INTERNED: ***REMOVED***
      ...
      void *result;
      if (gpr_atm_acq_load(&im->destroy_user_data) == (gpr_atm)destroy_func) ***REMOVED***
...
void *grpc_mdelem_set_user_data(grpc_mdelem md, void (*destroy_func)(void *),
  ...
  switch (GRPC_MDELEM_STORAGE(md)) ***REMOVED***
    ...
    case GRPC_MDELEM_STORAGE_INTERNED: ***REMOVED***
      ...
      if (gpr_atm_no_barrier_load(&im->destroy_user_data)) ***REMOVED***
        /* user data can only be set once */
        gpr_mu_unlock(&im->mu_user_data);
        if (destroy_func != NULL) ***REMOVED***
          destroy_func(user_data);
        ***REMOVED***
        return (void *)gpr_atm_no_barrier_load(&im->user_data);
      ***REMOVED***
...
bool grpc_mdelem_eq(grpc_mdelem a, grpc_mdelem b) ***REMOVED***
