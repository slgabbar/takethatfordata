...
grpc_call_error grpc_call_set_credentials(grpc_call *call,
  ...
  GRPC_API_TRACE("grpc_call_set_credentials(call=%p, creds=%p)", 2,
                 (call, creds));
  if (!grpc_call_is_client(call)) ***REMOVED***
...
grpc_auth_context *grpc_call_auth_context(grpc_call *call) ***REMOVED***
...
void grpc_auth_context_release(grpc_auth_context *context) ***REMOVED***
...
grpc_client_security_context *grpc_client_security_context_create(void) ***REMOVED***
  return gpr_zalloc(sizeof(grpc_client_security_context));
***REMOVED***

void grpc_client_security_context_destroy(void *ctx) ***REMOVED***
  grpc_exec_ctx exec_ctx = GRPC_EXEC_CTX_INIT;
  grpc_client_security_context *c = (grpc_client_security_context *)ctx;
  grpc_call_credentials_unref(&exec_ctx, c->creds);
  GRPC_AUTH_CONTEXT_UNREF(c->auth_context, "client_security_context");
  if (c->extension.instance != NULL && c->extension.destroy != NULL) ***REMOVED***
...
grpc_server_security_context *grpc_server_security_context_create(void) ***REMOVED***
  return gpr_zalloc(sizeof(grpc_server_security_context));
***REMOVED***

void grpc_server_security_context_destroy(void *ctx) ***REMOVED***
...
static grpc_auth_property_iterator empty_iterator = ***REMOVED***NULL, 0, NULL***REMOVED***;

grpc_auth_context *grpc_auth_context_create(grpc_auth_context *chained) ***REMOVED***
  grpc_auth_context *ctx = gpr_zalloc(sizeof(grpc_auth_context));
  gpr_ref_init(&ctx->refcount, 1);
  if (chained != NULL) ***REMOVED***
...
grpc_auth_context *grpc_auth_context_ref(grpc_auth_context *ctx,
  ...
  if (GRPC_TRACER_ON(grpc_trace_auth_context_refcount)) ***REMOVED***
  ...
  ***REMOVED***
#else
grpc_auth_context *grpc_auth_context_ref(grpc_auth_context *ctx) ***REMOVED***
  if (ctx == NULL) return NULL;
#endif
...
void grpc_auth_context_unref(grpc_auth_context *ctx, const char *file, int line,
  ...
  ***REMOVED***
#else
void grpc_auth_context_unref(grpc_auth_context *ctx) ***REMOVED***
  if (ctx == NULL) return;
#endif
  if (gpr_unref(&ctx->refcount)) ***REMOVED***
    size_t i;
    GRPC_AUTH_CONTEXT_UNREF(ctx->chained, "chained");
    if (ctx->properties.array != NULL) ***REMOVED***
      for (i = 0; i < ctx->properties.count; i++) ***REMOVED***
...
const char *grpc_auth_context_peer_identity_property_name(
    const grpc_auth_context *ctx) ***REMOVED***
...
int grpc_auth_context_peer_is_authenticated(const grpc_auth_context *ctx) ***REMOVED***
...
const grpc_auth_property *grpc_auth_property_iterator_next(
  ...
  ***REMOVED*** else ***REMOVED***
    while (it->index < it->ctx->properties.count) ***REMOVED***
      ...
      if (strcmp(it->name, prop->name) == 0) ***REMOVED***
        return prop;
      ***REMOVED***
    ***REMOVED***
...
grpc_auth_property_iterator grpc_auth_context_find_properties_by_name(
    const grpc_auth_context *ctx, const char *name) ***REMOVED***
  grpc_auth_property_iterator it = empty_iterator;
...
static void ensure_auth_context_capacity(grpc_auth_context *ctx) ***REMOVED***
  if (ctx->properties.count == ctx->properties.capacity) ***REMOVED***
...
void grpc_auth_context_add_property(grpc_auth_context *ctx, const char *name,
                                    const char *value, size_t value_length) ***REMOVED***
...
void grpc_auth_property_reset(grpc_auth_property *property) ***REMOVED***
  gpr_free(property->name);
  gpr_free(property->value);
  memset(property, 0, sizeof(grpc_auth_property));
***REMOVED***

static void auth_context_pointer_arg_destroy(grpc_exec_ctx *exec_ctx, void *p) ***REMOVED***
  GRPC_AUTH_CONTEXT_UNREF(p, "auth_context_pointer_arg");
***REMOVED***

static void *auth_context_pointer_arg_copy(void *p) ***REMOVED***
  return GRPC_AUTH_CONTEXT_REF(p, "auth_context_pointer_arg");
***REMOVED***

static int auth_context_pointer_cmp(void *a, void *b) ***REMOVED*** return GPR_ICMP(a, b); ***REMOVED***

static const grpc_arg_pointer_vtable auth_context_pointer_vtable = ***REMOVED***
    auth_context_pointer_arg_copy, auth_context_pointer_arg_destroy,
    auth_context_pointer_cmp***REMOVED***;

grpc_arg grpc_auth_context_to_arg(grpc_auth_context *p) ***REMOVED***
  return grpc_channel_arg_pointer_create(GRPC_AUTH_CONTEXT_ARG, p,
                                         &auth_context_pointer_vtable);
***REMOVED***

grpc_auth_context *grpc_auth_context_from_arg(const grpc_arg *arg) ***REMOVED***
