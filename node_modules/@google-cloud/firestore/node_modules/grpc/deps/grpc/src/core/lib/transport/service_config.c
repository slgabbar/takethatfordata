...
grpc_service_config* grpc_service_config_create(const char* json_string) ***REMOVED***
  ...
  if (service_config->json_tree == NULL) ***REMOVED***
...
static char* parse_json_method_name(grpc_json* json) ***REMOVED***
  ...
  for (grpc_json* child = json->child; child != NULL; child = child->next) ***REMOVED***
...
static bool parse_json_method_config(
  ...
  for (grpc_json* child = json->child; child != NULL; child = child->next) ***REMOVED***
    ...
    if (strcmp(child->key, "name") == 0) ***REMOVED***
      ...
      for (grpc_json* name = child->child; name != NULL; name = name->next) ***REMOVED***
...
grpc_slice_hash_table* grpc_service_config_create_method_config_table(
    ...
    void* (*create_value)(const grpc_json* method_config_json),
    void (*destroy_value)(grpc_exec_ctx* exec_ctx, void* value)) ***REMOVED***
  const grpc_json* json = service_config->json_tree;
