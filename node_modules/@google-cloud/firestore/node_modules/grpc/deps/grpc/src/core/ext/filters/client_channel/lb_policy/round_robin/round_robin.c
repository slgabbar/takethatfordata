...
typedef struct round_robin_lb_policy ***REMOVED***
...
***REMOVED*** subchannel_data;

struct rr_subchannel_list ***REMOVED***
...
static rr_subchannel_list *rr_subchannel_list_create(round_robin_lb_policy *p,
                                                     size_t num_subchannels) ***REMOVED***
  rr_subchannel_list *subchannel_list =
  ...
  if (GRPC_TRACER_ON(grpc_lb_round_robin_trace)) ***REMOVED***
...
static void rr_subchannel_list_destroy(grpc_exec_ctx *exec_ctx,
                                       rr_subchannel_list *subchannel_list) ***REMOVED***
...
static void rr_subchannel_list_shutdown_and_unref(
  ...
  ***REMOVED***
  ...
  for (size_t i = 0; i < subchannel_list->num_subchannels; i++) ***REMOVED***
    subchannel_data *sd = &subchannel_list->subchannels[i];
    if (sd->subchannel != NULL) ***REMOVED***  // if subchannel isn't shutdown, unsubscribe.
      if (GRPC_TRACER_ON(grpc_lb_round_robin_trace)) ***REMOVED***
        gpr_log(
            GPR_DEBUG,
...
static size_t get_next_ready_subchannel_index_locked(
    const round_robin_lb_policy *p) ***REMOVED***
  ...
  ***REMOVED***
  for (size_t i = 0; i < p->subchannel_list->num_subchannels; ++i) ***REMOVED***
...
static void update_last_ready_subchannel_index_locked(round_robin_lb_policy *p,
                                                      size_t last_ready_index) ***REMOVED***
...
static void rr_destroy(grpc_exec_ctx *exec_ctx, grpc_lb_policy *pol) ***REMOVED***
...
static void rr_shutdown_locked(grpc_exec_ctx *exec_ctx, grpc_lb_policy *pol) ***REMOVED***
  round_robin_lb_policy *p = (round_robin_lb_policy *)pol;
  if (GRPC_TRACER_ON(grpc_lb_round_robin_trace)) ***REMOVED***
  ...
  ***REMOVED***
  p->shutdown = true;
  pending_pick *pp;
  while ((pp = p->pending_picks)) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (!latest_is_current && p->latest_pending_subchannel_list != NULL &&
      !p->latest_pending_subchannel_list->shutting_down) ***REMOVED***
...
static void start_picking_locked(grpc_exec_ctx *exec_ctx,
                                 round_robin_lb_policy *p) ***REMOVED***
...
static void rr_exit_idle_locked(grpc_exec_ctx *exec_ctx, grpc_lb_policy *pol) ***REMOVED***
  round_robin_lb_policy *p = (round_robin_lb_policy *)pol;
  if (!p->started_picking) ***REMOVED***
...
static int rr_pick_locked(grpc_exec_ctx *exec_ctx, grpc_lb_policy *pol,
  ...
  ***REMOVED***
  if (p->subchannel_list != NULL) ***REMOVED***
    const size_t next_ready_index = get_next_ready_subchannel_index_locked(p);
    if (next_ready_index < p->subchannel_list->num_subchannels) ***REMOVED***
...
static void update_state_counters_locked(subchannel_data *sd) ***REMOVED***
  rr_subchannel_list *subchannel_list = sd->subchannel_list;
  if (sd->prev_connectivity_state == GRPC_CHANNEL_READY) ***REMOVED***
    GPR_ASSERT(subchannel_list->num_ready > 0);
    --subchannel_list->num_ready;
  ***REMOVED*** else if (sd->prev_connectivity_state == GRPC_CHANNEL_TRANSIENT_FAILURE) ***REMOVED***
    GPR_ASSERT(subchannel_list->num_transient_failures > 0);
    --subchannel_list->num_transient_failures;
  ***REMOVED*** else if (sd->prev_connectivity_state == GRPC_CHANNEL_SHUTDOWN) ***REMOVED***
    GPR_ASSERT(subchannel_list->num_shutdown > 0);
    --subchannel_list->num_shutdown;
  ***REMOVED*** else if (sd->prev_connectivity_state == GRPC_CHANNEL_IDLE) ***REMOVED***
    GPR_ASSERT(subchannel_list->num_idle > 0);
    --subchannel_list->num_idle;
  ***REMOVED***
  if (sd->curr_connectivity_state == GRPC_CHANNEL_READY) ***REMOVED***
    ++subchannel_list->num_ready;
  ***REMOVED*** else if (sd->curr_connectivity_state == GRPC_CHANNEL_TRANSIENT_FAILURE) ***REMOVED***
    ++subchannel_list->num_transient_failures;
  ***REMOVED*** else if (sd->curr_connectivity_state == GRPC_CHANNEL_SHUTDOWN) ***REMOVED***
    ++subchannel_list->num_shutdown;
  ***REMOVED*** else if (sd->curr_connectivity_state == GRPC_CHANNEL_IDLE) ***REMOVED***
    ++subchannel_list->num_idle;
  ***REMOVED***
***REMOVED***
...
static grpc_connectivity_state update_lb_connectivity_status_locked(
    grpc_exec_ctx *exec_ctx, subchannel_data *sd, grpc_error *error) ***REMOVED***
  ...
  if (subchannel_list->num_ready > 0) ***REMOVED*** /* 1) READY */
    grpc_connectivity_state_set(exec_ctx, &p->state_tracker, GRPC_CHANNEL_READY,
                                GRPC_ERROR_NONE, "rr_ready");
    new_state = GRPC_CHANNEL_READY;
  ***REMOVED*** else if (sd->curr_connectivity_state ==
             GRPC_CHANNEL_CONNECTING) ***REMOVED*** /* 2) CONNECTING */
    grpc_connectivity_state_set(exec_ctx, &p->state_tracker,
                                GRPC_CHANNEL_CONNECTING, GRPC_ERROR_NONE,
                                "rr_connecting");
    new_state = GRPC_CHANNEL_CONNECTING;
  ***REMOVED*** else if (p->subchannel_list->num_shutdown ==
             p->subchannel_list->num_subchannels) ***REMOVED*** /* 3) SHUTDOWN */
    grpc_connectivity_state_set(exec_ctx, &p->state_tracker,
                                GRPC_CHANNEL_SHUTDOWN, GRPC_ERROR_REF(error),
                                "rr_shutdown");
    p->shutdown = true;
    new_state = GRPC_CHANNEL_SHUTDOWN;
  ***REMOVED*** else if (subchannel_list->num_transient_failures ==
             p->subchannel_list->num_subchannels) ***REMOVED*** /* 4) TRANSIENT_FAILURE */
    grpc_connectivity_state_set(exec_ctx, &p->state_tracker,
                                GRPC_CHANNEL_TRANSIENT_FAILURE,
                                GRPC_ERROR_REF(error), "rr_transient_failure");
    new_state = GRPC_CHANNEL_TRANSIENT_FAILURE;
  ***REMOVED*** else if (subchannel_list->num_idle ==
             p->subchannel_list->num_subchannels) ***REMOVED*** /* 5) IDLE */
...
static void rr_connectivity_changed_locked(grpc_exec_ctx *exec_ctx, void *arg,
  ...
  ***REMOVED***
  ...
  if (p->shutdown) ***REMOVED***
    ...
    return;
  ***REMOVED***
  if (sd->subchannel_list->shutting_down && error == GRPC_ERROR_CANCELLED) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (sd->subchannel_list != p->subchannel_list &&
      sd->subchannel_list != p->latest_pending_subchannel_list) ***REMOVED***
    const char *reason = NULL;
    if (sd->subchannel_list->shutting_down) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (sd->curr_connectivity_state == GRPC_CHANNEL_SHUTDOWN) ***REMOVED***
    ...
    if (sd->user_data != NULL) ***REMOVED***
      ...
      sd->user_data = NULL;
    ***REMOVED***
    if (new_policy_connectivity_state == GRPC_CHANNEL_SHUTDOWN) ***REMOVED***
  ...
  ***REMOVED*** else ***REMOVED***  // sd not in SHUTDOWN
    if (sd->curr_connectivity_state == GRPC_CHANNEL_READY) ***REMOVED***
      if (sd->subchannel_list != p->subchannel_list) ***REMOVED***
      ...
      ***REMOVED***
      ...
      if (p->pending_picks != NULL) ***REMOVED***
...
static grpc_connectivity_state rr_check_connectivity_locked(
    grpc_exec_ctx *exec_ctx, grpc_lb_policy *pol, grpc_error **error) ***REMOVED***
  round_robin_lb_policy *p = (round_robin_lb_policy *)pol;
...
static void rr_update_locked(grpc_exec_ctx *exec_ctx, grpc_lb_policy *policy,
  ...
  ***REMOVED***
  ...
  if (addresses->num_addresses == 0) ***REMOVED***
    grpc_connectivity_state_set(
        exec_ctx, &p->state_tracker, GRPC_CHANNEL_TRANSIENT_FAILURE,
  ...
  ***REMOVED***
  ...
  if (p->latest_pending_subchannel_list != NULL && p->started_picking) ***REMOVED***
    if (GRPC_TRACER_ON(grpc_lb_round_robin_trace)) ***REMOVED***
      gpr_log(GPR_DEBUG,
  ...
  ***REMOVED***
  ...
  for (size_t i = 0; i < addresses->num_addresses; i++) ***REMOVED***
    ...
    ***REMOVED***
    ...
    if (sd->user_data_vtable != NULL) ***REMOVED***
      sd->user_data =
...
static const grpc_lb_policy_vtable round_robin_lb_policy_vtable = ***REMOVED***
    rr_destroy,
    rr_shutdown_locked,
    rr_pick_locked,
    ...
    rr_update_locked***REMOVED***;

static void round_robin_factory_ref(grpc_lb_policy_factory *factory) ***REMOVED******REMOVED***

static void round_robin_factory_unref(grpc_lb_policy_factory *factory) ***REMOVED******REMOVED***
...
static const grpc_lb_policy_factory_vtable round_robin_factory_vtable = ***REMOVED***
    round_robin_factory_ref, round_robin_factory_unref, round_robin_create,
    "round_robin"***REMOVED***;

static grpc_lb_policy_factory round_robin_lb_policy_factory = ***REMOVED***
    &round_robin_factory_vtable***REMOVED***;

static grpc_lb_policy_factory *round_robin_lb_factory_create() ***REMOVED***
  return &round_robin_lb_policy_factory;
***REMOVED***

/* Plugin registration */

void grpc_lb_policy_round_robin_init() ***REMOVED***
  grpc_register_lb_policy(round_robin_lb_factory_create());
  grpc_register_tracer(&grpc_lb_round_robin_trace);
***REMOVED***

void grpc_lb_policy_round_robin_shutdown() ***REMOVED******REMOVED***
