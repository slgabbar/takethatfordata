...
static void init_default_credentials(void) ***REMOVED*** gpr_mu_init(&g_state_mu); ***REMOVED***
...
***REMOVED*** compute_engine_detector;
...
static void on_compute_engine_detection_http_response(grpc_exec_ctx *exec_ctx,
  ...
  if (error == GRPC_ERROR_NONE && detector->response.status == 200 &&
      detector->response.hdr_count > 0) ***REMOVED***
    /* Internet providers can return a generic response to all requests, so
       it is necessary to check that metadata header is present also. */
    size_t i;
    for (i = 0; i < detector->response.hdr_count; i++) ***REMOVED***
      grpc_http_header *header = &detector->response.hdrs[i];
      if (strcmp(header->key, "Metadata-Flavor") == 0 &&
          strcmp(header->value, "Google") == 0) ***REMOVED***
        detector->success = 1;
        break;
      ***REMOVED***
    ***REMOVED***
...
static void destroy_pollset(grpc_exec_ctx *exec_ctx, void *p, grpc_error *e) ***REMOVED***
  grpc_pollset_destroy(exec_ctx, p);
***REMOVED***

static int is_stack_running_on_compute_engine(grpc_exec_ctx *exec_ctx) ***REMOVED***
  ...
  gpr_mu_lock(g_polling_mu);
  while (!detector.is_done) ***REMOVED***
    ...
    if (!GRPC_LOG_IF_ERROR(
            ...
            grpc_pollset_work(exec_ctx,
                              ...
                              gpr_inf_future(GPR_CLOCK_MONOTONIC)))) ***REMOVED***
      detector.is_done = 1;
      detector.success = 0;
    ***REMOVED***
  ***REMOVED***
...
static grpc_error *create_default_creds_from_path(
    grpc_exec_ctx *exec_ctx, char *creds_path, grpc_call_credentials **creds) ***REMOVED***
  grpc_json *json = NULL;
  ...
  ***REMOVED***
  ...
  if (grpc_auth_json_key_is_valid(&key)) ***REMOVED***
    result =
  ...
  ***REMOVED***
  ...
  if (grpc_auth_refresh_token_is_valid(&token)) ***REMOVED***
    result =
...
grpc_channel_credentials *grpc_google_default_credentials_create(void) ***REMOVED***
  ...
  if (default_credentials != NULL) ***REMOVED***
...
void grpc_flush_cached_google_default_credentials(void) ***REMOVED***
  grpc_exec_ctx exec_ctx = GRPC_EXEC_CTX_INIT;
  gpr_once_init(&g_once, init_default_credentials);
  gpr_mu_lock(&g_state_mu);
  if (default_credentials != NULL) ***REMOVED***
...
char *grpc_get_well_known_google_credentials_file_path(void) ***REMOVED***
  if (creds_path_getter != NULL) return creds_path_getter();
  return grpc_get_well_known_google_credentials_file_path_impl();
***REMOVED***

void grpc_override_well_known_credentials_path_getter(
    grpc_well_known_credentials_path_getter getter) ***REMOVED***
