...
grpc_slice grpc_chttp2_rst_stream_create(uint32_t id, uint32_t code,
                                         grpc_transport_one_way_stats *stats) ***REMOVED***
...
grpc_error *grpc_chttp2_rst_stream_parser_begin_frame(
    grpc_chttp2_rst_stream_parser *parser, uint32_t length, uint8_t flags) ***REMOVED***
  if (length != 4) ***REMOVED***
    char *msg;
...
grpc_error *grpc_chttp2_rst_stream_parser_parse(grpc_exec_ctx *exec_ctx,
  ...
  ***REMOVED***
  ...
  if (p->byte == 4) ***REMOVED***
    ...
    if (reason != GRPC_HTTP2_NO_ERROR || s->metadata_buffer[1].size == 0) ***REMOVED***
      char *message;
