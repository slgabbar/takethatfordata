...
static void security_handshaker_unref(grpc_exec_ctx *exec_ctx,
  ...
  if (gpr_unref(&h->refs)) ***REMOVED***
    ...
    if (h->endpoint_to_destroy != NULL) ***REMOVED***
      grpc_endpoint_destroy(exec_ctx, h->endpoint_to_destroy);
    ***REMOVED***
    if (h->read_buffer_to_destroy != NULL) ***REMOVED***
...
static void on_peer_checked_inner(grpc_exec_ctx *exec_ctx,
  ...
  ***REMOVED***
  ...
  if (zero_copy_protector == NULL) ***REMOVED***
...
static grpc_error *on_handshake_next_done_locked(
  ...
  ***REMOVED***
  ...
  if (handshaker_result != NULL) ***REMOVED***
  ...
  ***REMOVED*** else if (handshaker_result == NULL) ***REMOVED***
...
static void on_handshake_data_received_from_peer(grpc_exec_ctx *exec_ctx,
  ...
  ***REMOVED***
  ...
  for (i = 0; i < h->args->read_buffer->count; i++) ***REMOVED***
    bytes_received_size += GRPC_SLICE_LENGTH(h->args->read_buffer->slices[i]);
  ***REMOVED***
  if (bytes_received_size > h->handshake_buffer_size) ***REMOVED***
    h->handshake_buffer =
...
static void on_handshake_data_sent_to_peer(grpc_exec_ctx *exec_ctx, void *arg,
  ...
  ***REMOVED***
  // We may be done.
  if (h->handshaker_result == NULL) ***REMOVED***
    grpc_endpoint_read(exec_ctx, h->args->endpoint, h->args->read_buffer,
                       &h->on_handshake_data_received_from_peer);
  ***REMOVED*** else ***REMOVED***
    error = check_peer_locked(exec_ctx, h);
