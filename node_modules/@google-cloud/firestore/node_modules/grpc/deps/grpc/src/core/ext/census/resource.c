...
void initialize_resources(void) ***REMOVED***
...
static void delete_resource_locked(size_t rid) ***REMOVED***
...
void shutdown_resources(void) ***REMOVED***
  gpr_mu_lock(&resource_lock);
  for (size_t i = 0; i < n_resources; i++) ***REMOVED***
...
static bool validate_string(pb_istream_t *stream, const pb_field_t *field,
  ...
  switch (field->tag) ***REMOVED***
    case google_census_Resource_name_tag:
      // Name must have at least one character
      if (stream->bytes_left == 0) ***REMOVED***
      ...
      ***REMOVED***
      ...
      if (!pb_read(stream, (uint8_t *)vresource->name, stream->bytes_left)) ***REMOVED***
        return false;
      ***REMOVED***
      ...
      for (size_t i = 0; i < n_resources; i++) ***REMOVED***
        ...
        if (strcmp(compare->name, vresource->name) == 0) ***REMOVED***
    ...
    default:
      ...
      if (!pb_read(stream, NULL, stream->bytes_left)) ***REMOVED***
        return false;
      ***REMOVED***
      break;
  ***REMOVED***
...
static bool validate_units_helper(pb_istream_t *stream, int *count,
                                  google_census_Resource_BasicUnit **bup) ***REMOVED***
  while (stream->bytes_left) ***REMOVED***
    (*count)++;
    ...
    *bup = new_bup;
    uint64_t value;
    if (!pb_decode_varint(stream, &value)) ***REMOVED***
      return false;
    ***REMOVED***
...
static bool validate_resource_pb(const uint8_t *resource_pb,
                                 size_t resource_pb_size, size_t id) ***REMOVED***
  GPR_ASSERT(id < n_resources);
  if (resource_pb == NULL) ***REMOVED***
    return false;
  ***REMOVED***
  ...
  if (!pb_decode(&stream, google_census_Resource_fields, &vresource)) ***REMOVED***
    return false;
  ***REMOVED***
...
// resource_lock held.
size_t allocate_resource(void) ***REMOVED***
  ...
  if (n_resources == n_defined_resources) ***REMOVED***
  ...
  ***REMOVED*** else ***REMOVED***
    ...
    // Find a free id.
    for (size_t base = 0; base < n_resources; base++) ***REMOVED***
...
int32_t census_define_resource(const uint8_t *resource_pb,
                               size_t resource_pb_size) ***REMOVED***
  if (resource_pb == NULL) ***REMOVED***
    return -1;
  ***REMOVED***
  gpr_mu_lock(&resource_lock);
  ...
  if (!validate_resource_pb(resource_pb, resource_pb_size, id)) ***REMOVED***
...
void census_delete_resource(int32_t rid) ***REMOVED***
  gpr_mu_lock(&resource_lock);
  if (rid >= 0 && (size_t)rid < n_resources && resources[rid] != NULL) ***REMOVED***
    delete_resource_locked((size_t)rid);
  ***REMOVED***
  gpr_mu_unlock(&resource_lock);
***REMOVED***

int32_t census_resource_id(const char *name) ***REMOVED***
  gpr_mu_lock(&resource_lock);
  for (int32_t id = 0; (size_t)id < n_resources; id++) ***REMOVED***
    if (resources[id] != NULL && strcmp(resources[id]->name, name) == 0) ***REMOVED***
      gpr_mu_unlock(&resource_lock);
      return id;
    ***REMOVED***
  ***REMOVED***
...
int32_t define_resource(const resource *base) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (base->n_denominators != 0) ***REMOVED***
