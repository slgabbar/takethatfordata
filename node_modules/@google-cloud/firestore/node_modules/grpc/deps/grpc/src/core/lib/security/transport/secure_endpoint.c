...
typedef struct ***REMOVED***
  ...
  gpr_refcount ref;
***REMOVED*** secure_endpoint;

grpc_tracer_flag grpc_trace_secure_endpoint =
    GRPC_TRACER_INITIALIZER(false, "secure_endpoint");

static void destroy(grpc_exec_ctx *exec_ctx, secure_endpoint *secure_ep) ***REMOVED***
...
static void secure_endpoint_unref(grpc_exec_ctx *exec_ctx, secure_endpoint *ep,
                                  ...
                                  int line) ***REMOVED***
  if (GRPC_TRACER_ON(grpc_trace_secure_endpoint)) ***REMOVED***
    gpr_atm val = gpr_atm_no_barrier_load(&ep->ref.count);
    gpr_log(file, line, GPR_LOG_SEVERITY_DEBUG,
            "SECENDP unref %p : %s %" PRIdPTR " -> %" PRIdPTR, ep, reason, val,
            val - 1);
  ***REMOVED***
  if (gpr_unref(&ep->ref)) ***REMOVED***
    destroy(exec_ctx, ep);
  ***REMOVED***
***REMOVED***
...
static void secure_endpoint_unref(grpc_exec_ctx *exec_ctx,
                                  secure_endpoint *ep) ***REMOVED***
  if (gpr_unref(&ep->ref)) ***REMOVED***
    destroy(exec_ctx, ep);
  ***REMOVED***
***REMOVED***

static void secure_endpoint_ref(secure_endpoint *ep) ***REMOVED*** gpr_ref(&ep->ref); ***REMOVED***
#endif
...
static void call_read_cb(grpc_exec_ctx *exec_ctx, secure_endpoint *ep,
  ...
  if (GRPC_TRACER_ON(grpc_trace_secure_endpoint)) ***REMOVED***
    size_t i;
    for (i = 0; i < ep->read_buffer->count; i++) ***REMOVED***
...
static void on_read(grpc_exec_ctx *exec_ctx, void *user_data,
  ...
  ***REMOVED***
  ...
  if (ep->zero_copy_protector != NULL) ***REMOVED***
  ...
  ***REMOVED*** else ***REMOVED***
      ...
      while (message_size > 0 || keep_looping) ***REMOVED***
        ...
        ***REMOVED***
        ...
        if (cur == end) ***REMOVED***
          ...
          keep_looping = 1;
        ***REMOVED*** else if (unprotected_buffer_size_written > 0) ***REMOVED***
          keep_looping = 1;
        ***REMOVED*** else ***REMOVED***
          keep_looping = 0;
        ***REMOVED***
    ...
    ***REMOVED***
    ...
    if (cur != GRPC_SLICE_START_PTR(ep->read_staging_buffer)) ***REMOVED***
...
static void endpoint_read(grpc_exec_ctx *exec_ctx, grpc_endpoint *secure_ep,
  ...
  if (ep->leftover_bytes.count) ***REMOVED***
...
static void endpoint_write(grpc_exec_ctx *exec_ctx, grpc_endpoint *secure_ep,
  ...
  if (GRPC_TRACER_ON(grpc_trace_secure_endpoint)) ***REMOVED***
    for (i = 0; i < slices->count; i++) ***REMOVED***
      char *data =
    ...
    if (result == TSI_OK) ***REMOVED***
      ...
      do ***REMOVED***
        ...
        if (cur == end) ***REMOVED***
          flush_write_staging_buffer(ep, &cur, &end);
        ***REMOVED***
      ***REMOVED*** while (still_pending_size > 0);
      if (cur != GRPC_SLICE_START_PTR(ep->write_staging_buffer)) ***REMOVED***
        grpc_slice_buffer_add(
            &ep->output_buffer,
            grpc_slice_split_head(
...
static void endpoint_destroy(grpc_exec_ctx *exec_ctx,
                             grpc_endpoint *secure_ep) ***REMOVED***
  secure_endpoint *ep = (secure_endpoint *)secure_ep;
...
static char *endpoint_get_peer(grpc_endpoint *secure_ep) ***REMOVED***
  secure_endpoint *ep = (secure_endpoint *)secure_ep;
  return grpc_endpoint_get_peer(ep->wrapped_ep);
***REMOVED***

static int endpoint_get_fd(grpc_endpoint *secure_ep) ***REMOVED***
  secure_endpoint *ep = (secure_endpoint *)secure_ep;
...
static const grpc_endpoint_vtable vtable = ***REMOVED***endpoint_read,
                                            endpoint_write,
                                            ...
                                            endpoint_get_peer,
                                            endpoint_get_fd***REMOVED***;

grpc_endpoint *grpc_secure_endpoint_create(
    struct tsi_frame_protector *protector,
    struct tsi_zero_copy_grpc_protector *zero_copy_protector,
    grpc_endpoint *transport, grpc_slice *leftover_slices,
    size_t leftover_nslices) ***REMOVED***
  size_t i;
  ...
  for (i = 0; i < leftover_nslices; i++) ***REMOVED***
