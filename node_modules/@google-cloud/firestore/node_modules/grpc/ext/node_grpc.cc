...
typedef struct log_args ***REMOVED***
  gpr_log_func_args core_args;
  gpr_timespec timestamp;
***REMOVED*** log_args;

typedef struct logger_state ***REMOVED***
  ...
  bool logger_set;
***REMOVED*** logger_state;

logger_state grpc_logger_state;

static char *pem_root_certs = NULL;

void InitOpTypeConstants(Local<Object> exports) ***REMOVED***
...
void InitConnectivityStateConstants(Local<Object> exports) ***REMOVED***
  Nan::HandleScope scope;
...
NAN_METHOD(MetadataKeyIsLegal) ***REMOVED***
...
NAN_METHOD(MetadataNonbinValueIsLegal) ***REMOVED***
  if (!info[0]->IsString()) ***REMOVED***
...
NAN_METHOD(MetadataKeyIsBinary) ***REMOVED***
  if (!info[0]->IsString()) ***REMOVED***
    return Nan::ThrowTypeError(
...
static grpc_ssl_roots_override_result get_ssl_roots_override(
    char **pem_root_certs_ptr) ***REMOVED***
  *pem_root_certs_ptr = pem_root_certs;
  if (pem_root_certs == NULL) ***REMOVED***
 ...
 *ServerCredentials */
NAN_METHOD(SetDefaultRootsPem) ***REMOVED***
  if (!info[0]->IsString()) ***REMOVED***
    return Nan::ThrowTypeError(
...
NAUV_WORK_CB(LogMessagesCallback) ***REMOVED***
  Nan::HandleScope scope;
  ...
  while (!args.empty()) ***REMOVED***
    ...
    Local<Value> timestamp =
        Nan::New<v8::Date>(grpc::node::TimespecToMilliseconds(arg->timestamp))
            .ToLocalChecked();
    const int argc = 5;
    Local<Value> argv[argc] = ***REMOVED***file, line, severity, message, timestamp***REMOVED***;
...
void node_log_func(gpr_log_func_args *args) ***REMOVED***
...
NAN_METHOD(SetDefaultLoggerCallback) ***REMOVED***
  if (!info[0]->IsFunction()) ***REMOVED***
    return Nan::ThrowTypeError(
        "setDefaultLoggerCallback's argument must be a function");
  ***REMOVED***
  if (!grpc_logger_state.logger_set) ***REMOVED***
...
NAN_METHOD(SetLogVerbosity) ***REMOVED***
...
NAN_METHOD(ForcePoll) ***REMOVED***
  grpc::node::CompletionQueueForcePoll();
***REMOVED***

void init(Local<Object> exports) ***REMOVED***
  Nan::HandleScope scope;
  grpc_init();
