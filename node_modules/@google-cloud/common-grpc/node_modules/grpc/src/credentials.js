/**
 * @license
 * Copyright 2015 gRPC authors.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 *
 */

/**
 * Credentials module
 *
 * This module contains factory methods for two different credential types:
 * CallCredentials and ChannelCredentials. ChannelCredentials are things like
 * SSL credentials that can be used to secure a connection, and are used to
 * construct a Client object. CallCredentials genrally modify metadata, so they
 * can be attached to an individual method call.
 *
 * CallCredentials can be composed with other CallCredentials to create
 * CallCredentials. ChannelCredentials can be composed with CallCredentials
 * to create ChannelCredentials. No combined credential can have more than
 * one ChannelCredentials.
 *
 * For example, to create a client secured with SSL that uses Google
 * default application credentials to authenticate:
 *
 * @example
 * var channel_creds = credentials.createSsl(root_certs);
 * (new GoogleAuth()).getApplicationDefault(function(err, credential) ***REMOVED***
 *   var call_creds = credentials.createFromGoogleCredential(credential);
 *   var combined_creds = credentials.combineChannelCredentials(
 *       channel_creds, call_creds);
 *   var client = new Client(address, combined_creds);
 * ***REMOVED***);
 *
 * @namespace grpc.credentials
 */

'use strict';

var grpc = require('./grpc_extension');

/**
 * This cannot be constructed directly. Instead, instances of this class should
 * be created using the factory functions in ***REMOVED***@link grpc.credentials***REMOVED***
 * @constructor grpc.credentials~CallCredentials
 */
var CallCredentials = grpc.CallCredentials;

/**
 * This cannot be constructed directly. Instead, instances of this class should
 * be created using the factory functions in ***REMOVED***@link grpc.credentials***REMOVED***
 * @constructor grpc.credentials~ChannelCredentials
 */
var ChannelCredentials = grpc.ChannelCredentials;

var Metadata = require('./metadata.js');

var common = require('./common.js');

var constants = require('./constants');

var _ = require('lodash');

/**
 * @external GoogleCredential
 * @see https://github.com/google/google-auth-library-nodejs
 */

/**
 * Create an SSL Credentials object. If using a client-side certificate, both
 * the second and third arguments must be passed.
 * @memberof grpc.credentials
 * @alias grpc.credentials.createSsl
 * @kind function
 * @param ***REMOVED***Buffer=***REMOVED*** root_certs The root certificate data
 * @param ***REMOVED***Buffer=***REMOVED*** private_key The client certificate private key, if
 *     applicable
 * @param ***REMOVED***Buffer=***REMOVED*** cert_chain The client certificate cert chain, if applicable
 * @return ***REMOVED***grpc.credentials~ChannelCredentials***REMOVED*** The SSL Credentials object
 */
exports.createSsl = ChannelCredentials.createSsl;

/**
 * @callback grpc.credentials~metadataCallback
 * @param ***REMOVED***Error***REMOVED*** error The error, if getting metadata failed
 * @param ***REMOVED***grpc.Metadata***REMOVED*** metadata The metadata
 */

/**
 * @callback grpc.credentials~generateMetadata
 * @param ***REMOVED***Object***REMOVED*** params Parameters that can modify metadata generation
 * @param ***REMOVED***string***REMOVED*** params.service_url The URL of the service that the call is
 *     going to
 * @param ***REMOVED***grpc.credentials~metadataCallback***REMOVED*** callback
 */

/**
 * Create a gRPC credentials object from a metadata generation function. This
 * function gets the service URL and a callback as parameters. The error
 * passed to the callback can optionally have a 'code' value attached to it,
 * which corresponds to a status code that this library uses.
 * @memberof grpc.credentials
 * @alias grpc.credentials.createFromMetadataGenerator
 * @param ***REMOVED***grpc.credentials~generateMetadata***REMOVED*** metadata_generator The function
 *     that generates metadata
 * @return ***REMOVED***grpc.credentials~CallCredentials***REMOVED*** The credentials object
 */
exports.createFromMetadataGenerator = function(metadata_generator) ***REMOVED***
  return CallCredentials.createFromPlugin(function(service_url, cb_data,
                                                   callback) ***REMOVED***
    metadata_generator(***REMOVED***service_url: service_url***REMOVED***, function(error, metadata) ***REMOVED***
      var code = constants.status.OK;
      var message = '';
      if (error) ***REMOVED***
        message = error.message;
        if (error.hasOwnProperty('code') && _.isFinite(error.code)) ***REMOVED***
          code = error.code;
        ***REMOVED*** else ***REMOVED***
          code = constants.status.UNAUTHENTICATED;
        ***REMOVED***
        if (!metadata) ***REMOVED***
          metadata = new Metadata();
        ***REMOVED***
      ***REMOVED***
      callback(code, message, metadata._getCoreRepresentation(), cb_data);
    ***REMOVED***);
  ***REMOVED***);
***REMOVED***;

/**
 * Create a gRPC credential from a Google credential object.
 * @memberof grpc.credentials
 * @alias grpc.credentials.createFromGoogleCredential
 * @param ***REMOVED***external:GoogleCredential***REMOVED*** google_credential The Google credential
 *     object to use
 * @return ***REMOVED***grpc.credentials~CallCredentials***REMOVED*** The resulting credentials object
 */
exports.createFromGoogleCredential = function(google_credential) ***REMOVED***
  return exports.createFromMetadataGenerator(function(auth_context, callback) ***REMOVED***
    var service_url = auth_context.service_url;
    google_credential.getRequestMetadata(service_url, function(err, header) ***REMOVED***
      if (err) ***REMOVED***
        common.log(constants.logVerbosity.INFO, 'Auth error:' + err);
        callback(err);
        return;
      ***REMOVED***
      var metadata = new Metadata();
      metadata.add('authorization', header.Authorization);
      callback(null, metadata);
    ***REMOVED***);
  ***REMOVED***);
***REMOVED***;

/**
 * Combine a ChannelCredentials with any number of CallCredentials into a single
 * ChannelCredentials object.
 * @memberof grpc.credentials
 * @alias grpc.credentials.combineChannelCredentials
 * @param ***REMOVED***grpc.credentials~ChannelCredentials***REMOVED*** channel_credential The ChannelCredentials to
 *     start with
 * @param ***REMOVED***...grpc.credentials~CallCredentials***REMOVED*** credentials The CallCredentials to compose
 * @return ***REMOVED***grpc.credentials~ChannelCredentials***REMOVED*** A credentials object that combines all of the
 *     input credentials
 */
exports.combineChannelCredentials = function(channel_credential) ***REMOVED***
  var current = channel_credential;
  for (var i = 1; i < arguments.length; i++) ***REMOVED***
    current = current.compose(arguments[i]);
  ***REMOVED***
  return current;
***REMOVED***;

/**
 * Combine any number of CallCredentials into a single CallCredentials object
 * @memberof grpc.credentials
 * @alias grpc.credentials.combineCallCredentials
 * @param ***REMOVED***...grpc.credentials~CallCredentials***REMOVED*** credentials The CallCredentials to compose
 * @return ***REMOVED***grpc.credentials~CallCredentials***REMOVED*** A credentials object that combines all of the input
 *     credentials
 */
exports.combineCallCredentials = function() ***REMOVED***
  var current = arguments[0];
  for (var i = 1; i < arguments.length; i++) ***REMOVED***
    current = current.compose(arguments[i]);
  ***REMOVED***
  return current;
***REMOVED***;

/**
 * Create an insecure credentials object. This is used to create a channel that
 * does not use SSL. This cannot be composed with anything.
 * @memberof grpc.credentials
 * @alias grpc.credentials.createInsecure
 * @kind function
 * @return ***REMOVED***grpc.credentials~ChannelCredentials***REMOVED*** The insecure credentials object
 */
exports.createInsecure = ChannelCredentials.createInsecure;
