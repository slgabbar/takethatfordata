    ...
    switch (field) ***REMOVED***
      ...
      case "path":
        ...
        if (pathLen > pathFSize &&
            pathLen <= pathFSize + prefFSize) ***REMOVED***
          ...
          for ( var s = searchStart
              ; (s <= searchEnd)
              ; s ++ ) ***REMOVED***
            if (pathBuf[s] === slash || pathBuf[s] === bslash) ***REMOVED***
              found = s
              break
            ***REMOVED***
          ***REMOVED***

          if (found !== false) ***REMOVED***
...
// then use write256.
var MAXNUM = ***REMOVED*** 12: 077777777777
             , 11: 07777777777
             , 8 : 07777777
             , 7 : 0777777 ***REMOVED***
function writeNumeric (block, off, end, num) ***REMOVED***
  ...
  if (num instanceof Date ||
      Object.prototype.toString.call(num) === "[object Date]") ***REMOVED***
    num = num.getTime() / 1000
  ***REMOVED***

  if (num > maxNum || num < 0) ***REMOVED***
  ...
  ***REMOVED***
  ...
  // pad with "0" chars
  if (numStr.length < writeLen) ***REMOVED***
    numStr = (new Array(writeLen - numStr.length).join("0")) + numStr
  ***REMOVED***

  if (numStr.length !== writeLen - 1) ***REMOVED***
...
function write256 (block, off, end, num) ***REMOVED***
  ...
  ***REMOVED***
  ...
  var zero = true
  for (i = bytes; i > 0; i --) ***REMOVED***
    var byte = tuple[bytes - i]
    if (positive) buf[fill + i] = byte
    else if (zero && byte === 0) buf[fill + i] = 0
    else if (zero) ***REMOVED***
      zero = false
      buf[fill + i] = 0x100 - byte
    ***REMOVED*** else buf[fill + i] = 0xFF - byte
  ***REMOVED***
***REMOVED***

function writeText (block, off, end, str) ***REMOVED***
...
function calcSum (block) ***REMOVED***
  ...
  for (var i = 0; i < fieldOffs[fields.cksum]; i ++) ***REMOVED***
    sum += block[i]
  ***REMOVED***
...
function checkSum (block) ***REMOVED***
  ...
  for (var f = 0; fields[f] !== null; f ++) ***REMOVED***
    ...
    switch (field) ***REMOVED***
      case "ustar":
        ...
        if (val.toString() !== "ustar\0") ***REMOVED***
          this.ustar = false
          return
...
function parse256 (buf) ***REMOVED***
  ...
  var zero = false
    , tuple = []
  for (var i = buf.length - 1; i > 0; i --) ***REMOVED***
    var byte = buf[i]
    ...
    ***REMOVED*** else tuple.push(0xFF - byte)
  ***REMOVED***

  for (var sum = 0, i = 0, l = tuple.length; i < l; i ++) ***REMOVED***
...
function parseNumeric (f) ***REMOVED***
