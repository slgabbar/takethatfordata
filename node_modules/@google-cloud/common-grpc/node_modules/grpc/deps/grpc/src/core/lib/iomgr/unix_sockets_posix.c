...
grpc_error *grpc_resolve_unix_domain_address(const char *name,
  ...
  if (strlen(name) > GPR_ARRAY_SIZE(((struct sockaddr_un *)0)->sun_path) - 1) ***REMOVED***
    char *err_msg;
    grpc_error *err;
...
int grpc_is_unix_socket(const grpc_resolved_address *resolved_addr) ***REMOVED***
...
void grpc_unlink_if_unix_domain_socket(
  ...
  const struct sockaddr *addr = (const struct sockaddr *)resolved_addr->addr;
  if (addr->sa_family != AF_UNIX) ***REMOVED***
    return;
  ***REMOVED***
  struct sockaddr_un *un = (struct sockaddr_un *)resolved_addr->addr;
  struct stat st;

  if (stat(un->sun_path, &st) == 0 && (st.st_mode & S_IFMT) == S_IFSOCK) ***REMOVED***
    unlink(un->sun_path);
  ***REMOVED***
***REMOVED***
