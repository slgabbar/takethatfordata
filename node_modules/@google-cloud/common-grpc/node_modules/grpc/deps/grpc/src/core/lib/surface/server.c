...
static void server_delete(grpc_exec_ctx *exec_ctx, grpc_server *server) ***REMOVED***
  ...
  while ((rm = server->registered_methods) != NULL) ***REMOVED***
...
static void destroy_channel(grpc_exec_ctx *exec_ctx, channel_data *chand,
  ...
  if (GRPC_TRACER_ON(grpc_server_channel_trace) && error != GRPC_ERROR_NONE) ***REMOVED***
...
static void done_request_event(grpc_exec_ctx *exec_ctx, void *req,
  ...
  if (rc >= server->requested_calls_per_cq[rc->cq_idx] &&
      rc < server->requested_calls_per_cq[rc->cq_idx] +
               server->max_requested_calls_per_cq) ***REMOVED***
...
static void publish_new_rpc(grpc_exec_ctx *exec_ctx, void *arg,
  ...
  ***REMOVED***
  ...
  calld->state = PENDING;
  gpr_mu_unlock(&calld->mu_state);
  if (rm->pending_head == NULL) ***REMOVED***
...
static void server_on_recv_initial_metadata(grpc_exec_ctx *exec_ctx, void *ptr,
  ...
  ***REMOVED***
  ...
  if (0 != gpr_time_cmp(op_deadline, gpr_inf_future(op_deadline.clock_type))) ***REMOVED***
...
static void got_initial_metadata(grpc_exec_ctx *exec_ctx, void *ptr,
  ...
  ***REMOVED*** else ***REMOVED***
    gpr_mu_lock(&calld->mu_state);
    if (calld->state == NOT_STARTED) ***REMOVED***
      calld->state = ZOMBIED;
      gpr_mu_unlock(&calld->mu_state);
      GRPC_CLOSURE_INIT(&calld->kill_zombie_closure, kill_zombie, elem,
                        grpc_schedule_on_exec_ctx);
      GRPC_CLOSURE_SCHED(exec_ctx, &calld->kill_zombie_closure,
                         GRPC_ERROR_NONE);
    ***REMOVED*** else if (calld->state == PENDING) ***REMOVED***
      calld->state = ZOMBIED;
      gpr_mu_unlock(&calld->mu_state);
...
void grpc_server_start(grpc_server *server) ***REMOVED***
  ...
  for (i = 0; i < server->cq_count; i++) ***REMOVED***
    ...
    ***REMOVED***
    ...
    for (int j = 0; j < server->max_requested_calls_per_cq; j++) ***REMOVED***
...
static grpc_call_error queue_call_request(grpc_exec_ctx *exec_ctx,
  ...
  ***REMOVED***
  ...
  gpr_free(rc);
  if (gpr_stack_lockfree_push(rm->requests_per_cq[cq_idx], request_id)) ***REMOVED***
    /* this was the first queued request: we need to lock and start
       matching calls */
    gpr_mu_lock(&server->mu_call);
    while ((calld = rm->pending_head) != NULL) ***REMOVED***
      request_id = gpr_stack_lockfree_pop(rm->requests_per_cq[cq_idx]);
      if (request_id == -1) break;
      rm->pending_head = calld->pending_next;
      gpr_mu_unlock(&server->mu_call);
      gpr_mu_lock(&calld->mu_state);
      if (calld->state == ZOMBIED) ***REMOVED***
        gpr_mu_unlock(&calld->mu_state);
        GRPC_CLOSURE_INIT(
            &calld->kill_zombie_closure, kill_zombie,
            grpc_call_stack_element(grpc_call_get_call_stack(calld->call), 0),
