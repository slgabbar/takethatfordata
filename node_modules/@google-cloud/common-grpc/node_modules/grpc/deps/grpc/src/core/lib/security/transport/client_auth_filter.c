...
typedef struct ***REMOVED***
  ...
  grpc_slice host;
  grpc_slice method;
  /* pollset***REMOVED***_set***REMOVED*** bound to this call; if we need to make external
...
static void reset_auth_metadata_context(
    grpc_auth_metadata_context *auth_md_context) ***REMOVED***
  if (auth_md_context->service_url != NULL) ***REMOVED***
    gpr_free((char *)auth_md_context->service_url);
    auth_md_context->service_url = NULL;
  ***REMOVED***
  if (auth_md_context->method_name != NULL) ***REMOVED***
...
static void add_error(grpc_error **combined, grpc_error *error) ***REMOVED***
  if (error == GRPC_ERROR_NONE) return;
  if (*combined == GRPC_ERROR_NONE) ***REMOVED***
...
static void on_credentials_metadata(grpc_exec_ctx *exec_ctx, void *arg,
                                    grpc_error *input_error) ***REMOVED***
  ...
  if (error == GRPC_ERROR_NONE) ***REMOVED***
    ...
    for (size_t i = 0; i < calld->md_array.size; ++i) ***REMOVED***
...
void build_auth_metadata_context(grpc_security_connector *sc,
  ...
  reset_auth_metadata_context(&calld->auth_md_context);
  if (last_slash == NULL) ***REMOVED***
    gpr_log(GPR_ERROR, "No '/' found in fully qualified method name");
    service[0] = '\0';
  ***REMOVED*** else if (last_slash == service) ***REMOVED***
...
static void send_security_metadata(grpc_exec_ctx *exec_ctx,
  ...
  if (channel_call_creds == NULL && !call_creds_has_md) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (grpc_call_credentials_get_request_metadata(
          ...
          &calld->md_array, &calld->async_result_closure, &error)) ***REMOVED***
...
static void auth_start_transport_stream_op_batch(
  ...
  if (!batch->cancel_stream) ***REMOVED***
    ...
    if (batch->payload->context[GRPC_CONTEXT_SECURITY].value == NULL) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (batch->send_initial_metadata) ***REMOVED***
    for (grpc_linked_mdelem *l = batch->payload->send_initial_metadata
         ...
         l != NULL; l = l->next) ***REMOVED***
      grpc_mdelem md = l->md;
      /* Pointer comparison is OK for md_elems created from the same context.
       */
      if (grpc_slice_eq(GRPC_MDKEY(md), GRPC_MDSTR_AUTHORITY)) ***REMOVED***
        if (calld->have_host) ***REMOVED***
          grpc_slice_unref_internal(exec_ctx, calld->host);
        ***REMOVED***
        calld->host = grpc_slice_ref_internal(GRPC_MDVALUE(md));
        calld->have_host = true;
      ***REMOVED*** else if (grpc_slice_eq(GRPC_MDKEY(md), GRPC_MDSTR_PATH)) ***REMOVED***
        if (calld->have_method) ***REMOVED***
    ...
    ***REMOVED***
    if (calld->have_host) ***REMOVED***
      ...
      if (grpc_channel_security_connector_check_call_host(
              ...
              chand->auth_context, &calld->async_result_closure, &error)) ***REMOVED***
...
const grpc_channel_filter grpc_client_auth_filter = ***REMOVED***
