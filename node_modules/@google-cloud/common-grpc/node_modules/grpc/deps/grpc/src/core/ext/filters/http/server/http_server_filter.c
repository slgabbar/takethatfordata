...
typedef struct channel_data ***REMOVED*** uint8_t unused; ***REMOVED*** channel_data;
...
static void add_error(const char *error_name, grpc_error **cumulative,
                      grpc_error *new_err) ***REMOVED***
  if (new_err == GRPC_ERROR_NONE) return;
  if (*cumulative == GRPC_ERROR_NONE) ***REMOVED***
...
static grpc_error *server_filter_incoming_metadata(grpc_exec_ctx *exec_ctx,
  ...
  if (b->idx.named.method != NULL) ***REMOVED***
    if (grpc_mdelem_eq(b->idx.named.method->md, GRPC_MDELEM_METHOD_POST)) ***REMOVED***
      *calld->recv_initial_metadata_flags &=
          ~(GRPC_INITIAL_METADATA_CACHEABLE_REQUEST |
            GRPC_INITIAL_METADATA_IDEMPOTENT_REQUEST);
    ***REMOVED*** else if (grpc_mdelem_eq(b->idx.named.method->md,
                              GRPC_MDELEM_METHOD_PUT)) ***REMOVED***
      *calld->recv_initial_metadata_flags &=
          ~GRPC_INITIAL_METADATA_CACHEABLE_REQUEST;
      *calld->recv_initial_metadata_flags |=
          GRPC_INITIAL_METADATA_IDEMPOTENT_REQUEST;
    ***REMOVED*** else if (grpc_mdelem_eq(b->idx.named.method->md,
                              GRPC_MDELEM_METHOD_GET)) ***REMOVED***
      *calld->recv_initial_metadata_flags |=
          GRPC_INITIAL_METADATA_CACHEABLE_REQUEST;
      *calld->recv_initial_metadata_flags &=
          ~GRPC_INITIAL_METADATA_IDEMPOTENT_REQUEST;
  ...
  ***REMOVED***
  ...
  if (b->idx.named.te != NULL) ***REMOVED***
    if (!grpc_mdelem_eq(b->idx.named.te->md, GRPC_MDELEM_TE_TRAILERS)) ***REMOVED***
      add_error(error_name, &error,
                grpc_attach_md_to_error(
                    GRPC_ERROR_CREATE_FROM_STATIC_STRING("Bad header"),
                    b->idx.named.te->md));
  ...
  ***REMOVED***
  ...
  if (b->idx.named.scheme != NULL) ***REMOVED***
    if (!grpc_mdelem_eq(b->idx.named.scheme->md, GRPC_MDELEM_SCHEME_HTTP) &&
        ...
        !grpc_mdelem_eq(b->idx.named.scheme->md, GRPC_MDELEM_SCHEME_GRPC)) ***REMOVED***
      add_error(error_name, &error,
                grpc_attach_md_to_error(
                    GRPC_ERROR_CREATE_FROM_STATIC_STRING("Bad header"),
  ...
  ***REMOVED***
  ...
  if (b->idx.named.path == NULL) ***REMOVED***
    add_error(error_name, &error,
              grpc_error_set_str(
                  GRPC_ERROR_CREATE_FROM_STATIC_STRING("Missing header"),
                  GRPC_ERROR_STR_KEY, grpc_slice_from_static_string(":path")));
  ***REMOVED*** else if (*calld->recv_initial_metadata_flags &
             GRPC_INITIAL_METADATA_CACHEABLE_REQUEST) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (b->idx.named.host != NULL && b->idx.named.authority == NULL) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (b->idx.named.authority == NULL) ***REMOVED***
    add_error(
        error_name, &error,
        grpc_error_set_str(
            GRPC_ERROR_CREATE_FROM_STATIC_STRING("Missing header"),
...
static void hs_on_complete(grpc_exec_ctx *exec_ctx, void *user_data,
  ...
  if (calld->seen_path_with_query && calld->recv_message_ready != NULL) ***REMOVED***
...
static void hs_recv_message_ready(grpc_exec_ctx *exec_ctx, void *user_data,
  ...
  grpc_call_element *elem = (grpc_call_element *)user_data;
  call_data *calld = (call_data *)elem->call_data;
  if (calld->seen_path_with_query) ***REMOVED***
...
static grpc_error *hs_mutate_op(grpc_exec_ctx *exec_ctx,
  ...
  ***REMOVED***
  ...
  if (op->recv_message) ***REMOVED***
    ...
    if (op->payload->recv_message.recv_message_ready) ***REMOVED***
      op->payload->recv_message.recv_message_ready =
          &calld->hs_recv_message_ready;
    ***REMOVED***
    if (op->on_complete) ***REMOVED***
...
const grpc_channel_filter grpc_http_server_filter = ***REMOVED***
