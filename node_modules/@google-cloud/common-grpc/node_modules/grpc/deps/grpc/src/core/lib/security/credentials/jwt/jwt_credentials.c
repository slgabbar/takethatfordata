...
static void jwt_reset_cache(grpc_exec_ctx *exec_ctx,
                            grpc_service_account_jwt_access_credentials *c) ***REMOVED***
  GRPC_MDELEM_UNREF(exec_ctx, c->cached.jwt_md);
  c->cached.jwt_md = GRPC_MDNULL;
  if (c->cached.service_url != NULL) ***REMOVED***
...
static grpc_call_credentials_vtable jwt_vtable = ***REMOVED***
    jwt_destruct, jwt_get_request_metadata, jwt_cancel_get_request_metadata***REMOVED***;

grpc_call_credentials *
grpc_service_account_jwt_access_credentials_create_from_auth_json_key(
    grpc_exec_ctx *exec_ctx, grpc_auth_json_key key,
    gpr_timespec token_lifetime) ***REMOVED***
  grpc_service_account_jwt_access_credentials *c;
  ...
  ***REMOVED***
  ...
  if (gpr_time_cmp(token_lifetime, max_token_lifetime) > 0) ***REMOVED***
    gpr_log(GPR_INFO,
...
static char *redact_private_key(const char *json_key) ***REMOVED***
  ...
  ***REMOVED***
  ...
  while (current) ***REMOVED***
    if (current->type == GRPC_JSON_STRING &&
        strcmp(current->key, "private_key") == 0) ***REMOVED***
...
grpc_call_credentials *grpc_service_account_jwt_access_credentials_create(
    const char *json_key, gpr_timespec token_lifetime, void *reserved) ***REMOVED***
  if (GRPC_TRACER_ON(grpc_api_trace)) ***REMOVED***
    char *clean_json = redact_private_key(json_key);
    gpr_log(GPR_INFO,
            "grpc_service_account_jwt_access_credentials_create("
            "json_key=%s, "
            "token_lifetime="
            "gpr_timespec ***REMOVED*** tv_sec: %" PRId64
