...
char *grpc_dump_slice(grpc_slice s, uint32_t flags) ***REMOVED***
...
static int slice_find_separator_offset(const grpc_slice str, const char *sep,
  ...
  if (str_len < sep_len) ***REMOVED***
    return 0;
  ***REMOVED***

  for (i = 0; i <= str_len - sep_len; i++) ***REMOVED***
    if (memcmp(str_ptr + i, sep, sep_len) == 0) ***REMOVED***
      *begin = read_offset;
      *end = read_offset + i;
      return 1;
    ***REMOVED***
...
void grpc_slice_split(grpc_slice str, const char *sep, grpc_slice_buffer *dst) ***REMOVED***
  const size_t sep_len = strlen(sep);
  size_t begin, end;

  GPR_ASSERT(sep_len > 0);

  if (slice_find_separator_offset(str, sep, 0, &begin, &end) != 0) ***REMOVED***
    do ***REMOVED***
      grpc_slice_buffer_add_indexed(dst, grpc_slice_sub(str, begin, end));
    ***REMOVED*** while (slice_find_separator_offset(str, sep, end + sep_len, &begin,
                                         &end) != 0);
    grpc_slice_buffer_add_indexed(
        dst, grpc_slice_sub(str, end + sep_len, GRPC_SLICE_LENGTH(str)));
  ***REMOVED*** else ***REMOVED*** /* no sep found, add whole input */
    grpc_slice_buffer_add_indexed(dst, grpc_slice_ref_internal(str));
  ***REMOVED***
***REMOVED***

bool grpc_parse_slice_to_uint32(grpc_slice str, uint32_t *result) ***REMOVED***
