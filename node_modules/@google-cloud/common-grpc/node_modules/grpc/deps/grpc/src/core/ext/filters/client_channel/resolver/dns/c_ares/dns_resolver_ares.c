...
static const grpc_resolver_vtable dns_ares_resolver_vtable = ***REMOVED***
    dns_ares_destroy, dns_ares_shutdown_locked,
    dns_ares_channel_saw_error_locked, dns_ares_next_locked***REMOVED***;

static void dns_ares_shutdown_locked(grpc_exec_ctx *exec_ctx,
                                     grpc_resolver *resolver) ***REMOVED***
  ares_dns_resolver *r = (ares_dns_resolver *)resolver;
  if (r->have_retry_timer) ***REMOVED***
    grpc_timer_cancel(exec_ctx, &r->retry_timer);
  ***REMOVED***
  if (r->pending_request != NULL) ***REMOVED***
    grpc_cancel_ares_request(exec_ctx, r->pending_request);
  ***REMOVED***
  if (r->next_completion != NULL) ***REMOVED***
...
static bool value_in_json_array(grpc_json *array, const char *value) ***REMOVED***
  for (grpc_json *entry = array->child; entry != NULL; entry = entry->next) ***REMOVED***
    if (entry->type == GRPC_JSON_STRING && strcmp(entry->value, value) == 0) ***REMOVED***
      return true;
    ***REMOVED***
  ***REMOVED***
  return false;
...
static char *choose_service_config(char *service_config_choice_json) ***REMOVED***
  grpc_json *choices_json = grpc_json_parse_string(service_config_choice_json);
  if (choices_json == NULL || choices_json->type != GRPC_JSON_ARRAY) ***REMOVED***
    gpr_log(GPR_ERROR, "cannot parse service config JSON string");
    return NULL;
  ***REMOVED***
  char *service_config = NULL;
  for (grpc_json *choice = choices_json->child; choice != NULL;
       choice = choice->next) ***REMOVED***
    if (choice->type != GRPC_JSON_OBJECT) ***REMOVED***
      gpr_log(GPR_ERROR, "cannot parse service config JSON string");
      break;
    ***REMOVED***
    grpc_json *service_config_json = NULL;
    for (grpc_json *field = choice->child; field != NULL; field = field->next) ***REMOVED***
      // Check client language, if specified.
      if (strcmp(field->key, "clientLanguage") == 0) ***REMOVED***
        if (field->type != GRPC_JSON_ARRAY ||
            !value_in_json_array(field, "c++")) ***REMOVED***
          service_config_json = NULL;
          break;
        ***REMOVED***
      ***REMOVED***
      // Check client hostname, if specified.
      if (strcmp(field->key, "clientHostname") == 0) ***REMOVED***
        char *hostname = grpc_gethostname();
        if (hostname == NULL || field->type != GRPC_JSON_ARRAY ||
            !value_in_json_array(field, hostname)) ***REMOVED***
          service_config_json = NULL;
          break;
        ***REMOVED***
      ***REMOVED***
      // Check percentage, if specified.
      if (strcmp(field->key, "percentage") == 0) ***REMOVED***
        if (field->type != GRPC_JSON_NUMBER) ***REMOVED***
          service_config_json = NULL;
          break;
        ***REMOVED***
        ...
        if (sscanf(field->value, "%d", &percentage) != 1 ||
            random_pct > percentage || percentage == 0) ***REMOVED***
          service_config_json = NULL;
          break;
        ***REMOVED***
      ***REMOVED***
      // Save service config.
      if (strcmp(field->key, "serviceConfig") == 0) ***REMOVED***
        if (field->type == GRPC_JSON_OBJECT) ***REMOVED***
          service_config_json = field;
        ***REMOVED***
      ***REMOVED***
    ***REMOVED***
    if (service_config_json != NULL) ***REMOVED***
...
static void dns_ares_on_resolved_locked(grpc_exec_ctx *exec_ctx, void *arg,
  ...
  if (r->lb_addresses != NULL) ***REMOVED***
    ...
    if (r->service_config_json != NULL) ***REMOVED***
      ...
      if (service_config_string != NULL) ***REMOVED***
        ...
        if (service_config != NULL) ***REMOVED***
          const char *lb_policy_name =
              grpc_service_config_get_lb_policy_name(service_config);
          if (lb_policy_name != NULL) ***REMOVED***
  ...
  ***REMOVED***
  if (r->resolved_result != NULL) ***REMOVED***
...
static void dns_ares_next_locked(grpc_exec_ctx *exec_ctx,
  ...
  if (r->resolved_version == 0 && !r->resolving) ***REMOVED***
    gpr_backoff_reset(&r->backoff_state);
    dns_ares_start_resolving_locked(exec_ctx, r);
  ***REMOVED*** else ***REMOVED***
    dns_ares_maybe_finish_next_locked(exec_ctx, r);
...
static void dns_ares_maybe_finish_next_locked(grpc_exec_ctx *exec_ctx,
  ...
  if (r->next_completion != NULL &&
      r->resolved_version != r->published_version) ***REMOVED***
...
static void dns_ares_destroy(grpc_exec_ctx *exec_ctx, grpc_resolver *gr) ***REMOVED***
...
static grpc_resolver *dns_ares_create(grpc_exec_ctx *exec_ctx,
                                      ...
                                      const char *default_port) ***REMOVED***
  /* Get name from args. */
  ...
  if (0 != strcmp(args->uri->authority, "")) ***REMOVED***
  ...
  ***REMOVED***
  ...
  r->request_service_config = !grpc_channel_arg_get_integer(
      arg, (grpc_integer_options)***REMOVED***false, false, true***REMOVED***);
  r->interested_parties = grpc_pollset_set_create();
  if (args->pollset_set != NULL) ***REMOVED***
...
static void dns_ares_factory_ref(grpc_resolver_factory *factory) ***REMOVED******REMOVED***

static void dns_ares_factory_unref(grpc_resolver_factory *factory) ***REMOVED******REMOVED***

static grpc_resolver *dns_factory_create_resolver(
    grpc_exec_ctx *exec_ctx, grpc_resolver_factory *factory,
    grpc_resolver_args *args) ***REMOVED***
  return dns_ares_create(exec_ctx, args, "https");
***REMOVED***

static char *dns_ares_factory_get_default_host_name(
    grpc_resolver_factory *factory, grpc_uri *uri) ***REMOVED***
  const char *path = uri->path;
  if (path[0] == '/') ++path;
  return gpr_strdup(path);
***REMOVED***

static const grpc_resolver_factory_vtable dns_ares_factory_vtable = ***REMOVED***
    dns_ares_factory_ref, dns_ares_factory_unref, dns_factory_create_resolver,
    dns_ares_factory_get_default_host_name, "dns"***REMOVED***;
static grpc_resolver_factory dns_resolver_factory = ***REMOVED***&dns_ares_factory_vtable***REMOVED***;

static grpc_resolver_factory *dns_ares_resolver_factory_create() ***REMOVED***
  return &dns_resolver_factory;
***REMOVED***

void grpc_resolver_dns_ares_init(void) ***REMOVED***
  char *resolver = gpr_getenv("GRPC_DNS_RESOLVER");
  /* TODO(zyc): Turn on c-ares based resolver by default after the address
     sorter and the CNAME support are added. */
  if (resolver != NULL && gpr_stricmp(resolver, "ares") == 0) ***REMOVED***
...
void grpc_resolver_dns_ares_shutdown(void) ***REMOVED***
  char *resolver = gpr_getenv("GRPC_DNS_RESOLVER");
  if (resolver != NULL && gpr_stricmp(resolver, "ares") == 0) ***REMOVED***
    grpc_ares_cleanup();
  ***REMOVED***
...
void grpc_resolver_dns_ares_init(void) ***REMOVED******REMOVED***

void grpc_resolver_dns_ares_shutdown(void) ***REMOVED******REMOVED***
