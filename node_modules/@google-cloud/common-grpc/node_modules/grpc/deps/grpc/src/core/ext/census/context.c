...
static size_t validate_tag(const char *kv) ***REMOVED***
  size_t len = 1;
  char ch;
  while ((ch = *kv++) != 0) ***REMOVED***
    if (ch < MIN_VALID_TAG_CHAR || ch > MAX_VALID_TAG_CHAR) ***REMOVED***
      return 0;
    ***REMOVED***
    len++;
  ***REMOVED***
  if (len > CENSUS_MAX_TAG_KV_LEN) ***REMOVED***
    return 0;
  ***REMOVED***
  return len;
***REMOVED***
...
static char *decode_tag(struct raw_tag *tag, char *header, int offset) ***REMOVED***
...
static void tag_set_copy(struct tag_set *to, const struct tag_set *from) ***REMOVED***
...
static bool tag_set_delete_tag(struct tag_set *tags, const char *key,
                               size_t key_len) ***REMOVED***
  char *kvp = tags->kvm;
  for (int i = 0; i < tags->ntags_alloc; i++) ***REMOVED***
    uint8_t *flags = (uint8_t *)(kvp + FLAG_OFFSET);
    struct raw_tag tag;
    kvp = decode_tag(&tag, kvp, 0);
    if (CENSUS_TAG_IS_DELETED(tag.flags)) continue;
    if ((key_len == tag.key_len) && (memcmp(key, tag.key, key_len) == 0)) ***REMOVED***
...
static bool tag_set_add_tag(struct tag_set *tags, const census_tag *tag,
                            size_t key_len, size_t value_len) ***REMOVED***
  if (tags->ntags == CENSUS_MAX_PROPAGATED_TAGS) ***REMOVED***
    return false;
  ***REMOVED***
  const size_t tag_size = key_len + value_len + TAG_HEADER_SIZE;
  if (tags->kvm_used + tag_size > tags->kvm_size) ***REMOVED***
...
static void context_modify_tag(census_context *context, const census_tag *tag,
  ...
  bool added = false;
  if (CENSUS_TAG_IS_PROPAGATED(tag->flags)) ***REMOVED***
...
//    appropriate amount.
static void tag_set_flatten(struct tag_set *tags) ***REMOVED***
  ...
  for (int i = 0; i < tags->ntags_alloc; i++) ***REMOVED***
    ...
    if (found_deleted) ***REMOVED***
      if (!CENSUS_TAG_IS_DELETED(tag.flags)) ***REMOVED***
...
census_context *census_context_create(const census_context *base,
                                      ...
                                      census_context_status const **status) ***REMOVED***
...
const census_context_status *census_context_get_status(
    const census_context *context) ***REMOVED***
  return &context->status;
***REMOVED***

void census_context_destroy(census_context *context) ***REMOVED***
...
void census_context_initialize_iterator(const census_context *context,
                                        census_context_iterator *iterator) ***REMOVED***
  iterator->context = context;
  iterator->index = 0;
  if (context->tags[PROPAGATED_TAGS].ntags != 0) ***REMOVED***
    iterator->base = PROPAGATED_TAGS;
    iterator->kvm = context->tags[PROPAGATED_TAGS].kvm;
  ***REMOVED*** else if (context->tags[LOCAL_TAGS].ntags != 0) ***REMOVED***
...
int census_context_next_tag(census_context_iterator *iterator,
                            census_tag *tag) ***REMOVED***
  if (iterator->base < 0) ***REMOVED***
    return 0;
  ***REMOVED***
  struct raw_tag raw;
  ...
  tag->value = raw.value;
  tag->flags = raw.flags;
  if (++iterator->index == iterator->context->tags[iterator->base].ntags) ***REMOVED***
    do ***REMOVED***
      if (iterator->base == LOCAL_TAGS) ***REMOVED***
        iterator->base = -1;
        return 1;
      ***REMOVED***
    ***REMOVED*** while (iterator->context->tags[++iterator->base].ntags == 0);
    iterator->index = 0;
...
static bool tag_set_get_tag(const struct tag_set *tags, const char *key,
                            size_t key_len, census_tag *tag) ***REMOVED***
  char *kvp = tags->kvm;
  for (int i = 0; i < tags->ntags; i++) ***REMOVED***
    struct raw_tag raw;
    kvp = decode_tag(&raw, kvp, 0);
    if (key_len == raw.key_len && memcmp(raw.key, key, key_len) == 0) ***REMOVED***
      tag->key = raw.key;
      tag->value = raw.value;
      tag->flags = raw.flags;
      return true;
...
int census_context_get_tag(const census_context *context, const char *key,
  ...
  ***REMOVED***
  if (tag_set_get_tag(&context->tags[PROPAGATED_TAGS], key, key_len, tag) ||
      tag_set_get_tag(&context->tags[LOCAL_TAGS], key, key_len, tag)) ***REMOVED***
    return 1;
  ***REMOVED***
  return 0;
***REMOVED***
...
static size_t tag_set_encode(const struct tag_set *tags, char *buffer,
                             size_t buf_size) ***REMOVED***
  if (buf_size < ENCODED_HEADER_SIZE + tags->kvm_used) ***REMOVED***
    return 0;
  ***REMOVED***
...
static void tag_set_decode(struct tag_set *tags, const char *buffer,
  ...
  if (tags->ntags == 0) ***REMOVED***
    ...
    return;
  ***REMOVED***
  if (header_size != ENCODED_HEADER_SIZE) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (tag_header_size != TAG_HEADER_SIZE) ***REMOVED***
...
census_context *census_context_decode(const char *buffer, size_t size) ***REMOVED***
  census_context *context =
      (census_context *)gpr_malloc(sizeof(census_context));
