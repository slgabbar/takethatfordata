...
static size_t run_closures(grpc_exec_ctx *exec_ctx, grpc_closure_list list) ***REMOVED***
  size_t n = 0;
  ...
  while (c != NULL) ***REMOVED***
    ...
    if (GRPC_TRACER_ON(executor_trace)) ***REMOVED***
#ifndef NDEBUG
...
bool grpc_executor_is_threaded() ***REMOVED***
  return gpr_atm_no_barrier_load(&g_cur_threads) > 0;
***REMOVED***

void grpc_executor_set_threading(grpc_exec_ctx *exec_ctx, bool threading) ***REMOVED***
  gpr_atm cur_threads = gpr_atm_no_barrier_load(&g_cur_threads);
  if (threading) ***REMOVED***
    ...
    for (size_t i = 0; i < g_max_threads; i++) ***REMOVED***
  ...
  ***REMOVED*** else ***REMOVED***
    ...
    ***REMOVED***
    ...
    for (gpr_atm i = 0; i < g_cur_threads; i++) ***REMOVED***
...
void grpc_executor_init(grpc_exec_ctx *exec_ctx) ***REMOVED***
  grpc_register_tracer(&executor_trace);
  gpr_atm_no_barrier_store(&g_cur_threads, 0);
  grpc_executor_set_threading(exec_ctx, true);
***REMOVED***

void grpc_executor_shutdown(grpc_exec_ctx *exec_ctx) ***REMOVED***
  grpc_executor_set_threading(exec_ctx, false);
***REMOVED***

static void executor_thread(void *arg) ***REMOVED***
  ...
  for (;;) ***REMOVED***
    ...
    ***REMOVED***
    ...
    while (grpc_closure_list_empty(ts->elems) && !ts->shutdown) ***REMOVED***
      ts->queued_long_job = false;
      gpr_cv_wait(&ts->cv, &ts->mu, gpr_inf_future(GPR_CLOCK_REALTIME));
    ***REMOVED***
    if (ts->shutdown) ***REMOVED***
      if (GRPC_TRACER_ON(executor_trace)) ***REMOVED***
...
static void executor_push(grpc_exec_ctx *exec_ctx, grpc_closure *closure,
                          grpc_error *error, bool is_short) ***REMOVED***
  bool retry_push;
  if (is_short) ***REMOVED***
    ...
    ***REMOVED***
    ...
    for (;;) ***REMOVED***
      ...
      ***REMOVED***
      ...
      if (ts->queued_long_job) ***REMOVED***
        ...
        ***REMOVED***
        continue;
      ***REMOVED***
      if (grpc_closure_list_empty(ts->elems)) ***REMOVED***
      ...
      ***REMOVED***
      ...
      break;
    ***REMOVED***
    if (try_new_thread && gpr_spinlock_trylock(&g_adding_thread_lock)) ***REMOVED***
      cur_thread_count = (size_t)gpr_atm_no_barrier_load(&g_cur_threads);
      if (cur_thread_count < g_max_threads) ***REMOVED***
      ...
      ***REMOVED***
      gpr_spinlock_unlock(&g_adding_thread_lock);
    ***REMOVED***
    if (retry_push) ***REMOVED***
      GRPC_STATS_INC_EXECUTOR_PUSH_RETRIES(exec_ctx);
    ***REMOVED***
  ***REMOVED*** while (retry_push);
***REMOVED***
...
static const grpc_closure_scheduler_vtable executor_vtable_short = ***REMOVED***
    executor_push_short, executor_push_short, "executor"***REMOVED***;
static grpc_closure_scheduler executor_scheduler_short = ***REMOVED***
    &executor_vtable_short***REMOVED***;

static const grpc_closure_scheduler_vtable executor_vtable_long = ***REMOVED***
    executor_push_long, executor_push_long, "executor"***REMOVED***;
static grpc_closure_scheduler executor_scheduler_long = ***REMOVED***&executor_vtable_long***REMOVED***;

grpc_closure_scheduler *grpc_executor_scheduler(
    grpc_executor_job_length length) ***REMOVED***
