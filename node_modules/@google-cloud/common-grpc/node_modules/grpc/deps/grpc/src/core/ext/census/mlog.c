...
static void cl_block_set_bytes_committed(cl_block* block,
                                         size_t bytes_committed) ***REMOVED***
  gpr_atm_rel_store(&block->bytes_committed, (gpr_atm)bytes_committed);
***REMOVED***

static size_t cl_block_get_bytes_committed(cl_block* block) ***REMOVED***
...
static bool cl_block_try_disable_access(cl_block* block, int discard_data) ***REMOVED***
...
void census_log_initialize(size_t size_in_mb, int discard_old_records) ***REMOVED***
  ...
  for (uint32_t i = 0; i < g_log.num_blocks; ++i) ***REMOVED***
...
int64_t census_log_out_of_space_count(void) ***REMOVED***
  GPR_ASSERT(g_log.initialized);
