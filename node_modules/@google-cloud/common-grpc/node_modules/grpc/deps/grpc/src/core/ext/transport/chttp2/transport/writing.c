...
static void collapse_pings_from_into(grpc_chttp2_transport *t,
                                     ...
                                     grpc_chttp2_ping_queue *pq) ***REMOVED***
  for (size_t i = 0; i < GRPC_CHTTP2_PCL_COUNT; i++) ***REMOVED***
...
static void maybe_initiate_ping(grpc_exec_ctx *exec_ctx,
                                ...
                                grpc_chttp2_ping_type ping_type) ***REMOVED***
  ...
  ***REMOVED***
  if (!grpc_closure_list_empty(pq->lists[GRPC_CHTTP2_PCL_INFLIGHT])) ***REMOVED***
    ...
    if (GRPC_TRACER_ON(grpc_http_trace) ||
        GRPC_TRACER_ON(grpc_bdp_estimator_trace)) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (gpr_time_cmp(next_allowed_ping, now) > 0) ***REMOVED***
...
grpc_chttp2_begin_write_result grpc_chttp2_begin_write(
  ...
  if (t->dirtied_local_settings && !t->sent_local_settings) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (t->flow_control.remote_window > 0) ***REMOVED***
    while (grpc_chttp2_list_pop_stalled_by_transport(t, &s)) ***REMOVED***
  ...
  ***REMOVED***
  ...
  grpc_chttp2_begin_write_result result = ***REMOVED***false, false, false***REMOVED***;

  /* for each grpc_chttp2_stream that's become writable, frame it's data
     (according to available window sizes) and add to the output buffer */
  while (true) ***REMOVED***
    if (t->outbuf.length > target_write_size(t)) ***REMOVED***
      result.partial = true;
      break;
    ***REMOVED***

    if (!grpc_chttp2_list_pop_writable_stream(t, &s)) ***REMOVED***
      break;
    ***REMOVED***
    ...
    if (!sent_initial_metadata && s->send_initial_metadata != NULL) ***REMOVED***
      ...
      if (t->is_client || s->fetching_send_message != NULL ||
          ...
          !is_default_initial_metadata(s->send_initial_metadata)) ***REMOVED***
        grpc_encode_header_options hopt = ***REMOVED***
            ...
            .stats = &s->stats.outgoing***REMOVED***;
      ...
      ***REMOVED*** else ***REMOVED***
        ...
        if (s->send_initial_metadata->idx.named.status != NULL) ***REMOVED***
          extra_headers_for_trailing_metadata
              [num_extra_headers_for_trailing_metadata++] =
                  &s->send_initial_metadata->idx.named.status->md;
        ***REMOVED***
        if (s->send_initial_metadata->idx.named.content_type != NULL) ***REMOVED***
          extra_headers_for_trailing_metadata
              [num_extra_headers_for_trailing_metadata++] =
    ...
    ***REMOVED***
    ...
    if (stream_announce > 0) ***REMOVED***
      ...
      ***REMOVED***
      flow_control_writes++;
    ***REMOVED***
    if (sent_initial_metadata) ***REMOVED***
      /* send any body bytes, if allowed by flow control */
      if (s->flow_controlled_buffer.length > 0 ||
          s->compressed_data_buffer.length > 0) ***REMOVED***
        ...
        if (max_outgoing > 0) ***REMOVED***
          ...
          while ((s->flow_controlled_buffer.length > 0 ||
            ...
            if (s->compressed_data_buffer.length > 0) ***REMOVED***
              ...
              if (is_last_data_frame && s->send_trailing_metadata != NULL &&
                  s->stream_compression_ctx != NULL) ***REMOVED***
          ...
          ***REMOVED***
          if (!t->is_client) ***REMOVED***
            ...
            t->ping_recv_state.ping_strikes = 0;
          ***REMOVED***
          if (is_last_frame) ***REMOVED***
            s->send_trailing_metadata = NULL;
            s->sent_trailing_metadata = true;
            if (!t->is_client && !s->read_closed) ***REMOVED***
          ...
          ***REMOVED***
          message_writes++;
        ***REMOVED*** else if (t->flow_control.remote_window == 0) ***REMOVED***
          grpc_chttp2_list_add_stalled_by_transport(t, s);
          now_writing = true;
        ***REMOVED*** else if (stream_remote_window == 0) ***REMOVED***
      ...
      ***REMOVED***
      if (s->send_trailing_metadata != NULL &&
        ...
        if (grpc_metadata_batch_is_empty(s->send_trailing_metadata)) ***REMOVED***
  ...
  ***REMOVED***
  ...
  for (size_t i = 0; i < t->ping_ack_count; i++) ***REMOVED***
    grpc_slice_buffer_add(&t->outbuf,
