...
***REMOVED*** shadow_flow_control;

static void pretrace(shadow_flow_control* shadow_fc,
                     grpc_chttp2_transport_flowctl* tfc,
                     grpc_chttp2_stream_flowctl* sfc) ***REMOVED***
...
static char* fmt_int64_diff_str(int64_t old_val, int64_t new_val) ***REMOVED***
  char* str;
  if (old_val != new_val) ***REMOVED***
...
static char* fmt_uint32_diff_str(uint32_t old_val, uint32_t new_val) ***REMOVED***
  char* str;
  if (new_val > 0 && old_val != new_val) ***REMOVED***
...
static void posttrace(shadow_flow_control* shadow_fc,
                      grpc_chttp2_transport_flowctl* tfc,
                      grpc_chttp2_stream_flowctl* sfc, char* reason) ***REMOVED***
...
static char* urgency_to_string(grpc_chttp2_flowctl_urgency urgency) ***REMOVED***
  switch (urgency) ***REMOVED***
...
static void trace_action(grpc_chttp2_transport_flowctl* tfc,
                         grpc_chttp2_flowctl_action action) ***REMOVED***
...
static uint32_t grpc_chttp2_target_announced_window(
    const grpc_chttp2_transport_flowctl* tfc) ***REMOVED***
...
static void announced_window_delta_preupdate(grpc_chttp2_transport_flowctl* tfc,
                                             grpc_chttp2_stream_flowctl* sfc) ***REMOVED***
  if (sfc->announced_window_delta > 0) ***REMOVED***
    tfc->announced_stream_total_over_incoming_window -=
...
static void announced_window_delta_postupdate(
    grpc_chttp2_transport_flowctl* tfc, grpc_chttp2_stream_flowctl* sfc) ***REMOVED***
  if (sfc->announced_window_delta > 0) ***REMOVED***
    tfc->announced_stream_total_over_incoming_window +=
        sfc->announced_window_delta;
  ***REMOVED*** else ***REMOVED***
...
grpc_error* grpc_chttp2_flowctl_recv_data(grpc_chttp2_transport_flowctl* tfc,
                                          grpc_chttp2_stream_flowctl* sfc,
                                          int64_t incoming_frame_size) ***REMOVED***
  ...
  uint32_t acked_init_window =
      tfc->t->settings[GRPC_ACKED_SETTINGS]
                      [GRPC_CHTTP2_SETTINGS_INITIAL_WINDOW_SIZE];
  PRETRACE(tfc, sfc);
  if (incoming_frame_size > tfc->announced_window) ***REMOVED***
    char* msg;
    gpr_asprintf(&msg,
  ...
  ***REMOVED***
  ...
  if (sfc != NULL) ***REMOVED***
    ...
    if (incoming_frame_size > acked_stream_window) ***REMOVED***
      if (incoming_frame_size <= sent_stream_window) ***REMOVED***
        gpr_log(
            GPR_ERROR,
...
uint32_t grpc_chttp2_flowctl_maybe_send_transport_update(
    grpc_chttp2_transport_flowctl* tfc) ***REMOVED***
  PRETRACE(tfc, NULL);
  ...
  if (tfc->announced_window <= threshold_to_send_transport_window_update &&
      tfc->announced_window != target_announced_window) ***REMOVED***
...
uint32_t grpc_chttp2_flowctl_maybe_send_stream_update(
    grpc_chttp2_transport_flowctl* tfc, grpc_chttp2_stream_flowctl* sfc) ***REMOVED***
  PRETRACE(tfc, sfc);
  if (sfc->local_window_delta > sfc->announced_window_delta) ***REMOVED***
    uint32_t announce = (uint32_t)GPR_CLAMP(
...
void grpc_chttp2_flowctl_recv_transport_update(
    grpc_chttp2_transport_flowctl* tfc, uint32_t size) ***REMOVED***
  PRETRACE(tfc, NULL);
  tfc->remote_window += size;
...
void grpc_chttp2_flowctl_incoming_bs_update(grpc_chttp2_transport_flowctl* tfc,
  ...
  if (max_size_hint >= UINT32_MAX - sent_init_window) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (max_recv_bytes >= have_already) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (sfc->local_window_delta < max_recv_bytes) ***REMOVED***
...
static grpc_chttp2_flowctl_urgency delta_is_significant(
    ...
    grpc_chttp2_setting_id setting_id) ***REMOVED***
  int64_t delta = (int64_t)value -
                  (int64_t)tfc->t->settings[GRPC_LOCAL_SETTINGS][setting_id];
  // TODO(ncteisen): tune this
  if (delta != 0 && (delta <= -value / 5 || delta >= value / 5)) ***REMOVED***
...
grpc_chttp2_flowctl_action grpc_chttp2_flowctl_get_action(
  ...
  uint32_t target_announced_window = grpc_chttp2_target_announced_window(tfc);
  if (tfc->announced_window < target_announced_window / 2) ***REMOVED***
    action.send_transport_update = GRPC_CHTTP2_FLOWCTL_UPDATE_IMMEDIATELY;
  ***REMOVED***
  // TODO(ncteisen): tune this
  if (sfc != NULL && !sfc->s->read_closed) ***REMOVED***
    uint32_t sent_init_window =
        tfc->t->settings[GRPC_SENT_SETTINGS]
                        [GRPC_CHTTP2_SETTINGS_INITIAL_WINDOW_SIZE];
    if ((int64_t)sfc->local_window_delta >
        ...
        (int64_t)sfc->announced_window_delta + sent_init_window <=
            sent_init_window / 2) ***REMOVED***
      action.send_stream_update = GRPC_CHTTP2_FLOWCTL_UPDATE_IMMEDIATELY;
    ***REMOVED*** else if (sfc->local_window_delta > sfc->announced_window_delta) ***REMOVED***
...
grpc_chttp2_flowctl_action grpc_chttp2_flowctl_get_bdp_action(
  ...
  grpc_chttp2_flowctl_action action;
  memset(&action, 0, sizeof(action));
  if (tfc->enable_bdp_probe) ***REMOVED***
    action.need_ping = grpc_bdp_estimator_need_ping(&tfc->bdp_estimator);

    // get bdp estimate and update initial_window accordingly.
    int64_t estimate = -1;
    int32_t bdp = -1;
    if (grpc_bdp_estimator_get_estimate(&tfc->bdp_estimator, &estimate)) ***REMOVED***
      ...
      grpc_chttp2_flowctl_urgency init_window_update_urgency =
          delta_is_significant(tfc, bdp,
                               GRPC_CHTTP2_SETTINGS_INITIAL_WINDOW_SIZE);
      if (init_window_update_urgency != GRPC_CHTTP2_FLOWCTL_NO_ACTION_NEEDED) ***REMOVED***
    ...
    ***REMOVED***
    ...
    double bw_dbl = -1;
    if (grpc_bdp_estimator_get_bw(&tfc->bdp_estimator, &bw_dbl)) ***REMOVED***
      ...
      if (frame_size_urgency != GRPC_CHTTP2_FLOWCTL_NO_ACTION_NEEDED) ***REMOVED***
        if (frame_size_urgency > action.send_setting_update) ***REMOVED***
