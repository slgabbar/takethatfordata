...
static void ssl_config_pem_key_cert_pair_destroy(
    tsi_ssl_pem_key_cert_pair *kp) ***REMOVED***
  if (kp == NULL) return;
...
static grpc_security_status ssl_create_security_connector(
  ...
  for (size_t i = 0; args && i < args->num_args; i++) ***REMOVED***
    grpc_arg *arg = &args->args[i];
    if (strcmp(arg->key, GRPC_SSL_TARGET_NAME_OVERRIDE_ARG) == 0 &&
        arg->type == GRPC_ARG_STRING) ***REMOVED***
...
static grpc_channel_credentials_vtable ssl_vtable = ***REMOVED***
    ssl_destruct, ssl_create_security_connector, NULL***REMOVED***;

static void ssl_build_config(const char *pem_root_certs,
                             grpc_ssl_pem_key_cert_pair *pem_key_cert_pair,
                             grpc_ssl_config *config) ***REMOVED***
  if (pem_root_certs != NULL) ***REMOVED***
    config->pem_root_certs = gpr_strdup(pem_root_certs);
  ***REMOVED***
  if (pem_key_cert_pair != NULL) ***REMOVED***
...
static void ssl_server_destruct(grpc_exec_ctx *exec_ctx,
  ...
  size_t i;
  for (i = 0; i < c->config.num_key_cert_pairs; i++) ***REMOVED***
...
static grpc_server_credentials_vtable ssl_server_vtable = ***REMOVED***
    ssl_server_destruct, ssl_server_create_security_connector***REMOVED***;

static void ssl_build_server_config(
    const char *pem_root_certs, grpc_ssl_pem_key_cert_pair *pem_key_cert_pairs,
    size_t num_key_cert_pairs,
    grpc_ssl_client_certificate_request_type client_certificate_request,
    grpc_ssl_server_config *config) ***REMOVED***
  size_t i;
  config->client_certificate_request = client_certificate_request;
  if (pem_root_certs != NULL) ***REMOVED***
    config->pem_root_certs = gpr_strdup(pem_root_certs);
  ***REMOVED***
  if (num_key_cert_pairs > 0) ***REMOVED***
...
grpc_server_credentials *grpc_ssl_server_credentials_create(
    const char *pem_root_certs, grpc_ssl_pem_key_cert_pair *pem_key_cert_pairs,
    size_t num_key_cert_pairs, int force_client_auth, void *reserved) ***REMOVED***
