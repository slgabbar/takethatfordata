...
static jose_header *jose_header_from_json(grpc_exec_ctx *exec_ctx,
  ...
  h->buffer = buffer;
  for (cur = json->child; cur != NULL; cur = cur->next) ***REMOVED***
    if (strcmp(cur->key, "alg") == 0) ***REMOVED***
      ...
      if (cur->type != GRPC_JSON_STRING || strncmp(cur->value, "RS", 2) ||
          evp_md_from_alg(cur->value) == NULL) ***REMOVED***
...
grpc_jwt_verifier_status grpc_jwt_claims_check(const grpc_jwt_claims *claims,
  ...
  ***REMOVED***
  ...
  if (grpc_jwt_issuer_email_domain(claims->iss) != NULL &&
      claims->sub != NULL && strcmp(claims->iss, claims->sub) != 0) ***REMOVED***
    gpr_log(GPR_ERROR,
...
gpr_timespec grpc_jwt_verifier_max_delay = ***REMOVED***60, 0, GPR_TIMESPAN***REMOVED***;
...
static int RSA_set0_key(RSA *r, BIGNUM *n, BIGNUM *e, BIGNUM *d) ***REMOVED***
   ...
   */
  if ((r->n == NULL && n == NULL) || (r->e == NULL && e == NULL)) ***REMOVED***
    return 0;
  ***REMOVED***
...
static EVP_PKEY *pkey_from_jwk(grpc_exec_ctx *exec_ctx, const grpc_json *json,
  ...
  ***REMOVED***
  ...
  if (rsa == NULL) ***REMOVED***
    ...
    goto end;
  ***REMOVED***
  for (key_prop = json->child; key_prop != NULL; key_prop = key_prop->next) ***REMOVED***
    ...
    ***REMOVED***
  ***REMOVED***
  if (tmp_e == NULL || tmp_n == NULL) ***REMOVED***
    gpr_log(GPR_ERROR, "Missing RSA public key field.");
    goto end;
  ***REMOVED***
  if (!RSA_set0_key(rsa, tmp_n, tmp_e, NULL)) ***REMOVED***
...
static EVP_PKEY *find_verification_key(grpc_exec_ctx *exec_ctx,
  ...
  ***REMOVED***
  ...
  for (jkey = jwk_keys->child; jkey != NULL; jkey = jkey->next) ***REMOVED***
    grpc_json *key_prop;
    const char *alg = NULL;
    const char *kid = NULL;
    const char *kty = NULL;
    ...
    for (key_prop = jkey->child; key_prop != NULL; key_prop = key_prop->next) ***REMOVED***
...
static int verify_jwt_signature(EVP_PKEY *key, const char *alg,
  ...
  if (md_ctx == NULL) ***REMOVED***
    gpr_log(GPR_ERROR, "Could not create EVP_MD_CTX.");
    goto end;
  ***REMOVED***
  if (EVP_DigestVerifyInit(md_ctx, NULL, md, NULL, key) != 1) ***REMOVED***
...
static void on_keys_retrieved(grpc_exec_ctx *exec_ctx, void *user_data,
  ...
  ***REMOVED***
  ...
  if (verification_key == NULL) ***REMOVED***
...
static void retrieve_key_and_verify(grpc_exec_ctx *exec_ctx,
  ...
  iss = ctx->claims->iss;
  if (ctx->header->kid == NULL) ***REMOVED***
