...
static grpc_error *prepare_socket(const grpc_resolved_address *addr, int fd,
  ...
  if (err != GRPC_ERROR_NONE) goto error;
  if (!grpc_is_unix_socket(addr)) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (channel_args) ***REMOVED***
    for (size_t i = 0; i < channel_args->num_args; i++) ***REMOVED***
      if (0 == strcmp(channel_args->args[i].key, GRPC_ARG_SOCKET_MUTATOR)) ***REMOVED***
...
static void tc_on_alarm(grpc_exec_ctx *exec_ctx, void *acp, grpc_error *error) ***REMOVED***
  int done;
...
grpc_endpoint *grpc_tcp_client_create_from_fd(
    ...
    const char *addr_str) ***REMOVED***
  return grpc_tcp_create(exec_ctx, fd, channel_args, addr_str);
***REMOVED***

static void on_writable(grpc_exec_ctx *exec_ctx, void *acp, grpc_error *error) ***REMOVED***
  async_connect *ac = (async_connect *)acp;
  int so_error = 0;
...
static void tcp_client_connect_impl(grpc_exec_ctx *exec_ctx,
  ...
  ***REMOVED***
  if (dsmode == GRPC_DSMODE_IPV4) ***REMOVED***
    ...
    addr = &addr4_copy;
  ***REMOVED***
  if ((error = prepare_socket(addr, fd, channel_args)) != GRPC_ERROR_NONE) ***REMOVED***
    GRPC_CLOSURE_SCHED(exec_ctx, closure, error);
    return;
  ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (errno != EWOULDBLOCK && errno != EINPROGRESS) ***REMOVED***
