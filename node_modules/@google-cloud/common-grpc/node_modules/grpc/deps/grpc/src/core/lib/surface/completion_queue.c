...
***REMOVED*** cq_poller_vtable;

typedef struct non_polling_worker ***REMOVED***
  gpr_cv cv;
  bool kicked;
  struct non_polling_worker *next;
  struct non_polling_worker *prev;
***REMOVED*** non_polling_worker;

typedef struct ***REMOVED***
  gpr_mu mu;
  non_polling_worker *root;
  grpc_closure *shutdown;
***REMOVED*** non_polling_poller;

static size_t non_polling_poller_size(void) ***REMOVED***
  return sizeof(non_polling_poller);
***REMOVED***

static void non_polling_poller_init(grpc_pollset *pollset, gpr_mu **mu) ***REMOVED***
...
static grpc_error *non_polling_poller_work(grpc_exec_ctx *exec_ctx,
  ...
  ***REMOVED***
  ...
  while (!npp->shutdown && !w.kicked && !gpr_cv_wait(&w.cv, &npp->mu, deadline))
    ;
  if (&w == npp->root) ***REMOVED***
    npp->root = w.next;
    if (&w == npp->root) ***REMOVED***
      if (npp->shutdown) ***REMOVED***
...
static void non_polling_poller_shutdown(grpc_exec_ctx *exec_ctx,
  ...
  ***REMOVED*** else ***REMOVED***
    ...
    do ***REMOVED***
      gpr_cv_signal(&w->cv);
      w = w->next;
    ***REMOVED*** while (w != p->root);
  ***REMOVED***
***REMOVED***

static const cq_poller_vtable g_poller_vtable_by_poller_type[] = ***REMOVED***
    /* GRPC_CQ_DEFAULT_POLLING */
    ***REMOVED***.can_get_pollset = true,
     .can_listen = true,
     ...
     .destroy = grpc_pollset_destroy***REMOVED***,
    ...
    ***REMOVED***.can_get_pollset = false,
     ...
     .destroy = non_polling_poller_destroy***REMOVED***,
***REMOVED***;

typedef struct cq_vtable ***REMOVED***
...
typedef struct grpc_cq_event_queue ***REMOVED***
...
***REMOVED*** grpc_cq_event_queue;

typedef struct cq_next_data ***REMOVED***
...
***REMOVED*** cq_next_data;

typedef struct cq_pluck_data ***REMOVED***
...
***REMOVED*** cq_pluck_data;

/* Completion queue structure */
struct grpc_completion_queue ***REMOVED***
...
static const cq_vtable g_cq_vtable[] = ***REMOVED***
    /* GRPC_CQ_NEXT */
    ***REMOVED***GRPC_CQ_NEXT, sizeof(cq_next_data), cq_init_next, cq_shutdown_next,
     cq_destroy_next, cq_begin_op_for_next, cq_end_op_for_next, cq_next, NULL***REMOVED***,
    /* GRPC_CQ_PLUCK */
    ***REMOVED***GRPC_CQ_PLUCK, sizeof(cq_pluck_data), cq_init_pluck, cq_shutdown_pluck,
...
#define GRPC_SURFACE_TRACE_RETURNED_EVENT(cq, event)    \
  if (GRPC_TRACER_ON(grpc_api_trace) &&                 \
      (GRPC_TRACER_ON(grpc_cq_pluck_trace) ||           \
       (event)->type != GRPC_QUEUE_TIMEOUT)) ***REMOVED***          \
...
static void cq_event_queue_init(grpc_cq_event_queue *q) ***REMOVED***
  gpr_mpscq_init(&q->queue);
  q->queue_lock = GPR_SPINLOCK_INITIALIZER;
  gpr_atm_no_barrier_store(&q->num_queue_items, 0);
***REMOVED***

static void cq_event_queue_destroy(grpc_cq_event_queue *q) ***REMOVED***
  gpr_mpscq_destroy(&q->queue);
***REMOVED***

static bool cq_event_queue_push(grpc_cq_event_queue *q, grpc_cq_completion *c) ***REMOVED***
  gpr_mpscq_push(&q->queue, (gpr_mpscq_node *)c);
  return gpr_atm_no_barrier_fetch_add(&q->num_queue_items, 1) == 0;
***REMOVED***

static grpc_cq_completion *cq_event_queue_pop(grpc_cq_event_queue *q) ***REMOVED***
  grpc_cq_completion *c = NULL;
  if (gpr_spinlock_trylock(&q->queue_lock)) ***REMOVED***
...
static long cq_event_queue_num_items(grpc_cq_event_queue *q) ***REMOVED***
  return (long)gpr_atm_no_barrier_load(&q->num_queue_items);
***REMOVED***

grpc_completion_queue *grpc_completion_queue_create_internal(
    grpc_cq_completion_type completion_type,
    grpc_cq_polling_type polling_type) ***REMOVED***
  grpc_completion_queue *cq;
...
static void cq_init_next(void *ptr) ***REMOVED***
...
static void cq_destroy_next(void *ptr) ***REMOVED***
  cq_next_data *cqd = (cq_next_data *)ptr;
  GPR_ASSERT(cq_event_queue_num_items(&cqd->queue) == 0);
  cq_event_queue_destroy(&cqd->queue);
***REMOVED***

static void cq_init_pluck(void *ptr) ***REMOVED***
...
static void cq_destroy_pluck(void *ptr) ***REMOVED***
  cq_pluck_data *cqd = (cq_pluck_data *)ptr;
  GPR_ASSERT(cqd->completed_head.next == (uintptr_t)&cqd->completed_head);
***REMOVED***

grpc_cq_completion_type grpc_get_cq_completion_type(grpc_completion_queue *cq) ***REMOVED***
  return cq->vtable->cq_completion_type;
***REMOVED***

int grpc_get_cq_poll_num(grpc_completion_queue *cq) ***REMOVED***
  int cur_num_polls;
  gpr_mu_lock(cq->mu);
...
void grpc_cq_internal_ref(grpc_completion_queue *cq, const char *reason,
  ...
  if (GRPC_TRACER_ON(grpc_trace_cq_refcount)) ***REMOVED***
  ...
  ***REMOVED***
#else
void grpc_cq_internal_ref(grpc_completion_queue *cq) ***REMOVED***
#endif
...
#endif
  if (gpr_unref(&cq->owning_refs)) ***REMOVED***
...
#ifndef NDEBUG
static void cq_check_tag(grpc_completion_queue *cq, void *tag, bool lock_cq) ***REMOVED***
  int found = 0;
  if (lock_cq) ***REMOVED***
    gpr_mu_lock(cq->mu);
  ***REMOVED***

  for (int i = 0; i < (int)cq->outstanding_tag_count; i++) ***REMOVED***
    if (cq->outstanding_tags[i] == tag) ***REMOVED***
...
***REMOVED***
#else
static void cq_check_tag(grpc_completion_queue *cq, void *tag, bool lock_cq) ***REMOVED******REMOVED***
#endif
...
static bool atm_inc_if_nonzero(gpr_atm *counter) ***REMOVED***
  while (true) ***REMOVED***
    ...
    if (count == 0) ***REMOVED***
      return false;
    ***REMOVED*** else if (gpr_atm_full_cas(counter, count, count + 1)) ***REMOVED***
      break;
    ***REMOVED***
  ***REMOVED***
...
static bool cq_begin_op_for_next(grpc_completion_queue *cq, void *tag) ***REMOVED***
  cq_next_data *cqd = (cq_next_data *)DATA_FROM_CQ(cq);
  return atm_inc_if_nonzero(&cqd->pending_events);
***REMOVED***

static bool cq_begin_op_for_pluck(grpc_completion_queue *cq, void *tag) ***REMOVED***
  cq_pluck_data *cqd = (cq_pluck_data *)DATA_FROM_CQ(cq);
  return atm_inc_if_nonzero(&cqd->pending_events);
***REMOVED***

bool grpc_cq_begin_op(grpc_completion_queue *cq, void *tag) ***REMOVED***
#ifndef NDEBUG
  gpr_mu_lock(cq->mu);
  if (cq->outstanding_tag_count == cq->outstanding_tag_capacity) ***REMOVED***
...
static void cq_end_op_for_next(grpc_exec_ctx *exec_ctx,
                               ...
                               void (*done)(grpc_exec_ctx *exec_ctx,
                                            void *done_arg,
                                            grpc_cq_completion *storage),
                               void *done_arg, grpc_cq_completion *storage) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (!will_definitely_shutdown) ***REMOVED***
    ...
    if (is_first) ***REMOVED***
      ...
      if (kick_error != GRPC_ERROR_NONE) ***REMOVED***
        const char *msg = grpc_error_string(kick_error);
        gpr_log(GPR_ERROR, "Kick failed: %s", msg);
        GRPC_ERROR_UNREF(kick_error);
      ***REMOVED***
    ***REMOVED***
    if (gpr_atm_full_fetch_add(&cqd->pending_events, -1) == 1) ***REMOVED***
...
static void cq_end_op_for_pluck(grpc_exec_ctx *exec_ctx,
  ...
  ***REMOVED*** else ***REMOVED***
    ...
    for (int i = 0; i < cqd->num_pluckers; i++) ***REMOVED***
      if (cqd->pluckers[i].tag == tag) ***REMOVED***
...
typedef struct ***REMOVED***
  ...
  void *tag; /* for pluck */
  bool first_loop;
***REMOVED*** cq_is_finished_arg;

static bool cq_is_next_finished(grpc_exec_ctx *exec_ctx, void *arg) ***REMOVED***
  ...
  if (current_last_seen_things_queued_ever != a->last_seen_things_queued_ever) ***REMOVED***
    ...
    if (a->stolen_completion != NULL) ***REMOVED***
      return true;
    ***REMOVED***
  ***REMOVED***
  return !a->first_loop &&
...
#ifndef NDEBUG
static void dump_pending_tags(grpc_completion_queue *cq) ***REMOVED***
  ...
  gpr_mu_lock(cq->mu);
  for (size_t i = 0; i < cq->outstanding_tag_count; i++) ***REMOVED***
    char *s;
...
***REMOVED***
#else
static void dump_pending_tags(grpc_completion_queue *cq) ***REMOVED******REMOVED***
#endif
...
static grpc_event cq_next(grpc_completion_queue *cq, gpr_timespec deadline,
  ...
  cq_is_finished_arg is_finished_arg = ***REMOVED***
  ...
  for (;;) ***REMOVED***
    ...
    if (is_finished_arg.stolen_completion != NULL) ***REMOVED***
    ...
    ***REMOVED*** else ***REMOVED***
      ...
      if (cq_event_queue_num_items(&cqd->queue) > 0) ***REMOVED***
        iteration_deadline = gpr_time_0(GPR_CLOCK_MONOTONIC);
      ***REMOVED***
    ***REMOVED***

    if (gpr_atm_acq_load(&cqd->pending_events) == 0) ***REMOVED***
    ...
    ***REMOVED***
    ...
    if (!is_finished_arg.first_loop && gpr_time_cmp(now, deadline) >= 0) ***REMOVED***
      memset(&ret, 0, sizeof(ret));
  ...
  ***REMOVED***
  ...
  if (cq_event_queue_num_items(&cqd->queue) > 0 &&
      gpr_atm_acq_load(&cqd->pending_events) > 0) ***REMOVED***
    gpr_mu_lock(cq->mu);
    cq->poller_vtable->kick(&exec_ctx, POLLSET_FROM_CQ(cq), NULL);
    gpr_mu_unlock(cq->mu);
  ***REMOVED***
...
static void cq_shutdown_next(grpc_exec_ctx *exec_ctx,
  ...
  GRPC_CQ_INTERNAL_REF(cq, "shutting_down");
  gpr_mu_lock(cq->mu);
  if (cqd->shutdown_called) ***REMOVED***
    gpr_mu_unlock(cq->mu);
    GRPC_CQ_INTERNAL_UNREF(exec_ctx, cq, "shutting_down");
    return;
  ***REMOVED***
...
grpc_event grpc_completion_queue_next(grpc_completion_queue *cq,
                                      gpr_timespec deadline, void *reserved) ***REMOVED***
...
static int add_plucker(grpc_completion_queue *cq, void *tag,
  ...
  cq_pluck_data *cqd = (cq_pluck_data *)DATA_FROM_CQ(cq);
  if (cqd->num_pluckers == GRPC_MAX_COMPLETION_QUEUE_PLUCKERS) ***REMOVED***
    return 0;
  ***REMOVED***
...
static void del_plucker(grpc_completion_queue *cq, void *tag,
  ...
  cq_pluck_data *cqd = (cq_pluck_data *)DATA_FROM_CQ(cq);
  for (int i = 0; i < cqd->num_pluckers; i++) ***REMOVED***
    if (cqd->pluckers[i].tag == tag && cqd->pluckers[i].worker == worker) ***REMOVED***
      cqd->num_pluckers--;
...
static bool cq_is_pluck_finished(grpc_exec_ctx *exec_ctx, void *arg) ***REMOVED***
  cq_is_finished_arg *a = (cq_is_finished_arg *)arg;
  grpc_completion_queue *cq = a->cq;
  cq_pluck_data *cqd = (cq_pluck_data *)DATA_FROM_CQ(cq);
  ...
  if (current_last_seen_things_queued_ever != a->last_seen_things_queued_ever) ***REMOVED***
    ...
    while ((c = (grpc_cq_completion *)(prev->next & ~(uintptr_t)1)) !=
           &cqd->completed_head) ***REMOVED***
      if (c->tag == a->tag) ***REMOVED***
        prev->next = (prev->next & (uintptr_t)1) | (c->next & ~(uintptr_t)1);
        if (c == cqd->completed_tail) ***REMOVED***
...
static grpc_event cq_pluck(grpc_completion_queue *cq, void *tag,
  ...
  if (GRPC_TRACER_ON(grpc_cq_pluck_trace)) ***REMOVED***
    GRPC_API_TRACE(
    ...
    ***REMOVED***
    ...
    while ((c = (grpc_cq_completion *)(prev->next & ~(uintptr_t)1)) !=
      ...
      ***REMOVED***
      prev = c;
    ***REMOVED***
    if (gpr_atm_no_barrier_load(&cqd->shutdown)) ***REMOVED***
      gpr_mu_unlock(cq->mu);
      memset(&ret, 0, sizeof(ret));
      ret.type = GRPC_QUEUE_SHUTDOWN;
      break;
    ***REMOVED***
    if (!add_plucker(cq, tag, &worker)) ***REMOVED***
      gpr_log(GPR_DEBUG,
...
void grpc_completion_queue_shutdown(grpc_completion_queue *cq) ***REMOVED***
  grpc_exec_ctx exec_ctx = GRPC_EXEC_CTX_INIT;
...
void grpc_completion_queue_destroy(grpc_completion_queue *cq) ***REMOVED***
...
grpc_pollset *grpc_cq_pollset(grpc_completion_queue *cq) ***REMOVED***
  return cq->poller_vtable->can_get_pollset ? POLLSET_FROM_CQ(cq) : NULL;
***REMOVED***

bool grpc_cq_can_listen(grpc_completion_queue *cq) ***REMOVED***
