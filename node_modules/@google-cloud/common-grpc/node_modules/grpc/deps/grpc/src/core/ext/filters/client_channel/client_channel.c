...
static void method_parameters_free(grpc_exec_ctx *exec_ctx, void *value) ***REMOVED***
...
static bool parse_timeout(grpc_json *field, gpr_timespec *timeout) ***REMOVED***
  ...
  if (decimal_point != NULL) ***REMOVED***
    *decimal_point = '\0';
    timeout->tv_nsec = gpr_parse_nonnegative_int(decimal_point + 1);
    if (timeout->tv_nsec == -1) ***REMOVED***
      gpr_free(buf);
      return false;
    ***REMOVED***
    // There should always be exactly 3, 6, or 9 fractional digits.
    int multiplier = 1;
    switch (strlen(decimal_point + 1)) ***REMOVED***
      case 9:
        break;
      case 6:
        multiplier *= 1000;
...
static void *method_parameters_create_from_json(const grpc_json *json) ***REMOVED***
  ...
  gpr_timespec timeout = ***REMOVED***0, 0, GPR_TIMESPAN***REMOVED***;
  for (grpc_json *field = json->child; field != NULL; field = field->next) ***REMOVED***
...
static void set_channel_connectivity_state_locked(grpc_exec_ctx *exec_ctx,
  ...
  if (chand->lb_policy != NULL) ***REMOVED***
    ...
    ***REMOVED***
  ***REMOVED***
  if (GRPC_TRACER_ON(grpc_client_channel_trace)) ***REMOVED***
...
static void on_lb_policy_state_changed_locked(grpc_exec_ctx *exec_ctx,
  ...
  if (w->lb_policy == w->chand->lb_policy) ***REMOVED***
    ...
    ***REMOVED***
    if (publish_state == GRPC_CHANNEL_SHUTDOWN && w->chand->resolver != NULL) ***REMOVED***
...
static void on_resolver_result_changed_locked(grpc_exec_ctx *exec_ctx,
  ...
  ***REMOVED***
  ...
  if (chand->resolver_result != NULL) ***REMOVED***
    // Find LB policy name.
    const char *lb_policy_name = NULL;
    const grpc_arg *channel_arg =
        grpc_channel_args_find(chand->resolver_result, GRPC_ARG_LB_POLICY_NAME);
    if (channel_arg != NULL) ***REMOVED***
    ...
    ***REMOVED***
    ...
    if (channel_arg != NULL && channel_arg->type == GRPC_ARG_POINTER) ***REMOVED***
      ...
      ***REMOVED***
      if (found_balancer_address) ***REMOVED***
        if (lb_policy_name != NULL && strcmp(lb_policy_name, "grpclb") != 0) ***REMOVED***
          gpr_log(GPR_INFO,
    ...
    ***REMOVED***
    ...
    if (chand->lb_policy != NULL && !lb_policy_name_changed) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (lb_policy_name_dup != NULL) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (chand->retry_throttle_data != NULL) ***REMOVED***
    grpc_server_retry_throttle_data_unref(chand->retry_throttle_data);
  ***REMOVED***
  chand->retry_throttle_data = retry_throttle_data;
  // Swap out the method params table.
  if (chand->method_params_table != NULL) ***REMOVED***
  ...
  ***REMOVED***
  ...
  // error or shutdown.
  if (error != GRPC_ERROR_NONE || chand->resolver == NULL) ***REMOVED***
    if (GRPC_TRACER_ON(grpc_client_channel_trace)) ***REMOVED***
      gpr_log(GPR_DEBUG, "chand=%p: shutting down", chand);
    ***REMOVED***
    if (chand->resolver != NULL) ***REMOVED***
      if (GRPC_TRACER_ON(grpc_client_channel_trace)) ***REMOVED***
...
static void cc_get_channel_info(grpc_exec_ctx *exec_ctx,
  ...
  channel_data *chand = (channel_data *)elem->channel_data;
  gpr_mu_lock(&chand->info_mu);
  if (info->lb_policy_name != NULL) ***REMOVED***
    *info->lb_policy_name = chand->info_lb_policy_name == NULL
                                ? NULL
                                : gpr_strdup(chand->info_lb_policy_name);
  ***REMOVED***
  if (info->service_config_json != NULL) ***REMOVED***
...
static void cc_destroy_channel_elem(grpc_exec_ctx *exec_ctx,
  ...
  if (chand->resolver != NULL) ***REMOVED***
    GRPC_CLOSURE_SCHED(
        ...
        GRPC_ERROR_NONE);
  ***REMOVED***
  if (chand->client_channel_factory != NULL) ***REMOVED***
...
static void waiting_for_pick_batches_fail(grpc_exec_ctx *exec_ctx,
  ...
  ***REMOVED***
  if (calld->initial_metadata_batch != NULL) ***REMOVED***
    grpc_transport_stream_op_batch_finish_with_failure(
...
static void apply_service_config_to_call_locked(grpc_exec_ctx *exec_ctx,
  ...
  ***REMOVED***
  if (chand->method_params_table != NULL) ***REMOVED***
    ...
    if (calld->method_params != NULL) ***REMOVED***
      method_parameters_ref(calld->method_params);
      // If the deadline from the service config is shorter than the one
      // from the client API, reset the deadline timer.
      if (chand->deadline_checking_enabled &&
          gpr_time_cmp(calld->method_params->timeout,
                       gpr_time_0(GPR_TIMESPAN)) != 0) ***REMOVED***
        const gpr_timespec per_method_deadline =
            gpr_time_add(calld->call_start_time, calld->method_params->timeout);
        if (gpr_time_cmp(per_method_deadline, calld->deadline) < 0) ***REMOVED***
...
static void create_subchannel_call_locked(grpc_exec_ctx *exec_ctx,
  ...
  const grpc_connected_subchannel_call_args call_args = ***REMOVED***
      ...
      .call_combiner = calld->call_combiner***REMOVED***;
...
static void pick_done_locked(grpc_exec_ctx *exec_ctx, grpc_call_element *elem,
  ...
  call_data *calld = (call_data *)elem->call_data;
  channel_data *chand = (channel_data *)elem->channel_data;
  if (calld->connected_subchannel == NULL) ***REMOVED***
...
static void pick_callback_cancel_locked(grpc_exec_ctx *exec_ctx, void *arg,
  ...
  channel_data *chand = (channel_data *)elem->channel_data;
  call_data *calld = (call_data *)elem->call_data;
  if (calld->lb_policy != NULL) ***REMOVED***
    if (GRPC_TRACER_ON(grpc_client_channel_trace)) ***REMOVED***
...
static void on_complete(grpc_exec_ctx *exec_ctx, void *arg, grpc_error *error) ***REMOVED***
  grpc_call_element *elem = (grpc_call_element *)arg;
  call_data *calld = (call_data *)elem->call_data;
  if (calld->retry_throttle_data != NULL) ***REMOVED***
    if (error == GRPC_ERROR_NONE) ***REMOVED***
  ...
  ***REMOVED***
  if (batch->cancel_stream) ***REMOVED***
    ...
    ***REMOVED***
    ...
    if (calld->subchannel_call != NULL) ***REMOVED***
...
static void cc_destroy_call_elem(grpc_exec_ctx *exec_ctx,
  ...
  ***REMOVED***
  for (size_t i = 0; i < GRPC_CONTEXT_COUNT; ++i) ***REMOVED***
    if (calld->subchannel_call_context[i].value != NULL) ***REMOVED***
...
static void try_to_connect_locked(grpc_exec_ctx *exec_ctx, void *arg,
  ...
  ***REMOVED*** else ***REMOVED***
    ...
    if (!chand->started_resolving && chand->resolver != NULL) ***REMOVED***
      start_resolving_locked(exec_ctx, chand);
    ***REMOVED***
  ***REMOVED***
...
static external_connectivity_watcher *lookup_external_connectivity_watcher(
  ...
  while (w != NULL && w->on_complete != on_complete) ***REMOVED***
    w = w->next;
  ***REMOVED***
  gpr_mu_unlock(&chand->external_connectivity_watcher_list_mu);
  return w;
