...
/* Adjusts a heap so as to move a hole at position i closer to the root,
   ...
   its argument. */
static void adjust_upwards(grpc_timer **first, uint32_t i, grpc_timer *t) ***REMOVED***
  while (i > 0) ***REMOVED***
...
static void maybe_shrink(grpc_timer_heap *heap) ***REMOVED***
  if (heap->timer_count >= 8 &&
      heap->timer_count <= heap->timer_capacity / SHRINK_FULLNESS_FACTOR / 2) ***REMOVED***
...
static void note_changed_priority(grpc_timer_heap *heap, grpc_timer *timer) ***REMOVED***
  uint32_t i = timer->heap_index;
  uint32_t parent = (uint32_t)(((int)i - 1) / 2);
  if (heap->timers[parent]->deadline > timer->deadline) ***REMOVED***
...
void grpc_timer_heap_init(grpc_timer_heap *heap) ***REMOVED***
  memset(heap, 0, sizeof(*heap));
***REMOVED***

void grpc_timer_heap_destroy(grpc_timer_heap *heap) ***REMOVED*** gpr_free(heap->timers); ***REMOVED***

int grpc_timer_heap_add(grpc_timer_heap *heap, grpc_timer *timer) ***REMOVED***
  if (heap->timer_count == heap->timer_capacity) ***REMOVED***
    heap->timer_capacity =
...
void grpc_timer_heap_remove(grpc_timer_heap *heap, grpc_timer *timer) ***REMOVED***
  uint32_t i = timer->heap_index;
  if (i == heap->timer_count - 1) ***REMOVED***
    heap->timer_count--;
    maybe_shrink(heap);
    return;
  ***REMOVED***
...
int grpc_timer_heap_is_empty(grpc_timer_heap *heap) ***REMOVED***
  return heap->timer_count == 0;
***REMOVED***

grpc_timer *grpc_timer_heap_top(grpc_timer_heap *heap) ***REMOVED***
  return heap->timers[0];
***REMOVED***

void grpc_timer_heap_pop(grpc_timer_heap *heap) ***REMOVED***
