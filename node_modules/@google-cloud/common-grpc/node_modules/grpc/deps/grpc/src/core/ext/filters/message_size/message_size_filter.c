...
typedef struct message_size_limits ***REMOVED***
  int max_send_size;
  int max_recv_size;
***REMOVED*** message_size_limits;

static void message_size_limits_free(grpc_exec_ctx* exec_ctx, void* value) ***REMOVED***
  gpr_free(value);
***REMOVED***

static void* message_size_limits_create_from_json(const grpc_json* json) ***REMOVED***
  int max_request_message_bytes = -1;
  int max_response_message_bytes = -1;
  for (grpc_json* field = json->child; field != NULL; field = field->next) ***REMOVED***
    if (field->key == NULL) continue;
    if (strcmp(field->key, "maxRequestMessageBytes") == 0) ***REMOVED***
      if (max_request_message_bytes >= 0) return NULL;  // Duplicate.
      if (field->type != GRPC_JSON_STRING && field->type != GRPC_JSON_NUMBER) ***REMOVED***
        return NULL;
      ***REMOVED***
      max_request_message_bytes = gpr_parse_nonnegative_int(field->value);
      if (max_request_message_bytes == -1) return NULL;
    ***REMOVED*** else if (strcmp(field->key, "maxResponseMessageBytes") == 0) ***REMOVED***
...
static void recv_message_ready(grpc_exec_ctx* exec_ctx, void* user_data,
  ...
  if (*calld->recv_message != NULL && calld->limits.max_recv_size >= 0 &&
      (*calld->recv_message)->length > (size_t)calld->limits.max_recv_size) ***REMOVED***
    char* message_string;
...
static void start_transport_stream_op_batch(
  ...
  if (op->send_message && calld->limits.max_send_size >= 0 &&
      op->payload->send_message.send_message->length >
          (size_t)calld->limits.max_send_size) ***REMOVED***
    char* message_string;
...
static int default_size(const grpc_channel_args* args,
                        int without_minimal_stack) ***REMOVED***
  if (grpc_channel_args_want_minimal_stack(args)) ***REMOVED***
    return -1;
  ***REMOVED***
...
message_size_limits get_message_size_limits(
  ...
  for (size_t i = 0; i < channel_args->num_args; ++i) ***REMOVED***
    if (strcmp(channel_args->args[i].key, GRPC_ARG_MAX_SEND_MESSAGE_LENGTH) ==
        0) ***REMOVED***
      const grpc_integer_options options = ***REMOVED***lim.max_send_size, -1, INT_MAX***REMOVED***;
      lim.max_send_size =
          grpc_channel_arg_get_integer(&channel_args->args[i], options);
    ***REMOVED***
    if (strcmp(channel_args->args[i].key,
               GRPC_ARG_MAX_RECEIVE_MESSAGE_LENGTH) == 0) ***REMOVED***
      const grpc_integer_options options = ***REMOVED***lim.max_recv_size, -1, INT_MAX***REMOVED***;
      lim.max_recv_size =
          grpc_channel_arg_get_integer(&channel_args->args[i], options);
    ***REMOVED***
  ***REMOVED***
...
const grpc_channel_filter grpc_message_size_filter = ***REMOVED***
...
static bool maybe_add_message_size_filter(grpc_exec_ctx* exec_ctx,
  ...
  if (lim.max_send_size != -1 || lim.max_recv_size != -1) ***REMOVED***
    enable = true;
  ***REMOVED***
  const grpc_arg* a =
...
void grpc_message_size_filter_init(void) ***REMOVED***
...
void grpc_message_size_filter_shutdown(void) ***REMOVED******REMOVED***
