...
static grpc_error* cv_fd_init(grpc_wakeup_fd* fd_info) ***REMOVED***
  unsigned int i, newsize;
  int idx;
  gpr_mu_lock(&g_cvfds.mu);
  if (!g_cvfds.free_fds) ***REMOVED***
    newsize = GPR_MIN(g_cvfds.size * 2, g_cvfds.size + MAX_TABLE_RESIZE);
    g_cvfds.cvfds =
        (fd_node*)gpr_realloc(g_cvfds.cvfds, sizeof(fd_node) * newsize);
    for (i = g_cvfds.size; i < newsize; i++) ***REMOVED***
...
static grpc_error* cv_fd_wakeup(grpc_wakeup_fd* fd_info) ***REMOVED***
  cv_node* cvn;
  gpr_mu_lock(&g_cvfds.mu);
...
static grpc_error* cv_fd_consume(grpc_wakeup_fd* fd_info) ***REMOVED***
  gpr_mu_lock(&g_cvfds.mu);
...
static void cv_fd_destroy(grpc_wakeup_fd* fd_info) ***REMOVED***
  if (fd_info->read_fd == 0) ***REMOVED***
    return;
  ***REMOVED***
  gpr_mu_lock(&g_cvfds.mu);
...
static int cv_check_availability(void) ***REMOVED*** return 1; ***REMOVED***

const grpc_wakeup_fd_vtable grpc_cv_wakeup_fd_vtable = ***REMOVED***
    cv_fd_init, cv_fd_consume, cv_fd_wakeup, cv_fd_destroy,
    cv_check_availability***REMOVED***;
