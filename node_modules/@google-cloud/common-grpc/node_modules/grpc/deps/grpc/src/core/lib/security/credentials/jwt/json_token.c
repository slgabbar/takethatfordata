...
/* 1 hour max. */
gpr_timespec grpc_max_auth_token_lifetime() ***REMOVED***
  gpr_timespec out;
  out.tv_sec = 3600;
  out.tv_nsec = 0;
...
int grpc_auth_json_key_is_valid(const grpc_auth_json_key *json_key) ***REMOVED***
  return (json_key != NULL) &&
         strcmp(json_key->type, GRPC_AUTH_JSON_TYPE_INVALID);
***REMOVED***

grpc_auth_json_key grpc_auth_json_key_create_from_json(const grpc_json *json) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (prop_value == NULL ||
      strcmp(prop_value, GRPC_AUTH_JSON_TYPE_SERVICE_ACCOUNT)) ***REMOVED***
    goto end;
  ***REMOVED***
...
static char *encoded_jwt_header(const char *key_id, const char *algorithm) ***REMOVED***
...
static char *encoded_jwt_claim(const grpc_auth_json_key *json_key,
                               const char *audience,
                               gpr_timespec token_lifetime, const char *scope) ***REMOVED***
  grpc_json *json = grpc_json_create(GRPC_JSON_OBJECT);
  grpc_json *child = NULL;
  char *json_str = NULL;
  char *result = NULL;
  ...
  if (gpr_time_cmp(token_lifetime, grpc_max_auth_token_lifetime()) > 0) ***REMOVED***
...
static char *dot_concat_and_free_strings(char *str1, char *str2) ***REMOVED***
...
const EVP_MD *openssl_digest_from_algorithm(const char *algorithm) ***REMOVED***
  if (strcmp(algorithm, GRPC_JWT_RSA_SHA256_ALGORITHM) == 0) ***REMOVED***
    return EVP_sha256();
  ***REMOVED*** else ***REMOVED***
...
char *compute_and_encode_signature(const grpc_auth_json_key *json_key,
  ...
  ***REMOVED***
  ...
  if (EVP_DigestSignInit(md_ctx, NULL, md, NULL, key) != 1) ***REMOVED***
    gpr_log(GPR_ERROR, "DigestInit failed.");
    goto end;
  ***REMOVED***
  if (EVP_DigestSignUpdate(md_ctx, to_sign, strlen(to_sign)) != 1) ***REMOVED***
    gpr_log(GPR_ERROR, "DigestUpdate failed.");
    goto end;
  ***REMOVED***
  if (EVP_DigestSignFinal(md_ctx, NULL, &sig_len) != 1) ***REMOVED***
    gpr_log(GPR_ERROR, "DigestFinal (get signature length) failed.");
    goto end;
  ***REMOVED***
  sig = gpr_malloc(sig_len);
  if (EVP_DigestSignFinal(md_ctx, sig, &sig_len) != 1) ***REMOVED***
...
char *grpc_jwt_encode_and_sign(const grpc_auth_json_key *json_key,
                               const char *audience,
                               gpr_timespec token_lifetime, const char *scope) ***REMOVED***
  if (g_jwt_encode_and_sign_override != NULL) ***REMOVED***
...
void grpc_jwt_encode_and_sign_set_override(
    grpc_jwt_encode_and_sign_override func) ***REMOVED***
