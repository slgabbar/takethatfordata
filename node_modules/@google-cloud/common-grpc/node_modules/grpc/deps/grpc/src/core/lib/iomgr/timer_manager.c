...
// shutdown)
static bool wait_until(gpr_timespec next) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (!g_kicked) ***REMOVED***
    ...
    /* If there's no timed waiter, we should become one: that waiter waits only
       ...
       waiter) */
    if (gpr_time_cmp(next, inf_future) != 0) ***REMOVED***
      if (!g_has_timed_waiter ||
          (gpr_time_cmp(next, g_timed_waiter_deadline) < 0)) ***REMOVED***
    ...
    ***REMOVED***
    ...
    if (GRPC_TRACER_ON(grpc_timer_check_trace) &&
        gpr_time_cmp(next, inf_future) == 0) ***REMOVED***
...
static void timer_main_loop(grpc_exec_ctx *exec_ctx) ***REMOVED***
  ...
  for (;;) ***REMOVED***
    ...
    switch (grpc_timer_check(exec_ctx, now, &next)) ***REMOVED***
