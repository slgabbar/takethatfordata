/*
 *
 * Copyright 2015 gRPC authors.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 *
 */

#include <grpc/support/alloc.h>

#include <grpc/support/log.h>
#include <grpc/support/port_platform.h>
#include <stdlib.h>
#include <string.h>
#include "src/core/lib/profiling/timers.h"

static void *zalloc_with_calloc(size_t sz) ***REMOVED*** return calloc(sz, 1); ***REMOVED***

static void *zalloc_with_gpr_malloc(size_t sz) ***REMOVED***
  void *p = gpr_malloc(sz);
  memset(p, 0, sz);
  return p;
***REMOVED***

static gpr_allocation_functions g_alloc_functions = ***REMOVED***malloc, zalloc_with_calloc,
                                                     realloc, free***REMOVED***;

gpr_allocation_functions gpr_get_allocation_functions() ***REMOVED***
  return g_alloc_functions;
***REMOVED***

void gpr_set_allocation_functions(gpr_allocation_functions functions) ***REMOVED***
  GPR_ASSERT(functions.malloc_fn != NULL);
  GPR_ASSERT(functions.realloc_fn != NULL);
  GPR_ASSERT(functions.free_fn != NULL);
  if (functions.zalloc_fn == NULL) ***REMOVED***
    functions.zalloc_fn = zalloc_with_gpr_malloc;
  ***REMOVED***
  g_alloc_functions = functions;
***REMOVED***

void *gpr_malloc(size_t size) ***REMOVED***
  void *p;
  if (size == 0) return NULL;
  GPR_TIMER_BEGIN("gpr_malloc", 0);
  p = g_alloc_functions.malloc_fn(size);
  if (!p) ***REMOVED***
    abort();
  ***REMOVED***
  GPR_TIMER_END("gpr_malloc", 0);
  return p;
***REMOVED***

void *gpr_zalloc(size_t size) ***REMOVED***
  void *p;
  if (size == 0) return NULL;
  GPR_TIMER_BEGIN("gpr_zalloc", 0);
  p = g_alloc_functions.zalloc_fn(size);
  if (!p) ***REMOVED***
    abort();
  ***REMOVED***
  GPR_TIMER_END("gpr_zalloc", 0);
  return p;
***REMOVED***

void gpr_free(void *p) ***REMOVED***
  GPR_TIMER_BEGIN("gpr_free", 0);
  g_alloc_functions.free_fn(p);
  GPR_TIMER_END("gpr_free", 0);
***REMOVED***

void *gpr_realloc(void *p, size_t size) ***REMOVED***
  if ((size == 0) && (p == NULL)) return NULL;
  GPR_TIMER_BEGIN("gpr_realloc", 0);
  p = g_alloc_functions.realloc_fn(p, size);
  if (!p) ***REMOVED***
    abort();
  ***REMOVED***
  GPR_TIMER_END("gpr_realloc", 0);
  return p;
***REMOVED***

void *gpr_malloc_aligned(size_t size, size_t alignment_log) ***REMOVED***
  size_t alignment = ((size_t)1) << alignment_log;
  size_t extra = alignment - 1 + sizeof(void *);
  void *p = gpr_malloc(size + extra);
  void **ret = (void **)(((uintptr_t)p + extra) & ~(alignment - 1));
  ret[-1] = p;
  return (void *)ret;
***REMOVED***

void gpr_free_aligned(void *ptr) ***REMOVED*** gpr_free(((void **)ptr)[-1]); ***REMOVED***
