...
static grpc_uri *bad_uri(const char *uri_text, size_t pos, const char *section,
                         bool suppress_errors) ***REMOVED***
  char *line_prefix;
  size_t pfx_len;

  if (!suppress_errors) ***REMOVED***
...
static char *decode_and_copy_component(grpc_exec_ctx *exec_ctx, const char *src,
                                       size_t begin, size_t end) ***REMOVED***
...
static bool valid_hex(char c) ***REMOVED***
...
static size_t parse_pchar(const char *uri_text, size_t i) ***REMOVED***
  ...
  switch (c) ***REMOVED***
    ...
    case '=':
      return 1;
    case '%': /* pct-encoded */
      if (valid_hex(uri_text[i + 1]) && valid_hex(uri_text[i + 2])) ***REMOVED***
        return 2;
      ***REMOVED***
      return NOT_SET;
  ***REMOVED***
...
/* *( pchar / "?" / "/" ) */
static int parse_fragment_or_query(const char *uri_text, size_t *i) ***REMOVED***
  char c;
  while ((c = uri_text[*i]) != 0) ***REMOVED***
    const size_t advance = parse_pchar(uri_text, *i); /* pchar */
    switch (advance) ***REMOVED***
      case 0: /* uri_text[i] isn't in pchar */
        /* maybe it's ? or / */
        if (uri_text[*i] == '?' || uri_text[*i] == '/') ***REMOVED***
          (*i)++;
          break;
...
static void parse_query_parts(grpc_uri *uri) ***REMOVED***
  ...
  ***REMOVED***
  ...
  for (size_t i = 0; i < uri->num_query_parts; i++) ***REMOVED***
    ...
    if (num_query_param_parts > 1) ***REMOVED***
    ...
    ***REMOVED***
    for (size_t j = 2; j < num_query_param_parts; j++) ***REMOVED***
...
grpc_uri *grpc_uri_parse(grpc_exec_ctx *exec_ctx, const char *uri_text,
  ...
  for (i = scheme_begin; uri_text[i] != 0; i++) ***REMOVED***
    if (uri_text[i] == ':') ***REMOVED***
      scheme_end = i;
      break;
    ***REMOVED***
    if (uri_text[i] >= 'a' && uri_text[i] <= 'z') continue;
    if (uri_text[i] >= 'A' && uri_text[i] <= 'Z') continue;
    if (i != scheme_begin) ***REMOVED***
    ...
    ***REMOVED***
    break;
  ***REMOVED***
  if (scheme_end == NOT_SET) ***REMOVED***
    return bad_uri(uri_text, i, "scheme", suppress_errors);
  ***REMOVED***

  if (uri_text[scheme_end + 1] == '/' && uri_text[scheme_end + 2] == '/') ***REMOVED***
    authority_begin = scheme_end + 3;
    for (i = authority_begin; uri_text[i] != 0 && authority_end == NOT_SET;
         i++) ***REMOVED***
      if (uri_text[i] == '/' || uri_text[i] == '?' || uri_text[i] == '#') ***REMOVED***
        authority_end = i;
      ***REMOVED***
    ***REMOVED***
    if (authority_end == NOT_SET && uri_text[i] == 0) ***REMOVED***
      authority_end = i;
    ***REMOVED***
    if (authority_end == NOT_SET) ***REMOVED***
  ...
  ***REMOVED***
  ...
  for (i = path_begin; uri_text[i] != 0; i++) ***REMOVED***
    if (uri_text[i] == '?' || uri_text[i] == '#') ***REMOVED***
      path_end = i;
      break;
    ***REMOVED***
  ***REMOVED***
  if (path_end == NOT_SET && uri_text[i] == 0) ***REMOVED***
    path_end = i;
  ***REMOVED***
  if (path_end == NOT_SET) ***REMOVED***
    return bad_uri(uri_text, i, "path", suppress_errors);
  ***REMOVED***

  if (uri_text[i] == '?') ***REMOVED***
    query_begin = ++i;
    if (!parse_fragment_or_query(uri_text, &i)) ***REMOVED***
      return bad_uri(uri_text, i, "query", suppress_errors);
    ***REMOVED*** else if (uri_text[i] != 0 && uri_text[i] != '#') ***REMOVED***
  ...
  ***REMOVED***
  if (uri_text[i] == '#') ***REMOVED***
    ...
    ***REMOVED*** else if (uri_text[i] != 0) ***REMOVED***
...
const char *grpc_uri_get_query_arg(const grpc_uri *uri, const char *key) ***REMOVED***
  GPR_ASSERT(key != NULL);
  if (key[0] == '\0') return NULL;

  for (size_t i = 0; i < uri->num_query_parts; ++i) ***REMOVED***
    if (0 == strcmp(key, uri->query_parts[i])) ***REMOVED***
...
void grpc_uri_destroy(grpc_uri *uri) ***REMOVED***
  if (!uri) return;
  gpr_free(uri->scheme);
