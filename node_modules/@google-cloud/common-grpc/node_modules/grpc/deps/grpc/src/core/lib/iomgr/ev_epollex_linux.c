...
typedef struct pollable ***REMOVED***
  polling_obj po;
  int epfd;
...
static const char *polling_obj_type_string(polling_obj_type t) ***REMOVED***
  switch (t) ***REMOVED***
...
typedef struct pollset_worker_link ***REMOVED***
  grpc_pollset_worker *next;
  grpc_pollset_worker *prev;
***REMOVED*** pollset_worker_link;

typedef enum ***REMOVED***
  PWL_POLLSET,
  PWL_POLLABLE,
  POLLSET_WORKER_LINK_COUNT
***REMOVED*** pollset_worker_links;
...
static void ref_by(grpc_fd *fd, int n, const char *reason, const char *file,
                   int line) ***REMOVED***
  if (GRPC_TRACER_ON(grpc_trace_fd_refcount)) ***REMOVED***
    gpr_log(GPR_DEBUG,
...
static void fd_global_shutdown(void) ***REMOVED***
  gpr_mu_lock(&fd_freelist_mu);
  gpr_mu_unlock(&fd_freelist_mu);
  while (fd_freelist != NULL) ***REMOVED***
...
static grpc_fd *fd_create(int fd, const char *name) ***REMOVED***
  ...
  gpr_mu_lock(&fd_freelist_mu);
  if (fd_freelist != NULL) ***REMOVED***
    new_fd = fd_freelist;
    fd_freelist = fd_freelist->freelist_next;
  ***REMOVED***
  gpr_mu_unlock(&fd_freelist_mu);

  if (new_fd == NULL) ***REMOVED***
...
static void pollable_init(pollable *p, polling_obj_type type) ***REMOVED***
  po_init(&p->po, type);
  p->root_worker = NULL;
  p->epfd = -1;
***REMOVED***

static void pollable_destroy(pollable *p) ***REMOVED***
  po_destroy(&p->po);
  if (p->epfd != -1) ***REMOVED***
    close(p->epfd);
...
/* ensure that p->epfd, p->wakeup are initialized; p->po.mu must be held */
static grpc_error *pollable_materialize(pollable *p) ***REMOVED***
  if (p->epfd == -1) ***REMOVED***
    int new_epfd = epoll_create1(EPOLL_CLOEXEC);
    if (new_epfd < 0) ***REMOVED***
    ...
    ***REMOVED***
    ...
    if (epoll_ctl(new_epfd, EPOLL_CTL_ADD, p->wakeup.read_fd, &ev) != 0) ***REMOVED***
...
static grpc_error *pollable_add_fd(pollable *p, grpc_fd *fd) ***REMOVED***
  ...
  if (GRPC_TRACER_ON(grpc_polling_trace)) ***REMOVED***
...
static void pollset_maybe_finish_shutdown(grpc_exec_ctx *exec_ctx,
  ...
  if (pollset->shutdown_closure != NULL && pollset->root_worker == NULL &&
      pollset->kick_alls_pending == 0) ***REMOVED***
...
static void do_kick_all(grpc_exec_ctx *exec_ctx, void *arg,
                        grpc_error *error_unused) ***REMOVED***
  grpc_error *error = GRPC_ERROR_NONE;
  grpc_pollset *pollset = (grpc_pollset *)arg;
  gpr_mu_lock(&pollset->pollable_obj.po.mu);
  if (pollset->root_worker != NULL) ***REMOVED***
    grpc_pollset_worker *worker = pollset->root_worker;
    do ***REMOVED***
      GRPC_STATS_INC_POLLSET_KICK(exec_ctx);
      if (worker->pollable_obj != &pollset->pollable_obj) ***REMOVED***
        gpr_mu_lock(&worker->pollable_obj->po.mu);
      ***REMOVED***
      if (worker->initialized_cv && worker != pollset->root_worker) ***REMOVED***
        if (GRPC_TRACER_ON(grpc_polling_trace)) ***REMOVED***
    ...
    ***REMOVED*** while (worker != pollset->root_worker);
  ***REMOVED***
...
static void pollset_kick_all(grpc_exec_ctx *exec_ctx, grpc_pollset *pollset) ***REMOVED***
  pollset->kick_alls_pending++;
...
static grpc_error *pollset_kick_inner(grpc_pollset *pollset, pollable *p,
  ...
  if (specific_worker == NULL) ***REMOVED***
    ...
    ***REMOVED*** else ***REMOVED***
      ...
      ***REMOVED***
      return GRPC_ERROR_NONE;
    ***REMOVED***
  ***REMOVED*** else if (specific_worker->kicked) ***REMOVED***
    if (GRPC_TRACER_ON(grpc_polling_trace)) ***REMOVED***
  ...
  ***REMOVED*** else if (gpr_tls_get(&g_current_thread_worker) ==
    ...
    ***REMOVED***
    ...
    return GRPC_ERROR_NONE;
  ***REMOVED*** else if (specific_worker == p->root_worker) ***REMOVED***
    if (GRPC_TRACER_ON(grpc_polling_trace)) ***REMOVED***
...
static grpc_error *pollset_kick(grpc_exec_ctx *exec_ctx, grpc_pollset *pollset,
  ...
  GRPC_STATS_INC_POLLSET_KICK(exec_ctx);
  if (p != &pollset->pollable_obj) ***REMOVED***
    gpr_mu_lock(&p->po.mu);
  ***REMOVED***
...
static int poll_deadline_to_millis_timeout(gpr_timespec deadline,
                                           gpr_timespec now) ***REMOVED***
  gpr_timespec timeout;
  if (gpr_time_cmp(deadline, gpr_inf_future(deadline.clock_type)) == 0) ***REMOVED***
    return -1;
  ***REMOVED***

  if (gpr_time_cmp(deadline, now) <= 0) ***REMOVED***
    return 0;
  ***REMOVED***

  static const gpr_timespec round_up = ***REMOVED***
      0,                 /* tv_sec */
...
static grpc_error *fd_become_pollable_locked(grpc_fd *fd) ***REMOVED***
  grpc_error *error = GRPC_ERROR_NONE;
  static const char *err_desc = "fd_become_pollable";
  if (append_error(&error, pollable_materialize(&fd->pollable_obj), err_desc)) ***REMOVED***
...
static bool pollset_is_pollable_fd(grpc_pollset *pollset, pollable *p) ***REMOVED***
  return p != &g_empty_pollable && p != &pollset->pollable_obj;
***REMOVED***

static grpc_error *pollset_process_events(grpc_exec_ctx *exec_ctx,
                                          grpc_pollset *pollset, bool drain) ***REMOVED***
...
static void pollset_destroy(grpc_exec_ctx *exec_ctx, grpc_pollset *pollset) ***REMOVED***
  ...
  if (pollset_is_pollable_fd(pollset, pollset->current_pollable_obj)) ***REMOVED***
...
typedef enum ***REMOVED*** EMPTIED, NEW_ROOT, REMOVED ***REMOVED*** worker_remove_result;

static worker_remove_result worker_remove(grpc_pollset_worker **root,
                                          pollset_worker_links link,
                                          grpc_pollset_worker *worker) ***REMOVED***
  if (worker == *root) ***REMOVED***
...
static bool begin_worker(grpc_pollset *pollset, grpc_pollset_worker *worker,
  ...
  if (pollset_is_pollable_fd(pollset, worker->pollable_obj)) ***REMOVED***
  ...
  if (!worker_insert(&worker->pollable_obj->root_worker, PWL_POLLABLE,
    ...
    ***REMOVED***
    while (do_poll && worker->pollable_obj->root_worker != worker) ***REMOVED***
      if (gpr_cv_wait(&worker->cv, &worker->pollable_obj->po.mu, deadline)) ***REMOVED***
        if (GRPC_TRACER_ON(grpc_polling_trace)) ***REMOVED***
      ...
      ***REMOVED*** else if (worker->kicked) ***REMOVED***
        ...
        ***REMOVED***
        do_poll = false;
      ***REMOVED*** else if (GRPC_TRACER_ON(grpc_polling_trace) &&
                 worker->pollable_obj->root_worker != worker) ***REMOVED***
...
static void end_worker(grpc_exec_ctx *exec_ctx, grpc_pollset *pollset,
  ...
  if (NEW_ROOT ==
      worker_remove(&worker->pollable_obj->root_worker, PWL_POLLABLE, worker)) ***REMOVED***
  ...
  ***REMOVED***
  if (EMPTIED == worker_remove(&pollset->root_worker, PWL_POLLSET, worker)) ***REMOVED***
    pollset_maybe_finish_shutdown(exec_ctx, pollset);
  ***REMOVED***
***REMOVED***
...
static grpc_error *pollset_work(grpc_exec_ctx *exec_ctx, grpc_pollset *pollset,
                                ...
                                gpr_timespec now, gpr_timespec deadline) ***REMOVED***
  grpc_pollset_worker worker;
  if (0 && GRPC_TRACER_ON(grpc_polling_trace)) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (pollset->kicked_without_poller) ***REMOVED***
    ...
    return GRPC_ERROR_NONE;
  ***REMOVED***
  if (pollset->current_pollable_obj != &pollset->pollable_obj) ***REMOVED***
    gpr_mu_lock(&pollset->current_pollable_obj->po.mu);
  ***REMOVED***
  if (begin_worker(pollset, &worker, worker_hdl, &now, deadline)) ***REMOVED***
    ...
    ***REMOVED***
    gpr_mu_unlock(&pollset->pollable_obj.po.mu);
    if (pollset->event_cursor == pollset->event_count) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (worker.pollable_obj != &pollset->pollable_obj) ***REMOVED***
    gpr_mu_unlock(&worker.pollable_obj->po.mu);
  ***REMOVED***
  if (grpc_exec_ctx_has_work(exec_ctx)) ***REMOVED***
    gpr_mu_unlock(&pollset->pollable_obj.po.mu);
...
static grpc_error *pollset_add_fd_locked(grpc_exec_ctx *exec_ctx,
                                         ...
                                         bool fd_locked) ***REMOVED***
  static const char *err_desc = "pollset_add_fd";
  grpc_error *error = GRPC_ERROR_NONE;
  if (pollset->current_pollable_obj == &g_empty_pollable) ***REMOVED***
    if (GRPC_TRACER_ON(grpc_polling_trace)) ***REMOVED***
      gpr_log(GPR_DEBUG,
  ...
  ***REMOVED*** else if (pollset->current_pollable_obj == &pollset->pollable_obj) ***REMOVED***
    if (GRPC_TRACER_ON(grpc_polling_trace)) ***REMOVED***
    ...
    ***REMOVED***
    append_error(&error, pollable_add_fd(pollset->current_pollable_obj, fd),
                 err_desc);
  ***REMOVED*** else if (pollset->current_pollable_obj != &fd->pollable_obj) ***REMOVED***
...
static void pollset_set_del_pollset(grpc_exec_ctx *exec_ctx,
                                    grpc_pollset_set *pss, grpc_pollset *ps) ***REMOVED******REMOVED***
...
static void po_init(polling_obj *po, polling_obj_type type) ***REMOVED***
  gpr_mu_init(&po->mu);
  po->type = type;
  po->group = NULL;
  po->next = po;
...
static polling_group *pg_lock_latest(polling_group *pg) ***REMOVED***
  /* assumes pg unlocked; consumes ref, returns ref */
  gpr_mu_lock(&pg->po.mu);
  while (pg->po.group != NULL) ***REMOVED***
...
static void po_destroy(polling_obj *po) ***REMOVED***
  if (po->group != NULL) ***REMOVED***
...
static polling_group *pg_ref(polling_group *pg) ***REMOVED***
  gpr_ref(&pg->refs);
  return pg;
***REMOVED***

static void pg_unref(polling_group *pg) ***REMOVED***
  if (gpr_unref(&pg->refs)) ***REMOVED***
    po_destroy(&pg->po);
    gpr_free(pg);
  ***REMOVED***
***REMOVED***

static int po_cmp(polling_obj *a, polling_obj *b) ***REMOVED***
  if (a == b) return 0;
...
static void po_join(grpc_exec_ctx *exec_ctx, polling_obj *a, polling_obj *b) ***REMOVED***
  switch (po_cmp(a, b)) ***REMOVED***
    case 0:
      return;
    case 1:
    ...
    case -1:
      ...
      if (a->group == NULL) ***REMOVED***
        if (b->group == NULL) ***REMOVED***
          polling_obj *initial_po[] = ***REMOVED***a, b***REMOVED***;
        ...
        ***REMOVED***
      ***REMOVED*** else if (b->group == NULL) ***REMOVED***
        polling_group *a_group = pg_ref(a->group);
        gpr_mu_unlock(&a->mu);
        gpr_mu_unlock(&b->mu);
        pg_join(exec_ctx, a_group, b);
      ***REMOVED*** else if (a->group == b->group) ***REMOVED***
        /* nothing to do */
        gpr_mu_unlock(&a->mu);
        gpr_mu_unlock(&b->mu);
      ***REMOVED*** else ***REMOVED***
...
static void pg_notify(grpc_exec_ctx *exec_ctx, polling_obj *a, polling_obj *b) ***REMOVED***
  if (a->type == PO_FD && b->type == PO_POLLSET) ***REMOVED***
    pollset_add_fd_locked(exec_ctx, (grpc_pollset *)b, (grpc_fd *)a, true);
  ***REMOVED*** else if (a->type == PO_POLLSET && b->type == PO_FD) ***REMOVED***
...
static void pg_broadcast(grpc_exec_ctx *exec_ctx, polling_group *from,
                         polling_group *to) ***REMOVED***
  for (polling_obj *a = from->po.next; a != &from->po; a = a->next) ***REMOVED***
    for (polling_obj *b = to->po.next; b != &to->po; b = b->next) ***REMOVED***
      if (po_cmp(a, b) < 0) ***REMOVED***
        gpr_mu_lock(&a->mu);
        gpr_mu_lock(&b->mu);
      ***REMOVED*** else ***REMOVED***
...
static void pg_create(grpc_exec_ctx *exec_ctx, polling_obj **initial_po,
                      size_t initial_po_count) ***REMOVED***
  ...
  for (size_t i = 0; i < initial_po_count; i++) ***REMOVED***
    GPR_ASSERT(initial_po[i]->group == NULL);
    initial_po[i]->group = pg;
  ***REMOVED***
  for (size_t i = 1; i < initial_po_count; i++) ***REMOVED***
    initial_po[i]->prev = initial_po[i - 1];
  ***REMOVED***
  for (size_t i = 0; i < initial_po_count - 1; i++) ***REMOVED***
...
static void pg_join(grpc_exec_ctx *exec_ctx, polling_group *pg,
  ...
  for (polling_obj *existing = pg->po.next /* skip pg - it's just a stub */;
       existing != &pg->po; existing = existing->next) ***REMOVED***
    if (po_cmp(po, existing) < 0) ***REMOVED***
      gpr_mu_lock(&po->mu);
...
static void pg_merge(grpc_exec_ctx *exec_ctx, polling_group *a,
  ...
  ***REMOVED***
  ...
  while (b->po.next != &b->po) ***REMOVED***
    polling_obj *po = b->po.next;
    gpr_mu_lock(&po->mu);
    if (unref_count == unref_cap) ***REMOVED***
  ...
  ***REMOVED***
  gpr_mu_unlock(&a->po.mu);
  gpr_mu_unlock(&b->po.mu);
  for (size_t i = 0; i < unref_count; i++) ***REMOVED***
    pg_unref(unref[i]);
  ***REMOVED***
  gpr_free(unref);
  pg_unref(b);
