...
gpr_avl gpr_avl_create(const gpr_avl_vtable *vtable) ***REMOVED***
  gpr_avl out;
  out.vtable = vtable;
  out.root = NULL;
  return out;
...
static gpr_avl_node *ref_node(gpr_avl_node *node) ***REMOVED***
  if (node) ***REMOVED***
    gpr_ref(&node->refs);
  ***REMOVED***
  return node;
...
static void unref_node(const gpr_avl_vtable *vtable, gpr_avl_node *node,
  ...
  if (node == NULL) ***REMOVED***
    return;
  ***REMOVED***
  if (gpr_unref(&node->refs)) ***REMOVED***
...
static long node_height(gpr_avl_node *node) ***REMOVED***
  return node == NULL ? 0 : node->height;
***REMOVED***

#ifndef NDEBUG
static long calculate_height(gpr_avl_node *node) ***REMOVED***
  return node == NULL ? 0 : 1 + GPR_MAX(calculate_height(node->left),
                                        calculate_height(node->right));
***REMOVED***

static gpr_avl_node *assert_invariants(gpr_avl_node *n) ***REMOVED***
...
***REMOVED***
#else
static gpr_avl_node *assert_invariants(gpr_avl_node *n) ***REMOVED*** return n; ***REMOVED***
#endif

gpr_avl_node *new_node(void *key, void *value, gpr_avl_node *left,
                       gpr_avl_node *right) ***REMOVED***
...
static gpr_avl_node *get(const gpr_avl_vtable *vtable, gpr_avl_node *node,
                         void *key, void *user_data) ***REMOVED***
  long cmp;
...
void *gpr_avl_get(gpr_avl avl, void *key, void *user_data) ***REMOVED***
  gpr_avl_node *node = get(avl.vtable, avl.root, key, user_data);
  return node ? node->value : NULL;
***REMOVED***

int gpr_avl_maybe_get(gpr_avl avl, void *key, void **value, void *user_data) ***REMOVED***
  gpr_avl_node *node = get(avl.vtable, avl.root, key, user_data);
  if (node != NULL) ***REMOVED***
    *value = node->value;
    return 1;
...
static gpr_avl_node *rotate_left(const gpr_avl_vtable *vtable, void *key,
                                 ...
                                 gpr_avl_node *right, void *user_data) ***REMOVED***
...
static gpr_avl_node *rebalance(const gpr_avl_vtable *vtable, void *key,
                               void *value, gpr_avl_node *left,
                               gpr_avl_node *right, void *user_data) ***REMOVED***
  switch (node_height(left) - node_height(right)) ***REMOVED***
    case 2:
      if (node_height(left->left) - node_height(left->right) == -1) ***REMOVED***
      ...
      ***REMOVED***
    case -2:
      if (node_height(right->left) - node_height(right->right) == 1) ***REMOVED***
        return assert_invariants(
...
static gpr_avl_node *add_key(const gpr_avl_vtable *vtable, gpr_avl_node *node,
                             void *key, void *value, void *user_data) ***REMOVED***
  long cmp;
  if (node == NULL) ***REMOVED***
...
gpr_avl gpr_avl_add(gpr_avl avl, void *key, void *value, void *user_data) ***REMOVED***
...
static gpr_avl_node *in_order_head(gpr_avl_node *node) ***REMOVED***
  while (node->left != NULL) ***REMOVED***
    node = node->left;
  ***REMOVED***
  return node;
***REMOVED***

static gpr_avl_node *in_order_tail(gpr_avl_node *node) ***REMOVED***
  while (node->right != NULL) ***REMOVED***
    node = node->right;
  ***REMOVED***
  return node;
***REMOVED***
...
static gpr_avl_node *remove_key(const gpr_avl_vtable *vtable,
  ...
  ***REMOVED***
  cmp = vtable->compare_keys(node->key, key, user_data);
  if (cmp == 0) ***REMOVED***
    if (node->left == NULL) ***REMOVED***
      return ref_node(node->right);
    ***REMOVED*** else if (node->right == NULL) ***REMOVED***
      return ref_node(node->left);
    ***REMOVED*** else if (node->left->height < node->right->height) ***REMOVED***
...
gpr_avl gpr_avl_remove(gpr_avl avl, void *key, void *user_data) ***REMOVED***
  gpr_avl_node *old_root = avl.root;
...
gpr_avl gpr_avl_ref(gpr_avl avl, void *user_data) ***REMOVED***
  ref_node(avl.root);
  return avl;
***REMOVED***

void gpr_avl_unref(gpr_avl avl, void *user_data) ***REMOVED***
  unref_node(avl.vtable, avl.root, user_data);
***REMOVED***

int gpr_avl_is_empty(gpr_avl avl) ***REMOVED*** return avl.root == NULL; ***REMOVED***
