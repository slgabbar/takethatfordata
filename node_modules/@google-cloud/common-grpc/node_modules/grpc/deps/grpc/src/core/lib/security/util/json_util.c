...
const char *grpc_json_get_string_property(const grpc_json *json,
                                          const char *prop_name) ***REMOVED***
  grpc_json *child;
  for (child = json->child; child != NULL; child = child->next) ***REMOVED***
    if (strcmp(child->key, prop_name) == 0) break;
  ***REMOVED***
  if (child == NULL || child->type != GRPC_JSON_STRING) ***REMOVED***
