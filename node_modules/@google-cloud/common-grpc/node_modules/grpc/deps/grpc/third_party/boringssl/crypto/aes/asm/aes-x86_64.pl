...
sub hi() ***REMOVED*** my $r=shift;	$r =~ s/%[er]([a-d])x/%\1h/;	$r; ***REMOVED***
sub lo() ***REMOVED*** my $r=shift;	$r =~ s/%[er]([a-d])x/%\1l/;
			$r =~ s/%[er]([sd]i)/%\1l/;
			$r =~ s/%(r[0-9]+)[d]?/%\1b/;	$r; ***REMOVED***
sub LO() ***REMOVED*** my $r=shift; $r =~ s/%r([a-z]+)/%e\1/;
			$r =~ s/%r([0-9]+)/%r\1d/;	$r; ***REMOVED***
sub _data_word()
***REMOVED*** my $i;
    while(defined($i=shift)) ***REMOVED*** $code.=sprintf".long\t0x%08x,0x%08x\n",$i,$i; ***REMOVED***
***REMOVED***
sub data_word()
***REMOVED*** my $i;
  my $last=pop(@_);
    $code.=".long\t";
    while(defined($i=shift)) ***REMOVED*** $code.=sprintf"0x%08x,",$i; ***REMOVED***
    $code.=sprintf"0x%08x\n",$last;
***REMOVED***
...
***REMOVED*** my $i;
  my $last=pop(@_);
    $code.=".byte\t";
    while(defined($i=shift)) ***REMOVED*** $code.=sprintf"0x%02x,",$i&0xff; ***REMOVED***
    $code.=sprintf"0x%02x\n",$last&0xff;
***REMOVED***

sub encvert()
***REMOVED*** my $t3="%r8d";	# zaps $inp!
...
.Lenc_loop:
___
	if ($verticalspin) ***REMOVED*** &encvert(); ***REMOVED***
	else ***REMOVED***	&encstep(0,$s0,$s1,$s2,$s3);
...
$code.=<<___;
	...
	jnz	.Lenc_loop
___
	if ($verticalspin) ***REMOVED*** &enclastvert(); ***REMOVED***
	else ***REMOVED***	&enclast(0,$s0,$s1,$s2,$s3);
...
sub enccompactvert()
***REMOVED*** my ($t3,$t4,$t5)=("%r8d","%r9d","%r13d");
...
sub enctransform()
***REMOVED*** my ($t3,$r20,$r21)=($acc2,"%r8d","%r9d");
...
.Ldec_loop:
___
	if ($verticalspin) ***REMOVED*** &decvert(); ***REMOVED***
	else ***REMOVED***	&decstep(0,$s0,$s3,$s2,$s1);
...
$code.=<<___;
	...
	jnz	.Ldec_loop
___
	if ($verticalspin) ***REMOVED*** &declastvert(); ***REMOVED***
	else ***REMOVED***	&declast(0,$s0,$s3,$s2,$s1);
...
# %ecx=s2 and %edx=s3.
sub dectransform()
***REMOVED*** my ($tp10,$tp20,$tp40,$tp80,$acc0)=("%rax","%r8", "%r9", "%r10","%rbx");
