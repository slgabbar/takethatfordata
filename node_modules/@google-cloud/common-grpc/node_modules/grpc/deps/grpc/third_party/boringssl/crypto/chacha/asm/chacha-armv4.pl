...
***REMOVED*** my $opcode = $AUTOLOAD; $opcode =~ s/.*:://; $opcode =~ s/_/\./;
  my $arg = pop;
...
.LChaCha20_ctr32:
	...
	stmdb	sp!,***REMOVED***r0-r2,r4-r11,lr***REMOVED***
...
.Lshort:
#endif
	ldmia	r12,***REMOVED***r4-r7***REMOVED***		@ load counter and nonce
	sub	sp,sp,#4*(16)		@ off-load area
	sub	r14,r14,#64		@ .Lsigma
	stmdb	sp!,***REMOVED***r4-r7***REMOVED***		@ copy counter and nonce
	ldmia	r3,***REMOVED***r4-r11***REMOVED***		@ load key
	ldmia	r14,***REMOVED***r0-r3***REMOVED***		@ load sigma
	stmdb	sp!,***REMOVED***r4-r11***REMOVED***		@ copy key
	stmdb	sp!,***REMOVED***r0-r3***REMOVED***		@ copy sigma
...
.align	4
.Loop_outer:
	ldmia	sp,***REMOVED***r0-r9***REMOVED***		@ load key material
...
.Loop:
	subs	@t[3],@t[3],#1
___
	foreach (&ROUND(0, 4, 8,12)) ***REMOVED*** eval; ***REMOVED***
	foreach (&ROUND(0, 5,10,15)) ***REMOVED*** eval; ***REMOVED***
$code.=<<___;
	bne	.Loop
...
# endif
	eorhs	@x[2],@x[2],@t[2]
	eorhs	@x[3],@x[3],@t[3]
	 ldmia	@t[0],***REMOVED***@t[0]-@t[3]***REMOVED***	@ load key material
	str	@x[1],[r14,#-12]
	str	@x[2],[r14,#-8]
	str	@x[3],[r14,#-4]
...
# endif
	...
	ldmia	@x[0],***REMOVED***@x[0]-@x[7]***REMOVED***	@ load second half
...
.Ldone:
	add	sp,sp,#4*(32+3)
.Lno_data:
	ldmia	sp!,***REMOVED***r4-r11,pc***REMOVED***
...
.LChaCha20_neon:
	adr		r14,.Lsigma
	vstmdb		sp!,***REMOVED***d8-d15***REMOVED***		@ ABI spec says so
	stmdb		sp!,***REMOVED***r0-r3***REMOVED***

	vld1.32		***REMOVED***$b0-$c0***REMOVED***,[r3]		@ load key
	ldmia		r3,***REMOVED***r4-r11***REMOVED***		@ load key

	sub		sp,sp,#4*(16+16)
	vld1.32		***REMOVED***$d0***REMOVED***,[r12]		@ load counter and nonce
	add		r12,sp,#4*8
	ldmia		r14,***REMOVED***r0-r3***REMOVED***		@ load sigma
	vld1.32		***REMOVED***$a0***REMOVED***,[r14]!		@ load sigma
	vld1.32		***REMOVED***$t0***REMOVED***,[r14]		@ one
	vst1.32		***REMOVED***$c0-$d0***REMOVED***,[r12]		@ copy 1/2key|counter|nonce
	vst1.32		***REMOVED***$a0-$b0***REMOVED***,[sp]		@ copy sigma|1/2key
...
$code.=<<___;
	...
	add		@t[3],sp,#32
	vld1.32		***REMOVED***$t0-$t1***REMOVED***,[sp]		@ load key material
	vld1.32		***REMOVED***$t2-$t3***REMOVED***,[@t[3]]
	...
	vld1.8		***REMOVED***$t0-$t1***REMOVED***,[r12]!	@ load input
	 mov		@t[3],sp
	vld1.8		***REMOVED***$t2-$t3***REMOVED***,[r12]!
	veor		$a0,$a0,$t0		@ xor with input
	veor		$b0,$b0,$t1
	vld1.8		***REMOVED***$t0-$t1***REMOVED***,[r12]!
	veor		$c0,$c0,$t2
	veor		$d0,$d0,$t3
	vld1.8		***REMOVED***$t2-$t3***REMOVED***,[r12]!

	veor		$a1,$a1,$t0
	 vst1.8		***REMOVED***$a0-$b0***REMOVED***,[r14]!	@ store output
	veor		$b1,$b1,$t1
	vld1.8		***REMOVED***$t0-$t1***REMOVED***,[r12]!
	veor		$c1,$c1,$t2
	 vst1.8		***REMOVED***$c0-$d0***REMOVED***,[r14]!
	veor		$d1,$d1,$t3
	vld1.8		***REMOVED***$t2-$t3***REMOVED***,[r12]!

	veor		$a2,$a2,$t0
	 vld1.32	***REMOVED***$a0-$b0***REMOVED***,[@t[3]]!	@ load for next iteration
	 veor		$t0#hi,$t0#hi,$t0#hi
	 vldr		$t0#lo,[sp,#4*(16+4)]	@ four
	veor		$b2,$b2,$t1
	 vld1.32	***REMOVED***$c0-$d0***REMOVED***,[@t[3]]
	veor		$c2,$c2,$t2
	 vst1.8		***REMOVED***$a1-$b1***REMOVED***,[r14]!
	veor		$d2,$d2,$t3
	 vst1.8		***REMOVED***$c1-$d1***REMOVED***,[r14]!

	vadd.i32	$d0#lo,$d0#lo,$t0#lo	@ next counter value
	vldr		$t0#lo,[sp,#4*(16+0)]	@ one

	ldmia		sp,***REMOVED***@t[0]-@t[3]***REMOVED***	@ load key material
	add		@x[0],@x[0],@t[0]	@ accumulate key material
	ldr		@t[0],[r12],#16		@ load input
	 vst1.8		***REMOVED***$a2-$b2***REMOVED***,[r14]!
	add		@x[1],@x[1],@t[1]
	ldr		@t[1],[r12,#-12]
	 vst1.8		***REMOVED***$c2-$d2***REMOVED***,[r14]!
	add		@x[2],@x[2],@t[2]
	ldr		@t[2],[r12,#-8]
	add		@x[3],@x[3],@t[3]
	ldr		@t[3],[r12,#-4]
...
.Lbreak_neon:
	...
	ldr		@x[12],[sp,#4*(16+10)]
	ldr		@x[14],[sp,#4*(16+11)]
	 vldmia		@t[3],***REMOVED***d8-d15***REMOVED***			@ fulfill ABI requirement
	...
	add		@t[3],sp,#4*(20)
	vst1.32		***REMOVED***$a0-$b0***REMOVED***,[@t[3]]!		@ copy key
	add		sp,sp,#4*(20)			@ switch frame
	vst1.32		***REMOVED***$c0-$d0***REMOVED***,[@t[3]]
	mov		@t[3],#10
...
.Ltail_neon:
	...
	add		@t[0],sp,#4*(8)
	vst1.8		***REMOVED***$a0-$b0***REMOVED***,[sp]
	add		@t[2],sp,#4*(0)
	vst1.8		***REMOVED***$c0-$d0***REMOVED***,[@t[0]]
	b		.Loop_tail_neon
...
.L64_or_more_neon:
	...
	veor		$c0,$c0,$t2
	veor		$d0,$d0,$t3
	vst1.8		***REMOVED***$a0-$b0***REMOVED***,[r14]!
	vst1.8		***REMOVED***$c0-$d0***REMOVED***,[r14]!

	beq		.Ldone_neon

	add		@t[0],sp,#4*(8)
	vst1.8		***REMOVED***$a1-$b1***REMOVED***,[sp]
	add		@t[2],sp,#4*(0)
	vst1.8		***REMOVED***$c1-$d1***REMOVED***,[@t[0]]
...
.L128_or_more_neon:
	...
	add		@t[0],sp,#4*(8)
	vst1.8		***REMOVED***$a2-$b2***REMOVED***,[sp]
	add		@t[2],sp,#4*(0)
	vst1.8		***REMOVED***$c2-$d2***REMOVED***,[@t[0]]
...
# ifdef	__ARMEB__
	...
	rev		@x[7],@x[7]
# endif
	stmia		sp,***REMOVED***@x[0]-@x[7]***REMOVED***
	 add		@x[0],sp,#4*(16+8)
...
# ifdef	__ARMEB__
	...
	rev		@x[7],@x[7]
# endif
	stmia		@t[0],***REMOVED***@x[0]-@x[7]***REMOVED***
	 add		@t[2],sp,#4*(0)
