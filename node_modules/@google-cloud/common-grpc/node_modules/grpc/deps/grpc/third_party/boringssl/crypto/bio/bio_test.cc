...
#if !defined(OPENSSL_WINDOWS)
static int closesocket(int sock) ***REMOVED***
  return close(sock);
***REMOVED***

static void PrintSocketError(const char *func) ***REMOVED***
  perror(func);
***REMOVED***
#else
static void PrintSocketError(const char *func) ***REMOVED***
...
class ScopedSocket ***REMOVED***
 public:
  explicit ScopedSocket(int sock) : sock_(sock) ***REMOVED******REMOVED***
  ~ScopedSocket() ***REMOVED***
    closesocket(sock_);
  ***REMOVED***
...
static bool TestSocketConnect() ***REMOVED***
  static const char kTestMessage[] = "test";

  int listening_sock = socket(AF_INET, SOCK_STREAM, 0);
  if (listening_sock == -1) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (!inet_pton(AF_INET, "127.0.0.1", &sin.sin_addr)) ***REMOVED***
    PrintSocketError("inet_pton");
    return false;
  ***REMOVED***
  if (bind(listening_sock, (struct sockaddr *)&sin, sizeof(sin)) != 0) ***REMOVED***
    PrintSocketError("bind");
    return false;
  ***REMOVED***
  if (listen(listening_sock, 1)) ***REMOVED***
    PrintSocketError("listen");
    return false;
  ***REMOVED***
  socklen_t sockaddr_len = sizeof(sin);
  if (getsockname(listening_sock, (struct sockaddr *)&sin, &sockaddr_len) ||
      sockaddr_len != sizeof(sin)) ***REMOVED***
  ...
  ***REMOVED***
  ...
  char buf[5];
  if (recv(sock, buf, sizeof(buf), 0) != sizeof(kTestMessage)) ***REMOVED***
    PrintSocketError("read");
    return false;
  ***REMOVED***
  if (OPENSSL_memcmp(buf, kTestMessage, sizeof(kTestMessage))) ***REMOVED***
    return false;
  ***REMOVED***
...
static bool TestPrintf() ***REMOVED***
  // Test a short output, a very long one, and various sizes around
  // 256 (the size of the buffer) to ensure edge cases are correct.
  static const size_t kLengths[] = ***REMOVED*** 5, 250, 251, 252, 253, 254, 1023 ***REMOVED***;
  ...
  ***REMOVED***
  ...
  for (size_t i = 0; i < OPENSSL_ARRAY_SIZE(kLengths); i++) ***REMOVED***
    char string[1024];
    if (kLengths[i] >= sizeof(string)) ***REMOVED***
    ...
    ***REMOVED***
    ...
    if (ret < 0 || static_cast<size_t>(ret) != 5 + kLengths[i]) ***REMOVED***
    ...
    ***REMOVED***
    if (len != 5 + kLengths[i] ||
        ...
        strncmp((const char *)contents + 5, string, kLengths[i]) != 0) ***REMOVED***
      fprintf(stderr, "Contents did not match: %.*s\n", (int)len, contents);
      return false;
    ***REMOVED***

    if (!BIO_reset(bio.get())) ***REMOVED***
...
static bool ReadASN1(bool should_succeed, const uint8_t *data, size_t data_len,
                     size_t expected_len, size_t max_len) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (should_succeed != (ok == 1)) ***REMOVED***
    return false;
  ***REMOVED***

  if (should_succeed && (out_len != expected_len ||
                         OPENSSL_memcmp(data, out, expected_len) != 0)) ***REMOVED***
    return false;
  ***REMOVED***
...
static bool TestASN1() ***REMOVED***
  static const uint8_t kData1[] = ***REMOVED***0x30, 2, 1, 2, 0, 0***REMOVED***;
  static const uint8_t kData2[] = ***REMOVED***0x30, 3, 1, 2***REMOVED***;  /* truncated */
  static const uint8_t kData3[] = ***REMOVED***0x30, 0x81, 1, 1***REMOVED***;  /* should be short len */
  static const uint8_t kData4[] = ***REMOVED***0x30, 0x82, 0, 1, 1***REMOVED***;  /* zero padded. */
  ...
  if (!ReadASN1(true, kData1, sizeof(kData1), 4, 100) ||
      ...
      !ReadASN1(false, kData4, sizeof(kData4), 0, 100)) ***REMOVED***
    return false;
  ***REMOVED***

  static const size_t kLargePayloadLen = 8000;
  static const uint8_t kLargePrefix[] = ***REMOVED***0x30, 0x82, kLargePayloadLen >> 8,
                                         kLargePayloadLen & 0xff***REMOVED***;
  ...
  ***REMOVED***
  ...
  if (!ReadASN1(false, large.get(), sizeof(kLargePrefix) + kLargePayloadLen,
                sizeof(kLargePrefix) + kLargePayloadLen,
                kLargePayloadLen - 1)) ***REMOVED***
    fprintf(stderr, "max_len test failed.\n");
    return false;
  ***REMOVED***

  static const uint8_t kIndefPrefix[] = ***REMOVED***0x30, 0x80***REMOVED***;
...
static bool TestPair() ***REMOVED***
  // Run through the tests twice, swapping |bio1| and |bio2|, for symmetry.
  for (int i = 0; i < 2; i++) ***REMOVED***
    BIO *bio1, *bio2;
    if (!BIO_new_bio_pair(&bio1, 10, &bio2, 10)) ***REMOVED***
      return false;
    ***REMOVED***
    ...
    ***REMOVED***
    ...
    if (BIO_ctrl_get_write_guarantee(bio1) != 10 ||
        BIO_ctrl_get_read_request(bio1) != 0) ***REMOVED***
      return false;
    ***REMOVED***
    ...
    if (BIO_write(bio1, "12345", 5) != 5 ||
        ...
        BIO_ctrl_get_write_guarantee(bio1) != 10) ***REMOVED***
      return false;
    ***REMOVED***
    ...
    ***REMOVED***
    ...
    if (BIO_read(bio2, buf, 5) != -1 ||
        ...
        BIO_ctrl_get_read_request(bio1) != 5) ***REMOVED***
      return false;
    ***REMOVED***
    ...
    if (BIO_read(bio2, buf, 20) != -1 ||
        !BIO_should_read(bio2) ||
        BIO_ctrl_get_read_request(bio1) != 10) ***REMOVED***
      return false;
    ***REMOVED***
    ...
    ***REMOVED***
    ...
    if (BIO_ctrl_get_read_request(bio1) != 0) ***REMOVED***
      return false;
    ***REMOVED***
    ...
    ***REMOVED***
    ...
    if (BIO_write(bio1, "12345", 5) != 5 ||
        ...
        OPENSSL_memcmp(buf, "67890", 5) != 0) ***REMOVED***
      return false;
    ***REMOVED***
    ...
    if (BIO_write(bio1, "12345", 5) != 5 ||
        ...
        BIO_read(bio2, buf, sizeof(buf)) != 5 ||
        OPENSSL_memcmp(buf, "12345", 5) != 0 ||
        BIO_read(bio2, buf, sizeof(buf)) != 0) ***REMOVED***
      return false;
    ***REMOVED***

    // A closed write end may not be written to.
    if (BIO_ctrl_get_write_guarantee(bio1) != 0 ||
        BIO_write(bio1, "_____", 5) != -1) ***REMOVED***
      return false;
    ***REMOVED***

    uint32_t err = ERR_get_error();
    if (ERR_GET_LIB(err) != ERR_LIB_BIO ||
        ERR_GET_REASON(err) != BIO_R_BROKEN_PIPE) ***REMOVED***
      return false;
    ***REMOVED***
    ...
    if (BIO_write(bio2, "12345", 5) != 5 ||
        BIO_read(bio1, buf, sizeof(buf)) != 5 ||
        OPENSSL_memcmp(buf, "12345", 5) != 0) ***REMOVED***
      return false;
    ***REMOVED***
  ***REMOVED***
...
int main() ***REMOVED***
  ...
  if (wsa_err != 0) ***REMOVED***
    fprintf(stderr, "WSAStartup failed: %d\n", wsa_err);
    return 1;
  ***REMOVED***
  if (wsa_data.wVersion != wsa_version) ***REMOVED***
