  ...
  def WriteFiles(self, files, asm_outputs):
    ...
    with open('sources.bp', 'w+') as blueprint:
      ...
      for ((osname, arch), asm_files) in asm_outputs:
        ...
        blueprint.write('        android_%s: ***REMOVED***\n' % arch)
        blueprint.write('            srcs: [\n')
        for f in sorted(asm_files):
          blueprint.write('                "%s",\n' % f)
        blueprint.write('            ],\n')
        blueprint.write('        ***REMOVED***,\n')

        if arch == 'x86' or arch == 'x86_64':
          blueprint.write('        linux_%s: ***REMOVED***\n' % arch)
          blueprint.write('            srcs: [\n')
          for f in sorted(asm_files):
            blueprint.write('                "%s",\n' % f)
          blueprint.write('            ],\n')
...
"""
  ...
  def WriteFiles(self, files, asm_outputs):
    ...
    with open('BUILD.generated_tests.gni', 'w+') as out:
      ...
      out.write('template("create_tests") ***REMOVED***\n')
      ...
      for test in sorted(files['test']):
        ...
        out.write('  executable("%s") ***REMOVED***\n' % test_name)
        out.write('    sources = [\n')
        out.write('      "%s",\n' % test)
        out.write('    ]\n')
        out.write('    sources += test_support_sources\n')
        out.write('    if (defined(invoker.configs_exclude)) ***REMOVED***\n')
      ...
      out.write('  group(target_name) ***REMOVED***\n')
      out.write('    deps = [\n')
...
"""
  ...
  def WriteFiles(self, files, asm_outputs):
    with open('boringssl.gypi', 'w+') as gypi:
      gypi.write(self.header + '***REMOVED***\n  \'variables\': ***REMOVED***\n')
...
def WriteAsmFiles(perlasms):
  ...
  for osarch in OS_ARCH_COMBOS:
    ...
    for perlasm in perlasms:
      ...
      if output.endswith('-armx.$***REMOVED***ASM_EXT***REMOVED***'):
        output = output.replace('-armx',
                                '-armx64' if arch == 'aarch64' else '-armx32')
      output = output.replace('$***REMOVED***ASM_EXT***REMOVED***', asm_ext)
