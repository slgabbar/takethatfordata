...
func hkdfExtract(hash func() hash.Hash, salt, ikm []byte) []byte ***REMOVED***
	if salt == nil ***REMOVED***
...
func hkdfExpand(hash func() hash.Hash, prk, info []byte, length int) []byte ***REMOVED***
	hashSize := hash().Size()
	if length > 255*hashSize ***REMOVED***
		panic("hkdfExpand: length too long")
	***REMOVED***
	if len(prk) < hashSize ***REMOVED***
	...
	***REMOVED***
	...
	for length > 0 ***REMOVED***
		hmac.Reset()
		counter++
		hmac.Write(lastBlock)
		hmac.Write(info)
		hmac.Write([]byte***REMOVED***counter***REMOVED***)
		block := hmac.Sum(nil)
		lastBlock = block
