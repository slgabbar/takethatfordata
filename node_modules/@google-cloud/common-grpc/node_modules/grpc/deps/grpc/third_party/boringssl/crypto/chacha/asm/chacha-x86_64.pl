...
@x=("%eax","%ebx","%ecx","%edx",map("%r$***REMOVED***_***REMOVED***d",(8..11)),
    "%nox","%nox","%nox","%nox",map("%r$***REMOVED***_***REMOVED***d",(12..15)));
@t=("%esi","%edi");

sub ROUND ***REMOVED***			# critical path is 24 cycles per round
my ($a0,$b0,$c0,$d0)=@_;
...
sub SSSE3ROUND ***REMOVED***	# critical path is 20 "SIMD ticks" per round
	&paddd	($a,$b);
	&pxor	($d,$a);
	&pshufb	($d,$rot16);
...
.Loop4x:
___
	foreach (&SSSE3_lane_ROUND(0, 4, 8,12)) ***REMOVED*** eval; ***REMOVED***
	foreach (&SSSE3_lane_ROUND(0, 5,10,15)) ***REMOVED*** eval; ***REMOVED***
$code.=<<___;
	dec		%eax
	jnz		.Loop4x
...
.Loop8x:
___
	foreach (&AVX2_lane_ROUND(0, 4, 8,12)) ***REMOVED*** eval; ***REMOVED***
	foreach (&AVX2_lane_ROUND(0, 5,10,15)) ***REMOVED*** eval; ***REMOVED***
$code.=<<___;
	dec		%eax
	jnz		.Loop8x
...
sub AVX512ROUND ***REMOVED***	# critical path is 14 "SIMD ticks" per round
	&vpaddd	($a,$a,$b);
	&vpxord	($d,$d,$a);
	&vprold	($d,$d,16);
...
.Loop16x:
___
	foreach (&AVX512_lane_ROUND(0, 4, 8,12)) ***REMOVED*** eval; ***REMOVED***
	foreach (&AVX512_lane_ROUND(0, 5,10,15)) ***REMOVED*** eval; ***REMOVED***
$code.=<<___;
	dec		%eax
	jnz		.Loop16x
