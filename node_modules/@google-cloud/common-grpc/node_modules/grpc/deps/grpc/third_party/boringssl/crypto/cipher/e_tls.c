...
typedef struct ***REMOVED***
   ...
   * IV. */
  char implicit_iv;
***REMOVED*** AEAD_TLS_CTX;

OPENSSL_COMPILE_ASSERT(EVP_MAX_MD_SIZE < 256, mac_key_len_fits_in_uint8_t);

static void aead_tls_cleanup(EVP_AEAD_CTX *ctx) ***REMOVED***
...
static int aead_tls_init(EVP_AEAD_CTX *ctx, const uint8_t *key, size_t key_len,
                         ...
                         char implicit_iv) ***REMOVED***
  if (tag_len != EVP_AEAD_DEFAULT_TAG_LENGTH &&
      tag_len != EVP_MD_size(md)) ***REMOVED***
    OPENSSL_PUT_ERROR(CIPHER, CIPHER_R_UNSUPPORTED_TAG_SIZE);
    return 0;
  ***REMOVED***

  if (key_len != EVP_AEAD_key_length(ctx->aead)) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (!EVP_CipherInit_ex(&tls_ctx->cipher_ctx, cipher, NULL, &key[mac_key_len],
                         ...
                         dir == evp_aead_seal) ||
      !HMAC_Init_ex(&tls_ctx->hmac_ctx, key, mac_key_len, md, NULL)) ***REMOVED***
...
static int aead_tls_seal(const EVP_AEAD_CTX *ctx, uint8_t *out,
                         ...
                         const uint8_t *ad, size_t ad_len) ***REMOVED***
  AEAD_TLS_CTX *tls_ctx = (AEAD_TLS_CTX *)ctx->aead_state;
  size_t total = 0;

  if (!tls_ctx->cipher_ctx.encrypt) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (in_len + EVP_AEAD_max_overhead(ctx->aead) < in_len ||
      in_len > INT_MAX) ***REMOVED***
    /* EVP_CIPHER takes int as input. */
    OPENSSL_PUT_ERROR(CIPHER, CIPHER_R_TOO_LARGE);
    return 0;
  ***REMOVED***

  if (max_out_len < in_len + EVP_AEAD_max_overhead(ctx->aead)) ***REMOVED***
    OPENSSL_PUT_ERROR(CIPHER, CIPHER_R_BUFFER_TOO_SMALL);
    return 0;
  ***REMOVED***

  if (nonce_len != EVP_AEAD_nonce_length(ctx->aead)) ***REMOVED***
    OPENSSL_PUT_ERROR(CIPHER, CIPHER_R_INVALID_NONCE_SIZE);
    return 0;
  ***REMOVED***

  if (ad_len != 13 - 2 /* length bytes */) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (!HMAC_Init_ex(&tls_ctx->hmac_ctx, NULL, 0, NULL, NULL) ||
      ...
      !HMAC_Final(&tls_ctx->hmac_ctx, mac, &mac_len)) ***REMOVED***
    return 0;
  ***REMOVED***
  ...
  if (EVP_CIPHER_CTX_mode(&tls_ctx->cipher_ctx) == EVP_CIPH_CBC_MODE &&
      !tls_ctx->implicit_iv &&
      !EVP_EncryptInit_ex(&tls_ctx->cipher_ctx, NULL, NULL, NULL, nonce)) ***REMOVED***
    return 0;
  ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (block_size > 1) ***REMOVED***
    ...
    if (!EVP_EncryptUpdate(&tls_ctx->cipher_ctx, out + total, &len, padding,
                           (int)padding_len)) ***REMOVED***
      return 0;
    ***REMOVED***
    total += len;
  ***REMOVED***

  if (!EVP_EncryptFinal_ex(&tls_ctx->cipher_ctx, out + total, &len)) ***REMOVED***
    return 0;
  ***REMOVED***
  total += len;
...
static int aead_tls_open(const EVP_AEAD_CTX *ctx, uint8_t *out,
  ...
  ***REMOVED***
  ...
  if (in_len < HMAC_size(&tls_ctx->hmac_ctx)) ***REMOVED***
    OPENSSL_PUT_ERROR(CIPHER, CIPHER_R_BAD_DECRYPT);
    return 0;
  ***REMOVED***

  if (max_out_len < in_len) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (EVP_CIPHER_CTX_mode(&tls_ctx->cipher_ctx) == EVP_CIPH_CBC_MODE &&
      !tls_ctx->implicit_iv &&
      !EVP_DecryptInit_ex(&tls_ctx->cipher_ctx, NULL, NULL, NULL, nonce)) ***REMOVED***
    return 0;
  ***REMOVED***
  ...
  size_t total = 0;
  int len;
  if (!EVP_DecryptUpdate(&tls_ctx->cipher_ctx, out, &len, in, (int)in_len)) ***REMOVED***
    return 0;
  ***REMOVED***
  total += len;
  if (!EVP_DecryptFinal_ex(&tls_ctx->cipher_ctx, out + total, &len)) ***REMOVED***
    return 0;
  ***REMOVED***
  total += len;
  assert(total == in_len);
  ...
  if (EVP_CIPHER_CTX_mode(&tls_ctx->cipher_ctx) == EVP_CIPH_CBC_MODE) ***REMOVED***
    if (!EVP_tls_cbc_remove_padding(
            &padding_ok, &data_plus_mac_len, out, total,
            EVP_CIPHER_CTX_block_size(&tls_ctx->cipher_ctx),
            (unsigned)HMAC_size(&tls_ctx->hmac_ctx))) ***REMOVED***
  ...
  ***REMOVED***
  ...
  uint8_t *record_mac;
  if (EVP_CIPHER_CTX_mode(&tls_ctx->cipher_ctx) == EVP_CIPH_CBC_MODE &&
      EVP_tls_cbc_record_digest_supported(tls_ctx->hmac_ctx.md)) ***REMOVED***
    if (!EVP_tls_cbc_digest_record(tls_ctx->hmac_ctx.md, mac, &mac_len,
                                   ad_fixed, out, data_plus_mac_len, total,
                                   tls_ctx->mac_key, tls_ctx->mac_key_len)) ***REMOVED***
      OPENSSL_PUT_ERROR(CIPHER, CIPHER_R_BAD_DECRYPT);
      return 0;
    ***REMOVED***
  ...
  ***REMOVED*** else ***REMOVED***
    ...
    if (!HMAC_Init_ex(&tls_ctx->hmac_ctx, NULL, 0, NULL, NULL) ||
        ...
        !HMAC_Final(&tls_ctx->hmac_ctx, mac, &mac_len_u)) ***REMOVED***
      return 0;
    ***REMOVED***
    mac_len = mac_len_u;
...
static int aead_aes_128_cbc_sha1_tls_init(EVP_AEAD_CTX *ctx, const uint8_t *key,
                                          ...
                                          enum evp_aead_direction_t dir) ***REMOVED***
...
static const EVP_AEAD aead_aes_128_cbc_sha1_tls = ***REMOVED***
...
static const EVP_AEAD aead_aes_128_cbc_sha1_tls_implicit_iv = ***REMOVED***
...
static const EVP_AEAD aead_aes_128_cbc_sha256_tls = ***REMOVED***
...
static const EVP_AEAD aead_aes_256_cbc_sha1_tls = ***REMOVED***
...
static const EVP_AEAD aead_aes_256_cbc_sha1_tls_implicit_iv = ***REMOVED***
...
static const EVP_AEAD aead_aes_256_cbc_sha256_tls = ***REMOVED***
...
static const EVP_AEAD aead_aes_256_cbc_sha384_tls = ***REMOVED***
...
static const EVP_AEAD aead_des_ede3_cbc_sha1_tls = ***REMOVED***
...
static const EVP_AEAD aead_des_ede3_cbc_sha1_tls_implicit_iv = ***REMOVED***
...
static const EVP_AEAD aead_null_sha1_tls = ***REMOVED***
...
const EVP_AEAD *EVP_aead_aes_128_cbc_sha1_tls(void) ***REMOVED***
  return &aead_aes_128_cbc_sha1_tls;
***REMOVED***

const EVP_AEAD *EVP_aead_aes_128_cbc_sha1_tls_implicit_iv(void) ***REMOVED***
  return &aead_aes_128_cbc_sha1_tls_implicit_iv;
***REMOVED***

const EVP_AEAD *EVP_aead_aes_128_cbc_sha256_tls(void) ***REMOVED***
  return &aead_aes_128_cbc_sha256_tls;
***REMOVED***

const EVP_AEAD *EVP_aead_aes_256_cbc_sha1_tls(void) ***REMOVED***
  return &aead_aes_256_cbc_sha1_tls;
***REMOVED***

const EVP_AEAD *EVP_aead_aes_256_cbc_sha1_tls_implicit_iv(void) ***REMOVED***
  return &aead_aes_256_cbc_sha1_tls_implicit_iv;
***REMOVED***

const EVP_AEAD *EVP_aead_aes_256_cbc_sha256_tls(void) ***REMOVED***
  return &aead_aes_256_cbc_sha256_tls;
***REMOVED***

const EVP_AEAD *EVP_aead_aes_256_cbc_sha384_tls(void) ***REMOVED***
  return &aead_aes_256_cbc_sha384_tls;
***REMOVED***

const EVP_AEAD *EVP_aead_des_ede3_cbc_sha1_tls(void) ***REMOVED***
  return &aead_des_ede3_cbc_sha1_tls;
***REMOVED***

const EVP_AEAD *EVP_aead_des_ede3_cbc_sha1_tls_implicit_iv(void) ***REMOVED***
  return &aead_des_ede3_cbc_sha1_tls_implicit_iv;
***REMOVED***

const EVP_AEAD *EVP_aead_null_sha1_tls(void) ***REMOVED*** return &aead_null_sha1_tls; ***REMOVED***
