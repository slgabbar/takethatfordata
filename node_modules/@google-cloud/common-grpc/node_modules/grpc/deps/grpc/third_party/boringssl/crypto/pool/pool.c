...
static uint32_t CRYPTO_BUFFER_hash(const CRYPTO_BUFFER *buf) ***REMOVED***
  return OPENSSL_hash32(buf->data, buf->len);
***REMOVED***

static int CRYPTO_BUFFER_cmp(const CRYPTO_BUFFER *a, const CRYPTO_BUFFER *b) ***REMOVED***
...
void CRYPTO_BUFFER_POOL_free(CRYPTO_BUFFER_POOL *pool) ***REMOVED***
  if (pool == NULL) ***REMOVED***
    return;
  ***REMOVED***
...
CRYPTO_BUFFER *CRYPTO_BUFFER_new(const uint8_t *data, size_t len,
  ...
  ***REMOVED***
  ...
  if (len != 0 && buf->data == NULL) ***REMOVED***
    OPENSSL_free(buf);
    return NULL;
  ***REMOVED***
...
CRYPTO_BUFFER* CRYPTO_BUFFER_new_from_CBS(CBS *cbs, CRYPTO_BUFFER_POOL *pool) ***REMOVED***
  return CRYPTO_BUFFER_new(CBS_data(cbs), CBS_len(cbs), pool);
***REMOVED***

void CRYPTO_BUFFER_free(CRYPTO_BUFFER *buf) ***REMOVED***
  if (buf == NULL) ***REMOVED***
    return;
  ***REMOVED***
  ...
  if (pool == NULL) ***REMOVED***
    if (CRYPTO_refcount_dec_and_test_zero(&buf->references)) ***REMOVED***
...
int CRYPTO_BUFFER_up_ref(CRYPTO_BUFFER *buf) ***REMOVED***
...
const uint8_t *CRYPTO_BUFFER_data(const CRYPTO_BUFFER *buf) ***REMOVED***
  return buf->data;
***REMOVED***

size_t CRYPTO_BUFFER_len(const CRYPTO_BUFFER *buf) ***REMOVED***
  return buf->len;
***REMOVED***

void CRYPTO_BUFFER_init_CBS(const CRYPTO_BUFFER *buf, CBS *out) ***REMOVED***
