...
void CRYPTO_refcount_inc(CRYPTO_refcount_t *in_count) ***REMOVED***
  _Atomic CRYPTO_refcount_t *count = (_Atomic CRYPTO_refcount_t *) in_count;
  uint32_t expected = atomic_load(count);

  while (expected != CRYPTO_REFCOUNT_MAX) ***REMOVED***
    uint32_t new_value = expected + 1;
    if (atomic_compare_exchange_weak(count, &expected, new_value)) ***REMOVED***
      break;
    ***REMOVED***
  ***REMOVED***
***REMOVED***

int CRYPTO_refcount_dec_and_test_zero(CRYPTO_refcount_t *in_count) ***REMOVED***
  _Atomic CRYPTO_refcount_t *count = (_Atomic CRYPTO_refcount_t *)in_count;
  uint32_t expected = atomic_load(count);
  ...
  for (;;) ***REMOVED***
    if (expected == 0) ***REMOVED***
      abort();
    ***REMOVED*** else if (expected == CRYPTO_REFCOUNT_MAX) ***REMOVED***
      return 0;
