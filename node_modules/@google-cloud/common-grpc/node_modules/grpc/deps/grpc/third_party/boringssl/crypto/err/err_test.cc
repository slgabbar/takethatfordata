...
TEST(ErrTest, Overflow) ***REMOVED***
  for (unsigned i = 0; i < ERR_NUM_ERRORS*2; i++) ***REMOVED***
    ERR_put_error(1, 0 /* unused */, i+1, "test", 1);
  ***REMOVED***

  for (unsigned i = 0; i < ERR_NUM_ERRORS - 1; i++) ***REMOVED***
    SCOPED_TRACE(i);
...
TEST(ErrTest, ClearError) ***REMOVED***
  ASSERT_EQ(0u, ERR_get_error())
      << "ERR_get_error returned value before an error was added.";
...
TEST(ErrTest, Print) ***REMOVED***
  ERR_put_error(1, 0 /* unused */, 2, "test", 4);
  ERR_add_error_data(1, "testing");
  uint32_t packed_error = ERR_get_error();

  char buf[256];
  for (size_t i = 0; i <= sizeof(buf); i++) ***REMOVED***
    ERR_error_string_n(packed_error, buf, i);
  ***REMOVED***
***REMOVED***

TEST(ErrTest, Release) ***REMOVED***
  ERR_put_error(1, 0 /* unused */, 2, "test", 4);
...
static bool HasSuffix(const char *str, const char *suffix) ***REMOVED***
  size_t suffix_len = strlen(suffix);
  size_t str_len = strlen(str);
  if (str_len < suffix_len) ***REMOVED***
    return false;
  ***REMOVED***
  return strcmp(str + str_len - suffix_len, suffix) == 0;
***REMOVED***

TEST(ErrTest, PutMacro) ***REMOVED***
