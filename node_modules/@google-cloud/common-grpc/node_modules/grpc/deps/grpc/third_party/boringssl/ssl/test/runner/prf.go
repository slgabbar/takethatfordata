...
func splitPreMasterSecret(secret []byte) (s1, s2 []byte) ***REMOVED***
...
func pHash(result, secret, seed []byte, hash func() hash.Hash) ***REMOVED***
	h := hmac.New(hash, secret)
	h.Write(seed)
	a := h.Sum(nil)

	j := 0
	for j < len(result) ***REMOVED***
		h.Reset()
		h.Write(a)
		h.Write(seed)
		b := h.Sum(nil)
		todo := len(b)
		if j+todo > len(result) ***REMOVED***
			todo = len(result) - j
		***REMOVED***
...
func prf10(result, secret, label, seed []byte) ***REMOVED***
	hashSHA1 := sha1.New
	hashMD5 := md5.New
	...
	for i, b := range result2 ***REMOVED***
		result[i] ^= b
	***REMOVED***
***REMOVED***

// prf12 implements the TLS 1.2 pseudo-random function, as defined in RFC 5246, section 5.
func prf12(hashFunc func() hash.Hash) func(result, secret, label, seed []byte) ***REMOVED***
	return func(result, secret, label, seed []byte) ***REMOVED***
		labelAndSeed := make([]byte, len(label)+len(seed))
		copy(labelAndSeed, label)
		copy(labelAndSeed[len(label):], seed)
...
func prf30(result, secret, label, seed []byte) ***REMOVED***
	hashSHA1 := sha1.New()
	hashMD5 := md5.New()
...
func prfForVersion(version uint16, suite *cipherSuite) func(result, secret, label, seed []byte) ***REMOVED***
...
func masterFromPreMasterSecret(version uint16, suite *cipherSuite, preMasterSecret, clientRandom, serverRandom []byte) []byte ***REMOVED***
...
func extendedMasterFromPreMasterSecret(version uint16, suite *cipherSuite, preMasterSecret []byte, h finishedHash) []byte ***REMOVED***
	masterSecret := make([]byte, masterSecretLength)
...
func keysFromMasterSecret(version uint16, suite *cipherSuite, masterSecret, clientRandom, serverRandom []byte, macLen, keyLen, ivLen int) (clientMAC, serverMAC, clientKey, serverKey, clientIV, serverIV []byte) ***REMOVED***
	var seed [tlsRandomLength * 2]byte
...
func newFinishedHash(version uint16, cipherSuite *cipherSuite) finishedHash ***REMOVED***
	...
	***REMOVED***
	...
	ret.buffer = []byte***REMOVED******REMOVED***
	ret.version = version
	return ret
***REMOVED***
...
type finishedHash struct ***REMOVED***
	hash crypto.Hash
...
func (h *finishedHash) Write(msg []byte) (n int, err error) ***REMOVED***
	h.client.Write(msg)
	h.server.Write(msg)

	if h.version < VersionTLS12 ***REMOVED***
		h.clientMD5.Write(msg)
		h.serverMD5.Write(msg)
	***REMOVED***

	if h.buffer != nil ***REMOVED***
...
func (h finishedHash) Sum() []byte ***REMOVED***
	if h.version >= VersionTLS12 ***REMOVED***
...
// messages.
func finishedSum30(md5, sha1 hash.Hash, masterSecret []byte, magic []byte) []byte ***REMOVED***
	md5.Write(magic)
...
var ssl3ClientFinishedMagic = [4]byte***REMOVED***0x43, 0x4c, 0x4e, 0x54***REMOVED***
var ssl3ServerFinishedMagic = [4]byte***REMOVED***0x53, 0x52, 0x56, 0x52***REMOVED***

// clientSum returns the contents of the verify_data member of a client's
// Finished message.
func (h finishedHash) clientSum(baseKey []byte) []byte ***REMOVED***
	if h.version == VersionSSL30 ***REMOVED***
		return finishedSum30(h.clientMD5, h.client, baseKey, ssl3ClientFinishedMagic[:])
	***REMOVED***

	if h.version < VersionTLS13 ***REMOVED***
...
// Finished message.
func (h finishedHash) serverSum(baseKey []byte) []byte ***REMOVED***
	if h.version == VersionSSL30 ***REMOVED***
...
func (h finishedHash) hashForClientCertificateSSL3(masterSecret []byte) []byte ***REMOVED***
	md5Hash := md5.New()
...
func (h finishedHash) hashForChannelID(resumeHash []byte) []byte ***REMOVED***
	hash := sha256.New()
	hash.Write(channelIDLabel)
	if resumeHash != nil ***REMOVED***
...
func (h *finishedHash) discardHandshakeBuffer() ***REMOVED***
	h.buffer = nil
***REMOVED***
...
func hkdfExpandLabel(hash crypto.Hash, secret, label, hashValue []byte, length int) []byte ***REMOVED***
	if len(label) > 255 || len(hashValue) > 255 ***REMOVED***
...
func (h *finishedHash) appendContextHashes(b []byte) []byte ***REMOVED***
	b = h.client.Sum(b)
	return b
***REMOVED***
...
func (h *finishedHash) deriveSecret(label []byte) []byte ***REMOVED***
...
// in TLS 1.3.
func (h *finishedHash) certificateVerifyInput(context []byte) []byte ***REMOVED***
	const paddingLen = 64
...
// secret.
func deriveTrafficAEAD(version uint16, suite *cipherSuite, secret []byte, side trafficDirection) interface***REMOVED******REMOVED*** ***REMOVED***
...
func updateTrafficSecret(hash crypto.Hash, secret []byte) []byte ***REMOVED***
	return hkdfExpandLabel(hash, secret, applicationTrafficLabel, nil, hash.Size())
***REMOVED***

func computePSKBinder(psk, label []byte, cipherSuite *cipherSuite, transcript, truncatedHello []byte) []byte ***REMOVED***
