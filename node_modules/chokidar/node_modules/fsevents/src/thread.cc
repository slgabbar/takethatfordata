...
void FSEvents::threadStart() ***REMOVED***
  if (threadloop) return;
  pthread_create(&thread, NULL, &FSEvents::threadRun, this);
***REMOVED***

void HandleStreamEvents(ConstFSEventStreamRef stream, void *ctx, size_t numEvents, void *eventPaths, const FSEventStreamEventFlags eventFlags[], const FSEventStreamEventId eventIds[]) ***REMOVED***
  FSEvents * fse = (FSEvents *)ctx;
  size_t idx;
  fse->lock();
  for (idx=0; idx < numEvents; idx++) ***REMOVED***
...
void *FSEvents::threadRun(void *ctx) ***REMOVED***
  FSEvents *fse = (FSEvents*)ctx;
  FSEventStreamContext context = ***REMOVED*** 0, ctx, NULL, NULL, NULL ***REMOVED***;
...
void FSEvents::threadStop() ***REMOVED***
  if (!threadloop) return;
