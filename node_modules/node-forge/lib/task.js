...
var sTasks = ***REMOVED******REMOVED***;
var sNextTaskId = 0;
// debug access
forge.debug.set(cat, 'tasks', sTasks);

// a map of task type to task queue
var sTaskQueues = ***REMOVED******REMOVED***;
// debug access
 ...
 */
var sStateTable = ***REMOVED******REMOVED***;

sStateTable[READY] = ***REMOVED******REMOVED***;
sStateTable[READY][STOP] = READY;
sStateTable[READY][START] = RUNNING;
sStateTable[READY][CANCEL] = DONE;
sStateTable[READY][FAIL] = ERROR;

sStateTable[RUNNING] = ***REMOVED******REMOVED***;
...
sStateTable[BLOCKED] = ***REMOVED******REMOVED***;
...
sStateTable[SLEEPING] = ***REMOVED******REMOVED***;
...
sStateTable[DONE] = ***REMOVED******REMOVED***;
...
sStateTable[ERROR] = ***REMOVED******REMOVED***;
 ...
 */
var Task = function(options) ***REMOVED***
  // task id
  this.id = -1;
 ...
 */
Task.prototype.debug = function(msg) ***REMOVED***
  msg = msg || '';
 ...
 */
Task.prototype.next = function(name, subrun) ***REMOVED***
 ...
 * @return the current task (useful for chaining next() calls).
 */
Task.prototype.parallel = function(name, subrun) ***REMOVED***
  // juggle parameters if it looks like no name is given
  if(forge.util.isArray(name)) ***REMOVED***
    subrun = name;
  ...
  ***REMOVED***
  ...
  // proper time.
  return this.next(name, function(task) ***REMOVED***
    ...
    var startParallelTask = function(pname, pi) ***REMOVED***
      forge.task.start(***REMOVED***
        type: pname,
        run: function(task) ***REMOVED***
           subrun[pi](task);
        ***REMOVED***,
        success: function(task) ***REMOVED***
           ptask.unblock();
        ***REMOVED***,
        failure: function(task) ***REMOVED***
           ptask.unblock();
        ***REMOVED***
      ***REMOVED***);
    ***REMOVED***;

    for(var i = 0; i < subrun.length; i++) ***REMOVED***
 ...
 */
Task.prototype.stop = function() ***REMOVED***
 ...
 */
Task.prototype.start = function() ***REMOVED***
  this.error = false;
  this.state = sStateTable[this.state][START];

  // try to restart
  if(this.state === RUNNING) ***REMOVED***
    this.start = new Date();
    this.run(this);
    runNext(this, 0);
  ***REMOVED***
 ...
 */
Task.prototype.block = function(n) ***REMOVED***
  n = typeof(n) === 'undefined' ? 1 : n;
  this.blocks += n;
  if(this.blocks > 0) ***REMOVED***
 ...
 */
Task.prototype.unblock = function(n) ***REMOVED***
  n = typeof(n) === 'undefined' ? 1 : n;
  this.blocks -= n;
  if(this.blocks === 0 && this.state !== DONE) ***REMOVED***
    this.state = RUNNING;
    runNext(this, 0);
  ***REMOVED***
  return this.blocks;
 ...
 */
Task.prototype.sleep = function(n) ***REMOVED***
 ...
 */
Task.prototype.wait = function(cond) ***REMOVED***
  cond.wait(this);
***REMOVED***;
 ...
 */
Task.prototype.wakeup = function() ***REMOVED***
  if(this.state === SLEEPING) ***REMOVED***
 ...
 */
Task.prototype.cancel = function() ***REMOVED***
  this.state = sStateTable[this.state][CANCEL];
  // remove permits needed
  this.permitsNeeded = 0;
  // cancel timeouts
  if(this.timeoutId !== null) ***REMOVED***
    cancelTimeout(this.timeoutId);
    this.timeoutId = null;
  ***REMOVED***
  // remove subtasks
 ...
 */
Task.prototype.fail = function(next) ***REMOVED***
  // set error flag
  this.error = true;
  ...
  ***REMOVED*** else ***REMOVED***
    if(this.parent !== null) ***REMOVED***
      ...
      while(parent.parent !== null) ***REMOVED***
        // propagate task info
    ...
    ***REMOVED***
    ...
    if(this.failureCallback) ***REMOVED***
 ...
 */
var start = function(task) ***REMOVED***
  task.error = false;
  task.state = sStateTable[task.state][START];
  setTimeout(function() ***REMOVED***
    if(task.state === RUNNING) ***REMOVED***
 ...
 */
var runNext = function(task, recurse) ***REMOVED***
  ...
  var doNext = function(recurse) ***REMOVED***
    recurse++;
    if(task.state === RUNNING) ***REMOVED***
      if(swap) ***REMOVED***
        // update swap time
      ...
      ***REMOVED***
      ...
      if(task.subtasks.length > 0) ***REMOVED***
        // run next subtask
        ...
        subtask.run(subtask);
        if(!subtask.error) ***REMOVED***
           runNext(subtask, recurse);
        ***REMOVED***
      ***REMOVED*** else ***REMOVED***
        finish(task);

        if(!task.error) ***REMOVED***
          // chain back up and run parent
          if(task.parent !== null) ***REMOVED***
            // propagate task info
 ...
 */
var finish = function(task, suppressCallbacks) ***REMOVED***
  // subtask is now done
  task.state = DONE;
  ...
  ***REMOVED***
  ...
  if(task.parent === null) ***REMOVED***
    ...
    if(!(task.type in sTaskQueues)) ***REMOVED***
      forge.log.error(cat,
        '[%s][%s] task queue missing [%s]',
        task.id, task.name, task.type);
    ***REMOVED*** else if(sTaskQueues[task.type].length === 0) ***REMOVED***
      // report error if queue is empty
      forge.log.error(cat,
        '[%s][%s] task queue empty [%s]',
        task.id, task.name, task.type);
    ***REMOVED*** else if(sTaskQueues[task.type][0] !== task) ***REMOVED***
    ...
    ***REMOVED*** else ***REMOVED***
      ...
      if(sTaskQueues[task.type].length === 0) ***REMOVED***
        if(sVL >= 1) ***REMOVED***
    ...
    ***REMOVED***
    ...
    if(!suppressCallbacks) ***REMOVED***
      // call final callback if one exists
      if(task.error && task.failureCallback) ***REMOVED***
        task.failureCallback(task);
      ***REMOVED*** else if(!task.error && task.successCallback) ***REMOVED***
...
/* Tasks API */
module.exports = forge.task = forge.task || ***REMOVED******REMOVED***;
 ...
 */
forge.task.start = function(options) ***REMOVED***
  // create a new task
  var task = new Task(***REMOVED***
    run: options.run,
 ...
 */
forge.task.cancel = function(type) ***REMOVED***
  // find the task queue
  if(type in sTaskQueues) ***REMOVED***
 ...
 */
forge.task.createCondition = function() ***REMOVED***
  var cond = ***REMOVED***
  ...
  ***REMOVED***;
   ...
   */
  cond.wait = function(task) ***REMOVED***
    // only block once
    if(!(task.id in cond.tasks)) ***REMOVED***
       task.block();
  ...
  ***REMOVED***;
   ...
   */
  cond.notify = function() ***REMOVED***
    // since unblock() will run the next task from here, make sure to
    // clear the condition's blocked task list before unblocking
    var tmp = cond.tasks;
    cond.tasks = ***REMOVED******REMOVED***;
    for(var id in tmp) ***REMOVED***
      tmp[id].unblock();
    ***REMOVED***
  ***REMOVED***;
