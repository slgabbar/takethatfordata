module.exports = rimraf
rimraf.sync = rimrafSync

var assert = require("assert")
var path = require("path")
var fs = require("fs")
var glob = require("glob")

var defaultGlobOpts = ***REMOVED***
  nosort: true,
  silent: true
***REMOVED***

// for EMFILE handling
var timeout = 0

var isWindows = (process.platform === "win32")

function defaults (options) ***REMOVED***
  var methods = [
    'unlink',
    'chmod',
    'stat',
    'lstat',
    'rmdir',
    'readdir'
  ]
  methods.forEach(function(m) ***REMOVED***
    options[m] = options[m] || fs[m]
    m = m + 'Sync'
    options[m] = options[m] || fs[m]
  ***REMOVED***)

  options.maxBusyTries = options.maxBusyTries || 3
  options.emfileWait = options.emfileWait || 1000
  if (options.glob === false) ***REMOVED***
    options.disableGlob = true
  ***REMOVED***
  options.disableGlob = options.disableGlob || false
  options.glob = options.glob || defaultGlobOpts
***REMOVED***

function rimraf (p, options, cb) ***REMOVED***
  if (typeof options === 'function') ***REMOVED***
    cb = options
    options = ***REMOVED******REMOVED***
  ***REMOVED***

  assert(p, 'rimraf: missing path')
  assert.equal(typeof p, 'string', 'rimraf: path should be a string')
  assert.equal(typeof cb, 'function', 'rimraf: callback function required')
  assert(options, 'rimraf: invalid options argument provided')
  assert.equal(typeof options, 'object', 'rimraf: options should be object')

  defaults(options)

  var busyTries = 0
  var errState = null
  var n = 0

  if (options.disableGlob || !glob.hasMagic(p))
    return afterGlob(null, [p])

  options.lstat(p, function (er, stat) ***REMOVED***
    if (!er)
      return afterGlob(null, [p])

    glob(p, options.glob, afterGlob)
  ***REMOVED***)

  function next (er) ***REMOVED***
    errState = errState || er
    if (--n === 0)
      cb(errState)
  ***REMOVED***

  function afterGlob (er, results) ***REMOVED***
    if (er)
      return cb(er)

    n = results.length
    if (n === 0)
      return cb()

    results.forEach(function (p) ***REMOVED***
      rimraf_(p, options, function CB (er) ***REMOVED***
        if (er) ***REMOVED***
          if ((er.code === "EBUSY" || er.code === "ENOTEMPTY" || er.code === "EPERM") &&
              busyTries < options.maxBusyTries) ***REMOVED***
            busyTries ++
            var time = busyTries * 100
            // try again, with the same exact callback as this one.
            return setTimeout(function () ***REMOVED***
              rimraf_(p, options, CB)
            ***REMOVED***, time)
          ***REMOVED***

          // this one won't happen if graceful-fs is used.
          if (er.code === "EMFILE" && timeout < options.emfileWait) ***REMOVED***
            return setTimeout(function () ***REMOVED***
              rimraf_(p, options, CB)
            ***REMOVED***, timeout ++)
          ***REMOVED***

          // already gone
          if (er.code === "ENOENT") er = null
        ***REMOVED***

        timeout = 0
        next(er)
      ***REMOVED***)
    ***REMOVED***)
  ***REMOVED***
***REMOVED***

// Two possible strategies.
// 1. Assume it's a file.  unlink it, then do the dir stuff on EPERM or EISDIR
// 2. Assume it's a directory.  readdir, then do the file stuff on ENOTDIR
//
// Both result in an extra syscall when you guess wrong.  However, there
// are likely far more normal files in the world than directories.  This
// is based on the assumption that a the average number of files per
// directory is >= 1.
//
// If anyone ever complains about this, then I guess the strategy could
// be made configurable somehow.  But until then, YAGNI.
function rimraf_ (p, options, cb) ***REMOVED***
  assert(p)
  assert(options)
  assert(typeof cb === 'function')

  // sunos lets the root user unlink directories, which is... weird.
  // so we have to lstat here and make sure it's not a dir.
  options.lstat(p, function (er, st) ***REMOVED***
    if (er && er.code === "ENOENT")
      return cb(null)

    // Windows can EPERM on stat.  Life is suffering.
    if (er && er.code === "EPERM" && isWindows)
      fixWinEPERM(p, options, er, cb)

    if (st && st.isDirectory())
      return rmdir(p, options, er, cb)

    options.unlink(p, function (er) ***REMOVED***
      if (er) ***REMOVED***
        if (er.code === "ENOENT")
          return cb(null)
        if (er.code === "EPERM")
          return (isWindows)
            ? fixWinEPERM(p, options, er, cb)
            : rmdir(p, options, er, cb)
        if (er.code === "EISDIR")
          return rmdir(p, options, er, cb)
      ***REMOVED***
      return cb(er)
    ***REMOVED***)
  ***REMOVED***)
***REMOVED***

function fixWinEPERM (p, options, er, cb) ***REMOVED***
  assert(p)
  assert(options)
  assert(typeof cb === 'function')
  if (er)
    assert(er instanceof Error)

  options.chmod(p, 666, function (er2) ***REMOVED***
    if (er2)
      cb(er2.code === "ENOENT" ? null : er)
    else
      options.stat(p, function(er3, stats) ***REMOVED***
        if (er3)
          cb(er3.code === "ENOENT" ? null : er)
        else if (stats.isDirectory())
          rmdir(p, options, er, cb)
        else
          options.unlink(p, cb)
      ***REMOVED***)
  ***REMOVED***)
***REMOVED***

function fixWinEPERMSync (p, options, er) ***REMOVED***
  assert(p)
  assert(options)
  if (er)
    assert(er instanceof Error)

  try ***REMOVED***
    options.chmodSync(p, 666)
  ***REMOVED*** catch (er2) ***REMOVED***
    if (er2.code === "ENOENT")
      return
    else
      throw er
  ***REMOVED***

  try ***REMOVED***
    var stats = options.statSync(p)
  ***REMOVED*** catch (er3) ***REMOVED***
    if (er3.code === "ENOENT")
      return
    else
      throw er
  ***REMOVED***

  if (stats.isDirectory())
    rmdirSync(p, options, er)
  else
    options.unlinkSync(p)
***REMOVED***

function rmdir (p, options, originalEr, cb) ***REMOVED***
  assert(p)
  assert(options)
  if (originalEr)
    assert(originalEr instanceof Error)
  assert(typeof cb === 'function')

  // try to rmdir first, and only readdir on ENOTEMPTY or EEXIST (SunOS)
  // if we guessed wrong, and it's not a directory, then
  // raise the original error.
  options.rmdir(p, function (er) ***REMOVED***
    if (er && (er.code === "ENOTEMPTY" || er.code === "EEXIST" || er.code === "EPERM"))
      rmkids(p, options, cb)
    else if (er && er.code === "ENOTDIR")
      cb(originalEr)
    else
      cb(er)
  ***REMOVED***)
***REMOVED***

function rmkids(p, options, cb) ***REMOVED***
  assert(p)
  assert(options)
  assert(typeof cb === 'function')

  options.readdir(p, function (er, files) ***REMOVED***
    if (er)
      return cb(er)
    var n = files.length
    if (n === 0)
      return options.rmdir(p, cb)
    var errState
    files.forEach(function (f) ***REMOVED***
      rimraf(path.join(p, f), options, function (er) ***REMOVED***
        if (errState)
          return
        if (er)
          return cb(errState = er)
        if (--n === 0)
          options.rmdir(p, cb)
      ***REMOVED***)
    ***REMOVED***)
  ***REMOVED***)
***REMOVED***

// this looks simpler, and is strictly *faster*, but will
// tie up the JavaScript thread and fail on excessively
// deep directory trees.
function rimrafSync (p, options) ***REMOVED***
  options = options || ***REMOVED******REMOVED***
  defaults(options)

  assert(p, 'rimraf: missing path')
  assert.equal(typeof p, 'string', 'rimraf: path should be a string')
  assert(options, 'rimraf: missing options')
  assert.equal(typeof options, 'object', 'rimraf: options should be object')

  var results

  if (options.disableGlob || !glob.hasMagic(p)) ***REMOVED***
    results = [p]
  ***REMOVED*** else ***REMOVED***
    try ***REMOVED***
      options.lstatSync(p)
      results = [p]
    ***REMOVED*** catch (er) ***REMOVED***
      results = glob.sync(p, options.glob)
    ***REMOVED***
  ***REMOVED***

  if (!results.length)
    return

  for (var i = 0; i < results.length; i++) ***REMOVED***
    var p = results[i]

    try ***REMOVED***
      var st = options.lstatSync(p)
    ***REMOVED*** catch (er) ***REMOVED***
      if (er.code === "ENOENT")
        return

      // Windows can EPERM on stat.  Life is suffering.
      if (er.code === "EPERM" && isWindows)
        fixWinEPERMSync(p, options, er)
    ***REMOVED***

    try ***REMOVED***
      // sunos lets the root user unlink directories, which is... weird.
      if (st && st.isDirectory())
        rmdirSync(p, options, null)
      else
        options.unlinkSync(p)
    ***REMOVED*** catch (er) ***REMOVED***
      if (er.code === "ENOENT")
        return
      if (er.code === "EPERM")
        return isWindows ? fixWinEPERMSync(p, options, er) : rmdirSync(p, options, er)
      if (er.code !== "EISDIR")
        throw er

      rmdirSync(p, options, er)
    ***REMOVED***
  ***REMOVED***
***REMOVED***

function rmdirSync (p, options, originalEr) ***REMOVED***
  assert(p)
  assert(options)
  if (originalEr)
    assert(originalEr instanceof Error)

  try ***REMOVED***
    options.rmdirSync(p)
  ***REMOVED*** catch (er) ***REMOVED***
    if (er.code === "ENOENT")
      return
    if (er.code === "ENOTDIR")
      throw originalEr
    if (er.code === "ENOTEMPTY" || er.code === "EEXIST" || er.code === "EPERM")
      rmkidsSync(p, options)
  ***REMOVED***
***REMOVED***

function rmkidsSync (p, options) ***REMOVED***
  assert(p)
  assert(options)
  options.readdirSync(p).forEach(function (f) ***REMOVED***
    rimrafSync(path.join(p, f), options)
  ***REMOVED***)

  // We only end up here once we got ENOTEMPTY at least once, and
  // at this point, we are guaranteed to have removed all the kids.
  // So, we know that it won't be ENOENT or ENOTDIR or anything else.
  // try really hard to delete stuff on windows, because it has a
  // PROFOUNDLY annoying habit of not closing handles promptly when
  // files are deleted, resulting in spurious ENOTEMPTY errors.
  var retries = isWindows ? 100 : 1
  var i = 0
  do ***REMOVED***
    var threw = true
    try ***REMOVED***
      var ret = options.rmdirSync(p, options)
      threw = false
      return ret
    ***REMOVED*** finally ***REMOVED***
      if (++i < retries && threw)
        continue
    ***REMOVED***
  ***REMOVED*** while (true)
***REMOVED***
