"use strict";
/**
 * Copyright 2014 Google Inc. All Rights Reserved.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *      http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
Object.defineProperty(exports, "__esModule", ***REMOVED*** value: true ***REMOVED***);
var child_process_1 = require("child_process");
var fs = require("fs");
var os = require("os");
var path = require("path");
var util = require("util");
var transporters_1 = require("../transporters");
var computeclient_1 = require("./computeclient");
var iam_1 = require("./iam");
var jwtaccess_1 = require("./jwtaccess");
var jwtclient_1 = require("./jwtclient");
var oauth2client_1 = require("./oauth2client");
var refreshclient_1 = require("./refreshclient");
var GoogleAuth = /** @class */ (function () ***REMOVED***
    function GoogleAuth() ***REMOVED***
        /**
         * Caches a value indicating whether the auth layer is running on Google
         * Compute Engine.
         * @private
         */
        this._isGCE = undefined;
        // To save the contents of the JSON credential file
        this.jsonContent = null;
        this.cachedCredential = null;
        this.JWTClient = jwtclient_1.JWT;
        this.ComputeClient = computeclient_1.Compute;
        /**
         * Convenience field mapping in the IAM credential type.
         */
        this.IAMAuth = iam_1.IAMAuth;
        /**
         * Convenience field mapping in the Compute credential type.
         */
        this.Compute = computeclient_1.Compute;
        /**
         * Convenience field mapping in the JWT credential type.
         */
        this.JWT = jwtclient_1.JWT;
        /**
         * Convenience field mapping in the JWT Access credential type.
         */
        this.JWTAccess = jwtaccess_1.JWTAccess;
        /**
         * Convenience field mapping in the OAuth2 credential type.
         */
        this.OAuth2 = oauth2client_1.OAuth2Client;
        /**
         * Convenience field mapping to the UserRefreshClient credential type.
         */
        this.UserRefreshClient = refreshclient_1.UserRefreshClient;
    ***REMOVED***
    Object.defineProperty(GoogleAuth.prototype, "isGCE", ***REMOVED***
        // Note:  this properly is only public to satisify unit tests.
        // https://github.com/Microsoft/TypeScript/issues/5228
        get: function () ***REMOVED***
            return this._isGCE;
        ***REMOVED***,
        enumerable: true,
        configurable: true
    ***REMOVED***);
    Object.defineProperty(GoogleAuth.prototype, "cachedProjectId", ***REMOVED***
        // Note:  this properly is only public to satisify unit tests.
        // https://github.com/Microsoft/TypeScript/issues/5228
        set: function (projectId) ***REMOVED***
            this._cachedProjectId = projectId;
        ***REMOVED***,
        enumerable: true,
        configurable: true
    ***REMOVED***);
    /**
     * Obtains the default project ID for the application..
     * @param ***REMOVED***function=***REMOVED*** callback Optional callback.
     */
    GoogleAuth.prototype.getDefaultProjectId = function (callback) ***REMOVED***
        // In implicit case, supports three environments. In order of precedence,
        // the implicit environments are:
        //
        // * GCLOUD_PROJECT or GOOGLE_CLOUD_PROJECT environment variable
        // * GOOGLE_APPLICATION_CREDENTIALS JSON file
        // * Get default service project from
        //  ``$ gcloud beta auth application-default login``
        // * Google App Engine application ID (Not implemented yet)
        // * Google Compute Engine project ID (from metadata server) (Not
        // implemented yet)
        var _this = this;
        if (this._cachedProjectId) ***REMOVED***
            setImmediate(function () ***REMOVED***
                _this.callback(callback, null, _this._cachedProjectId);
            ***REMOVED***);
        ***REMOVED***
        else ***REMOVED***
            var my_callback_1 = function (err, projectId) ***REMOVED***
                if (!err && projectId) ***REMOVED***
                    _this._cachedProjectId = projectId;
                ***REMOVED***
                setImmediate(function () ***REMOVED***
                    _this.callback(callback, err, projectId);
                ***REMOVED***);
            ***REMOVED***;
            // environment variable
            if (this._getProductionProjectId(my_callback_1)) ***REMOVED***
                return;
            ***REMOVED***
            // json file
            this._getFileProjectId(function (err, projectId) ***REMOVED***
                if (err || projectId) ***REMOVED***
                    my_callback_1(err, projectId);
                    return;
                ***REMOVED***
                // Google Cloud SDK default project id
                _this._getDefaultServiceProjectId(function (err2, projectId2) ***REMOVED***
                    if (err2 || projectId2) ***REMOVED***
                        my_callback_1(err2, projectId2);
                        return;
                    ***REMOVED***
                    // Get project ID from Compute Engine metadata server
                    _this._getGCEProjectId(my_callback_1);
                ***REMOVED***);
            ***REMOVED***);
        ***REMOVED***
    ***REMOVED***;
    /**
     * Run the Google Cloud SDK command that prints the default project ID
     * @param ***REMOVED***function***REMOVED*** _callback Callback.
     * @api private
     */
    GoogleAuth.prototype._getSDKDefaultProjectId = function (_callback) ***REMOVED***
        child_process_1.exec('gcloud -q config list core/project --format=json', _callback);
    ***REMOVED***;
    /**
     * Obtains the default service-level credentials for the application..
     * @param ***REMOVED***function=***REMOVED*** callback Optional callback.
     */
    GoogleAuth.prototype.getApplicationDefault = function (callback) ***REMOVED***
        var _this = this;
        // If we've already got a cached credential, just return it.
        if (this.cachedCredential) ***REMOVED***
            setImmediate(function () ***REMOVED***
                _this.callback(callback, null, _this.cachedCredential, _this._cachedProjectId);
            ***REMOVED***);
        ***REMOVED***
        else ***REMOVED***
            // Inject our own callback routine, which will cache the credential once
            // it's been created. It also allows us to ensure that the ultimate
            // callback is always async.
            var my_callback_2 = function (err, result) ***REMOVED***
                if (!err && result) ***REMOVED***
                    _this.cachedCredential = result;
                    _this.getDefaultProjectId(function (err2, projectId) ***REMOVED***
                        setImmediate(function () ***REMOVED***
                            // Ignore default project error
                            _this.callback(callback, null, result, projectId);
                        ***REMOVED***);
                    ***REMOVED***);
                ***REMOVED***
                else ***REMOVED***
                    setImmediate(function () ***REMOVED***
                        _this.callback(callback, err, result);
                    ***REMOVED***);
                ***REMOVED***
            ***REMOVED***;
            // Check for the existence of a local environment variable pointing to the
            // location of the credential file. This is typically used in local
            // developer scenarios.
            if (this._tryGetApplicationCredentialsFromEnvironmentVariable(my_callback_2)) ***REMOVED***
                return;
            ***REMOVED***
            // Look in the well-known credential file location.
            if (this._tryGetApplicationCredentialsFromWellKnownFile(my_callback_2)) ***REMOVED***
                return;
            ***REMOVED***
            // Determine if we're running on GCE.
            this._checkIsGCE(function (err, gce) ***REMOVED***
                if (gce) ***REMOVED***
                    // For GCE, just return a default ComputeClient. It will take care of
                    // the rest.
                    my_callback_2(null, new _this.ComputeClient());
                ***REMOVED***
                else if (err) ***REMOVED***
                    my_callback_2(new Error('Unexpected error while acquiring application default ' +
                        'credentials: ' + err.message));
                ***REMOVED***
                else ***REMOVED***
                    // We failed to find the default credentials. Bail out with an error.
                    my_callback_2(new Error('Could not load the default credentials. Browse to ' +
                        'https://developers.google.com/accounts/docs/application-default-credentials for ' +
                        'more information.'));
                ***REMOVED***
            ***REMOVED***);
        ***REMOVED***
    ***REMOVED***;
    /**
     * Determines whether the auth layer is running on Google Compute Engine.
     * @param ***REMOVED***function=***REMOVED*** callback The callback.
     * @api private
     */
    GoogleAuth.prototype._checkIsGCE = function (callback) ***REMOVED***
        var _this = this;
        if (this._isGCE !== undefined) ***REMOVED***
            setImmediate(function () ***REMOVED***
                callback(null, _this._isGCE);
            ***REMOVED***);
        ***REMOVED***
        else ***REMOVED***
            if (!this.transporter) ***REMOVED***
                this.transporter = new transporters_1.DefaultTransporter();
            ***REMOVED***
            this.transporter.request(***REMOVED*** method: 'GET', uri: 'http://metadata.google.internal', json: true ***REMOVED***, function (err, body, res) ***REMOVED***
                if (err) ***REMOVED***
                    if (err.code !== 'ENOTFOUND') ***REMOVED***
                        // Unexpected error occurred. TODO(ofrobots): retry if this was
                        // a transient error.
                        return callback(err);
                    ***REMOVED***
                    _this._isGCE = false;
                    return callback(null, _this._isGCE);
                ***REMOVED***
                _this._isGCE = res && res.headers &&
                    res.headers['metadata-flavor'] === 'Google';
                callback(null, _this._isGCE);
            ***REMOVED***);
        ***REMOVED***
    ***REMOVED***;
    /**
     * Attempts to load default credentials from the environment variable path..
     * @param ***REMOVED***function=***REMOVED*** callback Optional callback.
     * @return ***REMOVED***boolean***REMOVED*** Returns true if the callback has been executed; false otherwise.
     * @api private
     */
    GoogleAuth.prototype._tryGetApplicationCredentialsFromEnvironmentVariable = function (callback) ***REMOVED***
        var _this = this;
        var credentialsPath = this._getEnv('GOOGLE_APPLICATION_CREDENTIALS');
        if (!credentialsPath || credentialsPath.length === 0) ***REMOVED***
            return false;
        ***REMOVED***
        this._getApplicationCredentialsFromFilePath(credentialsPath, function (err, result) ***REMOVED***
            var wrappedError = null;
            if (err) ***REMOVED***
                wrappedError = _this.createError('Unable to read the credential file specified by the GOOGLE_APPLICATION_CREDENTIALS ' +
                    'environment variable.', err);
            ***REMOVED***
            _this.callback(callback, wrappedError, result);
        ***REMOVED***);
        return true;
    ***REMOVED***;
    /**
     * Attempts to load default credentials from a well-known file location
     * @param ***REMOVED***function=***REMOVED*** callback Optional callback.
     * @return ***REMOVED***boolean***REMOVED*** Returns true if the callback has been executed; false otherwise.
     * @api private
     */
    GoogleAuth.prototype._tryGetApplicationCredentialsFromWellKnownFile = function (callback) ***REMOVED***
        // First, figure out the location of the file, depending upon the OS type.
        var location = null;
        if (this._isWindows()) ***REMOVED***
            // Windows
            location = this._getEnv('APPDATA');
        ***REMOVED***
        else ***REMOVED***
            // Linux or Mac
            var home = this._getEnv('HOME');
            if (home) ***REMOVED***
                location = this._pathJoin(home, '.config');
            ***REMOVED***
        ***REMOVED***
        // If we found the root path, expand it.
        if (location) ***REMOVED***
            location = this._pathJoin(location, 'gcloud');
            location =
                this._pathJoin(location, 'application_default_credentials.json');
            location = this._mockWellKnownFilePath(location);
            // Check whether the file exists.
            if (!this._fileExists(location)) ***REMOVED***
                location = null;
            ***REMOVED***
        ***REMOVED***
        // The file does not exist.
        if (!location) ***REMOVED***
            return false;
        ***REMOVED***
        // The file seems to exist. Try to use it.
        this._getApplicationCredentialsFromFilePath(location, callback);
        return true;
    ***REMOVED***;
    /**
     * Attempts to load default credentials from a file at the given path..
     * @param ***REMOVED***string=***REMOVED*** filePath The path to the file to read.
     * @param ***REMOVED***function=***REMOVED*** callback Optional callback.
     * @api private
     */
    GoogleAuth.prototype._getApplicationCredentialsFromFilePath = function (filePath, callback) ***REMOVED***
        var error = null;
        // Make sure the path looks like a string.
        if (!filePath || filePath.length === 0) ***REMOVED***
            error = new Error('The file path is invalid.');
        ***REMOVED***
        // Make sure there is a file at the path. lstatSync will throw if there is
        // nothing there.
        if (!error) ***REMOVED***
            try ***REMOVED***
                // Resolve path to actual file in case of symlink. Expect a thrown error
                // if not resolvable.
                filePath = fs.realpathSync(filePath);
                if (!fs.lstatSync(filePath).isFile()) ***REMOVED***
                    throw new Error();
                ***REMOVED***
            ***REMOVED***
            catch (err) ***REMOVED***
                error = this.createError(util.format('The file at %s does not exist, or it is not a file.', filePath), err);
            ***REMOVED***
        ***REMOVED***
        // Now open a read stream on the file, and parse it.
        if (!error) ***REMOVED***
            try ***REMOVED***
                var stream_1 = this._createReadStream(filePath);
                this.fromStream(stream_1, callback);
            ***REMOVED***
            catch (err) ***REMOVED***
                error = this.createError(util.format('Unable to read the file at %s.', filePath), err);
            ***REMOVED***
        ***REMOVED***
        if (error) ***REMOVED***
            this.callback(callback, error);
        ***REMOVED***
    ***REMOVED***;
    /**
     * Create a credentials instance using the given input options.
     * @param ***REMOVED***object=***REMOVED*** json The input object.
     * @param ***REMOVED***function=***REMOVED*** callback Optional callback.
     */
    GoogleAuth.prototype.fromJSON = function (json, callback) ***REMOVED***
        var _this = this;
        var client = null;
        if (!json) ***REMOVED***
            this.callback(callback, new Error('Must pass in a JSON object containing the Google auth settings.'));
            return;
        ***REMOVED***
        // Set the JSON contents
        this.jsonContent = json;
        if (json.type === 'authorized_user') ***REMOVED***
            client = new refreshclient_1.UserRefreshClient();
        ***REMOVED***
        else ***REMOVED***
            client = new jwtclient_1.JWT();
        ***REMOVED***
        client.fromJSON(json, function (err) ***REMOVED***
            if (err) ***REMOVED***
                _this.callback(callback, err);
            ***REMOVED***
            else ***REMOVED***
                _this.callback(callback, null, client);
            ***REMOVED***
        ***REMOVED***);
    ***REMOVED***;
    /**
     * Create a credentials instance using the given input stream.
     * @param ***REMOVED***object=***REMOVED*** stream The input stream.
     * @param ***REMOVED***function=***REMOVED*** callback Optional callback.
     */
    GoogleAuth.prototype.fromStream = function (stream, callback) ***REMOVED***
        var _this = this;
        if (!stream) ***REMOVED***
            setImmediate(function () ***REMOVED***
                _this.callback(callback, new Error('Must pass in a stream containing the Google auth settings.'));
            ***REMOVED***);
            return;
        ***REMOVED***
        var s = '';
        stream.setEncoding('utf8');
        stream.on('data', function (chunk) ***REMOVED***
            s += chunk;
        ***REMOVED***);
        stream.on('end', function () ***REMOVED***
            try ***REMOVED***
                var data = JSON.parse(s);
                _this.fromJSON(data, callback);
            ***REMOVED***
            catch (err) ***REMOVED***
                _this.callback(callback, err);
            ***REMOVED***
        ***REMOVED***);
    ***REMOVED***;
    /**
     * Create a credentials instance using the given API key string.
     * @param ***REMOVED***string***REMOVED*** - The API key string
     * @param ***REMOVED***function=***REMOVED*** - Optional callback function
     */
    GoogleAuth.prototype.fromAPIKey = function (apiKey, callback) ***REMOVED***
        var _this = this;
        var client = new this.JWTClient();
        client.fromAPIKey(apiKey, function (err) ***REMOVED***
            if (err) ***REMOVED***
                _this.callback(callback, err);
            ***REMOVED***
            else ***REMOVED***
                _this.callback(callback, null, client);
            ***REMOVED***
        ***REMOVED***);
    ***REMOVED***;
    /**
     * Determines whether the current operating system is Windows.
     * @api private
     */
    GoogleAuth.prototype._isWindows = function () ***REMOVED***
        var sys = this._osPlatform();
        if (sys && sys.length >= 3) ***REMOVED***
            if (sys.substring(0, 3).toLowerCase() === 'win') ***REMOVED***
                return true;
            ***REMOVED***
        ***REMOVED***
        return false;
    ***REMOVED***;
    /**
     * Creates a file stream. Allows mocking.
     * @api private
     */
    GoogleAuth.prototype._createReadStream = function (filePath) ***REMOVED***
        return fs.createReadStream(filePath);
    ***REMOVED***;
    /**
     * Gets the value of the environment variable with the given name. Allows
     * mocking.
     * @api private
     */
    GoogleAuth.prototype._getEnv = function (name) ***REMOVED***
        return process.env[name];
    ***REMOVED***;
    /**
     * Gets the current operating system platform. Allows mocking.
     * @api private
     */
    GoogleAuth.prototype._osPlatform = function () ***REMOVED***
        return os.platform();
    ***REMOVED***;
    /**
     * Determines whether a file exists. Allows mocking.
     * @api private
     */
    GoogleAuth.prototype._fileExists = function (filePath) ***REMOVED***
        return fs.existsSync(filePath);
    ***REMOVED***;
    /**
     * Joins two parts of a path. Allows mocking.
     * @api private
     */
    GoogleAuth.prototype._pathJoin = function (item1, item2) ***REMOVED***
        return path.join(item1, item2);
    ***REMOVED***;
    /**
     * Allows mocking of the path to a well-known file.
     * @api private
     */
    GoogleAuth.prototype._mockWellKnownFilePath = function (filePath) ***REMOVED***
        return filePath;
    ***REMOVED***;
    // Executes the given callback if it is not null.
    GoogleAuth.prototype.callback = function (c, err) ***REMOVED***
        var args = [];
        for (var _i = 2; _i < arguments.length; _i++) ***REMOVED***
            args[_i - 2] = arguments[_i];
        ***REMOVED***
        if (c) ***REMOVED***
            return c.apply(null, Array.prototype.slice.call(arguments, 1));
        ***REMOVED***
    ***REMOVED***;
    // Creates an Error containing the given message, and includes the message
    // from the optional err passed in.
    GoogleAuth.prototype.createError = function (message, err) ***REMOVED***
        var s = message || '';
        if (err) ***REMOVED***
            var errorMessage = String(err);
            if (errorMessage && errorMessage.length > 0) ***REMOVED***
                if (s.length > 0) ***REMOVED***
                    s += ' ';
                ***REMOVED***
                s += errorMessage;
            ***REMOVED***
        ***REMOVED***
        return Error(s);
    ***REMOVED***;
    /**
     * Loads the default project of the Google Cloud SDK.
     * @param ***REMOVED***function***REMOVED*** _callback Callback.
     * @api private
     */
    GoogleAuth.prototype._getDefaultServiceProjectId = function (_callback) ***REMOVED***
        var _this = this;
        this._getSDKDefaultProjectId(function (err, stdout) ***REMOVED***
            var projectId = null;
            if (!err && stdout) ***REMOVED***
                try ***REMOVED***
                    projectId = JSON.parse(stdout).core.project;
                ***REMOVED***
                catch (err) ***REMOVED***
                    projectId = null;
                ***REMOVED***
            ***REMOVED***
            // Ignore any errors
            _this.callback(_callback, null, projectId);
        ***REMOVED***);
    ***REMOVED***;
    /**
     * Loads the project id from environment variables.
     * @param ***REMOVED***function***REMOVED*** _callback Callback.
     * @api private
     */
    GoogleAuth.prototype._getProductionProjectId = function (_callback) ***REMOVED***
        var _this = this;
        var projectId = this._getEnv('GCLOUD_PROJECT') || this._getEnv('GOOGLE_CLOUD_PROJECT');
        if (projectId) ***REMOVED***
            setImmediate(function () ***REMOVED***
                _this.callback(_callback, null, projectId);
            ***REMOVED***);
        ***REMOVED***
        return projectId;
    ***REMOVED***;
    /**
     * Loads the project id from the GOOGLE_APPLICATION_CREDENTIALS json file.
     * @param ***REMOVED***function***REMOVED*** _callback Callback.
     * @api private
     */
    GoogleAuth.prototype._getFileProjectId = function (_callback) ***REMOVED***
        var _this = this;
        if (this.cachedCredential) ***REMOVED***
            // Try to read the project ID from the cached credentials file
            setImmediate(function () ***REMOVED***
                _this.callback(_callback, null, _this.cachedCredential.projectId);
            ***REMOVED***);
            return;
        ***REMOVED***
        // Try to load a credentials file and read its project ID
        var pathExists = this._tryGetApplicationCredentialsFromEnvironmentVariable(function (err, result) ***REMOVED***
            if (!err && result) ***REMOVED***
                _this.callback(_callback, null, result.projectId);
                return;
            ***REMOVED***
            _this.callback(_callback, err);
        ***REMOVED***);
        if (!pathExists) ***REMOVED***
            this.callback(_callback, null);
        ***REMOVED***
    ***REMOVED***;
    /**
     * Gets the Compute Engine project ID if it can be inferred.
     * Uses 169.254.169.254 for the metadata server to avoid request
     * latency from DNS lookup.
     * See https://cloud.google.com/compute/docs/metadata#metadataserver
     * for information about this IP address. (This IP is also used for
     * Amazon EC2 instances, so the metadata flavor is crucial.)
     * See https://github.com/google/oauth2client/issues/93 for context about
     * DNS latency.
     *
     * @param ***REMOVED***function***REMOVED*** _callback Callback.
     * @api private
     */
    GoogleAuth.prototype._getGCEProjectId = function (_callback) ***REMOVED***
        var _this = this;
        if (!this.transporter) ***REMOVED***
            this.transporter = new transporters_1.DefaultTransporter();
        ***REMOVED***
        this.transporter.request(***REMOVED***
            method: 'GET',
            uri: 'http://169.254.169.254/computeMetadata/v1/project/project-id',
            headers: ***REMOVED*** 'Metadata-Flavor': 'Google' ***REMOVED***
        ***REMOVED***, function (err, body, res) ***REMOVED***
            if (err || !res || res.statusCode !== 200 || !body) ***REMOVED***
                _this.callback(_callback);
                return;
            ***REMOVED***
            // Ignore any errors
            _this.callback(_callback, null, body);
        ***REMOVED***);
    ***REMOVED***;
    /**
     * The callback function handles a credential object that contains the
     * client_email and private_key (if exists).
     * getCredentials checks for these values from the user JSON at first.
     * If it doesn't exist, and the environment is on GCE, it gets the
     * client_email from the cloud metadata server.
     * @param callback Callback that handles the credential object that contains
     * a client_email and optional private key, or the error.
     * returned
     */
    GoogleAuth.prototype.getCredentials = function (callback) ***REMOVED***
        var _this = this;
        if (this.jsonContent) ***REMOVED***
            var credential = ***REMOVED***
                client_email: this.jsonContent.client_email,
                private_key: this.jsonContent.private_key
            ***REMOVED***;
            callback(null, credential);
        ***REMOVED***
        else ***REMOVED***
            this._checkIsGCE(function (err, gce) ***REMOVED***
                if (err) ***REMOVED***
                    callback(err);
                ***REMOVED***
                else if (!gce) ***REMOVED***
                    callback(new Error('Unknown error.'));
                ***REMOVED***
                else ***REMOVED***
                    // For GCE, return the service account details from the metadata
                    // server
                    _this.transporter.request(***REMOVED***
                        method: 'GET',
                        uri: 'http://metadata.google.internal/computeMetadata/v1/instance/service-accounts/?recursive=true',
                        headers: ***REMOVED*** 'Metadata-Flavor': 'Google' ***REMOVED***
                    ***REMOVED***, function (err, body, res) ***REMOVED***
                        if (err || !res || res.statusCode !== 200 || !body ||
                            !body.default || !body.default.email) ***REMOVED***
                            if (callback) ***REMOVED***
                                callback(new Error('Failure from metadata server.'));
                            ***REMOVED***
                        ***REMOVED***
                        else ***REMOVED***
                            // Callback with the body
                            var credential = ***REMOVED*** client_email: body.default.email ***REMOVED***;
                            if (callback) ***REMOVED***
                                callback(null, credential);
                            ***REMOVED***
                        ***REMOVED***
                    ***REMOVED***);
                ***REMOVED***
            ***REMOVED***);
        ***REMOVED***
    ***REMOVED***;
    /**
     * Export DefaultTransporter as a static property of the class.
     */
    GoogleAuth.DefaultTransporter = transporters_1.DefaultTransporter;
    return GoogleAuth;
***REMOVED***());
exports.GoogleAuth = GoogleAuth;
//# sourceMappingURL=googleauth.js.map