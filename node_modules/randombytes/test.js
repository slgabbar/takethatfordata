var test = require('tape')
var randomBytes = require('./')

test('sync', function (t) ***REMOVED***
  t.plan(4)
  t.equals(randomBytes(0).length, 0, 'len: ' + 0)
  t.equals(randomBytes(3).length, 3, 'len: ' + 3)
  t.equals(randomBytes(30).length, 30, 'len: ' + 30)
  t.equals(randomBytes(300).length, 300, 'len: ' + 300)
***REMOVED***)

test('async', function (t) ***REMOVED***
  t.plan(4)

  randomBytes(0, function (err, resp) ***REMOVED***
    if (err) throw err

    t.equals(resp.length, 0, 'len: ' + 0)
  ***REMOVED***)

  randomBytes(3, function (err, resp) ***REMOVED***
    if (err) throw err

    t.equals(resp.length, 3, 'len: ' + 3)
  ***REMOVED***)

  randomBytes(30, function (err, resp) ***REMOVED***
    if (err) throw err

    t.equals(resp.length, 30, 'len: ' + 30)
  ***REMOVED***)

  randomBytes(300, function (err, resp) ***REMOVED***
    if (err) throw err

    t.equals(resp.length, 300, 'len: ' + 300)
  ***REMOVED***)
***REMOVED***)

if (process.browser) ***REMOVED***
  test('requesting to much throws', function (t) ***REMOVED***
    t.plan(1)
    t.throws(function () ***REMOVED***
      randomBytes(65537)
    ***REMOVED***)
  ***REMOVED***)

  test('requesting to much throws async', function (t) ***REMOVED***
    t.plan(1)
    t.throws(function () ***REMOVED***
      randomBytes(65537, function () ***REMOVED***
        t.ok(false, 'should not get here')
      ***REMOVED***)
    ***REMOVED***)
  ***REMOVED***)
***REMOVED***
