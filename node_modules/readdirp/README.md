...
var stream = readdirp(***REMOVED*** root: path.join(__dirname), fileFilter: '*.js' ***REMOVED***);
stream
  .on('warn', function (err) ***REMOVED*** 
    console.error('non-fatal error', err); 
    // optionally call stream.destroy() here in order to abort and cause 'close' to be emitted
  ***REMOVED***)
  .on('error', function (err) ***REMOVED*** console.error('fatal error', err); ***REMOVED***)
  .pipe(es.mapSync(function (entry) ***REMOVED*** 
    return ***REMOVED*** path: entry.path, size: entry.stat.size ***REMOVED***;
  ***REMOVED***))
  .pipe(es.stringify())
  .pipe(process.stdout);
```
...
- function with err and res parameters, e.g., `function (err, res) ***REMOVED*** ... ***REMOVED***`
- **err**: array of errors that occurred during the operation, **res may still be present, even if errors occurred**
- **res**: collection of file/directory [entry infos](#entry-info)

### fileProcessed

- function with [entry info](#entry-info) parameter e.g., `function (entryInfo) ***REMOVED*** ... ***REMOVED***`
...
// Glob file filter
readdirp(***REMOVED*** root: './test/bed', fileFilter: '*.js' ***REMOVED***)
  .on('data', function (entry) ***REMOVED***
    // do something with each JavaScript file entry
  ***REMOVED***);

// Combined glob file filters
readdirp(***REMOVED*** root: './test/bed', fileFilter: [ '*.js', '*.json' ] ***REMOVED***)
  .on('data', function (entry) ***REMOVED***
    // do something with each JavaScript and Json file entry 
  ***REMOVED***);

// Combined negated directory filters
readdirp(***REMOVED*** root: './test/bed', directoryFilter: [ '!.git', '!*modules' ] ***REMOVED***)
  .on('data', function (entry) ***REMOVED***
    // do something with each file entry found outside '.git' or any modules directory 
  ***REMOVED***);

// Function directory filter
readdirp(***REMOVED*** root: './test/bed', directoryFilter: function (di) ***REMOVED*** return di.name.length === 9; ***REMOVED*** ***REMOVED***)
  .on('data', function (entry) ***REMOVED***
    // do something with each file entry found inside directories whose name has length 9
  ***REMOVED***);

// Limiting depth
readdirp(***REMOVED*** root: './test/bed', depth: 1 ***REMOVED***)
  .on('data', function (entry) ***REMOVED***
