...
We also provide Async as a collection of ES2015 modules. You can now `import ***REMOVED***each***REMOVED*** from 'async-es'` or `import waterfall from 'async-es/waterfall'`. If you are using only a few Async functions, and are using a ES bundler such as Rollup, this can significantly lower your build size.
...
- Async is also available as a collection of ES2015 modules in the new `async-es` package. (`import ***REMOVED***forEachSeries***REMOVED*** from 'async-es'`) ([#984](https://github.com/caolan/async/issues/984), [#996](https://github.com/caolan/async/issues/996))
