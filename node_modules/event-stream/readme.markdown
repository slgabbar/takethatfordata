...
if(!module.parent) ***REMOVED***
  ...
  process.stdin                        //connect streams together with `pipe`
    ...
    .pipe(es.map(function (data, cb) ***REMOVED*** //turn this async function into a stream
      cb(null
...
es.through(function write(data) ***REMOVED***
    this.emit('data', data)
    //this.pause() 
  ***REMOVED***,
  function end () ***REMOVED*** //optional
    this.emit('end')
  ***REMOVED***)
...
``` js
fs.createReadStream(file, ***REMOVED***flags: 'r'***REMOVED***)
  .pipe(es.split())
  .pipe(es.map(function (line, cb) ***REMOVED***
...
for another behaviour, transforms created by `es.parse(***REMOVED***error: true***REMOVED***)` will
...
``` js
	var s = new stream.Readable(***REMOVED***objectMode: true***REMOVED***).wrap(es.readArray([1,2,3]));
```
...
``` js
  var es = require('event-stream')
    , reader = es.readArray([1, 2, 3])
    , writer = es.writeArray(function (err, array)***REMOVED***
...
readStream.pipe(es.wait(function (err, body) ***REMOVED***
...
process.stdin.pipe(reduce(function(acc, data) ***REMOVED***
  return acc + data.length;
***REMOVED***, 0)).on("data", function(length) ***REMOVED***
