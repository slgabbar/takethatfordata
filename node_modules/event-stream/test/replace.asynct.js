...
exports ['fizz buzz'] = function (test) ***REMOVED***

  var readThis = d.map(1, 100, function (i) ***REMOVED***
    return (
     ! (i % 3 || i % 5) ? "FB" :
      !(i % 3) ? "F" :
      !(i % 5) ? "B" :
  ...
  ***REMOVED***) //array of multiples of 3 < 100

  var reader = es.readArray(readThis)
  var join = es.wait(function (err, string)***REMOVED***
...
exports ['fizz buzz replace'] = function (test) ***REMOVED***
  var split = es.split(/(1)/)
  var replace = es.replace('7', 'seven')
//  var x = spec(replace).through()
  split
    .pipe(replace)
    .pipe(es.join(function (err, string) ***REMOVED***
      it(string).equal(fizz7buzz)
    ***REMOVED***))

    replace.on('close', function () ***REMOVED***
//      x.validate()
      test.done()
    ***REMOVED***)
...
exports ['fizz buzz replace whitespace using regexp'] = function (test) ***REMOVED***
  var split = es.split(/(1)/)
...
//  var x = spec(replace).through()
  split
    ...
    ***REMOVED***))
    ...
    replaceTrailing.on('close', function () ***REMOVED***
//      x.validate()
      test.done()
    ***REMOVED***)
