# takethatfordata
Advanced Basketball Scouting Web Application

This is an advanced scouting web application geared for high-school coaches and programs. This project is for CMPS183 at UCSC.

Before trying to run the application make sure to run "npm install" first. Then run node server.js or nodemon server.js and go to the specified local host.


Sam Gabbard
slgabbar@ucsc.edu
1/21/18

